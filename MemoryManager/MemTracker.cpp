...
void *AllocationUnit::operator new(
size_t size
)
{
assert(size
== sizeof(AllocationUnit));
return TagAllocator.
...
void AllocationUnit::operator delete(
void *memory
)
{
TagAllocator.
    ...
    if (!leakLogFile)
        ...
        for (AllocationUnit *tag = Tags.PopHead(); tag; tag = Tags.PopHead()) {
            ...
            switch (tag->AllocationType) {
                case AllocType_Unknown:
                    allocationTypeName = "unknown";
                    break;
                case AllocType_New:
                    allocationTypeName = "new";
                    break;
                case AllocType_VectorNew:
                    allocationTypeName = "new[]";
                    break;
                case AllocType_Malloc:
                    allocationTypeName = "malloc";
                    break;
                case AllocType_Calloc:
                    allocationTypeName = "calloc";
                    break;
                case AllocType_Realloc:
                    allocationTypeName = "realloc";
                    break;
                case AllocType_Unvalidated:
                    allocationTypeName = "unvalidated";
                    break;
                default:
                    __assume(false);
            }
            ...
            fprintf(leakLogFile, "0x%08p\t0x%04IX (%Iu)\t%.128s\t%.128s\t%d\t%s\r\n", tag->ReportedAddress,
                    tag->ReportedSize, tag->ReportedSize, tag->SourceFile, tag->SourceFunction, tag->SourceLine,
                    allocationTypeName);
...
void
MemoryTracker::SetThreadLocalInformation(const char *source_file, const char *source_function, const int source_line) {
...
DECLSPEC_RESTRICT void *
MemoryTracker::Allocate(const size_t size, const AllocType type, const char *source_file, const char *source_function,
...
DECLSPEC_RESTRICT void *
MemoryTracker::Reallocate(void *memory, const size_t size, const char *source_file, const char *source_function,
                          const int source_line) {
...
void MemoryTracker::Free(void *memory, const DeallocType type, const char *source_file, const char *source_function,
                         const int source_line) {
