...
void CheckMemoryManagerConfiguration(int32 config) {
    if (config != MEMORYMANAGER_RELEASECONFIGURATION) {
        MessageBoxW(NULL,
                    L"This is a friendly warning to remind you that running the release-mode memory manager with debug-mode executables is not recommended and may cause unexpected results.",
                    L"MemoryManager.dll", MB_OK | MB_ICONWARNING);
    };
};
