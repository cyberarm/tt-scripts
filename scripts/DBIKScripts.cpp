...
ScriptRegistrant<DB_Shield_Generator_Vehicle> DB_Shield_Generator_Vehicle_Registrant("DB_Shield_Generator_Vehicle",
                                                                                     "ShieldPreset:string,ShieldUp_Sound:string,ShieldDown_Sound:string");
...
void DB_Guard_Bot::Timer_Expired(GameObject *obj, int number) {
    if (number == 87) {
        ...
        if (obj->As_SoldierGameObj()->Get_State() == SoldierAIState::AI_STATE_IDLE &&
            Commands->Get_Distance(Commands->Get_Position(obj), HomePosition) > 5) {
    ...
    if (number == 1) {
        ...
        if (follow) {
            if (obj && !obj->As_SoldierGameObj()->Is_On_Ladder() && !obj->As_SoldierGameObj()->Is_In_Elevator()) {
                if (Commands->Get_Distance(Commands->Get_Position(obj), Commands->Get_Position(follow)) >
                    distance * 1.5f) {
        ...
        if (PowerOn != BasePower) {
            ...
            if (BasePower == true) {
                ...
                if (!WeaponObj) {
                    ...
                    if (WeaponObj) {
                        WeaponID = Commands->Get_ID(WeaponObj);
                        Attach_Script_Once_V(WeaponObj, "dblaney_Nod_Obelisk_Weapon", "%f,%f,%i", MaxRange, ChargeTime,
                                             GroundOnly);
                        //Commands->Attach_Script(WeaponObj, "Obelisk_Weapon_CnC", "");
                    }
                }
...
ScriptRegistrant<dblaney_Nod_Obelisk> dblaney_Nod_Obelisk_Registrant("dblaney_Nod_Obelisk",
                                                                     "ObeliskEmitterPreset=Nod_Obelisk:string,MaxRange=150:float,ChargeTime=2:float,GroundOnly=0:int,OffsetHeight=22:float");
...
void dblaney_Nod_Obelisk_Structure::Custom(GameObject *ObeliskObj, int type, int Param, GameObject *Sender) {
    if (type == CUSTOM_EVENT_BUILDING_POWER_CHANGED) {
        ...
        Commands->Send_Custom_Event(ObeliskObj, ObeliskObj, CUSTOM_EVENT_BUILDING_POWER_CHANGED,
                                    Commands->Get_Building_Power(ObeliskObj), 0);
    }
}

ScriptRegistrant<dblaney_Nod_Obelisk_Structure> dblaney_Nod_Obelisk_Structure_Registrant(
        "dblaney_Nod_Obelisk_Structure",
        "ObeliskEmitterPreset=Nod_Obelisk:string,MaxRange=150:float,ChargeTime=2:float,GroundOnly=0:int,OffsetHeight=22:float");
...
ScriptRegistrant<dblaney_Nod_Obelisk_Weapon> dblaney_Nod_Obelisk_Weapon_Registrant("dblaney_Nod_Obelisk_Weapon",
                                                                                   "MaxRange=150:float,ChargeTime=2:float,GroundOnly=0:int,Team=0:int");
...
void DB_Power_Down_Console::Poked(GameObject *obj, GameObject *poker) {
    ...
    if (Is_Base_Powered(PTTEAM(team)) && !reset &&
        ...
        if (Get_Player_Count() <= 1) {
            message.Format("%s base power offline. Power will reset in approximately %g seconds.",
                           Get_Team_Name(PTTEAM(team)),
                           Get_Float_Parameter("Power_Time") + Get_Float_Parameter("SinglePlayerBonus"));
            Send_Message(255, 200, 200, message);
            Commands->Start_Timer(obj, this,
                                  Get_Float_Parameter("Power_Time") + Get_Float_Parameter("SinglePlayerBonus"), 1);
        } else {
            message.Format("%s base power offline. Power will reset in approximately %g seconds.",
                           Get_Team_Name(PTTEAM(team)), Get_Float_Parameter("Power_Time"));
            Send_Message(255, 200, 200, message);
...
ScriptRegistrant<DB_Power_Down_Console> DB_Power_Down_Console_Registrant("DB_Power_Down_Console",
                                                                         "Power_Time:float=60,Reset_Time:float=5,Sound:string,Sound2:string,SinglePlayerBonus:float=0");
...
void DB_Power_Down_Building_Console::Poked(GameObject *obj, GameObject *poker) {
    ...
    if (Is_Base_Powered(Get_Object_Type(building)) && !Is_Building_Dead(building) && !reset) {
        ...
        StringClass message;
        if (Get_Player_Count() <= 1) {
            message.Format("%s %s offline. Power to the %s will be reset in approximately %g seconds.",
                           Get_Team_Name(PTTEAM(team)), Get_Translated_Preset_Name(building),
                           Get_Translated_Preset_Name(building),
                           Get_Float_Parameter("Power_Time") + Get_Float_Parameter("SinglePlayerBonus"));
            Send_Message(255, 200, 200, message);
            Commands->Start_Timer(obj, this,
                                  Get_Float_Parameter("Power_Time") + Get_Float_Parameter("SinglePlayerBonus"), 1);
            ...
            message.Format("%s %s offline. Power to the %s will be reset in approximately %g seconds.",
                           Get_Team_Name(PTTEAM(team)), Get_Translated_Preset_Name(building),
                           Get_Translated_Preset_Name(building), Get_Float_Parameter("Power_Time"));
            Send_Message(255, 200, 200, message);
            Commands->Start_Timer(obj, this, Get_Float_Parameter("Power_Time"), 1);
        }
    }
...
void DB_Power_Down_Building_Console::Timer_Expired(GameObject *obj, int number) {
    if (number == 1) {
        ...
        if (Is_Base_Powered(Get_Object_Type(building)) && !Is_Building_Dead(building)) {
            ...
            StringClass message;
            message.Format("%s %s online.", Get_Team_Name(Get_Object_Type(building)),
                           Get_Translated_Preset_Name(building));
            Send_Message(255, 200, 200, message);
        }
        reset = true;
...
ScriptRegistrant<DB_Power_Down_Building_Console> DB_Power_Down_Building_Console_Registrant(
        "DB_Power_Down_Building_Console",
        "BuildingID:int=0,Power_Time:float=60,Reset_Time:float=5,Sound:string,Sound2:string,SinglePlayerBonus:float=0");
...
ScriptRegistrant<DB_Disable_Injured_Animations> DB_Disable_Injured_Animations_Registrant(
        "DB_Disable_Injured_Animations", "");
...
void DB_Grant_Keycard_Killed::Killed(GameObject *obj, GameObject *killer) {
    Send_Message(255, 255, 255,
                 StringClass::getFormattedString("Level %i Keycard acquired.", Get_Int_Parameter("KeyLevel")));
...
ScriptRegistrant<DB_Grant_Keycard_Killed> DB_Grant_Keycard_Killed_Registrant("DB_Grant_Keycard_Killed",
                                                                             "KeyLevel=1:int");
...
void DB_Infantry_Place_Buildable_Object::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == Get_Int_Parameter("DeployCustom")) {
        ...
        if (!canRegen && reloadTime) {
            Send_Message_Player(obj, 255, 255, 255,
                                "You must repair the last one you placed before you can place another.");
            return;
        }
        if (reloadTime) {
        ...
        char params[220];
        sprintf(params, "%d,%d,%s,%d,%d", this->Get_ID(), Commands->Get_ID(obj), Get_Parameter("RepairedPreset"),
                Get_Int_Parameter("MatchTeam"), Commands->Get_Player_Type(obj));
...
ScriptRegistrant<DB_Infantry_Place_Buildable_Object> DB_Infantry_Place_Buildable_Object_Registrant(
        "DB_Infantry_Place_Buildable_Object",
        "WeaponPreset=null:string,DeployPreset=null:string,RepairedPreset=null:string,PlaceablePreview=null:string,UnPlaceablePreview=null:string,EnableCustom:int,DisableCustom:int,DeployCustom:int,RegainTimer:int,Cost:float,DeploymentBlockedMessage=Cannot deploy when near buildings.:string,PlacementMessage=You can now place another.:string,GrantWeapon=1:int,RegenStartsAfterRepair=1:int,MatchTeam=0:int,DestroyedRegenTime=1:int");
...
ScriptRegistrant<DB_Turret_Sound> DB_Turret_Sound_Registrant("DB_Turret_Sound",
                                                             "Sound_Emitter:string,Min_Rotate_Amount=0.05:float");
    ...
    if (message == BOMBARD_TOGGLE) {
        if (bombard == false) {
            ...
            if (turret) {
                ...
                bombard = true;
                Commands->Send_Custom_Event(turret, turret, CUSTOM_AI_DISABLEAI, 0, 0);
                Set_HUD_Help_Text_Player_Text(sender, 7403,
                                              "Manual Targeting set, Press 'T' to switch to auto targeting",
            ...
            if (turret) {
                ...
                Commands->Send_Custom_Event(turret, turret, CUSTOM_AI_ENABLEAI, 0, 0);
                Set_HUD_Help_Text_Player_Text(sender, 7403,
                                              "Auto Targeting set, Press 'T' to set manual target location",
                                              Vector3(0.3f, 0.3f, 1.0f));
            }
        }
    }
...
ScriptRegistrant<DB_Turret_Spawn> DB_Turret_Spawn_Registrant("DB_Turret_Spawn",
                                                             "Turret_Preset:string,Bone_Name=Tur_Mount:string");
...
ScriptRegistrant<DB_Spawned_Visible_Ammo> DB_Spawned_Visible_Ammo_Registrant("DB_Spawned_Visible_Ammo",
                                                                             "Ammo_Preset:string,Bone_Name=AmmoMount:string");
...
ScriptRegistrant<DB_Vehicle_Visible_Ammo> DB_Vehicle_Visible_Ammo_Registrant("DB_Vehicle_Visible_Ammo",
                                                                             "Animation:string,StartEnabled=0:int,EnableMessage=9320945:int,DisableMessage=9320946:int");
...
void DB_Orca_Lifter::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == DEPLOY_COMPLETE_CUSTOM) {
        ...
        OBBoxClass zoneBoundingBox(Commands->Get_Bone_Position(obj, Get_Parameter("Zone_Bone")),
                                   Get_Vector3_Parameter("Zone_Size"), rotation);
        if (GameObject *reloadZone = Create_Zone("Script_Zone_All", zoneBoundingBox)) {
            zoneID = Commands->Get_ID(reloadZone);
            Attach_Script_Once_V(reloadZone, "DB_capturable_helipadzone_reload", "%i,%s", obj->Get_ID(),
                                 Get_Parameter("Reload_Preset"));
        ...
        zoneID = 0;
    } else if (message == CUSTOM_EVENT_VEHICLE_EXITED && zoneID &&
               obj->As_VehicleGameObj()->Get_Occupant_Count() == 0) {
...
ScriptRegistrant<DB_Orca_Lifter> DB_Orca_Lifter_Registrant("DB_Orca_Lifter",
                                                           "Zone_Bone=Origin:string,Zone_Size:vector3,Reload_Preset:string");
...
void DB_Turret_Bombard_Player::Created(GameObject *obj) {
    ...
    Set_HUD_Help_Text_Player_Text(obj, 7403, "Press the 'T' key to toggle auxilery turret manual targeting",
                                  Vector3(0.3f, 0.3f, 1.0f));
}
...
ScriptRegistrant<DB_Turret_Bombard_Player> DB_Turret_Bombard_Player_Registrant("DB_Turret_Bombard_Player",
                                                                               "Key=Deploy:string,ID=0:int");
...
ScriptRegistrant<DB_Health_Sound_Timer> DB_Health_Sound_Timer_Registrant("DB_Health_Sound_Timer",
                                                                         "Time:float,TimerNum:int,Repeat:int,Sound:string,Min_Health:float,Max_Health:float");
    ...
    if (type == CUSTOM_EVENT_VEHICLE_ENTERED) {
        ...
        if (seat == Get_Int_Parameter("Seat")) {
            ...
            if (staticAnim) {
                ...
                Commands->Set_Animation(object, Get_Parameter("Animation"), true, 0, Get_Animation_Frame(obj), -1,
                                        false);
            }
        }
    }
...
ScriptRegistrant<DB_Visible_Passenger> DB_Visible_Passenger_Registrant("DB_Visible_Passenger",
                                                                       "Seat=0:int,BoneName=seat0:string,Animation=s_a_human.h_a_a0a0:string,StaticAnim=0:int,StaticFrame=0:int");
...
void DB_Drop_Wreckage_On_Death::Killed(GameObject *obj, GameObject *killer) {
    if (Get_Damage_Warhead() != 25) {
        if (Find_Named_Definition(Get_Parameter("Wreckage_Preset")) && obj->As_VehicleGameObj()) {
            ...
            if (CurTank) {
                PhysicalGameObj *Shell = (PhysicalGameObj *) Commands->Create_Object("Mounted",
...
void DB_Wreckage_Rebuildable::Damaged(GameObject *obj, GameObject *damager, float amount) {
    ...
    if (amount < 0) {
        ...
        if (TotalHealth == MaxHealth) {
            GameObject * CurTank = Commands->Create_Object(Get_Parameter("Vehicle_Preset"),
                                                           Commands->Get_Position(obj));
...
ScriptRegistrant<DB_Drop_Wreckage_On_Death> DB_Drop_Wreckage_On_Death_Registrant("DB_Drop_Wreckage_On_Death",
                                                                                 "Wreckage_Preset:string");
ScriptRegistrant<DB_Wreckage_Rebuildable> DB_Wreckage_Rebuildable_Registrant("DB_Wreckage_Rebuildable",
...
void DB_Damage_Vehicle_Occupants_Area_Killed::Killed(GameObject *obj, GameObject *shooter) {
    ...
    char prms[512];
    if (shooter)
        sprintf(prms, "%f,%s,%f,%i", Get_Float_Parameter("Occupants_Damage_Amount"),
                Get_Parameter("Occupants_Damage_Warhead"), Get_Float_Parameter("Damage_Distance"), shooter->Get_ID());
    else
        sprintf(prms, "%f,%s,%f,%i", Get_Float_Parameter("Occupants_Damage_Amount"),
                Get_Parameter("Occupants_Damage_Warhead"), Get_Float_Parameter("Damage_Distance"), 0);
...
void DB_Damage_Vehicle_Occupants_Area_Timer::Timer_Expired(GameObject *obj, int number) {
    if (number == 10000) {
        ...
        if (damager)
            Damage_All_Objects_Area_By_Team(Get_Float_Parameter("Occupants_Damage_Amount"),
                                            Get_Parameter("Occupants_Damage_Warhead"), Commands->Get_Position(obj),
                                            Get_Float_Parameter("Damage_Distance"), damager, 1, 0, 2);
        else
            Damage_All_Objects_Area_By_Team(Get_Float_Parameter("Occupants_Damage_Amount"),
                                            Get_Parameter("Occupants_Damage_Warhead"), Commands->Get_Position(obj),
                                            Get_Float_Parameter("Damage_Distance"), 0, 1, 0, 2);
        Commands->Destroy_Object(obj);
    }
}

ScriptRegistrant<DB_Damage_Vehicle_Occupants_Area_Timer> DB_Damage_Vehicle_Occupants_Area_Timer_Registrant(
        "DB_Damage_Vehicle_Occupants_Area_Timer",
        "Occupants_Damage_Amount=1000:float,Occupants_Damage_Warhead=warhead:string,Damage_Distance=1000:float,ShooterID=0:int");
ScriptRegistrant<DB_Damage_Vehicle_Occupants_Area_Killed> DB_Damage_Vehicle_Occupants_Area_Killed_Registrant(
        "DB_Damage_Vehicle_Occupants_Area_Killed",
        "Area_Damaging_Explosion=explosion:string,Occupants_Damage_Warhead=warhead:string,Occupants_Damage_Amount=1000:float,Damage_Distance=1000:float");
...
void DB_Set_PT_Slot::Created(GameObject *obj) {
    ...
    PurchaseSettingsDefClass * PT = PurchaseSettingsDefClass::Find_Definition((PurchaseSettingsDefClass::TYPE) Type,
...
ScriptRegistrant<DB_Set_PT_Slot> DB_Set_PT_Slot_Registrant("DB_Set_PT_Slot",
                                                           "Team=0:int,Type=0:int,Slot=0:int,Alt=-1:int,Preset=CnC_Nod_Light_Tank:String");
...
ScriptRegistrant<DB_Spawn_When_Killed> DB_Spawn_When_Killed_Registrant("DB_Spawn_When_Killed",
                                                                       "PresetName:string,DropHeight=0:float,SameFacing=1:int,HealthPercent=100.0000:float,ArmorPercent=100.0000:float,MatchObjectTeam=0:int");
ScriptRegistrant<DB_Replace_When_Repaired> DB_Replace_When_Repaired_Registrant("DB_Replace_When_Repaired",
                                                                               "PresetName:string,DropHeight=0:float,SameFacing=1:int,HealthPercent=100.0000:float,ArmorPercent=100.0000:float,MatchRepairTeam=0:int");
...
ScriptRegistrant<DB_Force_Fire> DB_Force_Fire_Registrant("DB_Force_Fire",
                                                         "UseSecondaryFire=0:int,ResetTime=0:float,StartDelay=0:float");