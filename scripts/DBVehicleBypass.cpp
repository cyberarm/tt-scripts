...
class DB_Check_Airdrop : public ScriptImpClass {
    ...
    void Timer_Expired(GameObject *obj, int Number) {
        if (Number == 12) {
            if (obj->As_VehicleGameObj()->Get_Lock_Owner() &&
                Commands->Get_Player_Type(obj->As_VehicleGameObj()->Get_Lock_Owner()) == 1 &&
                !obj->Find_Observer("DAAirDroppedVehicleObserverClass")) {
                GameObject * DavesArrow = Commands->Find_Object(Get_Int_Parameter("ArrowID"));
                if (DavesArrow) {
                    Send_Message_Player(obj->As_VehicleGameObj()->Get_Lock_Owner(), 150, 150, 255,
                                        "Your Vehicle will be Airdropped nearby.");
...
ScriptRegistrant<DB_CinematicVehicleFactoryBypass> DB_CinematicVehicleFactoryBypass_Registrant(
        "DB_CinematicVehicleFactoryBypass", "PresetName:string");
...
void Bypass_Drop_Off_Control::Created(GameObject *obj) {
    ...
    if (vehicle) {
        ...
        GameObject * trans = Commands->Create_Object_At_Bone(transbone, Team == 1 ? "GDI_Transport_Helicopter"
