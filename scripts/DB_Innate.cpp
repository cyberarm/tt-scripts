...
bool Can_Position_Target_Object(PhysicalGameObj *attacker, Vector3 attackPosition, PhysicalGameObj *target) {
    ...
    PhysRayCollisionTestClass collisionTest(LineSegClass(attackPosition, bullseyePosition), &castResult,
...
DB_Innate_Soldier::DB_Innate_Soldier() : State(1), StateTimer(0), HomeFacing(9999999), ActTime(0), UseWeapon(0),
                                         Aggressiveness(0.5), TakeCoverProbability(0.5), IsStationary(false),
                                         LastState(0), Weapon(0),
                                         ConversationTimer(((WWMath::Random_Float() * 9.0f) + 1.0f)) {
}
...
char *SoldierAIStateNames[6] = {"Relaxed Idle", "Alert Idle", "Footsteps Heard", "Bullet Heard", "Gunshot Heard",
                                "Enemy Seen"};
    ...
    if (!obj->Is_Hibernating()) {
        ...
        if (sobj->Is_Innate_Enabled(SOLDIER_INNATE_EVENT_BULLET_HEARD)) {
            ...
            if (damager)// && FreeRandom.Get_Float() <0.8f)
            {
                ...
                if (sobj->Is_Enemy(damager->As_DamageableGameObj()) && damager != Target &&
                    Commands->Get_Health(damager) > 0)
        ...
        if (pobj) {
            if (pobj->Get_Defense_Object()->Get_Health() > 0) {
                ...
                if (!target || target == enemy ||
                    (target->Get_Position(&targetpos), enemy->Get_Position(&seenpos), obj->Get_Position(&objpos), (
                            (seenpos - objpos).Length2() <= (targetpos - objpos).Length2()))) {
                    SoldierGameObj *sobj = obj->As_SoldierGameObj();
                    bool state = false;
...
bool DB_Innate_Soldier::Set_State(SoldierGameObj *obj, int state, Vector3 const &pos, ScriptableGameObj *target) {
    ...
    if (StatePriorities[State] <= StatePriorities[state]) {
        if (State != state) {
            if (Debug)
                Send_Message(225, 255, 255, StringClass::getFormattedString("%s -> %s", SoldierAIStateNames[State],
        ...
        StateTimer = 0;
        if (((State == BULLET_HEARD) || (State == GUNSHOT_HEARD) || (State == ENEMY_SEEN && state < ENEMY_SEEN)) &&
            ActTime > 1) {
            ActTime = 1;
        }
        Vector3 position;
...
StateDataStr StateData[6] = {{9999, 0},
                             {180,  0},
                             {6,    1},
                             {20,   1},
                             {4,    3},
...
void DB_Innate_Soldier::Think_Function(SoldierGameObj *obj, bool b) {
    ...
    if (StateTimer >= StateData[State].timer) {
        ...
        if (Debug)
            Send_Message(225, 255, 255, StringClass::getFormattedString("%s -> %s", SoldierAIStateNames[State],
                                                                        SoldierAIStateNames[StateData[State].state]));
...
void DB_Innate_Soldier::State_Act_Attack(SoldierGameObj *obj) {
    ...
    ActTime = 3;
    Action_Attack_Object(obj, (PhysicalGameObj *) target, range,
                         (1.0f - Aggressiveness) > FreeRandom.Get_Float() * 5.0f, pos3, 0.5f);
}
...
void DB_Innate_Soldier::Action_Face_Location(SoldierGameObj *obj, Vector3 const &position, SoldierAIState aistate,
                                             bool crouched) {
    ActionParamsStruct params;
...
void DB_Innate_Soldier::Action_Goto_Location(SoldierGameObj *obj, Vector3 const &position, SoldierAIState aistate,
                                             float speed, float distance, bool crouched) {
    Vector3 pos = position;
...
void
DB_Innate_Soldier::Action_Goto_Location_Facing(SoldierGameObj *obj, Vector3 const &position, SoldierAIState aistate,
                                               Vector3 const &position2, float speed, float distance, bool crouched) {
    Vector3 pos = position;
    Stay_Within_Home(obj, &pos, &distance);
    ActionParamsStruct params;
    params.Set_Basic(Get_ID(), (float) StatePriorities[State], 9000000, aistate);
...
void DB_Innate_Soldier::Action_Attack_Object(SoldierGameObj *obj, PhysicalGameObj *target, float range, bool crouched,
                                             Vector3 const &position, float distance) {
    Vector3 pos = position;
    Stay_Within_Home(obj, &pos, &distance);
    if (IsStationary) {
        distance = 10000;
...
ScriptRegistrant<DB_Innate_Soldier> DB_Innate_Soldier_Registrant("DB_Innate_Soldier",
                                                                 "Aggressiveness=0.5:float,HomeRange=0:Float,Stationary=0:int,Debug=0:int");
