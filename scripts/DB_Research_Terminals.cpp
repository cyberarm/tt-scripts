...
void Set_Model_Custom(GameObject * obj,
...
{
Commands->
Set_Model(obj, Model_Name
);
Commands->
Send_Custom_Event(obj, obj,
REATTACH_BONES_CUSTOM,0,0);
}
}
...
void DB_Research_Infantry_Armor_Terminal::Poked(GameObject *obj, GameObject *poker) {
    if (Allow_Poke) {
            ...
            if (!Upgrade_Completed) {
                if (!Upgrading_In_Progress) {
                    if (!Is_Building_Dead(Find_Building_By_Type(Terminal_Team, BuildingConstants::TYPE_COM_CENTER))) {
                        if (Commands->Get_Money(poker) >= Upgrade_Add_Funds_Cost * InsertScale) {
                            if (Is_Base_Powered(Terminal_Team)) {
                                if (Upgrade_Current_Funds < Upgrade_Total_Cost) {
                                    ...
                                    Send_Message_Team(Terminal_Team, 175, 175, 255, StringClass::getFormattedString(
                                            "%s added %g to the %s Fund. Status: %g/%g", Get_Player_Name(poker),
                                            Upgrade_Add_Funds_Cost * InsertScale, Get_Translated_Preset_Name(obj),
                                            Upgrade_Current_Funds * CostScale, Upgrade_Total_Cost * CostScale));
                                    if (Upgrade_Current_Funds >= Upgrade_Total_Cost) {
                                        Upgrading_In_Progress = true;
                                        Commands->Start_Timer(obj, this,
                                                              Research_Technology::UPGRADE_INFANTRY_ARMOR_TIME,
                                                              Research_Technology::UPGRADE_INFANTRY_ARMOR_TIMER_NUMBER);
                        ...
                        Send_Message_Player(poker, 255, 255, 255, StringClass::getFormattedString(
                                "Sorry but your Communications Center is dead, researching of the Upgrade %s is not possible!",
                                Get_Translated_Preset_Name(obj)));
                    }
                } else {
                    Create_2D_Wave_Sound_Dialog_Player(poker, Upgrade_In_Progress_Sound);
                    Send_Message_Player(poker, 255, 255, 255, StringClass::getFormattedString(
                            "Sorry the %s's research is already in progress!", Get_Translated_Preset_Name(obj)));
                }
            } else {
                Create_2D_Wave_Sound_Dialog_Player(poker, Upgrade_Access_Denied_Sound);
        ...
        Allow_Poke = false;
        Commands->Start_Timer(obj, this, 0.25f, 700); //Allow poking the terminal after 2.5 seconds again
    }
}
...
void DB_Research_Infantry_Armor_Terminal::Timer_Expired(GameObject *obj, int number) {
    if (number == 700) {
        ...
        //if (!Is_Building_Dead(Find_Building_By_Type(Terminal_Team,BuildingConstants::TYPE_COM_CENTER)))
        {
            ...
            Send_Custom_Event_To_Objects_With_Script(obj, "DB_Research_Listener",
                                                     Research_Technology::UPGRADE_INFANTRY_ARMOR_CUSTOM, UpgradeID, 0);
...
ScriptRegistrant<DB_Research_Infantry_Armor_Terminal> DB_Research_Infantry_Armor_Terminal_Registrant(
        "DB_Research_Infantry_Armor_Terminal",
        "Terminal_Team=0:int,Upgrade_In_Progress_Sound=snd:string,Upgrade_In_Progress_Text=txt:string,Upgrade_Money_Added_Sound=snd:string,Upgrade_Access_Denied_Sound=snd:string,Upgrade_Completed_Sound=snd:string,Upgrade_Complete_Text=txt:string,Upgrade_Insufficient_Funds_Sound=snd:string,Upgrade_Total_Cost=0:float,Upgrade_Add_Funds_Cost=0:float,UpgradeID=1:int");
                    ...
                    if (!Is_Building_Dead(Find_Construction_Yard(Terminal_Team))) {
                        if (Is_Building_Dead(Find_Building_By_Type(Terminal_Team, BuildingType))) {
                            if (Commands->Get_Money(poker) >= Upgrade_Add_Funds_Cost * InsertScale) {
                                if (Is_Base_Powered(Terminal_Team)) {
                                    if (Upgrade_Current_Funds < Upgrade_Total_Cost) {
                                        ...
                                        Commands->Give_Money(poker, -Upgrade_Add_Funds_Cost * InsertScale, false);
                                        Create_2D_Wave_Sound_Dialog_Player(poker, Upgrade_Money_Added_Sound);
                                        Send_Message_Team(Terminal_Team, 150, 255, 150, StringClass::getFormattedString(
                                                "%s added %g to the %s Fund. Status: %g/%g", Get_Player_Name(poker),
                                                Upgrade_Add_Funds_Cost * InsertScale, Get_Translated_Preset_Name(obj),
                                                Upgrade_Current_Funds * CostScale, Upgrade_Total_Cost * CostScale));
                                        if (Upgrade_Current_Funds >= Upgrade_Total_Cost) {
                                    ...
                                    if (Upgrade_Current_Funds < (Upgrade_Total_Cost)) {
                                        ...
                                        if (Upgrade_Current_Funds >= (Upgrade_Total_Cost)) {
                                            ...
                                            Commands->Start_Timer(obj, this,
                                                                  ...
                                                                  Research_Technology::UPGRADE_INFANTRY_ARMOR_TIMER_NUMBER);
                                            if (Upgrade_In_Progress_Text) {
                                                Send_Message_Team(Terminal_Team, 100, 255, 100,
                                                                  Upgrade_In_Progress_Text);
                                            }
                                            Create_2D_WAV_Sound_Team_Dialog(Upgrade_In_Progress_Sound, Terminal_Team);
                                        }
                                    }
                            ...
                            Send_Message_Player(poker, 255, 255, 255,
                                                StringClass::getFormattedString("Structure is not Destroyed!",
                                                                                Get_Translated_Preset_Name(obj)));
                        }
                    } else {
                        Create_2D_Wave_Sound_Dialog_Player(poker, Upgrade_Access_Denied_Sound);
                        Send_Message_Player(poker, 255, 255, 255, StringClass::getFormattedString(
                                "Access Denied: Construction Yard is Destroyed!", Get_Translated_Preset_Name(obj)));
                    }
                } else {
                    Create_2D_Wave_Sound_Dialog_Player(poker, Upgrade_Access_Denied_Sound);
                    Send_Message_Player(poker, 255, 255, 255,
                                        StringClass::getFormattedString("Building Construction is already in progress!",
                                                                        Get_Translated_Preset_Name(obj)));
                }
            } else {
                Create_2D_Wave_Sound_Dialog_Player(poker, Upgrade_Access_Denied_Sound);
                Send_Message_Player(poker, 255, 255, 255,
                                    "Access Denied: Building Construction has already been completed!");
            }
        } else {
            Create_2D_Wave_Sound_Dialog_Player(poker, Upgrade_Access_Denied_Sound);
            Send_Message_Player(poker, 255, 255, 255, "Access Denied: You are on the wrong team!");
...
ScriptRegistrant<DB_Rebuild_Building_Terminal> DB_Rebuild_Building_Terminal_Registrant("DB_Rebuild_Building_Terminal",
...
ScriptRegistrant<DB_Research_Infantry_Armor_Ability> DB_Research_Infantry_Armor_Ability_Registrant(
        "DB_Research_Infantry_Armor_Ability",
        "Extra_Armor_Amount=0:float,Unit_Upgraded_Sound=bla:string,UpgradeID=1:int");
...
ScriptRegistrant<DB_Research_Infantry_Hitpoints_Ability> DB_Research_Infantry_Hitpoints_Ability_Registrant(
        "DB_Research_Infantry_Hitpoints_Ability",
        "Extra_Health_Amount=0:float,Extra_Armor_Amount=0:float,Model:string,Unit_Upgraded_Sound=bla:string,UpgradeID=1:int");
...
void DB_Research_Weapon_Upgrade::Created(GameObject *obj) {
    ...
    if (UpgradeComplete.Get(UpgradeID, 0)) {
        ...
        if (Find_Named_Definition(PowerUpPreset)) {
            ...
            Console_Input(StringClass::getFormattedString("msg invalid weapon preset: %s - %s",
                                                          Get_Translated_Preset_Name(obj), PowerUpPreset));
        }
    }
...
ScriptRegistrant<DB_Research_Weapon_Upgrade> DB_Research_Weapon_Upgrade_Registrant("DB_Research_Weapon_Upgrade",
                                                                                   "PowerupPreset:string,Model:string,Unit_Upgraded_Sound=bla:string,UpgradeID=1:int,Remove_Weapon_Preset=0:string,Secondary_Weapon=0:int");
...
ScriptRegistrant<DB_Research_Armor_Model> DB_Research_Armor_Model_Registrant("DB_Research_Armor_Model",
                                                                             "HealthShieldBonus:float,Model:string,Unit_Upgraded_Sound=bla:string,UpgradeID=1:int");
...
ScriptRegistrant<DB_Research_Credit_Tick> DB_Research_Credit_Tick_Registrant("DB_Research_Credit_Tick",
                                                                             "TickRate:float,Model:string,Unit_Upgraded_Sound=bla:string,UpgradeID=1:int");
...
void DB_Tiberium_Field::Entered(GameObject *obj, GameObject *enterer) {
    if (Is_Script_Attached(enterer, "DB_Tiberium_Harvester")) {
        if (Zonetype == 0) //green
            Commands->Send_Custom_Event(obj, enterer, CUSTOM_EVENT_GREENTIB_ENTERED, 0,
                                        0);//tell harv a green zone entered
        else if (Zonetype == 1) //blue
            Commands->Send_Custom_Event(obj, enterer, CUSTOM_EVENT_BLUETIB_ENTERED, 0,
                                        0);//tell harv a blue zone entered
...
ScriptRegistrant<DB_DeployableTank_Upgradeable> DB_DeployableTank_Upgradeable_Registrant(
        "DB_DeployableTank_Upgradeable",
