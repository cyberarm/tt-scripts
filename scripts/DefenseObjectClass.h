...
public:
    ...
    enum {
        MAX_MAX_HEALTH_NEW = 10000
    };//500};
    enum {
        MAX_MAX_SHIELD_STRENGTH_NEW = 10000
    };//500};
    ...
    Get_Health(void) const;

    void SCRIPTS_API

    Set_Health_Max(float health);

    float SCRIPTS_API

    Get_Health_Max(void) const;
    ...
    Get_Shield_Strength(void) const;

    void SCRIPTS_API

    Set_Shield_Strength_Max(float str);

    float SCRIPTS_API

    Get_Shield_Strength_Max(void) const;

    void SCRIPTS_API
    Set_Shield_Type( ArmorType
    type );
