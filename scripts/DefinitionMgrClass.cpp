    ...
    if (DefinitionHash) {
        ...
        if (!definition) {
            for (int j = 0; j < _DefinitionCount; j++) {
                ...
                if (def) {
                    if ((def->Get_Class_ID() == class_id) || (Get_Definition_Type(def->Get_Class_ID()) == class_id) ||
                        (twiddle && (def->Get_Class_ID() == 0xE000))) {
...
void DefinitionMgrClass::Free_Definitions() {
    if (DefinitionHash) {
        for (HashTemplateIterator<StringClass, DynamicVectorClass<DefinitionClass *> *> iter(
                *DefinitionHash); iter; ++iter)
...
int DefinitionMgrClass::fnCompareDefinitionsCallback(const void *elem1, const void *elem2) {
    DefinitionClass *d1 = *(DefinitionClass * *)
    elem1;
    DefinitionClass *d2 = *(DefinitionClass * *)
    elem2;
    int ret = 1;
...
bool DefinitionMgrClass::Load_Objects(ChunkLoadClass &cload) {
    while (cload.Open_Chunk()) {
        ...
        if (factory) {
            DefinitionClass *def = (DefinitionClass * )
            factory->Load(cload);
            if (def) {
                Prepare_Definition_Array();
