...
void JMG_New_Ship_Weapon_Control_System::Damaged(GameObject *obj, GameObject *damager, float damage) {
    if (Commands->Get_ID(Get_Vehicle(damager)) == Commands->Get_ID(obj) && damage == 0.0f &&
        Get_Total_Bullets(obj, "Weapon_Ship_Plasma_Shot_Normal") < 4)
    ...
    if (Commands->Get_Random(0.0f, 1.0f) > 0.94f)
        if (Random < 0.33)
            ...
            if (Commands->Get_Random(0.0f, 1.0f) > 0.75f)
                ...
                PowerupType = 13;
        } else if (Commands->Get_Random(0.0f, 1.0f) < 0.5)
            PowerupType = 12;
        else
            PowerupType = 16;
...
void JMG_CMTB_Main_Game_Control::Created(GameObject *obj) {
    ...
    for (int x = 0; x < MaxGamePlayerCount; x++) {
        ...
        MiniGamePlayerControlSystem[x].SpectatorObject = Commands->Create_Object_At_Bone(obj, "Spectator_Vehicle",
                                                                                         Spot);
        Commands->Attach_Script(MiniGamePlayerControlSystem[x].SpectatorObject, "JMG_CMTB_Vehicle_Face_Turret", "");
        MiniGamePlayerControlSystem[x].specCreateSpot = Commands->Get_Position(
                MiniGamePlayerControlSystem[x].SpectatorObject);
        MiniGamePlayerControlSystem[x].SpectatorObject->As_PhysicalGameObj()->Peek_Physical_Object()->Set_Collision_Group(
...
void JMG_CMTB_Main_Game_Control::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == 57447 && NextLevelDelay == -1) {
        ...
        for (int x = 0; x < MaxGamePlayerCount; x++) {
            if (!Get_GameObj(MiniGamePlayerControlSystem[x].PlayerID) ||
                !Commands->Find_Object(MiniGamePlayerControlSystem[x].GamePlayerID))
        ...
        if (!StartGame) {
            char playermsg[256];
            sprintf(playermsg, "%s is ready to start the game; however, %d others are not!", Get_Player_Name(sender),
                    TotalPlayerCount - ReadyPlayerCount);
            for (int x = 1; x < 128; x++) {
        ...
        for (int x = 0; x < MaxGamePlayerCount; x++) {
            ...
            RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                    MiniGamePlayerControlSystem[x].PlayerID)->RoundsPlayed++;
            MaxPlayerCount++;
        }
    ...
    if (message == 57448) {
        if (!Commands->Find_Object(MiniGamePlayerControlSystem[param].GamePlayerID)) {
            switch (CurrentMusic) {
                case MusicGameOver:
                    Set_Background_Music_Player(sender, "Thunder.mp3");
                    break;
                case MusicNormal:
                    Set_Background_Music_Player(sender, "Android.mp3");
                    break;
                case MusicBoss:
                    Set_Background_Music_Player(sender, "Escape.mp3");
                    break;
            }
...
void JMG_CMTB_Main_Game_Control::Timer_Expired(GameObject *obj, int number) {
    if (number == 5435) {
        ...
        for (int x = 0; x < MaxGamePlayerCount; x++) {
            if (!MiniGamePlayerControlSystem[x].GamePlayerID)
                continue;
            RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                    MiniGamePlayerControlSystem[x].PlayerID)->PlayTime++;
            RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                    MiniGamePlayerControlSystem[x].PlayerID)->LastPlayTime = currentTime;
            if (MiniGamePlayerControlSystem[x].ShipObject)
                switch (MiniGamePlayerControlSystem[x].ShipObject->Type) {
                    case PlayerCloak:
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->TimeCloaked++;
                        break;
                    case PlayerShield:
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->TimeShielded++;
                        break;
                    case PlayerShipDisrupter:
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->TimeDisrupted++;
                        break;
                }
        }
        ...
        if (NextLevelDelay != -1) {
            if (NextLevelDelay) {
                        ...
                        switch (MiniGamePlayerControlSystem[x].SelectedPerk) {
                            case 1:
                                if (JMG_CMTB_Main_Game_Control::CMTBLevel == 1 ||
                                    !(JMG_CMTB_Main_Game_Control::CMTBLevel % 25))
                                    Commands->Send_Custom_Event(obj, MiniGamePlayerControlSystem[x].PlayerShip,
                                                                CMTBPowerUpCustomMessage + 5, -1337, 0);
                                break;
                            case 2:
                                if (JMG_CMTB_Main_Game_Control::CMTBLevel == 1)
                                    Commands->Send_Custom_Event(obj, MiniGamePlayerControlSystem[x].PlayerShip,
                                                                CMTBPowerUpCustomMessage + 14, -1337, 0);
                                break;
                            case 3:
                                if (Commands->Get_Random(0.0f, 1.0f) < 0.165)
                                    Commands->Send_Custom_Event(obj, MiniGamePlayerControlSystem[x].PlayerShip,
                                                                CMTBPowerUpCustomMessage + 13, -1337, 0);
                                break;
                            case 4:
                                if (Commands->Get_Random(0.0f, 1.0f) < 0.05)
                                    Commands->Send_Custom_Event(obj, MiniGamePlayerControlSystem[x].PlayerShip,
                                                                CMTBPowerUpCustomMessage + 12, -1337, 0);
                                break;
                            case 5:
                                if (Commands->Get_Random(0.0f, 1.0f) < 0.05)
                                    Commands->Send_Custom_Event(obj, MiniGamePlayerControlSystem[x].PlayerShip,
                                                                CMTBPowerUpCustomMessage + 16, -1337, 0);
                                break;
                        }
                    }
                    if (!(JMG_CMTB_Main_Game_Control::CMTBLevel % 6)) {
                        unsigned int Count = (int) (JMG_CMTB_Main_Game_Control::CMTBLevel * 0.5);
                        for (unsigned int x = 0; x < Count; x++) {//Drones
                            ...
                            MoveablePhysClass *mphys = obj->As_PhysicalGameObj()
                                                       ? Comet->As_PhysicalGameObj()->Peek_Physical_Object()->As_MoveablePhysClass()
                                                       : NULL;
                        ...
                        for (unsigned int x = 0; x < Count; x++) {//Comet2
                            GameObject * Comet = Commands->Create_Object(
                                    ((JMG_CMTB_Main_Game_Control::CMTBLevel % 10) ? "Comet2" : "CMTB_UFO"),
                                    Random_Map_Position());
                            Commands->Set_Facing(Comet, Commands->Get_Random(-180.0f, 180.0f));
                            if (JMG_CMTB_Main_Game_Control::CMTBLevel % 10) {
                                char params[512];
                                sprintf(params, "2,%.3f %.3f %.3f", Commands->Get_Position(Comet).X +
                                                                    (Commands->Get_Random(0.0, 1.0f) < 0.5 ? 1 : -1) *
                                                                    Commands->Get_Random(15, 25),
                                        (Commands->Get_Random(0.0, 1.0f) < 0.5 ? 1 : -1) *
                                        Commands->Get_Position(Comet).Y + Commands->Get_Random(15, 25),
                                        Commands->Get_Position(Comet).Z);
                                Commands->Attach_Script(Comet, "JMG_CMTB_Comet_Script", params);
                            } else
                                Commands->Attach_Script(Comet, "JMG_CMTB_UFO_Random_Fire_Control", "1");
                            MoveablePhysClass *mphys = Comet->As_PhysicalGameObj()
                                                       ? Comet->As_PhysicalGameObj()->Peek_Physical_Object()->As_MoveablePhysClass()
                                                       : NULL;
                            while (mphys && !mphys->Can_Teleport(Matrix3D(Commands->Get_Position(Comet))))
                                Commands->Set_Position(Comet, Random_Map_Position());
                ...
                for (int x = 1; x < 128; x++) {
                    ...
                    if ((JMG_CMTB_Main_Game_Control::CMTBLevel - 1) % 10 &&
                        (JMG_CMTB_Main_Game_Control::CMTBLevel - 1) % 25)
            ...
            for (int x = 0; x < MaxGamePlayerCount; x++) {
                ...
                if (!MiniGamePlayerControlSystem[x].PlayerShip && !MiniGamePlayerControlSystem[x].NumberOfLives) {
                    if (MiniGamePlayerControlSystem[x].SpectatorObject &&
                        Commands->Get_Shield_Strength(MiniGamePlayerControlSystem[x].SpectatorObject) != 0.0f)
            ...
            if (!ContinueGame) {
                ...
                for (int x = 0; x < MaxGamePlayerCount; x++) {
                    ...
                    RenCometBustersScoreSystem::CMTBHighScoresNode *TempNode = RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                            MiniGamePlayerControlSystem[x].PlayerID);
                    ...
                    } else if (MiniGamePlayerControlSystem[x].SelectedPerk)
                        ...
                        if (teamHasPerks) {
                            if (TempNode->TeamHighestLevel < JMG_CMTB_Main_Game_Control::CMTBLevel) {
                                ...
                                sprintf(ScoreMessage,
                                        "You scored %.2f points and through teamwork made it to a new high level as a team: Level %d while using perks!",
                                        MiniGamePlayerControlSystem[x].Score, JMG_CMTB_Main_Game_Control::CMTBLevel);
                                TempNode->TeamHighestLevel = JMG_CMTB_Main_Game_Control::CMTBLevel;
                                PerkSystemIncreasePlayerPerkUnlockAmount(Player, 61,
                                                                         JMG_CMTB_Main_Game_Control::CMTBLevel);
                            } else
                                sprintf(ScoreMessage,
                                        "You scored %.2f points and through teamwork made it to Level %d while using perks!",
                                        MiniGamePlayerControlSystem[x].Score, JMG_CMTB_Main_Game_Control::CMTBLevel);
                        } else {
                            if (TempNode->HighestCleanTeamLevel < JMG_CMTB_Main_Game_Control::CMTBLevel) {
                                Create_2D_Sound_Player(Player, "SFX.Primary_Mission_Complete");
                                sprintf(ScoreMessage,
                                        "You scored %.2f points and through teamwork made it to a new high level as a team: Level %d!",
                                        MiniGamePlayerControlSystem[x].Score, JMG_CMTB_Main_Game_Control::CMTBLevel);
                                ...
                                sprintf(ScoreMessage,
                                        "You scored %.2f points and through teamwork made it to Level %d!",
                                        MiniGamePlayerControlSystem[x].Score, JMG_CMTB_Main_Game_Control::CMTBLevel);
                        }
                    else if (teamHasPerks) {
                        if (TempNode->HighestLevel < JMG_CMTB_Main_Game_Control::CMTBLevel) {
                            Create_2D_Sound_Player(Player, "SFX.Primary_Mission_Complete");
                            sprintf(ScoreMessage,
                                    "You scored %.2f points and made it to a new personal highest level: Level %d while using perks!",
                                    MiniGamePlayerControlSystem[x].Score, JMG_CMTB_Main_Game_Control::CMTBLevel);
                            ...
                            sprintf(ScoreMessage, "You scored %.2f points and made it to level %d while using perks!",
                                    MiniGamePlayerControlSystem[x].Score, MiniGamePlayerControlSystem[x].HighestLevel);
                    } else {
                        if (TempNode->HighestCleanLevel < JMG_CMTB_Main_Game_Control::CMTBLevel) {
                            Create_2D_Sound_Player(Player, "SFX.Primary_Mission_Complete");
                            sprintf(ScoreMessage,
                                    "You scored %.2f points and made it to a new personal highest level: Level %d!",
                                    MiniGamePlayerControlSystem[x].Score, JMG_CMTB_Main_Game_Control::CMTBLevel);
                            TempNode->HighestCleanLevel = JMG_CMTB_Main_Game_Control::CMTBLevel;
                            PerkSystemIncreasePlayerPerkUnlockAmount(Player, 61, JMG_CMTB_Main_Game_Control::CMTBLevel);
                        } else
                            sprintf(ScoreMessage, "You scored %.2f points and made it to level %d",
                                    MiniGamePlayerControlSystem[x].Score, JMG_CMTB_Main_Game_Control::CMTBLevel);
                    }
            ...
            if (LastPlayerCount != ContinueGame)
                for (int x = 0; x < MaxGamePlayerCount; x++)
                    Commands->Set_Health(MiniGamePlayerControlSystem[x].SpectatorObject,
                                         (float) JmgUtility::MathClampInt(ContinueGame, 1, 8));
            LastPlayerCount = ContinueGame;
        }
        ...
        if (UFOSpawnTime) {
            ...
            if (!UFOSpawnTime) {
                ...
                if (CometCount != LastCometCount || NextLevelDelay == -1) {
                    ...
                    MoveablePhysClass *mphys = UFO->As_PhysicalGameObj()
                                               ? UFO->As_PhysicalGameObj()->Peek_Physical_Object()->As_MoveablePhysClass()
                                               : NULL;
    ...
    if (number == 5436) {//OutOfBoundsObjectsCheck
        Vector3 Pos = Commands->Get_Position(obj);
        ...
        for (int x = 0; x < MaxGamePlayerCount; x++) {
            ...
            if (MiniGamePlayerControlSystem[x].DoublePointsTime) {
                ...
                if (!MiniGamePlayerControlSystem[x].DoublePointsTime) {
                    ...
                    if (Driver) {
                        JmgUtility::DisplayChatMessage(Driver, 200, 200, 125,
                                                       "Your double points powerup has expired!");
...
void JMG_CMTB_Main_Game_Control::Create_Player_Ship(GameObject *obj, int PlayerNumber) {
    ...
    if (abs(MiniGamePlayerControlSystem[PlayerNumber].LastNewLifeScore -
            MiniGamePlayerControlSystem[PlayerNumber].Score) > 100000.0f) {// Give a new life
        ...
        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                MiniGamePlayerControlSystem[PlayerNumber].PlayerID)->NewLivesAcquired++;
        Set_Max_Shield_Strength(MiniGamePlayerControlSystem[PlayerNumber].SpectatorObject,
                                (float) JmgUtility::MathClampInt(
                                        MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives + 1, 5, 100));
        Commands->Set_Shield_Strength(MiniGamePlayerControlSystem[PlayerNumber].SpectatorObject,
                                      (float) MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives + 1);
        if (MiniGamePlayerControlSystem[PlayerNumber].PlayerShip) {
            if (Commands->Get_Max_Health(MiniGamePlayerControlSystem[PlayerNumber].PlayerShip) <
                MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives + 1)
                Set_Max_Health(MiniGamePlayerControlSystem[PlayerNumber].PlayerShip,
                               (float) MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives + 1);
            Commands->Set_Health(MiniGamePlayerControlSystem[PlayerNumber].PlayerShip,
                                 (float) MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives + 1);
        }
    }
    if (NextLevelDelay != -1 && MiniGamePlayerControlSystem[PlayerNumber].RespawnTime > 0 &&
        MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives > 0)
        MiniGamePlayerControlSystem[PlayerNumber].RespawnTime--;
    if (!MiniGamePlayerControlSystem[PlayerNumber].ReplacementShip &&
        MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives > 0) {// Recreate the replacement ship if it was killed
        char Spot[32];
        sprintf(Spot, "CreateS%d", PlayerNumber);
        MiniGamePlayerControlSystem[PlayerNumber].ReplacementShip = Commands->Create_Object_At_Bone(obj, "Player_Ships",
                                                                                                    Spot);
        char ShipName[32];
        ...
        Commands->Set_Animation(MiniGamePlayerControlSystem[PlayerNumber].ReplacementShip, ShipName, false, 0, 0, 0,
                                false);
        Commands->Set_Health(MiniGamePlayerControlSystem[PlayerNumber].ReplacementShip,
                             (float) MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives);
    }//TODO enable end of line if spawning broken
    if (NextLevelDelay != -1 && MiniGamePlayerControlSystem[PlayerNumber].RespawnTime <= 0 &&
        MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives > 0 &&
        !MiniGamePlayerControlSystem[PlayerNumber].PlayerShip)//5 && Get_Vehicle_Driver(MiniGamePlayerControlSystem[PlayerNumber].SpectatorObject))
    {
        ...
        MiniGamePlayerControlSystem[PlayerNumber].ShipObject = RenCometBustersList.FindObject(
                MiniGamePlayerControlSystem[PlayerNumber].PlayerShip);
        ...
        Commands->Set_Health(MiniGamePlayerControlSystem[PlayerNumber].PlayerShip,
                             (float) MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives);
        Commands->Set_Obj_Radar_Blip_Shape(MiniGamePlayerControlSystem[PlayerNumber].PlayerShip,
                                           RADAR_BLIP_SHAPE_VEHICLE);
        Commands->Set_Health(MiniGamePlayerControlSystem[PlayerNumber].ReplacementShip, 5.0f);
        Set_Max_Shield_Strength(MiniGamePlayerControlSystem[PlayerNumber].SpectatorObject,
                                (float) JmgUtility::MathClampInt(
                                        MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives, 5, 100));
        Commands->Set_Shield_Strength(MiniGamePlayerControlSystem[PlayerNumber].SpectatorObject,
                                      (float) MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives);
        MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives--;
        switch (MiniGamePlayerControlSystem[PlayerNumber].SelectedPerk) {
            case 6:
                if (Commands->Get_Random(0.0f, 1.0f) < 0.2)
                    Commands->Send_Custom_Event(obj, MiniGamePlayerControlSystem[PlayerNumber].PlayerShip,
                                                CMTBPowerUpCustomMessage + 10, -1337, 1.0f);
                break;
            case 7:
                if (Commands->Get_Random(0.0f, 1.0f) < 0.05)
                    Commands->Send_Custom_Event(obj, MiniGamePlayerControlSystem[PlayerNumber].PlayerShip,
                                                CMTBPowerUpCustomMessage + 4, -1337, 1.0f);
                break;
            case 8:
                if (Commands->Get_Random(0.0f, 1.0f) < 0.025)
                    Commands->Send_Custom_Event(obj, MiniGamePlayerControlSystem[PlayerNumber].PlayerShip,
                                                CMTBPowerUpCustomMessage + 3, -1337, 1.0f);
                break;
            case 9:
                if (Commands->Get_Random(0.0f, 1.0f) < 0.025)
                    Commands->Send_Custom_Event(obj, MiniGamePlayerControlSystem[PlayerNumber].PlayerShip,
                                                CMTBPowerUpCustomMessage + 15, -1337, 1.0f);
                break;
        }
    } else if (MiniGamePlayerControlSystem[PlayerNumber].PlayerShip &&
               Get_Vehicle(Player) != MiniGamePlayerControlSystem[PlayerNumber].PlayerShip) {
        ...
        if (Player) {
            Commands->Set_Position(Player,
                                   Commands->Get_Position(MiniGamePlayerControlSystem[PlayerNumber].SpectatorObject));
...
void JMG_CMTB_UFO_Random_Fire_Control::Timer_Expired(GameObject *obj, int number) {
    ...
    if (number == 5433) {
        ...
        Commands->Start_Timer(obj, this,
                              Commands->Get_Random(0.25, 1.25) * JMG_CMTB_Main_Game_Control::DifficultyMultiplierInv,
                              5434);
    }
    if (number == 5434) {
...
void JMG_CMTB_UFO_Random_Fire_Control::Killed(GameObject *obj, GameObject *damager) {
    if (!(Get_Int_Parameter("UFOBoss") && RenCometBustersList.CountType(UFOBoss) == 1))
        if (Commands->Get_Random_Int(0, 100) <
            20 + (JMG_CMTB_Main_Game_Control::CMTBLevel * 0.2))// was originally 0.1 and 10
...
void JMG_CMTB_Ship_Control_Script::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    ...
    if (message == 7154148) {
        for (int x = 0; x < MaxGamePlayerCount; x++)
            if (MiniGamePlayerControlSystem[x].PlayerShip == obj) {
                switch (MiniGamePlayerControlSystem[x].SelectedPerk) {
                    case 1:
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125, "Selected Special Ability: Extra Life");
                        break;
                    case 2:
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                                       "Selected Special Ability: Hyperjump Powerup");
                        break;
                    case 3:
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                                       "Selected Special Ability: Double Points");
                        break;
                    case 4:
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125, "Selected Special Ability: Energy Regen");
                        break;
                    case 5 :
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                                       "Selected Special Ability: Weapons Upgrade");
                        break;
                    case 6:
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125, "Selected Special Ability: Drone");
                        break;
                    case 7:
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                                       "Selected Special Ability: High Explosive Warhead");
                        break;
                    case 8:
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125, "Selected Special Ability: SOS Beacon");
                        break;
                    case 9:
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125, "Selected Special Ability: Drone Swarm");
                        break;
                    default:
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125, "Selected Special Ability: None");
                        break;
                }
                char inventory[220];
                sprintf(inventory, "Hyper Drive Charge %.0f.",
                        MiniGamePlayerControlSystem[x].HyperspaceDriveCharge / 5.0f * 100.0f);
                ...
                if (MiniGamePlayerControlSystem[x].DoublePointsTime) {
                    sprintf(inventory, "Double Points time %d seconds.",
                            MiniGamePlayerControlSystem[x].DoublePointsTime);
                    JmgUtility::DisplayChatMessage(sender, 200, 200, 125, inventory);
                }
    ...
    if (message == 77454830) {
        if (!ForfeitLives) {
            JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                           "Press ForfeitLives key again to forfeit your lives!");
            ForfeitLives = 5;
        } else {
            for (int x = 0; x < MaxGamePlayerCount; x++)
                if (MiniGamePlayerControlSystem[x].PlayerShip == obj) {
                    ...
                    if (!theplayer)
                        return;
                    char playermsg[256];
                    sprintf(playermsg, "%s forfeited %s lives!", Get_Player_Name(theplayer),
                            (JmgUtility::JMG_Get_Sex(theplayer) == 'A' ? "his" : "her"));
                    for (int y = 1; y < 128; y++) {
    ...
    if (message == 77454812) {
        ...
        if (MyObject->Type == PlayerShield)
            ...
            for (int x = 0; x < MaxGamePlayerCount; x++)
                if (MiniGamePlayerControlSystem[x].PlayerShip == obj)
                    RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                            MiniGamePlayerControlSystem[x].PlayerID)->ShieldRaisedCount++;
        } else {
    ...
    if (message == 77454813) {
        ...
        if (MyObject->Type == PlayerShield) {
            ...
            for (int x = 0; x < MaxGamePlayerCount; x++)
                if (MiniGamePlayerControlSystem[x].PlayerShip == obj)
                    RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                            MiniGamePlayerControlSystem[x].PlayerID)->ShipCloakedCount++;
    ...
    if (message == 77454815 && MyObject->Type != PlayerSuper) {
        if (EngineDisabled) {
            JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                           "Cannot deploy a High Yield Warhead while engines are off-line!");
            Create_2D_Sound_Player(sender, "SFX.Comet_Busters_UI_Failed_Sound");
            return;
        }
        if (MyObject->Type == PlayerShipDisrupter) {
            JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                           "Cannot deploy a High Yield Warhead while disrupter is active!");
            Create_2D_Sound_Player(sender, "SFX.Comet_Busters_UI_Failed_Sound");
            return;
        }
        ...
        for (int x = 0; x < MaxGamePlayerCount; x++)
            if (MiniGamePlayerControlSystem[x].PlayerShip == obj)
                if (MiniGamePlayerControlSystem[x].SuperBombs) {
                    RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                            MiniGamePlayerControlSystem[x].PlayerID)->HEWUsed++;
                    MiniGamePlayerControlSystem[x].SuperBombs--;
                } else {
                    JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                                   "You do not have any high yield warheads to deploy!");
                    Create_2D_Sound_Player(sender, "SFX.Comet_Busters_UI_Failed_Sound");
                    return;
                }
        for (int x = 1; x < 128; x++) {
            ...
            if (!Player)
                continue;
            JmgUtility::DisplayChatMessage(Player, 200, 125, 125,
                                           "//////Warning: A HIGH YIELD WARHEAD IS BEING CHARGED!\\\\\\\\\\\\");
        ...
        for (int x = 0; x < MaxGamePlayerCount; x++)
            if (MiniGamePlayerControlSystem[x].PlayerShip == obj) {
                switch (message - CMTBPowerUpCustomMessage) {
                    ...
                    case 2:
                        if (Power >= 100) {
                            JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                           "Energy Cell Stocks are full!");
                        ...
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->PowerPowerups++;
                    ...
                    case 3:
                        if (MiniGamePlayerControlSystem[x].HasSOSBeacon) {
                            JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                           "You can only carry one SOS Beacon at a time!");
                            goto SHIPFAILEDTOGETPOWERUP;
                        }
                        PerkSystemIncreasePlayerPerkUnlockAmount(Get_Vehicle_Driver(obj), 112, 1);
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->SOSBeacons++;
                        MiniGamePlayerControlSystem[x].HasSOSBeacon = true;
                        JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                       "You have aquired a SOS Beacon!");
                    ...
                    case 4:
                        if (MiniGamePlayerControlSystem[x].SuperBombs >= MaxNumberOfHighYieldWarheads) {
                            JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                           "Sorry, your cargo bay is full!");
                            goto SHIPFAILEDTOGETPOWERUP;
                        }
                        PerkSystemIncreasePlayerPerkUnlockAmount(Get_Vehicle_Driver(obj), 111, 1);
                        MiniGamePlayerControlSystem[x].SuperBombs++;
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->HEWPowerups++;
                        JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                       "You have aquired a high yield warhead!");
                        Create_2D_Sound_Player(Get_Vehicle_Driver(obj), "SFX.Comet_Busters_Pickup_Powerup");
                        break;
                    case 5:
                        PerkSystemIncreasePlayerPerkUnlockAmount(Get_Vehicle_Driver(obj), 105, 1);
                        JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                       "You have aquired another life!");
                        ...
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->ExtraLifePowerups++;
                        Set_Max_Shield_Strength(MiniGamePlayerControlSystem[x].SpectatorObject,
                                                (float) JmgUtility::MathClampInt(
                                                        MiniGamePlayerControlSystem[x].NumberOfLives + 1, 5, 100));
                        Commands->Set_Shield_Strength(MiniGamePlayerControlSystem[x].SpectatorObject,
                                                      (float) MiniGamePlayerControlSystem[x].NumberOfLives + 1);
                        if (MiniGamePlayerControlSystem[x].PlayerShip) {
                            if (Commands->Get_Max_Health(MiniGamePlayerControlSystem[x].PlayerShip) <
                                MiniGamePlayerControlSystem[x].NumberOfLives + 1)
                                Set_Max_Health(MiniGamePlayerControlSystem[x].PlayerShip,
                                               (float) MiniGamePlayerControlSystem[x].NumberOfLives + 1);
                            Commands->Set_Health(MiniGamePlayerControlSystem[x].PlayerShip,
                                                 (float) MiniGamePlayerControlSystem[x].NumberOfLives + 1);
                        }
                        Create_2D_Sound_Player(Get_Vehicle_Driver(obj), "SFX.Comet_Busters_Pickup_Powerup");
                        break;
                    ...
                    case 11:
                        ...
                        if (MiniGamePlayerControlSystem[x].DoublePointsTime) {
                            RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                    MiniGamePlayerControlSystem[x].PlayerID)->DoublePointsPoints += (int) Points;
                            Points *= 2;
                        }
                        ...
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->PointPowerups++;
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->PowerupPoints += Points;
                        break;
                    case 10:
                        if (MiniGamePlayerControlSystem[x].DronePowerup) {
                            JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                           "You can only carry one Drone at a time!");
                            goto SHIPFAILEDTOGETPOWERUP;
                        }
                        PerkSystemIncreasePlayerPerkUnlockAmount(Get_Vehicle_Driver(obj), 110, 1);
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->PickedUpDrone++;
                        MiniGamePlayerControlSystem[x].DronePowerup = true;
                        JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                       "You have aquired a Drone!");
                        Create_2D_Sound_Player(Get_Vehicle_Driver(obj), "SFX.Comet_Busters_Pickup_Powerup");
                        break;
                    case 12:
                        PerkSystemIncreasePlayerPerkUnlockAmount(Get_Vehicle_Driver(obj), 50, 1);
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->RegenPowerups++;
                        RapidPowerRegen = 120;
                        JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                       "You have aquired a rapid energy cell regen for 2 minutes!");
                        Create_2D_Sound_Player(Get_Vehicle_Driver(obj), "SFX.Comet_Busters_Pickup_Powerup_Health");
                        break;
                    case 13:
                        PerkSystemIncreasePlayerPerkUnlockAmount(Get_Vehicle_Driver(obj), 107, 1);
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->DoublePointsPowerups++;
                        MiniGamePlayerControlSystem[x].DoublePointsTime = 60;
                        JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                       "You have aquired a double points powerup for 1 minute!");
                        Create_2D_Sound_Player(Get_Vehicle_Driver(obj), "SFX.Comet_Busters_Pickup_Powerup_Points");
                        break;
                    case 14:
                        if (MiniGamePlayerControlSystem[x].HyperspaceDriveCharge >= 5) {
                            JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                           "Sorry, your hyperdrive is already fully charged!");
                            goto SHIPFAILEDTOGETPOWERUP;
                        }
                        ...
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->DeerPowerups++;
                        char drivemsg[220];
                        sprintf(drivemsg, "Hyperdrive %.0f%% charged%s", 100.0f *
                                                                         (MiniGamePlayerControlSystem[x].HyperspaceDriveCharge /
                                                                          (1.00f * MaxHyperdriveCharge)),
                                (CouldJump ? "." : "; automated systems are now on-line!"));
                    ...
                    case 15:
                        if (MiniGamePlayerControlSystem[x].DroneSwarmPowerup) {
                            JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                           "Your data base only has enough room for one drone swarm code!");
                            goto SHIPFAILEDTOGETPOWERUP;
                        }
                        PerkSystemIncreasePlayerPerkUnlockAmount(Get_Vehicle_Driver(obj), 113, 1);
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->PickedUpDroneSwarm++;
                        MiniGamePlayerControlSystem[x].DroneSwarmPowerup = true;
                        JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                       "You have aquired drone swarm calling codes!");
                        Create_2D_Sound_Player(Get_Vehicle_Driver(obj), "SFX.Comet_Busters_Pickup_Powerup");
                        break;
                    case 16:
                        ...
                        ExtraShotActive = 120;
                        JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 127, 255, 0,
                                                       "You have aquired a 2 minute weapon upgrade!");
                        Create_2D_Sound_Player(Get_Vehicle_Driver(obj), "SFX.Comet_Busters_Pickup_Powerup_Health");
                ...
                GameObject * Powerup = Commands->Create_Object("CMTB_Powerup_Object",
                                                               Vector3(Commands->Get_Position(obj).X +
                                                                       cos(Random * PI180) * 3.0f,
                                                                       Commands->Get_Position(obj).Y +
                                                                       sin(Random * PI180) * 3.0f, 0.25));
                if (param == -1337)
                    param = 0;
                char params[256];
    ...
    if (message == 77454818) {
        ...
        if (JMG_CMTB_Main_Game_Control::PlayerSOSBeaconID) {
            JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                           "Sorry, there is already a SOS Beacon deployed, please wait for it to burn out before deploying another!");
            Create_2D_Sound_Player(sender, "SFX.Comet_Busters_UI_Failed_Sound");
            return;
        }
        if (!(JMG_CMTB_Main_Game_Control::CMTBLevel % 25)) {
            JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                           "Due to sensor interference, it would do no good to deploy a beacon on this level!");
            Create_2D_Sound_Player(sender, "SFX.Comet_Busters_UI_Failed_Sound");
            return;
        }
        char Params[32];
            ...
            if (MiniGamePlayerControlSystem[x].PlayerShip == obj) {
                if (MiniGamePlayerControlSystem[x].HasSOSBeacon) {
                    if (MiniGamePlayerControlSystem[x].CargoShipIDs[6]) {
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                                       "Sorry, you can only deploy one SOS Beacon every 5 minutes!");
                        Create_2D_Sound_Player(sender, "SFX.Comet_Busters_UI_Failed_Sound");
                        return;
                    }
                    sprintf(Params, "%d", x);
                    ...
                    MiniGamePlayerControlSystem[x].HasSOSBeacon = false;
                    RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                            MiniGamePlayerControlSystem[x].PlayerID)->UsedSOSBeacons++;
                } else {
            ...
            if (MiniGamePlayerControlSystem[x].PlayerShip == obj) {
                if (MiniGamePlayerControlSystem[x].DronePowerup) {
                    if (MiniGamePlayerControlSystem[x].Drones[0] && MiniGamePlayerControlSystem[x].Drones[1]) {
                        JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                                       "Sorry, you can only have two drones following you at a time!");
                        Create_2D_Sound_Player(sender, "SFX.Comet_Busters_UI_Failed_Sound");
                        return;
                    }
                    ...
                    MiniGamePlayerControlSystem[x].DronePowerup = false;
                    RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                            MiniGamePlayerControlSystem[x].PlayerID)->UsedDrones++;
                    char params[32];
                    ...
                    JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                                   "Sorry, you don't have any Drones to deploy!");
                    Create_2D_Sound_Player(sender, "SFX.Comet_Busters_UI_Failed_Sound");
                    return;
                }
                break;
    ...
    if (message == 77454833) {
        if (JMG_CMTB_Main_Game_Control::DroneSwarmCount > 2) {
            JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                           "Sorry, a drone swarm is already present in this sector!");
            Create_2D_Sound_Player(sender, "SFX.Comet_Busters_UI_Failed_Sound");
            return;
        }
        if (MyObject->Type == PlayerShipJumping) {
            JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                           "Cannot transmit drone swarm call codes while in hyperspace!");
            Create_2D_Sound_Player(sender, "SFX.Comet_Busters_UI_Failed_Sound");
            return;
        }
        for (int x = 0; x < MaxGamePlayerCount; x++)
            if (MiniGamePlayerControlSystem[x].PlayerShip == obj) {
                if (MiniGamePlayerControlSystem[x].DroneSwarmPowerup) {
                    ...
                    MiniGamePlayerControlSystem[x].DroneSwarmPowerup = false;
                    RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                            MiniGamePlayerControlSystem[x].PlayerID)->UsedDronesSwarm++;
                    char params[32];
                    ...
                    JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                                   "Sorry, you don't have the codes to call a drone swarm!");
                    Create_2D_Sound_Player(sender, "SFX.Comet_Busters_UI_Failed_Sound");
                    return;
                }
                break;
    ...
    if (message >= 77454820 && message <= 77454827) {
        ...
        if (!MiniGamePlayerControlSystem[MyPlayerNumber].NumberOfLives) {
            JmgUtility::DisplayChatMessage(sender, 200, 125, 200,
                                           "Sorry, you don't have any extra lives to grant to other players!");
            return;
        }
        char LifeMsg[220];
        GameObject * OtherPlayer = Commands->Find_Object(MiniGamePlayerControlSystem[PlayerNumber].GamePlayerID);
        if (PlayerRequestingALife[PlayerNumber] && MiniGamePlayerControlSystem[MyPlayerNumber].PlayerShip &&
            MiniGamePlayerControlSystem[MyPlayerNumber].NumberOfLives) {
            RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                    MiniGamePlayerControlSystem[MyPlayerNumber].PlayerID)->GrantedOtherPlayersLives++;
            RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                    MiniGamePlayerControlSystem[Get_Player_ID(OtherPlayer)].PlayerID)->ReceivedOtherPlayersLives++;
            PlayerRequestingALife[PlayerNumber] = false;
            MiniGamePlayerControlSystem[MyPlayerNumber].NumberOfLives--;
            Set_Max_Health(MiniGamePlayerControlSystem[MyPlayerNumber].PlayerShip, (float) JmgUtility::MathClampInt(
                    MiniGamePlayerControlSystem[MyPlayerNumber].NumberOfLives + 1, 5, 100));
            Commands->Set_Health(MiniGamePlayerControlSystem[MyPlayerNumber].PlayerShip,
                                 (float) MiniGamePlayerControlSystem[MyPlayerNumber].NumberOfLives + 1);
            Set_Max_Shield_Strength(MiniGamePlayerControlSystem[MyPlayerNumber].SpectatorObject,
                                    (float) JmgUtility::MathClampInt(
                                            MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives + 1, 5, 100));
            Commands->Set_Shield_Strength(MiniGamePlayerControlSystem[MyPlayerNumber].SpectatorObject,
                                          (float) MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives + 1);
            Commands->Apply_Damage(obj, 0.0f, "None", 0);
            MiniGamePlayerControlSystem[PlayerNumber].NumberOfLives++;
            sprintf(LifeMsg, "You have granted player %d (%s) another life, thanks for being a team player!",
                    PlayerNumber, Get_Player_Name_By_ID(Get_Player_ID(OtherPlayer)));
            JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 200, 125, 200, LifeMsg);
            if (Commands->Is_A_Star(OtherPlayer)) {
                sprintf(LifeMsg, "%s has granted you another life, try to make this one last!",
                        Get_Player_Name_By_ID(Get_Player_ID(sender)));
    ...
    if (message == 77454831) {
        ...
        for (int x = 0; x < MaxGamePlayerCount; x++)
            if (MiniGamePlayerControlSystem[x].PlayerShip == obj)
                if (MiniGamePlayerControlSystem[x].HyperspaceDriveCharge) {
                    RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                            MiniGamePlayerControlSystem[x].PlayerID)->JumpsUsed++;
                    MiniGamePlayerControlSystem[x].HyperspaceDriveCharge--;
                    char percentMessage[200];
                    sprintf(percentMessage, "(Hyperdrive charge at %.0f%%)", 100.0f *
                                                                             (MiniGamePlayerControlSystem[x].HyperspaceDriveCharge /
                                                                              (1.00f * MaxHyperdriveCharge)));
                    ...
                    if (Driver) {
                        char drivemsg[400];
                        sprintf(drivemsg,
                                "IMMINENT COLLISION CONDITION DETECTED! AUTOMATED HYPERSPACE JUMP TRIGGERED!%s",
                                MiniGamePlayerControlSystem[x].HyperspaceDriveCharge ? percentMessage : " ");
                        JmgUtility::DisplayChatMessage(Driver, 200, 125, 200, drivemsg);
                    }
                    if (!MiniGamePlayerControlSystem[x].HyperspaceDriveCharge) {
                        JmgUtility::DisplayChatMessage(Driver, 200, 125, 125,
                                                       "HYPER DRIVES DEPLETED, HYPERSPACE NO LONGER AVAILABLE!");
                        Create_2D_Sound_Player(Driver, "SFX.Comet_Busters_UI_Failed_Sound");
                    }
                    break;
                }
...
void JMG_CMTB_Ship_Control_Script::Timer_Expired(GameObject *obj, int number) {
    ...
    if (number == 453216) {
        ...
        if (ExtraShotActive) {
            ...
            for (int x = 0; x < MaxGamePlayerCount; x++)
                if (MiniGamePlayerControlSystem[x].PlayerShip == obj)
                    RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                            MiniGamePlayerControlSystem[x].PlayerID)->UpgadedWeaponTime++;
        }
        if (OverPoweredWeapons)
            DrainPower(obj);
    ...
    if (number == 453213) {
        ...
        float Direction =
                Commands->Get_Bone_Position(obj, "Backward").Z - Commands->Get_Bone_Position(obj, "Forward").Z;
...
void JMG_CMTB_Ship_Control_Script::Damaged(GameObject *obj, GameObject *damager, float damage) {
    if (!damage && damager == Get_Vehicle_Driver(obj) && !ChangingWeapons) {
        ...
        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                Get_Player_ID(Get_Vehicle_Driver(obj)))->RoundsFired++;
    ...
    for (int x = 0; x < MaxGamePlayerCount; x++)
        if (MiniGamePlayerControlSystem[x].PlayerShip == obj) {
            ...
            if (!Player)
                continue;
            RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                    MiniGamePlayerControlSystem[x].PlayerID)->Deaths++;
            if (!MiniGamePlayerControlSystem[x].NumberOfLives) {
                ...
                for (int y = 0; y < MaxGamePlayerCount; y++) {
                    ...
                    if (y == x)
                        ...
                        char LifeMsg[220];
                        sprintf(LifeMsg, "Player %d (%s) is out of lives!", x,
                                Get_Player_Name_By_ID(MiniGamePlayerControlSystem[x].PlayerID));
        ...
        if (Power <= 0) {
            ...
            if (Player) {
                for (int x = 0; x < MaxGamePlayerCount; x++)
                    if (MiniGamePlayerControlSystem[x].PlayerShip == obj)
                        RenCometBustersScoreControl.Get_Current_Player_Score_Node(
                                MiniGamePlayerControlSystem[x].PlayerID)->RanOutOfPowerCount++;
                JmgUtility::DisplayChatMessage(Get_Vehicle_Driver(obj), 200, 200, 125,
                                               "Power levels critical! Deactivating auxiliary systems!");
...
void JMG_CMTB_Comet_Script::Created(GameObject *obj) {
    ...
    if (Commands->Get_Random(0.0f, 1.0f) < 0.25f && JMG_CMTB_Main_Game_Control::CMTBLevel)
        Set_Current_Clip_Bullets(obj,
                                 max((int) ((Commands->Get_Random_Int(0, JMG_CMTB_Main_Game_Control::CMTBLevel) + 1) *
                                            JMG_CMTB_Main_Game_Control::DifficultyMultiplier), 1));
    else
        Set_Current_Clip_Bullets(obj, max((int) (JMG_CMTB_Main_Game_Control::CMTBLevel *
                                                 JMG_CMTB_Main_Game_Control::DifficultyMultiplier), 1));
...
void JMG_CMTB_Comet_Script::Destroyed(GameObject *obj) {
    ...
    if (!Get_Int_Parameter("Size")) {
        if (Commands->Get_Random_Int(0, 100) <
            10 + (JMG_CMTB_Main_Game_Control::CMTBLevel * 0.2))// was originally 0.1 and 8
            JMG_CMTB_Random_Ship_Powerup(Commands->Get_Position(obj));
        return;
    }
    int Number = 2;
    ...
    if (Get_Int_Parameter("Size") && JMG_CMTB_Main_Game_Control::CMTBLevel > 10 &&
        Commands->Get_Random_Int(0, 100) < (5.0f + JMG_CMTB_Main_Game_Control::CMTBLevel * 0.1f)) {
        for (int x = 0; x < Get_Int_Parameter("Size") * 2; x++) {
            ...
            MoveablePhysClass *mphys = Mine->As_PhysicalGameObj()
                                       ? Mine->As_PhysicalGameObj()->Peek_Physical_Object()->As_MoveablePhysClass()
                                       : NULL;
            if (mphys)
    ...
    char params[512];
    sprintf(params, "%d,%.3f %.3f %.3f",
            Get_Int_Parameter("Size") == 3 ? Get_Int_Parameter("Size") - 2 : Get_Int_Parameter("Size") - 1,
            Commands->Get_Position(obj).X, Commands->Get_Position(obj).Y,
            Get_Int_Parameter("Size") == 3 ? 0.5 : Commands->Get_Position(obj).Z);
            ...
            if (!Player)
                ...
                if (!RequestLifeDelay) {
                    ...
                    for (int x = 0; x < MaxGamePlayerCount; x++) {
                        ...
                        if (TempPlayer && (MiniGamePlayerControlSystem[x].NumberOfLives &&
                                           MiniGamePlayerControlSystem[x].PlayerShip) ||
                            (!MiniGamePlayerControlSystem[x].PlayerShip &&
                             MiniGamePlayerControlSystem[x].NumberOfLives > 1)) {
                            char LifeMsg[220];
                            sprintf(LifeMsg,
                                    "%s is out of lives, you can give them one of you own by pressing the ShipPlayer%d key!",
                                    Get_Player_Name_By_ID(MiniGamePlayerControlSystem[machineNumber].PlayerID),
                                    machineNumber);
            ...
            if (player) {
                ...
                if (readyPlayers) {
                    ...
                    if (!(MiniGamePlayerControlSystem[machineNumber].kickoutOfSpecTime % 10))
                        Set_HUD_Help_Text_Player_Text(player, 7233, "Press spacebar to start the game!",
                                                      Vector3(0.19599999f, 1.0f, 0.19599999f));
...
void JMG_CMTB_Spectator_Object::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == 7154148) {
        ...
        switch (MiniGamePlayerControlSystem[machineNumber].SelectedPerk) {
            case 1:
                ...
                break;
            case 2:
                JmgUtility::DisplayChatMessage(sender, 200, 200, 125, "Selected Special Ability: Hyperjump Powerup");
                break;
            case 3:
                JmgUtility::DisplayChatMessage(sender, 200, 200, 125, "Selected Special Ability: Double Points");
                break;
            case 4:
                JmgUtility::DisplayChatMessage(sender, 200, 200, 125, "Selected Special Ability: Energy Regen");
                break;
            case 5 :
                JmgUtility::DisplayChatMessage(sender, 200, 200, 125, "Selected Special Ability: Weapons Upgrade");
                break;
            case 6:
                JmgUtility::DisplayChatMessage(sender, 200, 200, 125, "Selected Special Ability: Drone");
                break;
    ...
    if (message == 77454814) {
        ...
        if (hasPerksToSelect && !MiniGamePlayerControlSystem[machineNumber].SecondSpace &&
            !MiniGamePlayerControlSystem[machineNumber].SelectedPerk) {
            JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                           "You have special abilities that you could assign, or press space again to start the round:");
            SpecialAbilitiesList(sender);
            MiniGamePlayerControlSystem[machineNumber].SecondSpace = true;
            return;
        }
        MiniGamePlayerControlSystem[machineNumber].isReady = true;
        Commands->Send_Custom_Event(Get_Vehicle_Driver(obj), Commands->Find_Object(Get_Int_Parameter("ControllerID")),
                                    57447, 0, 0);
    }
    ...
    if (message == CUSTOM_EVENT_VEHICLE_ENTERED) {
        ...
        if (!JMG_CMTB_Main_Game_Control::GameInProgress)
            if (!MiniGamePlayerControlSystem[machineNumber].endGamePadding)
                Set_HUD_Help_Text_Player_Text(sender, 7233, "Press spacebar to start the game!",
                                              Vector3(0.19599999f, 1.0f, 0.19599999f));
    }
    if (message == 77454817) {
        if (!JMG_CMTB_Main_Game_Control::GameInProgress) {
            JmgUtility::DisplayChatMessage(sender, 200, 125, 200,
                                           "A game must be in progress in order to request lives!");
            return;
        }
        if (MiniGamePlayerControlSystem[machineNumber].SpectatorObject == obj) {
            if (PlayerRequestingALife[machineNumber]) {
                ...
                PlayerRequestingALife[machineNumber] = false;
            } else if (!MiniGamePlayerControlSystem[machineNumber].NumberOfLives &&
                       !MiniGamePlayerControlSystem[machineNumber].PlayerShip) {
                int AvailablePlayers = 0;
                for (int y = 0; y < MaxGamePlayerCount; y++)
                    if ((MiniGamePlayerControlSystem[y].NumberOfLives && MiniGamePlayerControlSystem[y].PlayerShip) ||
                        (!MiniGamePlayerControlSystem[y].PlayerShip &&
                         MiniGamePlayerControlSystem[y].NumberOfLives > 1))
                        AvailablePlayers++;
                if (AvailablePlayers) {
                    RequestLifeDelay = 5;
                    PlayerRequestingALife[machineNumber] = true;
                    JmgUtility::DisplayChatMessage(sender, 200, 125, 200,
                                                   "Your request for a life has been dispatched to the other players in game!");
                } else
                    JmgUtility::DisplayChatMessage(sender, 200, 125, 200,
                                                   "Sorry, no one in game has any lives to spare!");
            } else
                JmgUtility::DisplayChatMessage(sender, 200, 125, 200,
                                               "Sorry, you must be out of lives before you can request more!");
        }
    }
    if (message >= 77454820 && message <= 77454829) {
        if (JMG_CMTB_Main_Game_Control::GameInProgress) {
            JmgUtility::DisplayChatMessage(sender, 200, 200, 125,
                                           "Sorry, you cannot change your special ability once the game is in progress.");
            return;
        }
        ResetCurrentAbility(sender);
...
void JMG_CMTB_Spectator_Object::SpecialAbilitiesList(GameObject *player) {
    if (CheckIfPlayerHasPerkUnlocked(player, 105))
        JmgUtility::DisplayChatMessage(player, 200, 200, 125,
                                       "1 - Start the game with an Extra Life and get an additional one every 25 levels.");
    ...
    if (CheckIfPlayerHasPerkUnlocked(player, 107))
        JmgUtility::DisplayChatMessage(player, 200, 200, 125,
                                       "3 - 16.5% chance of getting Double Points at the start of each level.");
    if (CheckIfPlayerHasPerkUnlocked(player, 108))
        JmgUtility::DisplayChatMessage(player, 200, 200, 125,
                                       "4 - 5% chance of getting Energy Regen at the start of each level.");
    if (CheckIfPlayerHasPerkUnlocked(player, 109))
        JmgUtility::DisplayChatMessage(player, 200, 200, 125,
                                       "5 - 5% chance of getting Weapons Upgrade at the start of each level.");
    ...
    if (CheckIfPlayerHasPerkUnlocked(player, 111))
        JmgUtility::DisplayChatMessage(player, 200, 200, 125,
                                       "7 - 5% chance of being granted a High Explosive Warhead on spawn.");
    if (CheckIfPlayerHasPerkUnlocked(player, 112))
        JmgUtility::DisplayChatMessage(player, 200, 200, 125,
                                       "8 - 2.5% chance of being granted a SOS Beacon on spawn.");
    if (CheckIfPlayerHasPerkUnlocked(player, 113))
        JmgUtility::DisplayChatMessage(player, 200, 200, 125,
                                       "9 - 2.5% chance of being granted a Drone Swarm on spawn.");
}
...
void JMG_CMTB_Arcade_Machine::Poked(GameObject *obj, GameObject *poker) {
    ...
    Commands->Send_Custom_Event(poker, Commands->Find_Object(Get_Int_Parameter("ControllerID")), 57448,
                                Get_Int_Parameter("MachineNumber"), 0.0f);
}
...
void JMG_CMTB_Renegade_Code::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if ((message >= 77454812 && message <= 77454819) || (message >= 77454820 && message <= 77454829) ||
        message == 7154148 || message == 77454830 || message == 77454833) {
        if (sender != obj)
            return;
...
void JMG_CMTB_Motion_Mine::Timer_Expired(GameObject *obj, int number) {
    if (number == 5437) {
        ...
        if (JmgUtility::SimpleDistance(Pos, LastPosition) > 0.05) {
            StuckCount = 0;
            Speed = max(0.1f, Clamp_Value(Speed + 0.05f * JMG_CMTB_Main_Game_Control::DifficultyMultiplierMine, 0.1f,
                                          (1.25f + JMG_CMTB_Main_Game_Control::CMTBLevel * 0.01f) *
                                          JMG_CMTB_Main_Game_Control::DifficultyMultiplierMine));
        } else {
            NewRandom = true;
            Speed = max(0.1f, 0.1f * JMG_CMTB_Main_Game_Control::DifficultyMultiplierMine);
            ...
            if (FollowObject) {
                StuckCount++;
                if (!(StuckCount % 10)) {
                    Pos.X += Commands->Get_Random(1.0f, 5.0f) *
                             (cos(Commands->Get_Random(-180.0f, 180.0f) * 3.14159265f / 180));
                    Pos.Y += Commands->Get_Random(1.0f, 5.0f) *
                             (sin(Commands->Get_Random(-180.0f, 180.0f) * 3.14159265f / 180));
                    Commands->Set_Position(obj, Pos);
                    LastPosition = Commands->Get_Position(obj);
                }
                if (StuckCount == 24)
        ...
        if (Target && !movingToLastSeenSpot) {
            ...
            if (!ArriveDistance)
                ...
                if (NewRandom) {
                    ...
                    TargetPos.X += Commands->Get_Random(5.0f, 20.0f) *
                                   (cos(Commands->Get_Random(-180.0f, 180.0f) * 3.14159265f / 180));
                    TargetPos.Y += Commands->Get_Random(5.0f, 20.0f) *
                                   (sin(Commands->Get_Random(-180.0f, 180.0f) * 3.14159265f / 180));
                    NewTarget = 40;
                }
    ...
    if (number == 5436) {
        ArriveDistance = 0;
        if (!movingToLastSeenSpot && LastTargetObject && LastTargetObject->Type == PlayerCloak &&
            Commands->Get_Random(0.0f, 1.0f) < 0.33f) {
        ...
        if (!movingToLastSeenSpot && (!LastTargetObject ||
                                      (LastTargetObject->AllowReplaceTime || LastTargetObject->Type == Mine ||
                                       LastTargetObject->Type == MineBoss) ||
                                      (newTargetObject && newTargetObject->ObjectID != LastTargetObject->ObjectID &&
                                       Commands->Get_Random(0.0f, 1.0f) < 0.05f))) {
            if (newTargetObject)
...
void JMG_CMTB_The_Moon_Script::Timer_Expired(GameObject *obj, int number) {
    if (number == 5437) {
        ...
        Commands->Start_Timer(obj, this,
                              Commands->Get_Random(5.0f, 8.75f) * JMG_CMTB_Main_Game_Control::DifficultyMultiplierInv,
                              5437);
    }
}
...
void JMG_CMTB_The_Moon_Script::Killed(GameObject *obj, GameObject *damager) {
    if (damager || !JMG_CMTB_Main_Game_Control::GameInProgress)//TODO remove this to remove end game lage blast
    {
        ...
        char params[512];
        sprintf(params, "%d,%.3f %.3f %.3f", 3, Commands->Get_Position(obj).X, Commands->Get_Position(obj).Y,
                Commands->Get_Position(obj).Z);
        for (int x = 0; x < Number; x++) {
...
void JMG_CMTB_Ship_Super_Weapon::Timer_Expired(GameObject *obj, int number) {
    ...
    if (number == 5438) {
        ...
        float NewDistance = Commands->Get_Distance(Commands->Get_Bone_Position(obj, "Pos150"),
                                                   Commands->Get_Position(obj));
        RenCometBustersList.KillInRange(Get_Vehicle_Driver(Ship), Commands->Get_Position(obj),
                                        LastDistance * LastDistance, NewDistance * NewDistance);
...
void JMG_CMTB_Powerup_Script::Created(GameObject *obj) {
    switch (Get_Int_Parameter("Custom")) {
        case 0:
            ...
            Commands->Set_Animation(obj, "p_Energy_0.p_Energy_0", false, 0, Get_Int_Parameter("LifeTime") * 30.0f,
                                    1899.0f, 0);
            break;
        case 1:
            Commands->Set_Model(obj, "p_Energy_1");
            Commands->Set_Animation(obj, "p_Energy_1.p_Energy_1", false, 0, Get_Int_Parameter("LifeTime") * 30.0f,
                                    1899.0f, 0);
            break;
        case 2:
            Commands->Set_Model(obj, "p_Energy_2");
            Commands->Set_Animation(obj, "p_Energy_2.p_Energy_2", false, 0, Get_Int_Parameter("LifeTime") * 30.0f,
                                    1899.0f, 0);
            break;
        case 3:
            Commands->Set_Model(obj, "p_SOSBeacon");
            Commands->Set_Animation(obj, "p_SOSBeacon.p_SOSBeacon", false, 0, Get_Int_Parameter("LifeTime") * 30.0f,
                                    1899.0f, 0);
            break;
        case 4:
            Commands->Set_Model(obj, "p_sw_HEW");
            Commands->Set_Animation(obj, "p_sw_HEW.p_sw_HEW", false, 0, Get_Int_Parameter("LifeTime") * 30.0f, 1899.0f,
                                    0);
            break;
        case 5:
            Commands->Set_Model(obj, "p_ExtraShip");
            Commands->Set_Animation(obj, "p_ExtraShip.p_ExtraShip", false, 0, Get_Int_Parameter("LifeTime") * 30.0f,
                                    1899.0f, 0);
            break;
        case 6:
            Commands->Set_Model(obj, "p_Points_0");
            Commands->Set_Animation(obj, "p_Points_0.p_Points_0", false, 0, Get_Int_Parameter("LifeTime") * 30.0f,
                                    1899.0f, 0);
            break;
        case 7:
            Commands->Set_Model(obj, "p_Points_1");
            Commands->Set_Animation(obj, "p_Points_1.p_Points_1", false, 0, Get_Int_Parameter("LifeTime") * 30.0f,
                                    1899.0f, 0);
            break;
        case 8:
            Commands->Set_Model(obj, "p_Points_2");
            Commands->Set_Animation(obj, "p_Points_2.p_Points_2", false, 0, Get_Int_Parameter("LifeTime") * 30.0f,
                                    1899.0f, 0);
            break;
        case 9:
            Commands->Set_Model(obj, "p_Points_3");
            Commands->Set_Animation(obj, "p_Points_3.p_Points_3", false, 0, Get_Int_Parameter("LifeTime") * 30.0f,
                                    1899.0f, 0);
            break;
        case 10:
            Commands->Set_Model(obj, "p_Drone");
            Commands->Set_Animation(obj, "p_Drone.p_Drone", false, 0, Get_Int_Parameter("LifeTime") * 30.0f, 1899.0f,
                                    0);
            break;
        case 11:
            Commands->Set_Model(obj, "p_Points_4");
            Commands->Set_Animation(obj, "p_Points_4.p_Points_4", false, 0, Get_Int_Parameter("LifeTime") * 30.0f,
                                    1899.0f, 0);
            break;
        case 12:
            Commands->Set_Model(obj, "p_Energy_Regen");
            Commands->Set_Animation(obj, "p_Energy_Regen.p_Energy_Regen", false, 0,
                                    Get_Int_Parameter("LifeTime") * 30.0f, 1899.0f, 0);
            break;
        case 13:
            Commands->Set_Model(obj, "P_DoublePoints");
            Commands->Set_Animation(obj, "P_DoublePoints.P_DoublePoints", false, 0,
                                    Get_Int_Parameter("LifeTime") * 30.0f, 1899.0f, 0);
            break;
        case 14:
        ...
        case 15:
            ...
            Commands->Set_Animation(obj, "p_Swarm.p_Swarm", false, 0, Get_Int_Parameter("LifeTime") * 30.0f, 1899.0f,
                                    0);
            break;
        case 16:
            Commands->Set_Model(obj, "p_weapUpgrade");
            Commands->Set_Animation(obj, "p_weapUpgrade.p_weapUpgrade", false, 0, Get_Int_Parameter("LifeTime") * 30.0f,
                                    1899.0f, 0);
            break;
    }
...
void JMG_CMTB_Cargo_Ship_Script::Created(GameObject *obj) {
    ...
    for (int x = 0; x < 6; x++) {
        ...
        SPOT = Commands->Create_Object("CMTB_Cargo_Ship_Turret", Vector3(0.0f, 0.0f, 0.0f));
                ...
                if (Player == damager) {
                    ...
                    if (!MiniGamePlayerControlSystem[x].CargoWarnedCount) {
                        ...
                        if (MiniGamePlayerControlSystem[x].CargoWarnedCount > 5)
                            JmgUtility::DisplayChatMessage(damager, 160, 127, 50,
                                                           "Galactic Cargo Ship: You were warned, open fire men!");
                        else
                            switch (MiniGamePlayerControlSystem[x].CargoTotalWarnedCount) {
                                case 0:
                                    JmgUtility::DisplayChatMessage(damager, 160, 127, 50,
                                                                   "Galactic Cargo Ship: Friendly fire! Repeat friendly fire! Please check your targets!");
                                    break;
                                case 1:
                                    JmgUtility::DisplayChatMessage(damager, 160, 127, 50,
                                                                   "Galactic Cargo Ship: Stop firing immanently! We are not your enemies here!");
                                    break;
                                case 2:
                                    JmgUtility::DisplayChatMessage(damager, 160, 127, 50,
                                                                   "Galactic Cargo Ship: Final warning, stop firing or we will return fire!");
                                    break;
                                case 3:
                                    JmgUtility::DisplayChatMessage(damager, 160, 127, 50,
                                                                   "Galactic Cargo Ship: You were warned, open fire men!");
                    ...
                    MiniGamePlayerControlSystem[x].CargoWarnedCount++;
                    if (MiniGamePlayerControlSystem[x].CargoWarnedCount > 5 ||
                        MiniGamePlayerControlSystem[x].CargoTotalWarnedCount > 2)
                        for (int y = 0; y < 6; y++) {
...
void JMG_CMTB_Cargo_Ship_Script::Killed(GameObject *obj, GameObject *damager) {
    ...
    if (SPOT) {
        ...
        for (int x = 0; x < MaxGamePlayerCount; x++) {
            ...
            if (!Player)
                continue;
            JmgUtility::DisplayChatMessage(Player, 160, 127, 50,
                                           "Galactic Cargo Ship: Structural integrity failing, everyone abandon ship!");
        }
    }
...
void JMG_CMTB_Cargo_Ship_Turret::Enemy_Seen(GameObject *obj, GameObject *seen) {
    ...
    if (HuntingPlayer || Commands->Is_A_Star(seen) || Get_Vehicle_Driver(seen) || presetId == PLAYERDRONE_ID ||
        presetId == 1000000074 || presetId == 1000000077)
        return;
        ...
        if (!Target) {
            ...
            if (TargetDistance > 10000)
                ...
                if (FireWeapons > 3) {
                    ...
                    Vector3 Movement = Vector3(LastPosition[0].X - LastPosition[1].X,
                                               LastPosition[0].Y - LastPosition[1].Y, 0.0f);
                    Vector3 Movement2 = Vector3(LastPosition[1].X - LastPosition[2].X,
                                                LastPosition[1].Y - LastPosition[2].Y, 0.0f);
                    Vector3 Acceleration = Vector3(SafeDiv(Movement.X, Movement2.X), SafeDiv(Movement.Y, Movement2.Y),
                                                   0.0f);
...
void JMG_CMTB_Player_Drone::Created(GameObject *obj) {
    ...
    sprintf(myAnimation, "FighterDrone%d.FighterDrone%d", Get_Int_Parameter("PlayerNumber"),
            Get_Int_Parameter("PlayerNumber"));
...
void JMG_CMTB_Player_Drone::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (!MyObject) {// Protect against crashes caused by accessing data nodes before ::Created has finished
...
void JMG_CMTB_Player_Drone::Timer_Expired(GameObject *obj, int number) {
    ...
    if (number == 5436) {
        ...
        if (MiniGamePlayerControlSystem[Get_Int_Parameter("PlayerNumber")].PlayerShip &&
            Get_Int_Parameter("PlayerEscort"))
            Object = RenCometBustersList.CheckForDroneTarget(obj, Commands->Get_Position(
                    MiniGamePlayerControlSystem[Get_Int_Parameter("PlayerNumber")].PlayerShip), &TargetSize);
        else
    ...
    if (number == 5437) {
        ...
        int ObjectID = RenCometBustersList.FindDroneAvoidObjectInRange(obj, &Pos, 56.25f + CurrentSpeed,
                                                                       &AvoidanceDistance);
        ...
        if (MyState != DroneReturning)
            if (MiniGamePlayerControlSystem[Get_Int_Parameter("PlayerNumber")].PlayerShip &&
                Get_Int_Parameter("PlayerEscort"))
                if (JmgUtility::SimpleDistance(Commands->Get_Position(
                        MiniGamePlayerControlSystem[Get_Int_Parameter("PlayerNumber")].PlayerShip), Pos) > 1000)
        ...
        AI_Update(obj);
        if ((Commands->Get_Bone_Position(obj, "LWing").Z > Pos.Z ||
             Commands->Get_Bone_Position(obj, "RWing").Z > Pos.Z) && !ShieldOverride) {
            for (int x = 0; x < 2; x++)
...
void JMG_CMTB_Player_Drone::AI_Update(GameObject *obj, bool Reset) {
    ...
    switch (MyState) {
        ...
        case DroneReturning:
            ...
            if (MiniGamePlayerControlSystem[Get_Int_Parameter("PlayerNumber")].PlayerShip &&
                Get_Int_Parameter("PlayerEscort")) {
                Facing = Commands->Get_Facing(
                        MiniGamePlayerControlSystem[Get_Int_Parameter("PlayerNumber")].PlayerShip);
                TargetPos = Commands->Get_Position(
                        MiniGamePlayerControlSystem[Get_Int_Parameter("PlayerNumber")].PlayerShip);
                if (MyState == DroneReturning &&
                    JmgUtility::SimpleDistance(Commands->Get_Position(obj), TargetPos) < 400.0f)
                    MyState = DroneGuarding;
            } else {
                if (!SearchTimeDelay) {
                    ...
                    Rotation = JmgUtility::MathClampDegrees(
                            Commands->Get_Facing(obj) - Commands->Get_Random(-45.0f, 45.0f)) * 3.14159265f / 180;
                    TargetPos.X += Distance * cos(Rotation);
                    TargetPos.Y += Distance * sin(Rotation);
                    params.Set_Basic(this, 100, 1);
                    params.Set_Movement(TargetPos, JmgUtility::MathClamp(
                                                Commands->Get_Distance(Commands->Get_Position(obj), TargetPos) / 10.0f, 1.0, 5), 0.0f,
                                        false);
            ...
            params.Set_Basic(this, 100, 1);
            params.Set_Movement(TargetPos, JmgUtility::MathClamp(
                    Commands->Get_Distance(Commands->Get_Position(obj), TargetPos) / 10.0f, 1.0, 5), 0.0f, false);
            params.ShutdownEngineOnArrival = true;
            Commands->Action_Goto(obj, params);
            break;
        case DroneAvoiding:
...
void JMG_CMTB_Poke_End_Map::Poked(GameObject *obj, GameObject *poker) {
    ...
    if (JMG_CMTB_Main_Game_Control::GameInProgress) {
        JmgUtility::DisplayChatMessage(poker, 0, 255, 0,
                                       "Please wait for the current game to end before pulling the switch.");
        return;
    }
    if (startDelay) {
...
ScriptRegistrant<JMG_Create_Ship_On_Poke> JMG_Create_Ship_On_Poke_Registrant("JMG_Create_Ship_On_Poke",
                                                                             "CreateSpot:vector3,ShipPreset=Comet_Busters_Ship_Blue:string,SpawnSpotExplosion=Explosion_Clear_Spawn_Point:string,InvisibleObject=Invisible_Spectator_Box:string");
ScriptRegistrant<JMG_Advanced_Bounce_Zone> JMG_Advanced_Bounce_Zone_Registrant("JMG_Advanced_Bounce_Zone",
                                                                               "XAmount=0:float,YAmount=0:float,ZAmount=0:float");
ScriptRegistrant<JMG_Ship_Random_Hyperspace> JMG_Ship_Random_Hyperspace_Registrant("JMG_Ship_Random_Hyperspace",
                                                                                   "HyperspacingModel=cometbshs:string,HyperSpaceSound=Ship_Teleport:string");
ScriptRegistrant<JMG_Random_Music> JMG_Random_Music_Registrant("JMG_Random_Music",
                                                               "Song1:string,Song1Time:float,Song2:string,Song2Time:float,Song3:string,Song3Time:float");
...
ScriptRegistrant<JMG_Kill_Self_And_Sender_On_Custom> JMG_Kill_Self_And_Sender_On_Custom_Registrant(
        "JMG_Kill_Self_And_Sender_On_Custom", "Message:int");
ScriptRegistrant<JMG_Ship_Shield_Control_Script> JMG_Ship_Shield_Control_Script_Registrant(
        "JMG_Ship_Shield_Control_Script", "");
ScriptRegistrant<JMG_Kill_Self_And_Sender_On_Custom_Ship> JMG_Kill_Self_And_Sender_On_Custom_Ship_Registrant(
        "JMG_Kill_Self_And_Sender_On_Custom_Ship", "Message:int");
ScriptRegistrant<JMG_Ship_Shield_Script> JMG_Ship_Shield_Script_Registrant("JMG_Ship_Shield_Script", "ID:int");
ScriptRegistrant<JMG_Renetbusters_Object> JMG_Renetbusters_Object_Registrant("JMG_Renetbusters_Object",
                                                                             "size:float,Power_Drain:int");
ScriptRegistrant<JMG_New_Ship_Weapon_Control_System> JMG_New_Ship_Weapon_Control_System_Registrant(
        "JMG_New_Ship_Weapon_Control_System", "");
ScriptRegistrant<JMG_Comet_Control_Script> JMG_Comet_Control_Script_Registrant("JMG_Comet_Control_Script",
                                                                               "Comet_Size:float,Spawn_Comets=0:int,Comet_Holder=null:string,Smaller_Comet=null:string");
ScriptRegistrant<JMG_Attach_And_Remove> JMG_Attach_And_Remove_Registrant("JMG_Attach_And_Remove",
                                                                         "Smaller_Comet:string");
ScriptRegistrant<JMG_User_Auto_Enter_Ship> JMG_User_Auto_Enter_Ship_Registrant("JMG_User_Auto_Enter_Ship",
                                                                               "Player_Ship=RB_Blue_Player_Ship:string");
...
ScriptRegistrant<JMG_CMTB_Ship_Control_Script> JMG_CMTB_Ship_Control_Script_Registrant("JMG_CMTB_Ship_Control_Script",
                                                                                       "");
ScriptRegistrant<JMG_CMTB_UFO_Random_Fire_Control> JMG_CMTB_UFO_Random_Fire_Control_Registrant(
        "JMG_CMTB_UFO_Random_Fire_Control", "UFOBoss:int");
ScriptRegistrant<JMG_CMTB_Comet_Script> JMG_CMTB_Comet_Script_Registrant("JMG_CMTB_Comet_Script",
                                                                         "Size=2:int,TargetPosition=0.0 0.0 0.0:Vector3");
ScriptRegistrant<JMG_CMTB_Zone_Realign_Enter> JMG_CMTB_Zone_Realign_Enter_Registrant("JMG_CMTB_Zone_Realign_Enter",
                                                                                     "Position=0.0 0.0 0.0:Vector3");
ScriptRegistrant<JMG_CMTB_Spectator_Object> JMG_CMTB_Spectator_Object_Registrant("JMG_CMTB_Spectator_Object",
                                                                                 "ControllerID:int,Number:int");
ScriptRegistrant<JMG_CMTB_Arcade_Machine> JMG_CMTB_Arcade_Machine_Registrant("JMG_CMTB_Arcade_Machine",
                                                                             "ControllerID=0:int,MachineNumber=0:int");
ScriptRegistrant<JMG_CMTB_Fake_Player_Attach_Script> JMG_CMTB_Fake_Player_Attach_Script_Registrant(
        "JMG_CMTB_Fake_Player_Attach_Script", "MySoldierID:int,Number:int");
...
ScriptRegistrant<JMG_CMTB_Ship_Super_Weapon> JMG_CMTB_Ship_Super_Weapon_Registrant("JMG_CMTB_Ship_Super_Weapon",
                                                                                   "ControllerShipID:int");
ScriptRegistrant<JMG_CMTB_Powerup_Script> JMG_CMTB_Powerup_Script_Registrant("JMG_CMTB_Powerup_Script",
                                                                             "Custom:int,LifeTime:int");
ScriptRegistrant<JMG_CMTB_Cargo_Ship_Script> JMG_CMTB_Cargo_Ship_Script_Registrant("JMG_CMTB_Cargo_Ship_Script",
                                                                                   "PlayerNumber:int");
ScriptRegistrant<JMG_CMTB_Cargo_Ship_Turret> JMG_CMTB_Cargo_Ship_Turret_Registrant("JMG_CMTB_Cargo_Ship_Turret", "");
ScriptRegistrant<JMG_CMTB_Player_Drone> JMG_CMTB_Player_Drone_Registrant("JMG_CMTB_Player_Drone",
                                                                         "PlayerNumber:int,Power:int,RePosition:int,PlayerEscort:int");
...
ScriptRegistrant<JMG_CMTB_Poke_Change_Difficulty> JMG_CMTB_Poke_Change_Difficulty_Registrant(
        "JMG_CMTB_Poke_Change_Difficulty", "");
