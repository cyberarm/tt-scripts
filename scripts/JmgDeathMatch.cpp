...
void JMG_DeathMatch_Spawn_Character::Timer_Expired(GameObject *obj, int number) {
    if (number == 1) {
        ...
        RP2DeathMatchSpawnSystem::RP2DeathMatchSpawnNode *spawnPoint = RP2DeathMatchSpawnControl.findSpawnPoint(
...
void JMG_DeathMatch_Teleport_Zone_Script::Entered(GameObject *obj, GameObject *enter)//For Doors
{
    if (enter->As_SoldierGameObj() && Commands->Is_A_Star(enter)) {
        char params[1024];
        sprintf(params, "%.2f %.2f %.2f,%f,%f,%s,%s,%s,%s,%s", Teleport_Pos.X, Teleport_Pos.Y, Teleport_Pos.Z,
                Get_Float_Parameter("Enter_Rotation"), Get_Float_Parameter("Exit_Rotation"),
                Get_Parameter("Enter_Animation"), Get_Parameter("Exit_Animation"),
                Get_Parameter("Enter_Path_Animation"), Get_Parameter("Exit_Path_Animation"),
                Get_Parameter("Bone_Attach_Name"));
...
void JMG_DeathMatch_Damage_When_Outside_Of_Range::Created(GameObject *obj) {
    ...
    vehicleDamageDistance =
            Get_Float_Parameter("Vehicle_Damage_Distance") * Get_Float_Parameter("Vehicle_Damage_Distance");
    aircraftWarnDistance =
            Get_Float_Parameter("Aircraft_Warn_Distance") * Get_Float_Parameter("Aircraft_Warn_Distance");
    aircraftDamageDistance =
            Get_Float_Parameter("Aircraft_Damage_Distance") * Get_Float_Parameter("Aircraft_Damage_Distance");
            ...
            if (!playerVehicle) {
                ...
                if (Is_VTOL(playerVehicle)) {
                    if (dist > aircraftWarnDistance) {
                        screenEffectOn[x] = true;
                        float TempDistCalc =
                                1 - (dist - maxSurviveDistance) / (aircraftWarnDistance - maxSurviveDistance);
                        Set_Screen_Fade_Color_Player(player, 0.25f, 0.0f, 0.0f, 0.1f);
                        Set_Screen_Fade_Opacity_Player(player, TempDistCalc, 0.1f);
                    } else if (screenEffectOn[x]) {
                        screenEffectOn[x] = false;
                    ...
                    if (dist > vehicleWarnDistance) {
                        screenEffectOn[x] = true;
                        float TempDistCalc =
                                1 - (dist - maxSurviveDistance) / (vehicleWarnDistance - maxSurviveDistance);
                        Set_Screen_Fade_Color_Player(player, 0.25f, 0.0f, 0.0f, 0.1f);
                        Set_Screen_Fade_Opacity_Player(player, TempDistCalc, 0.1f);
                    } else if (screenEffectOn[x]) {
                        screenEffectOn[x] = false;
...
void JMG_DeathMatch_Random_Weapon_Spawner::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == CUSTOM_EVENT_POWERUP_GRANTED && Commands->Get_ID(sender) == powerupId && spawnLimit && enabled) {
        ...
        if (Get_Float_Parameter("RespawnTimeVariation"))
            respawnTime += Commands->Get_Random(-Get_Float_Parameter("RespawnTimeVariation"),
                                                Get_Float_Parameter("RespawnTimeVariation"));
        powerupId = 0;
...
ScriptRegistrant<JMG_DeathMatch_Random_Spawn_Point> JMG_DeathMatch_Random_Spawn_Point_Registrant(
        "JMG_DeathMatch_Random_Spawn_Point", "");
ScriptRegistrant<JMG_DeathMatch_Teleport_Zone_Script> JMG_DeathMatch_Teleport_Zone_Script_Registrant(
        "JMG_DeathMatch_Teleport_Zone_Script",
        "TeleportLocation:Vector3,Object_ID=0:int,Enter_Rotation=0.0:float,Exit_Rotation=0.0:float,Enter_Animation=s_a_human.h_a_a0b1:string,Exit_Animation=s_a_human.walkinanim:string,Enter_Path_Animation=walkin:string,Exit_Path_Animation=walkout:string,Bone_Attach_Name=humanpos:string");
ScriptRegistrant<JMG_DeathMatch_Teleport_Zone_Script_Attach> JMG_DeathMatch_Teleport_Zone_Script_Attach_Registrant(
        "JMG_DeathMatch_Teleport_Zone_Script_Attach",
        "ExitPosition:Vector3,Enter_Rotation=0.0:float,Exit_Rotation=0.0:float,Enter_Animation=null:string,Exit_Animation=null:string,Enter_Path_Animation=null:string,Exit_Path_Animation=null:string,Bone_Attach_Name=null:string");
ScriptRegistrant<JMG_DeathMatch_Damage_When_Outside_Of_Range> JMG_DeathMatch_Damage_When_Outside_Of_Range_Registrant(
        "JMG_DeathMatch_Damage_When_Outside_Of_Range",
        "Warn_Distance=0.0:float,Damage_Distance=0.0:float,Vehicle_Warn_Distance=0.0:float,Vehicle_Damage_Distance=0.0:float,Aircraft_Warn_Distance=0.0:float,Aircraft_Damage_Distance=0.0:float,Max_Survive_Distance=0.0:float");
ScriptRegistrant<JMG_DeathMatch_Random_Weapon_Spawner> JMG_DeathMatch_Random_Weapon_Spawner_Registrant(
        "JMG_DeathMatch_Random_Weapon_Spawner",
        "PowerupPreset:string,InnitalSpawnDelay:float,RespawnTime:float,RespawnTimeVariation:float,SpawnLimit=-1:int,EnableMessage=0:int");
ScriptRegistrant<JMG_DeathMatch_Random_Weapon_Spawner_Attached> JMG_DeathMatch_Random_Weapon_Spawner_Attached_Registrant(
        "JMG_DeathMatch_Random_Weapon_Spawner_Attached", "ControllerId:int");
