...
void LoopedAnimationController::PlayAnimation(const char *animation, int startFrame, int endFrame, int nRepetitions) {
    if (m_objID != NULL) {
        if (GameObject *pObj = Commands->Find_Object(m_objID)) {
            ...
            // Start the animation
            Commands->Set_Animation(pObj, m_animationName, false, NULL, (float) m_startFrame, (float) m_endFrame,
                                    false);
        }
    }
}
...
void LoopedAnimationController::StopAnimation() {
    if (m_objID != NULL) {
        if (GameObject *pObj = Commands->Find_Object(m_objID)) {
            if (m_animationName != NULL && Get_Animation_Name(pObj) &&
                _stricmp(m_animationName, Get_Animation_Name(pObj)) == 0)
                pObj->As_PhysicalGameObj()->Clear_Animation();
