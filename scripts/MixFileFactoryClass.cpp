...
MixFileFactoryClass::MixFileFactoryClass
        (const char *filePath, FileFactoryClass *parentFileFactory)
        : FileFactoryClass(), Factory(0), UnkOffset(0), FileCount(0), MixFilenameOffset(0), IsValid(false),
          FileAdded(false) {
        ...
        if (f->Is_Available()) {
            ...
            if (IsValid) {
                ...
                if (IsValid) {
                    ...
                    IsValid = (f->Read(&FileInformation[0], FileCount * sizeof(FileInfoStruct)) ==
                               (int) (FileCount * sizeof(FileInfoStruct)));
                    if (IsValid) {
...
void BuildMixFile(const char *path, DynamicVectorClass<StringClass> filenames,
                  DynamicVectorClass<FileFactoryClass *> filefactories) {
