...
public:
    ...
    void Set_Visibility_Mode(Visibility_Mode_Type type) {
        Flags &= ~VISIBILITY_MODE_MASK;
        Flags |= ((uint) type << VISIBILITY_MODE_SHIFT) & VISIBILITY_MODE_MASK;
    }

    Visibility_Mode_Type Get_Visibility_Mode() const {
        return (Visibility_Mode_Type) ((Flags & VISIBILITY_MODE_MASK) >> VISIBILITY_MODE_SHIFT);
    }

    void Set_Collision_Group(Collision_Group_Type group) {
        Flags &= ~(uint) COLLISION_MASK;
        Flags |= (uint) group & COLLISION_MASK;
    }
