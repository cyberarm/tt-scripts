...
PresetClass::PresetClass() : m_NextPreset(0), m_PrevPreset(0), parent(0), definition(0), IsTemp(false), IsGood(true),
                             ParentID(0), m_DefinitionID(0) {
}
    ...
    while (cload.Open_Chunk()) {
        switch (cload.Cur_Chunk_ID()) {
            ...
            case 256:
                ...
                if (definition) {
                    definition->Set_User_Data((uint32)
                    this);
                }
                break;
            default:
...
PresetClass *
PresetMgrClass::Find_Next_Preset(PresetClass *preset, uint32 ClassID, int MatchType, bool FollowTwiddlers) {
