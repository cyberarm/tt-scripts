...
public:
    ...
    TT_INLINE explicit Quaternion(bool init) {
        if (init) {
            X = 0.0f;
            Y = 0.0f;
            Z = 0.0f;
    ...
    TT_INLINE explicit Quaternion(float a, float b, float c, float d) {
        X = a;
        Y = b;
        Z = c;
        W = d;
    ...
    friend TT_INLINE Quaternion

    operator*(const Vector3 &a, const Quaternion &b);
    ...
    TT_INLINE void Set(float a = 0.0, float b = 0.0, float c = 0.0, float d = 1.0) {
        X = a;
        Y = b;
        Z = c;
        W = d;
...
operator*(float scl, const Quaternion &a) {
    return Quaternion(scl * a[0], scl * a[1], scl * a[2], scl * a[3]);
}

TT_INLINE Quaternion

operator*(const Quaternion &a, float scl) {
    return scl * a;
}
