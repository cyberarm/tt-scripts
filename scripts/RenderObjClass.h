...
public:
    ...
    virtual void Set_Animation_Hidden(
            int onoff) { if (onoff) { Bits &= ~IS_NOT_ANIMATION_HIDDEN; } else { Bits |= IS_NOT_ANIMATION_HIDDEN; }}

    virtual int Is_Force_Visible() const { return Bits & IS_FORCE_VISIBLE; }

    virtual void
    Set_Force_Visible(int onoff) { if (onoff) { Bits |= IS_FORCE_VISIBLE; } else { Bits &= ~IS_FORCE_VISIBLE; }}

    virtual int Has_User_Lighting() const { return Bits & HAS_USER_LIGHTING; }

    virtual void
    Set_Has_User_Lighting(bool onoff) { if (onoff) { Bits |= HAS_USER_LIGHTING; } else { Bits &= ~HAS_USER_LIGHTING; }}
    ...
    virtual void Set_Collision_Type(int type) {
        ...
        Bits |= (type & COLLISION_TYPE_MASK) | 1;
    }
    ...
    void Set_Sub_Objects_Match_LOD(
            int onoff) { if (onoff) { Bits |= SUBOBJS_MATCH_LOD; } else { Bits &= ~SUBOBJS_MATCH_LOD; }}

    int Is_Sub_Objects_Match_LOD_Enabled(void) { return Bits & SUBOBJS_MATCH_LOD; }

    void Set_Sub_Object_Transforms_Dirty(
            bool onoff) { if (onoff) { Bits |= SUBOBJ_TRANSFORMS_DIRTY; } else { Bits &= ~SUBOBJ_TRANSFORMS_DIRTY; }}
    ...
    void
    Set_User_Lighting_Flag(); //Set the HAS_USER_LIGHTING flag appropriatly for this renderobj, intended to be called during vertex solve.

    void Register_For_Rendering(bool hint_static = false);

}; // 0088  00A0

TT_INLINE const SphereClass
&

RenderObjClass::Get_Bounding_Sphere(void) const {
...
TT_INLINE const AABoxClass
&

RenderObjClass::Get_Bounding_Box(void) const {
    if (!Bounding_Volumes_Valid()) {
        Update_Cached_Bounding_Volumes();
    }
...
TT_INLINE const Matrix3D
&

RenderObjClass::Get_Transform(void) const {
    Validate_Transform();
    return Transform;
}
...
RenderObjClass::Get_Transform(bool &is_transform_identity) const {
    Validate_Transform();
...
RenderObjClass::Get_Transform_No_Validity_Check(void) const {
    return Transform;
}
