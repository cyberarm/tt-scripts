...
public:
    ...
    Re_Init(const SoldierGameObjDef &definition);
    ...
    * the 'dummy' version of the driver is killed. */
    virtual void
    Apply_Damage_IgnoreVehicleCheck(const OffenseObjectClass &damager, float scale = 1.0f, int alternate_skin = -1);
    ...
    static float
    Say_Dynamic_Dialogue(int text_id, SoldierGameObj *speaker = NULL, AudibleSoundClass **sound_obj_to_return = NULL);
    ...
    void Set_Can_Steal_Vehicles(bool onoff) {
        CanStealVehicles = onoff;
        Set_Object_Dirty_Bit(BIT_RARE, true);
    }

    bool Can_Steal_Vehicles() { return CanStealVehicles; }

    void Set_Can_Drive_Vehicles(bool onoff) {
        CanDriveVehicles = onoff;
        Set_Object_Dirty_Bit(BIT_RARE, true);
    }

    bool Can_Drive_Vehicles() { return CanDriveVehicles; }

    void Set_Block_Action_Key(bool onoff) {
        BlockActionKey = onoff;
        Set_Object_Dirty_Bit(BIT_RARE, true);
    }

    bool Block_Action_Key() { return BlockActionKey; }

    void Set_Freeze(bool onoff) {
        Freeze = onoff;
        Set_Object_Dirty_Bit(BIT_RARE, true);
    }
    ...
    void Set_Can_Play_Damage_Animations(bool onoff) {
        CanPlayDamageAnimations = onoff;
        Set_Object_Dirty_Bit(BIT_RARE, true);
    }

    bool Can_Play_Damage_Animations() { return CanPlayDamageAnimations; }

    void Set_Scale_Across_Network(float scale) {
        NetworkRescale = scale;
        Set_Object_Dirty_Bit(BIT_RARE, true);
    }
    ...
    void Set_Override_Muzzle_Direction(bool override) {
        OverrideMuzzleDirection = override;
        Set_Object_Dirty_Bit(BIT_RARE, true);
    };

    bool Get_Override_Muzzle_Direction() { return OverrideMuzzleDirection; };

    void Set_Bot_Tag(const char *tag) {
        botTag = tag;
        Set_Object_Dirty_Bit(BIT_RARE, true);
    }
