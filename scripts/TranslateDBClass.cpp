...
REF_DEF2(TT_NOOP(HashTemplateClass<StringClass, TDBObjClass *>), TranslateDBClass::m_ObjectHash, 0x0085FCD0,
         0x0085EEB8);
...
bool TranslateDBClass::Save(ChunkSaveClass &csave) {
    ...
    for (int i = 0; i < m_ObjectList.Count(); i++) {
        ...
        if (obj) {
            ...
            if ((FilterType == FILTER_DISABLED) || ((FilterType == FILTER_IF_EQUAL) && !b) ||
                ((FilterType == FILTER_IF_NOT_EQUAL) && b)) {
...
TDBCategoryClass *TranslateDBClass::Find_Category(uint32
id)
{
for (
int i = 0;
...
TDBObjClass *TranslateDBClass::Get_First_Object(uint32
categoryId)
{
for (
int i = 0;
...
TDBObjClass *TranslateDBClass::Get_Next_Object(uint32
categoryId,
TDBObjClass *object
)
{
for (
int i = object->Get_ID() - 999;
i<m_ObjectList.
        ...
        */
        ...
        for (int i = 0; i < m_CategoryList.Count(); i++) {
            ...
            if (cat) {
                ...
                TDBObjClass * obj;
                for (obj = TranslateDBClass::Get_First_Object(
                        cat->Get_ID()); obj; obj = TranslateDBClass::Get_Next_Object(cat->Get_ID(), obj)) {
                    fwprintf_s(f, L"%u,%hs,%u,%hs\r\n", obj->Get_ID(), (const char *) obj->Get_ID_Desc(),
                               obj->Get_Sound_ID(), (const char *) obj->Get_Animation_Name());
...
void TranslateDBClass::Import_Strings(const char *filename) {
    ...
    if (f) {
        ...
        while (fgetws(input, input_max, f)) {
            ...
            while (wcslen(input) > 0 && (input[wcslen(input) - 1] == L' ' || input[wcslen(input) - 1] == L'\r' ||
                                         input[wcslen(input) - 1] == L'\n'))
            ...
            if (input[0] == L'[' && wcschr(input, L',') != NULL) {
                ...
                currentObj = NULL;
            } else if (currentCategory && (wcschr(input, L',') != NULL && (wcschr(input, L'=') == NULL ||
                                                                           wcschr(input, L',') <
                                                                           wcschr(input, L'=')))) {
                // String
                uint32 id;
            ...
            } else if (currentObj && (wcschr(input, L'=') != NULL &&
                                      (wcschr(input, L',') == NULL || wcschr(input, L'=') < wcschr(input, L',')))) {
                // String Translation
                uint32 lang_id;
