...
public:
    ...
    const VehicleGameObjDef SCRIPTS_API
    &
    ...
    void Set_Scripts_Can_Fire(bool b) {
        CanFire = b;
    ...
    void Set_Lock_Team(int team) {
        LockTeam = (char) team;
        Set_Object_Dirty_Bit(BIT_RARE, true);
    } //0 = nod, 1 = gdi, 2 = both
    int Get_Lock_Team() { return LockTeam; }

    void Set_Can_Be_Stolen(bool onoff) {
        CanBeStolen = onoff;
        Set_Object_Dirty_Bit(BIT_RARE, true);
    }
    ...
    void Set_Color(Vector3 &color) {
        if (UndergroundEffect) {
            UndergroundEffect->Set_Color(color);
