...
*LPFNSOSCALLBACK)        (
SoundSceneObjClass *sound_obj, uint32
user_param);
typedef void (_stdcall
*LPFNEOSCALLBACK)        (
SoundSceneObjClass *sound_obj, uint32
...
typedef void (_stdcall
*LPFNHEARDCALLBACK)    (
LogicalListenerClass *listener, LogicalSoundClass
*sound_obj,
uint32 user_param
);
typedef void (_stdcall
...
public:
    ...
    bool Load_From_Registry(const char *subkey_name, StringClass &device_name, bool &is_stereo, int &bits, int &hertz,
                            bool &sound_enabled, bool &music_enabled, bool &dialog_enabled,
                            bool &cinematic_sound_enabled, float &sound_volume, float &music_volume,
                            float &dialog_volume, float &cinematic_volume, int &speaker_types);
    ...
    bool Set_Max_2D_Sample_Count(int count = DEF_2D_SAMPLE_COUNT) {
        ...
        Allocate_2D_Handles();
        return true;
    } // Reimplementation. No idea whether this matches the original function!
    ...
    int
    Create_Instant_Sound(int definition_id, const Matrix3D &tm, RefCountClass *user_obj = NULL, uint32 user_data = 0,
                         int classid_hint = CLASSID_3D);

    int
    Create_Instant_Sound(const char *def_name, const Matrix3D &tm, RefCountClass *user_obj = NULL, uint32 user_data = 0,
                         int classid_hint = CLASSID_3D);
    ...
    AudibleSoundClass *
    Create_Continuous_Sound(const char *def_name, RefCountClass *user_obj = NULL, uint32 user_data = 0,
                            int classid_hint = CLASSID_3D);
...
public:
    ...
    SoundBufferClass *
    Create_Sound_Buffer(unsigned char *file_image, unsigned long bytes, const char *string_id, bool is_3d);

    bool Cache_Buffer(SoundBufferClass *buffer, const char *string_id);

    bool Free_Cache_Space(int bytes);

    static unsigned int AILCALLBACK

    File_Open_Callback(char const *filename, unsigned int *file_handle);

    static void AILCALLBACK

    File_Close_Callback(unsigned int file_handle);

    static signed int AILCALLBACK

    File_Seek_Callback(unsigned int file_handle, signed int offset, unsigned int type);

    static unsigned int AILCALLBACK

    File_Read_Callback(unsigned int file_handle, void *buffer, unsigned int bytes);

private:
#ifndef W3DVIEWER
    static REF_DECL(WWAudioClass
    *, _theInstance);
#else
