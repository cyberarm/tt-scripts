...
FARPROC GetProcAddressCheck(HMODULE hModule, LPCSTR lpProcName) {
    ...
    if (!x) {
        MessageBox(NULL, "Error getting function address from bandtest.dll", lpProcName,
...
*/
            if ((!Exe) || (Exe == 1)) {
                if (GetFileAttributes("scripts2.dll") == 0xFFFFFFFF) {
                    ...
                    MessageBox(NULL, "scripts2.dll was not found. Are you sure you installed scripts.dll correctly?",
            ...
            if (wwscripts) {
                ...
                MessageBox(NULL, "Unfortunately, there was an error loading scripts2.dll. Please report this bug!",
...
bool SCRIPTSAPI Set_Script_Commands(ScriptCommandsClass *commands) {
    ...
    Display_GDI_Player_Terminal_Player = (_Display_GDI_Player_Terminal_Player) Address(tt,
                                                                                       "New_Display_GDI_Player_Terminal_Player");
    Display_NOD_Player_Terminal_Player = (_Display_NOD_Player_Terminal_Player) Address(tt,
                                                                                       "New_Display_NOD_Player_Terminal_Player");
    Set_Screen_Fade_Color_Player = (_Set_Screen_Fade_Color_Player) Address(tt, "New_Set_Screen_Fade_Color_Player");
    Set_Screen_Fade_Opacity_Player = (_Set_Screen_Fade_Opacity_Player) Address(tt,
                                                                               "New_Set_Screen_Fade_Opacity_Player");
    ...
    Create_3D_WAV_Sound_At_Bone_Player = (_Create_3D_WAV_Sound_At_Bone_Player) Address(tt,
                                                                                       "New_Create_3D_WAV_Sound_At_Bone_Player");
    Create_3D_Sound_At_Bone_Player = (_Create_3D_Sound_At_Bone_Player) Address(tt,
                                                                               "New_Create_3D_Sound_At_Bone_Player");
    ...
    removeConnectionAcceptanceFilter = (removeConnectionAcceptanceFilterType) Address(tt,
    ...
    if (GetTTVersion() != TT_VERSION) {
        Console_Output("Fatal Error: scripts.dll requires a bandtest.dll version of %f. You are running %f.",
                       TT_VERSION, GetTTVersion());
        return false;
    }
