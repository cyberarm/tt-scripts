...
void dp88_AR_Deployable_Infantry::Created(GameObject *obj) {
    ...
    if (!Is_Valid_Preset(Get_Parameter("deployedObjectPreset"))) {
        Send_Message(255, 255, 255,
                     "dp88_AR_Deployable_Infantry :: Script aborting due to invalid deployedObjectPreset");
        Destroy_Script();
        return;
    }
    ...
    hasRookieWeaponPowerup = (strcmp(Get_Parameter("deployedWeaponPowerup"), "null") != 0 &&
                              Is_Valid_Preset(Get_Parameter("deployedWeaponPowerup"))) ? true : false;
    hasVeteranWeaponPowerup = (strcmp(Get_Parameter("deployedWeaponPowerup_veteran"), "null") != 0 &&
                               Is_Valid_Preset(Get_Parameter("deployedWeaponPowerup_veteran"))) ? true : false;
    hasEliteWeaponPowerup = (strcmp(Get_Parameter("deployedWeaponPowerup_elite"), "null") != 0 &&
                             Is_Valid_Preset(Get_Parameter("deployedWeaponPowerup_elite"))) ? true : false;
    ...
    int soundId = 0;
    cannotDeployStringId = (Is_Valid_String_ID(Get_Int_Parameter("cannotDeployStringId"))) ? Get_Int_Parameter(
            "cannotDeployStringId") : 0;
    if (cannotDeployStringId && (soundId = Get_String_Sound_ID(cannotDeployStringId)) != 0 &&
        Is_Valid_Preset_ID(soundId) && Find_Definition(soundId)->Get_Class_ID() == 0x5000)
...
void dp88_AR_Deployable_Infantry::KeyHook() {
    ...
    if (!obj || Get_Vehicle(obj) || obj->As_SoldierGameObj()->Is_On_Ladder() ||
        obj->As_SoldierGameObj()->Is_In_Elevator())
        return;
    ...
    if (deployed) {
        ...
        if (time(NULL) - lastDeploy < undeployTime) {
            ...
            str.Format("You cannot undeploy yet, you must wait at least %.*f seconds after deploying to undeploy.",
                       (((int) undeployTime) == undeployTime) ? 0 : 2, undeployTime);
        ...
        if (time(NULL) - lastDeploy < deployTime) {
            StringClass str(true);
            str.Format("You cannot deploy yet, you must wait at least %.*f seconds after undeploying to deploy again.",
                       (((int) deployTime) == deployTime) ? 0 : 2, deployTime);
            Send_Message_Player(obj, DP88_RGB_WARNING_MSG, str);
            return;
        }
        ...
        if (fabs(velocity.X) > 0.001 || fabs(velocity.Y) > 0.001 || fabs(velocity.Z) > 0.001
            || (closest && Commands->Get_Distance(Commands->Get_Position(obj), Commands->Get_Position(closest)) <
                           Get_Float_Parameter("deployedObjectSpaceRequired"))) {
...
void dp88_AR_Deployable_Infantry::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    ...
    if (sender == obj && type == CUSTOM_VETERANCY_PROMOTED) {
        ...
        if (deployed) {
            ...
            bool grantWeapon = ((currentVetLevel == 2 && hasEliteWeaponPowerup) ||
                                (oldVetLevel == 0 && hasVeteranWeaponPowerup)) ? true : false;
            if (grantWeapon) {
...
void dp88_AR_Deployable_Infantry::Deploy(GameObject *obj) {
    ...
    GameObject * deployedObject = Commands->Create_Object(Get_Parameter("deployedObjectPreset"),
...
void dp88_AR_CLEG_target::Damaged(GameObject *obj, GameObject *damager, float amount) {
    ...
    if (damager
        ...
        && (!Get_Vehicle(
            damager))                                // Check they are not in a vehicle, as current_weapon will not show this
...
void dp88_Ore_Miner::Created(GameObject *obj) {
  ...
  * which is required to prevent the default harvester AI taking over */
    if (m_bUseAI)
        Commands->Send_Custom_Event(obj, obj, CUSTOM_MINER_AI_SEARCH_FOR_ORE, 1,
                                    (float) Get_Int_Parameter("AI_Init_Delay"));
}
...
void dp88_Ore_Miner::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    ...
    if (type == CUSTOM_MINER_ENTERED_ORE_FIELD) {
        ...
        dp88_Ore_Field *pOreFieldScript = (!pOreField) ? NULL : (dp88_Ore_Field *) (Find_Script_On_Object(pOreField,
                                                                                                          "dp88_Ore_Field"));
        ...
        if (m_oreMined < m_oreCapacity && pOreFieldScript &&
            (pOreFieldScript->IsInfinite() || pOreFieldScript->NumOreUnits() > 0))
            ...
            if (m_bUseAI)
                ...
                if (m_oreMined < m_oreCapacity) {
                    StringClass str;
                    str.Format(
                            "The %s field is depleted, find another %s field or dock at the Refinery to process the %s you have collected so far...",
                            m_resourceName, m_resourceName, m_resourceName);
                    Send_Message_Player(Get_Vehicle_Driver(obj), DP88_RGB_GENERAL_MSG, str);
                } else {
                    StringClass str;
                    str.Format("Fully loaded with %s, dock at the Refinery to process the %s into credits",
        ...
        if (Get_Vehicle_Driver(obj) != NULL) {
            ...
            message.Format(
                    "%s unloaded successfully, you have earned %d credits for each player and %d points for yourself",
                    s.Peek_Buffer(), m_oreValue, m_oreValue / 10);
...
void dp88_Ore_Miner::DriveToOreField(GameObject *obj) {
    ...
    for (SLNode<GameObject> *oreFieldNode = oreFields.Head();
         oreFieldNode != NULL; oreFieldNode = oreFieldNode->Next()) {
        if (GameObject *oreField = oreFieldNode->Data()) {
            ...
            int availableUnits = min(m_oreCapacity, (pOreFieldScript->IsInfinite() ? m_oreCapacity
                                                                                   : (int) pOreFieldScript->NumOreUnits()));
            if (0 == availableUnits)
                continue;

            Vector3 fieldPosition = Commands->Get_Position(oreField);
            double fieldScore = (availableUnits * pOreFieldScript->GetOreValue()) / (0.5 * Commands->Get_Distance(
                    Commands->Get_Position(oreField), Commands->Get_Position(obj)));
...
void dp88_AR_Chrono_Miner::Damaged(GameObject *obj, GameObject *damager, float amount) {
    ...
    if (m_bUseAI && Commands->Get_Health(obj) < (Commands->Get_Max_Health(obj) *
                                                 (Get_Float_Parameter("emergencyChronoshiftHealthThreshold") / 100.0f))
...
void dp88_AR_Chrono_Miner::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    ...
    if (type == CUSTOM_EVENT_VEHICLE_ENTERED && driverId == NULL) {
        ...
        GameObject * refinery = Find_Refinery(Commands->Get_Player_Type(obj));
        if (refinery != NULL &&
            Commands->Get_Distance(Commands->Get_Position(refinery), Commands->Get_Position(obj)) > 150.0f) {
...
void dp88_AR_Chrono_Miner::KeyHook() {
    ...
    if (m_aiState != CMINER_AISTATE_CHRONOSHIFTING) {
        if (!Start_Chronoshift(obj))
            Send_Message_Player(Get_Vehicle_Driver(obj), 153, 204, 25,
                                "Unable to chronoshift, all target zones are unavailable...");
    }
}
...
void dp88_AR_Chrono_Miner::ReturnToRefinery(GameObject *obj) {
    ...
    if (!Start_Chronoshift(obj)) {
        ...
        Send_Message_Team(Get_Object_Type(obj), 153, 204, 25,
                          "The AI Chronominer was unable to chronoshift, please clear the area around the refinery");
...
bool dp88_AR_Chrono_Miner::Start_Chronoshift(GameObject *obj) {
    ...
    if (refinery != NULL) {
        ...
        for (SLNode<GameObject> *x = chronoZones.Head(); x != NULL; x = x->Next()) {
            ...
            if (Vector3::Distance_Squared(zonePos, refineryPos) > maxDist * maxDist ||
                !CanChronoshiftToLocation(obj, zonePos))
                continue;

            // Is this zone in use for another chronoshift operation? If so then we cannot use it
            chronozone_script = (dp88_AR_Chrono_Miner_Chronozone *) Find_Script_On_Object(zone,
            ...
            if (!chronozone_script || (chronozone_script->chronominer_id != NULL
                                       ...
                                       &&
                                       Commands->Get_Health(Commands->Find_Object(chronozone_script->chronominer_id)) >
                                       0)) {
                continue;
            }
            ...
            Commands->Send_Custom_Event(obj, obj, CUSTOM_CHRONO_MINER_DO_CHRONOSHIFT, Commands->Get_ID(zone),
                                        Get_Float_Parameter("chronoshift_time"));
            ...
            if (Is_Valid_Preset(Get_Parameter("chronoshift_out_effect_preset"))) {
                ...
                GameObject * effectObject = Commands->Create_Object(Get_Parameter("chronoshift_out_effect_preset"),
                                                                    Commands->Get_Position(obj));
            ...
            if (Is_Valid_Preset(Get_Parameter("chronoshift_in_effect_preset"))) {
                // Create effect object
                GameObject * effectObject = Commands->Create_Object(Get_Parameter("chronoshift_in_effect_preset"),
                                                                    Commands->Get_Position(zone));
...
void dp88_AR_Chrono_Miner::Do_Chronoshift(GameObject *obj, int target_zone_id) {
    ...
    if (zone != NULL) {
        ...
        dp88_AR_Chrono_Miner_Chronozone *chronozone_script = (dp88_AR_Chrono_Miner_Chronozone *) Find_Script_On_Object(
                zone, "dp88_AR_Chrono_Miner_Chronozone");
...
void dp88_AR_Chrono_Miner_Chronozone::Created(GameObject *obj) {
    ...
    if (!gameController) {
        Console_Output(
                "dp88_AR_Chrono_Miner_Chronozone - Unable to find Game Controller, unable to continue. Destroying script...\n");
        Destroy_Script();
        return;
    }
...
void dp88_Ore_Field::UpdateAnimationFrame(GameObject *pObj) {
    if (m_oreCapacity != 0 && m_strAnimation != NULL) {
        int frame = m_nAnimationFullFrame - (int) ceil(
                (m_nAnimationFullFrame - m_nAnimationEmptyFrame) * ((float) m_nOreUnits / m_oreCapacity));
...
void dp88_Aircraft_LandingZone_Aircraft::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == CUSTOM_TRANSITION_VTOL_LAND_ZONE && param == 1) {
        ...
        if (landingZoneCount == 1) {
            ...
            Commands->Set_Animation(obj, Get_Parameter("landing_anim_name"), false, 0,
                                    Get_Float_Parameter("landing_anim_first_frame"),
                                    Get_Float_Parameter("landing_anim_last_frame"), false);
        }
        ...
        if (landingZoneCount == 0) {
            ...
            Commands->Set_Animation(obj, Get_Parameter("landing_anim_name"), false, 0,
                                    Get_Float_Parameter("landing_anim_last_frame"),
                                    Get_Float_Parameter("landing_anim_first_frame"), false);
        }
...
void dp88_RemoteControlConsole::Poked(GameObject *obj, GameObject *poker) {
    ...
    if (pilotID == NULL && vehicleID == NULL) {
        ...
        if (!Create_Vehicle(Get_Parameter("vehiclePreset"), 0.0, Commands->Find_Object(pilotID),
                            Get_Object_Type(Commands->Find_Object(pilotID)))) {
...
void dp88_RemoteControlConsole::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    ...
    if (type == CUSTOM_REMOTECONTROL_CREATED && param == Get_Int_Parameter("remoteControlID") && vehicleID == NULL &&
        pilotID != NULL) {
...
void dp88_RemoteControlConsole::UpdateAnimation(GameObject *obj) {
    ...
    if (!m_bEnabled)
        m_pLoopedAnimCtrl->PlayAnimation(Get_Parameter("animationName"), Get_Int_Parameter("animFrameDisabled1"),
                                         Get_Int_Parameter("animFrameDisabled2"));

    else if (NULL != pilotID)
        m_pLoopedAnimCtrl->PlayAnimation(Get_Parameter("animationName"), Get_Int_Parameter("animFrameActive1"),
                                         Get_Int_Parameter("animFrameActive2"));

    else if (NULL != vehicleID) {
        m_pLoopedAnimCtrl->PlayAnimation(Get_Parameter("animationName"), Get_Int_Parameter("animFrameIdle1"),
                                         Get_Int_Parameter("animFrameIdle2"));
        bPokable = true;
    } else if (m_nChargeTime > 0)
        m_pLoopedAnimCtrl->PlayAnimation(Get_Parameter("animationName"), Get_Int_Parameter("animFrameCharging1"),
                                         Get_Int_Parameter("animFrameCharging2"));

    else {
        m_pLoopedAnimCtrl->PlayAnimation(Get_Parameter("animationName"), Get_Int_Parameter("animFrameAvailable1"),
                                         Get_Int_Parameter("animFrameAvailable2"));
        bPokable = true;
    }
...
void dp88_RemoteControlVehicle::Created(GameObject *obj) {
    ...
    Send_Custom_Event_To_Objects_With_Script(obj, "dp88_RemoteControlConsole", CUSTOM_REMOTECONTROL_CREATED,
                                             Get_Int_Parameter("remoteControlID"), 0);
}
...
void dp88_RemoteControlVehicle::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    ...
    if (type == CUSTOM_EVENT_VEHICLE_ENTERED) {
        ...
        if (Commands->Get_ID(sender) == pilotID) {
            ...
            // Report driver exit to our console
            if (consoleID != NULL)
                Commands->Send_Custom_Event(obj, Commands->Find_Object(consoleID), CUSTOM_REMOTECONTROL_DRIVEREXIT, 1,
                                            0);
        }
    }
...
void dp88_AR_Paradrop::Created(GameObject *pObj) {
    if (!pObj->As_SoldierGameObj()) {
        Console_Output(
                "[%d:%s:%s] Critical Error: This script is only compatible with soldier game objects. Destroying script...\n",
                Commands->Get_ID(pObj), Commands->Get_Preset_Name(pObj), this->Get_Name());
        Destroy_Script();
        return;
    }
    ...
    if (strlen(Get_Parameter("Animation")) > 0) {
        ...
        m_pAnimController->PlayAnimation(Get_Parameter("Animation"), Get_Int_Parameter("Animation_First_Frame"),
                                         Get_Int_Parameter("Animation_Last_Frame"),
...
void dp88_AR_Prism_Tower::calculateTowerMap() {
            ...
            for (int j = i + 1; j < MAX_TOWERS; ++j) {
                if (prismTowerIds[j] > 0) {
                    ...
                    if (CanAssistTower(t1, t2, ((dp88_AR_Prism_Tower *) prismTowerScripts[i])->primary_maxRange)) {
                        ...
                        towerConnections[(connectionCount * 3) + 2] = (int) Commands->Get_Distance(
                                Commands->Get_Position(t1), Commands->Get_Position(t2));
                        connectionCount++;
                    }
                }
            }
    ...
    for (int i = 0; i < connectionCount; ++i) {
        ...
        if (!calculateTowerMapPathSearch(sortedTowerConnections, connectionCount, sortedTowerConnections[i * 4],
                                         sortedTowerConnections[(i * 4) + 1]))
...
bool
dp88_AR_Prism_Tower::calculateTowerMapPathSearch(int *sortedConnections, int numConnections, int tower1, int tower2) {
    ...
    for (int i = 0; i < numConnections; ++i) {
        ...
        if (sortedConnections[(i * 4) + 3] != 1) {
            ...
            if (sortedConnections[i * 4] == tower1)
                bFound = calculateTowerMapPathSearch(sortedConnections, numConnections, tower2,
                                                     sortedConnections[(i * 4) + 1]);
            else if (sortedConnections[(i * 4) + 1] == tower1)
                bFound = calculateTowerMapPathSearch(sortedConnections, numConnections, tower2,
                                                     sortedConnections[i * 4]);
            else if (sortedConnections[i * 4] == tower2)
                bFound = calculateTowerMapPathSearch(sortedConnections, numConnections, tower1,
                                                     sortedConnections[(i * 4) + 1]);
...
void dp88_AR_Prism_Tower::Killed(GameObject *obj, GameObject *killer) {
    ...
    for (int i = 0; i < adjacentTowerCount; i++) {
        if (Commands->Find_Object(adjacentTowers[i]) != obj) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(adjacentTowers[i]), CUSTOM_PRISMTOWER_STOP_CHARGING,
                                        0, 0.0f);
        }
    }
...
void dp88_AR_Prism_Tower::SendAssistanceRequests(GameObject *obj) {
    ...
    for (int i = 0; i < adjacentTowerCount; ++i) {
        if (adjacentTowers[i] != m_target->Get_ID() && Commands->Find_Object(adjacentTowers[i]) != obj)
            Commands->Send_Custom_Event(obj, Commands->Find_Object(adjacentTowers[i]),
                                        CUSTOM_PRISMTOWER_REQUEST_CHARGING, (int) targetPriority, 0.0f);
    }
}
...
// Deployable Infantry
ScriptRegistrant<dp88_AR_Deployable_Infantry> dp88_AR_Deployable_Infantry_Registrant("dp88_AR_Deployable_Infantry",
                                                                                     "deployedObjectPreset=null:string,deployedObjectSpaceRequired=6:float,deployAnimation=obj.obj:string,deployTime=4:float,undeployAnimation=obj.obj:string,undeployTime=4:float,deployedWeaponPowerup=null:string,deployedWeaponPowerup_veteran=null:string,deployedWeaponPowerup_elite=null:string,cannotDeployStringId=0:int,deployKeyhook=IDeploy:string,deployedArmourType=null:string,deployedArmourType_veteran=null:string,deployedArmourType_elite=null:string,deploySound:string,undeploySound:string");

// CLEG
ScriptRegistrant<dp88_AR_CLEG> dp88_AR_CLEG_Registrant("dp88_AR_CLEG", "");

// CLEG resistance script
ScriptRegistrant<dp88_AR_CLEG_target> dp88_AR_CLEG_target_Registrant("dp88_AR_CLEG_target",
                                                                     "resistance=20:int,clegEffectPreset=null:string");
...
ScriptRegistrant<dp88_Aircraft_LandingZone_Aircraft> dp88_Aircraft_LandingZone_Aircraft_Registrant(
        "dp88_Aircraft_LandingZone_Aircraft",
        "landing_anim_name=modelfile.animfile:string,landing_anim_first_frame=0.0:float,landing_anim_last_frame=25.0:float,require_landing_zone=1:int");
...
ScriptRegistrant<dp88_RemoteControlConsole> dp88_RemoteControlConsole_Registrant("dp88_RemoteControlConsole",
                                                                                 "remoteControlID:int,vehiclePreset:string,cost:int,team:int,chargeTime=0:int,accessDeniedSound:string,consoleOfflineSound:string,consoleChargingSound:string,insufficientFundsSound:string,connectionEstablishedSound:string,animationName:string,animFrameAvailable1=0:int,animFrameAvailable2=0:int,animFrameCharging1=0:int,animFrameCharging2=0:int,animFrameActive1=0:int,animFrameActive2=0:int,animFrameIdle1=0:int,animFrameIdle2=0:int,animFrameDisabled1=0:int,animFrameDisabled2=0:int");
ScriptRegistrant<dp88_RemoteControlVehicle> dp88_RemoteControlVehicle_Registrant("dp88_RemoteControlVehicle",
                                                                                 "remoteControlID:int");


// Demo Truck
ScriptRegistrant<dp88_AR_DemoTruck> dp88_AR_DemoTruck_Registrant("dp88_AR_DemoTruck",
                                                                 "explosionPreset=Explosion_NukeBeacon:string,cinematicPreset=null:string,buildingDamageRange=50.0:float,buildingDamageWarhead=Nuke:string,buildingDamageStrength=2200.00:float");


// Paradrop scripts
ScriptRegistrant<dp88_AR_paradrop_Console> dp88_AR_paradrop_Console_Registrant("dp88_AR_paradrop_Console",
                                                                               "team=1:int");


// Prism Tower script
ScriptRegistrant<dp88_AR_Prism_Tower> dp88_AR_Prism_Tower_Registrant("dp88_AR_Prism_Tower",
                                                                     "Priority_Infantry=1.0:float,Splash_Infantry=0:int,Priority_Light_Vehicle=5.0:float,Priority_Heavy_Vehicle=7.0:float,Priority_VTOL=0.0:float,Min_Attack_Range=0:int,Max_Attack_Range=150:int,Animation_Model:string,Animation_Model_Bone:string,Animation:string,Animation_Idle_Start_Frame:int,Animation_Idle_End_Frame:int,Animation_Unpowered_Start_Frame:int,Animation_Unpowered_End_Frame:int,Animation_Charge_Start_Frame:int,Animation_Charge_End_Frame:int,Charge_Sound:string,Modifier_Distance=0.25:float,Modifier_Target_Damage=0.1:float,Modifier_Target_Value=0.25:float,Requires_Power=1:int,Debug=0:int,Detects_Stealth=1:int");
