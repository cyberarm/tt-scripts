...
void dp88_AR_Rocketeer::Created(GameObject *pObj) {
    ...
    if (strlen(Get_Parameter("Flying_Preset")) <= 0 || !Is_Valid_Preset(Get_Parameter("Flying_Preset"))) {
        Console_Output(
                "[%d:%s:%s] Critical Error: The specified flying preset, \"%s\", is not vaild. Destroying script...\n",
                Commands->Get_ID(pObj), Commands->Get_Preset_Name(pObj), this->Get_Name(),
                Get_Parameter("Flying_Preset"));
        Destroy_Script();
        return;
    }
    ...
    if (NULL != m_nVehicleID) {
        if (GameObject *pVehicle = Commands->Find_Object(m_nVehicleID)) {
            if (pVehicle->As_VehicleGameObj()) {
                int sound = pVehicle->As_VehicleGameObj()->Get_Definition().Get_Engine_Sound(
...
void dp88_AR_Rocketeer::Toggle_Flight_Mode(SoldierGameObj *pSoldier, bool bState) {
    ...
    if (bState && NULL == m_nVehicleID) {
        ...
        Attach_Script_V(pVehicle, "JFW_Death_Send_Custom", "%d,%d,0", Commands->Get_ID(pSoldier),
                        CUSTOM_ROCKETEER_VEHICLEKILLED);
...
void dp88_AR_MirageTank::Created(GameObject *obj) {
    ...
    if (!gameController) {
        Console_Output(
                "dp88_AR_MirageTank - Unable to find Game Controller, unable to continue. Destroying script...\n");
        Destroy_Script();
        return;
    }
...
void dp88_AR_MirageTank::setHidden(GameObject *obj, bool hide) {
    ...
    if (hide && NULL == m_mirageID) {
        ...
        dp88_AR_GameController *gameControllerScript = (NULL != gameController)
                                                       ? (dp88_AR_GameController *) Find_Script_On_Object(
                        gameController, "dp88_ar_gameController") : NULL;
        ...
        if (gameControllerScript->mirageTank_disguisePresets[0] == 0
            ...
            Console_Output(
                    "[%d:%s:%s] No valid Mirage Tank disguises have been set in the GameController, aborting script!",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
            Destroy_Script();
            return;
        }
        ...
        miragePosition.Z -= 3;
        GameObject * mirage = Commands->Create_Object(gameControllerScript->mirageTank_disguisePresets[disguiseID],
                                                      miragePosition);
...
void dp88_AR_IFV::deploy() {
    ...
    Commands->Set_Animation_Frame(obj, Get_Parameter("Turret_Frames_Animation"),
                                  Get_Int_Parameter("Switching_Anim_Frame"));
    ...
    isDeployed = true;
    isDeploying = true;
    Set_HUD_Help_Text_Player(Commands->Find_Object(pilotID), Get_Int_Parameter("String"),
...
void dp88_AR_IFVDriver::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    ...
    if (type == CUSTOM_IFV_REQUESTDRIVERDATA) {
        ...
        Commands->Send_Custom_Event(obj, sender, CUSTOM_IFV_DRIVER_ANIMFRAME, Get_Int_Parameter("IFV_ModelAnimFrame"),
                                    0.0f);

        if (Is_Valid_Preset(Get_Parameter("IFV_WeaponPowerup_Rookie")))
            Commands->Send_Custom_Event(obj, sender, CUSTOM_IFV_DRIVER_ROOKIEPOWID,
                                        Get_Definition_ID(Get_Parameter("IFV_WeaponPowerup_Rookie")), 0.0f);
        if (Is_Valid_Preset(Get_Parameter("IFV_WeaponPowerup_Veteran")))
            Commands->Send_Custom_Event(obj, sender, CUSTOM_IFV_DRIVER_VETERANPOWID,
                                        Get_Definition_ID(Get_Parameter("IFV_WeaponPowerup_Veteran")), 0.0f);
        if (Is_Valid_Preset(Get_Parameter("IFV_WeaponPowerup_Elite")))
            Commands->Send_Custom_Event(obj, sender, CUSTOM_IFV_DRIVER_ELITEPOWID,
                                        Get_Definition_ID(Get_Parameter("IFV_WeaponPowerup_Elite")), 0.0f);
    }
}
...
void
dp88_AR_Tesla_Coil::loadSettings(GameObject *pSelf, bool loadSecondaryFireSettings, bool loadBuildingTargetSettings) {
...
void dp88_AR_Tesla_Coil::Damaged(GameObject *obj, GameObject *damager, float amount) {
    ...
    if (Get_Damage_Warhead() == m_chargeWarheadID /*&& chargerScript->m_lastChargeTime < time(NULL)-1*/ ) {
        ...
        int supercharge_chargesRequired = (dp88_AI_ChargedTurret::checkPowerState(obj)) ? m_chargesSupercharge :
                                          m_chargesPowerOn + m_chargesSupercharge;
...
// IFV
ScriptRegistrant<dp88_AR_IFV> dp88_AR_IFV_Registrant("dp88_AR_IFV",
                                                     "Turret_Frames_Animation=v_all_ifv.v_all_ifv:string,Switch_Time=10:int,Switching_Anim_Frame=2:int,Keyhook=VDeploy:string,String:int,Color:vector3,Sound:string");
ScriptRegistrant<dp88_AR_IFVDriver> dp88_AR_IFVDriver_Registrant("dp88_AR_IFVDriver",
                                                                 "IFV_ModelAnimFrame=1:int,IFV_WeaponPowerup_Rookie=null:string,IFV_WeaponPowerup_Veteran=null:string,IFV_WeaponPowerup_Elite=null:string");
