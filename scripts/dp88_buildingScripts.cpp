                ...
                if (allWeakpointsDestroyed) {
                    ...
                    for (int i = 0; i < numChildren; i++) {
                        if (i != childIndex) {
                            GameObject * child = Commands->Find_Object(children[i].objectId);
                            if (child != NULL)
                                Commands->Send_Custom_Event(obj, child, CUSTOM_BUILDINGSCRIPTS_BUILDINGOFFLINE, 0,
                                                            0.0f);
                        }
                    }
...
void dp88_buildingScripts_componentBaseClass::PlayAnimation(GameObject *obj, const char *animation, int nStartFrame,
                                                            int nEndFrame, bool bLooped) {
...
void dp88_buildingScripts_minorWeakpoint::Damaged(GameObject *obj, GameObject *damager, float amount) {
    ...
    else if (m_bIsDestroyed && amount <= 0.0f &&
...
void dp88_buildingScripts_minorWeakpoint::Killed(GameObject *obj, GameObject *killer) {
    ...
    Console_Output("[%d:%s:%s] Script Error: A minor weakpoint has been killed...\n", Commands->Get_ID(obj),
                   Commands->Get_Preset_Name(obj), this->Get_Name());
}
...
void dp88_buildingScripts_functionProduceVehicles::OnBuildingOffline(GameObject *obj) {
    ...
    auto vehiclesDef = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_VEHICLES,
                                                                 (PurchaseSettingsDefClass::TEAM) PTTEAM(
                                                                         Get_Object_Type(obj)));
    if (NULL != vehiclesDef) {
        vehiclesDef->Set_Page_Disabled(true);
    }
    auto secretVehiclesDef = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_SECRET_VEHICLES,
                                                                       (PurchaseSettingsDefClass::TEAM) PTTEAM(
                                                                               Get_Object_Type(obj)));
...
void dp88_buildingScripts_functionProduceAircraft::OnBuildingOffline(GameObject *obj) {
    ...
    auto airDef = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_AIR,
                                                            (PurchaseSettingsDefClass::TEAM) PTTEAM(
                                                                    Get_Object_Type(obj)));
    if (NULL != airDef) {
...
void dp88_buildingScripts_functionProduceNavalUnits::OnBuildingOffline(GameObject *obj) {
    ...
    auto navalDef = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_NAVAL,
                                                              (PurchaseSettingsDefClass::TEAM) PTTEAM(
                                                                      Get_Object_Type(obj)));
    if (NULL != navalDef) {
...
void dp88_buildingScripts_functionSpawnZone::Created(GameObject *obj) {
    ...
    if (Is_Valid_Preset(Get_Parameter("scriptZonePreset")))
        ...
        Console_Output(
                ...
                Get_Parameter("scriptZonePreset"));
        Destroy_Script();
    }
}
...
void dp88_buildingScripts_capturePoint::Created(GameObject *obj) {
    GameObject * parent = RegisterWithParent(obj, Get_Parameter("Building_Controller_Preset"),
                                             BUILDING_CHILD_TYPE_CAPTUREPOINT);
    if (!parent)
        return;
    ...
    if (parent->As_BuildingGameObj()) {
        Console_Output(
                "[%d:%s:%s] Critical Error: Building controller '%s' is of type BuildingGameObj, capturing is not possible. Destroying script...\n",
                Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
...
void dp88_buildingScripts_capturePoint::Damaged(GameObject *obj, GameObject *damager, float amount) {
    ...
    if (Commands->Get_Player_Type(damager) == 0) {
        ...
        if (health == 0.25f && currentTeam != 0) {
            ...
            Set_Object_Type(obj, currentTeam);
            Commands->Send_Custom_Event(obj, Commands->Find_Object(m_parentId), CUSTOM_BUILDINGSCRIPTS_BUILDINGCAPTURED,
                                        currentTeam, 0.0f);
        }
...
void dp88_buildingScripts_spawnPart::Timer_Expired(GameObject *obj, int number) {
    ...
    if (number == TIMER_SPAWNVEHICLEPART && partObjectId == NULL) {
        ...
        GameObject * part = Commands->Create_Object_At_Bone(obj, Get_Parameter("Building_Part_Preset"),
...
ScriptRegistrant<dp88_buildingScripts_functionMoneyTrickle> dp88_buildingScripts_functionMoneyTrickle_Registrant(
        "dp88_buildingScripts_functionMoneyTrickle", "creditsPerSec=3.0:float");
ScriptRegistrant<dp88_buildingScripts_functionSpawnTeamZone> dp88_buildingScripts_functionSpawnTeamZone_Registrant(
        "dp88_buildingScripts_functionSpawnTeamZone",
        "scriptZonePreset_team0:string,scriptZonePreset_team1:string,scriptZonePreset_team2:string,scriptZoneBone:string,scriptZoneSize:vector3,destroyZoneWhenDisabled=1:int");
ScriptRegistrant<dp88_buildingScripts_functionMoneyGrant> dp88_buildingScripts_functionMoneyGrant_Registrant(
        "dp88_buildingScripts_functionMoneyGrant", "credits:float,onceOnly=1:int");
ScriptRegistrant<dp88_buildingScripts_spawnPart> dp88_buildingScripts_spawnPart_Registrant(
        "dp88_buildingScripts_spawnPart", "Building_Part_Preset:string,Spawn_At_Bone:string,IsDecorative=0:int");
