...
void dp88_customAI::Init(GameObject *obj) {
    if (!obj->As_SmartGameObj()) {
        Console_Output(
                "[%d:%s:%s] Critical Error: Only object of type SmartGameObj can use the dp88_AI_* series of scripts. Destroying script...\n",
...
void dp88_customAI::loadSettings(GameObject *obj, bool loadSecondaryFireSettings, bool loadBuildingTargetSettings) {
    ...
    primary_infantry = (loadSecondaryFireSettings) ? ((Get_Int_Parameter("Weapon_Infantry") == 2) ? false : true)
                                                   : true;
    primary_lightVehicle = (loadSecondaryFireSettings) ? ((Get_Int_Parameter("Weapon_Light_Vehicle") == 2) ? false
                                                                                                           : true)
                                                       : true;
    primary_heavyVehicle = (loadSecondaryFireSettings) ? ((Get_Int_Parameter("Weapon_Heavy_Vehicle") == 2) ? false
                                                                                                           : true)
                                                       : true;
    primary_VTOL = (loadSecondaryFireSettings) ? ((Get_Int_Parameter("Weapon_VTOL") == 2) ? false : true) : true;
    primary_building = (loadSecondaryFireSettings && loadBuildingTargetSettings) ? ((Get_Int_Parameter(
            "Weapon_Building") == 2) ? false : true) : true;
...
float dp88_customAI::getPriority(GameObject *obj, GameObject *target) {
    ...
    if (debug)
        fprintf(debugFile, "Calculating priority of %d (%s)\n", Commands->Get_ID(target),
                Commands->Get_Preset_Name(target));
    ...
    if ((Commands->Get_Health(target) + Commands->Get_Shield_Strength(target)) == 0
        || Commands->Get_Player_Type(target) == SCRIPT_PLAYERTYPE_NEUTRAL ||
        Commands->Get_Player_Type(target) == SCRIPT_PLAYERTYPE_SPECTATOR) {
        if (debug) fprintf(debugFile, "Target %d is dead or unteamed, ignoring\n", Commands->Get_ID(target));
        return 0.0;
    }

    if (target->As_VehicleGameObj() && !IsVehicleAIEnabled(target->As_VehicleGameObj()) &&
    ...
    if (!m_bCanDetectStealth && target->As_SmartGameObj() && target->As_SmartGameObj()->Is_Stealthed() &&
        getDistance(obj, target) > target->As_SmartGameObj()->Get_Stealth_Fade_Distance()) {
        if (debug)
            fprintf(debugFile, "Target %d is currently stealthed and stealth detection is disabled\n",
                    Commands->Get_ID(target));
        return 0.0;
    }
    ...
    if (!target->As_BuildingGameObj()) {
        ...
        if (debug)
            fprintf(debugFile, "\tTarget Value Modifier: Adding %.4f priority ((Value %d /10) * modifier %.2f)\n",
    ...
    if (debug)
        fprintf(debugFile,
                "\tTarget Damage Modifier: Adding %.4f priority ((Target Damage Taken %.2f) /10) * modifier %.2f)\n",
                ((maxHitPts - totalHitPts) / 10) * modifier_target_damage, maxHitPts - totalHitPts,
    ...
    if (debug)
        fprintf(debugFile,
                "\tDistance Modifier: Subtracting %.4f priority ((Distance to target %.2f) /10) * modifier %.2f)\n",
...
dp88_customAI::TargetType dp88_customAI::GetTargetType(GameObject *target) {
    ...
    if ((Get_Vehicle_Mode(target) == VEHICLE_TYPE_FLYING) || (target->As_SoldierGameObj() && (Get_Fly_Mode(target) ||
                                                                                              Find_Script_On_Object(
                                                                                                      target,
                                                                                                      "MS_Paradrop") ||
                                                                                              Find_Script_On_Object(
                                                                                                      target,
    ...
    if (target->As_SoldierGameObj())
        return SOLDIER;
    if (target->As_VehicleGameObj() && (Is_Script_Attached(target, "dp88_AI_Marker_HeavyVehicle") ||
...
void dp88_AI_Unit::Created(GameObject *obj) {
    ...
    Console_Output(
            "[%d:%s:%s] Critical Error: Vehicle AI base class created function should not be called. Destroying script...\n",
            Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
    Destroy_Script();
}
...
void dp88_AI_Unit::Timer_Expired(GameObject *obj, int number) {
    ...
    if (TIMER_AI_THINK == number) {
        ...
        if (m_target) {
            ...
            else if (!m_bMovingToTarget && ShouldPursueTarget(obj, m_target) &&
                     getDistance(obj, m_target) > GetPreferredAttackRange(obj, m_target)) {
...
void dp88_AI_Unit::AttackTarget(GameObject *obj, GameObject *target) {
    ...
    params.Set_Attack(target, (float) ((m_bTargetPrimaryFire) ? primary_maxRange : secondary_maxRange), 0.0,
                      m_bTargetPrimaryFire);
...
void
dp88_AI_Tank_Offensive::loadSettings(GameObject *obj, bool loadSecondaryFireSettings, bool loadBuildingTargetSettings) {
...
dp88_AI_Objective *dp88_AI_Tank_Offensive::ChooseNewObjective(GameObject *obj) {
    ...
    dp88_AI_Objective *pObjective = dp88_AI_Objective::GetBestObjective(obj, dp88_AI_Objective::TYPE_OFFENSIVE,
                                                                        modifier_distance, ignoredObjectives);
    if (NULL == pObjective)
...
void dp88_AI_Turret::Damaged(GameObject *obj, GameObject *damager, float amount) {
    if (!damager || amount <= 0 || !checkTeam(obj, damager) || Get_Object_Type(obj) == -2 ||
        obj->As_SmartGameObj()->Is_Enemy_Seen_Enabled() == false) {
        return;
    }
    ...
    if (number == TIMER_AI_THINK) {
        if (hasTarget) {
            if (!m_target || !checkPowerState(obj)
                ...
                || (m_target->As_VehicleGameObj() && !IsVehicleAIEnabled(m_target->As_VehicleGameObj()) &&
                    IsVehicleEmpty(m_target->As_VehicleGameObj()))) {
...
bool dp88_AI_Turret::checkTeam(GameObject *obj, GameObject *target) {
    return !(Commands->Get_Player_Type(target) == 2 ||
...
bool dp88_AI_Turret::checkRange(GameObject *obj, GameObject *target, bool primary) {
    ...
    if ((primary && distance < primary_minRange) || (!primary && distance < secondary_minRange)) {
        if (debug)
            fprintf(debugFile, "Target is too close to attack (distance: %.3f < min range: %d)\n", distance,
                    (primary) ? primary_minRange : secondary_minRange);
        return false;
    }
    if ((primary && distance > primary_maxRange) || (!primary && distance > secondary_maxRange)) {
        if (debug)
            fprintf(debugFile, "Target is too far away to attack (distance: %.3f > max range: %d)\n", distance,
                    (primary) ? primary_maxRange : secondary_maxRange);
        return false;
    }
...
void dp88_AI_PopupTurret::Created(GameObject *pSelf) {
    ...
    if (pSpotter) {
        ...
        Commands->Set_Animation_Frame(pSpotter, Get_Parameter("Deploy_Animation"),
                                      Get_Int_Parameter("Deploy_Animation_Frames"));
...
void dp88_AI_PopupTurret::Deploy(GameObject *pSelf) {
    if (m_deploymentState == STATE_UNDEPLOYED || m_deploymentState == STATE_UNDEPLOYING) {
        ...
        Commands->Set_Animation(pSelf, Get_Parameter("Deploy_Animation"), false, 0, Get_Animation_Frame(pSelf),
                                (float) Get_Int_Parameter("Deploy_Animation_Frames"), false);
...
void dp88_AI_PopupTurret::Undeploy(GameObject *pSelf) {
    if (m_deploymentState == STATE_DEPLOYED) {
        ...
        Commands->Set_Animation(pSelf, Get_Parameter("Deploy_Animation"), false, 0,
                                (float) Get_Int_Parameter("Deploy_Animation_Frames"), 0, false);
...
void dp88_AI_ChargedTurret::loadSettings(GameObject *pSelf, bool loadSecondaryFireSettings,
...
void dp88_AI_ChargedTurret::StartCharging(GameObject *pSelf) {
    ...
    if (!m_bIsPreReloading && !m_bIsCharging && !m_bIsDischarging && checkPowerState(pSelf) && m_target) {
        ...
        m_pLoopedAnimCtrl->PlayAnimation(Get_Parameter("Animation"), Get_Int_Parameter("Animation_Charge_Start_Frame"),
                                         Get_Int_Parameter("Animation_Charge_End_Frame"), 1);
...
void dp88_AI_ChargedTurret::ApplyIdleAnimation(GameObject *pSelf) {
    ...
    if (m_bPowerState)
        m_pLoopedAnimCtrl->PlayAnimation(Get_Parameter("Animation"), Get_Int_Parameter("Animation_Idle_Start_Frame"),
                                         Get_Int_Parameter("Animation_Idle_End_Frame"));
    else
        m_pLoopedAnimCtrl->PlayAnimation(Get_Parameter("Animation"),
                                         Get_Int_Parameter("Animation_Unpowered_Start_Frame"),
                                         Get_Int_Parameter("Animation_Unpowered_End_Frame"));
}
...
dp88_AI_ChargedTurret_AnimationObserver::dp88_AI_ChargedTurret_AnimationObserver(dp88_AI_ChargedTurret *pParent)
        : ObserverImpClass("dp88_AI_ChargedTurret_AnimationObserver") {
    m_pParent = pParent;
}
...
unsigned char dp88_AI_Objective::GetUnitType(GameObject *obj) {
    ...
    if (obj->As_VehicleGameObj()) {
        if (Is_Script_Attached(obj, "dp88_AI_heavyVehicleMarker") ||
            Is_Script_Attached(obj, "dp88_AI_Marker_HeavyVehicle"))
...
dp88_AI_Objective *
dp88_AI_Objective::GetBestObjective(GameObject *obj, unsigned int objective_type, float distance_modifier,
                                    DynamicVectorClass<int> ignoredObjectives) {
    ...
    for (int i = 0; i < Objectives.Count(); ++i) {
        ...
        if (objective_type != Objectives[i]->GetType() || (objTeam != 2 && team != objTeam) ||
            (ignoredObjectives.Length() && ignoredObjectives.ID(Objectives[i]->GetGameObject()->Get_ID()) != -1))
            continue;

        float priority = Objectives[i]->GetPriority(obj, distance_modifier);
        float distance = Vector3::Distance_Squared(Commands->Get_Position(obj),
                                                   Commands->Get_Position(Objectives[i]->GetGameObject()));
...
int dp88_AI_Objective::CountUnitObjectives(int team, unsigned int objective_type, GameObject *obj,
    ...
    for (int i = 0; i < Objectives.Count(); ++i) {
        int objTeam = Objectives[i]->GetTeam();
        if (objective_type != Objectives[i]->GetType() || (objTeam != 2 && team != objTeam) ||
            Objectives[i]->GetPriority(obj, distance_modifier) == 0)
            continue;
        count++;
    }
...
ScriptRegistrant<dp88_AI_PopupTurret_Spotter> dp88_AI_PopupTurret_Spotter_Registrant("dp88_AI_PopupTurret_Spotter",
                                                                                     "tId:int");
