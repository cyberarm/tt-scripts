...
void dp88_Veterancy_Controller::Created(GameObject *obj) {
    dp88_Veterancy_Settings::GetInstance().valueMultiplier[1] =
            1.0f + (Get_Int_Parameter("Veteran_Value_Multiplier_Percent") / 100.0f);
    dp88_Veterancy_Settings::GetInstance().valueMultiplier[2] =
            1.0f + (Get_Int_Parameter("Elite_Value_Multiplier_Percent") / 100.0f);
    dp88_Veterancy_Settings::GetInstance().carryOverPercentI =
            Get_Int_Parameter("Change_Character_Carry_Over_Percent_Inf") / 100.0f;
    dp88_Veterancy_Settings::GetInstance().carryOverPercentV =
            Get_Int_Parameter("Change_Character_Carry_Over_Percent_Veh") / 100.0f;
...
void dp88_veterancyUnit::Created(GameObject *obj) {
    ...
    hasVeteranWeaponPowerup = (strlen(Get_Parameter("veteran_weaponPowerup")) > 0 &&
                               Is_Valid_Preset(Get_Parameter("veteran_weaponPowerup"))) ? true : false;
    hasEliteWeaponPowerup = (strlen(Get_Parameter("elite_weaponPowerup")) > 0 &&
                             Is_Valid_Preset(Get_Parameter("elite_weaponPowerup"))) ? true : false;
...
void dp88_veterancyUnit::Damaged(GameObject *obj, GameObject *damager, float amount) {
    ...
    float pointsValue = (float) Get_Int_Parameter("pointsValue") *
                        dp88_Veterancy_Settings::GetInstance().valueMultiplier[currentLevel];
    ...
    if (amount > 0 && Get_Object_Type(damager) != Get_Object_Type(obj) && abs(Get_Object_Type(obj)) < 2) {
        ...
        Points given: 50% of ((pointsValue / total hit points) * damage repaired) */
             ...
             && (!obj->As_VehicleGameObj() || Get_Vehicle_Driver(obj) != NULL ||
                 Get_Vehicle_Mode(obj) == VEHICLE_TYPE_TURRET)) {
...
void dp88_veterancyUnit::Destroyed(GameObject *obj) {
    ...
    if (!deregistered && Get_Player_ID(obj) >= 0) {
        ...
        carryOverData.infantryVeterancyPoints =
                infantryVeterancyPoints * dp88_Veterancy_Settings::GetInstance().carryOverPercentI;
        carryOverData.vehicleVeterancyPoints =
                vehicleVeterancyPoints * dp88_Veterancy_Settings::GetInstance().carryOverPercentV;
    }
...
void dp88_veterancyUnit::KeyHook(const char *logicalKey) {
    ...
    if (0 == strcmp(logicalKey, ShowVeterancyPointsHookName)) {
        ...
        if (infantryVeteranRequirement > 0)
            message.Format(
                    "You currently have %.2f infantry veterancy points. A %s requires %d points for Veteran status and %d points for Elite status.",
                    infantryVeterancyPoints, str, infantryVeteranRequirement, infantryEliteRequirement);
        else
            message.Format("You currently have %.2f infantry veterancy points. A %s cannot be promoted",
                           infantryVeterancyPoints, str);
        delete[] str;
        ...
        if (NULL != vehicle) {
            ...
            if (vehicleVeteranRequirement > 0)
                message.Format(
                        "You currently have %.2f vehicle veterancy points. A %s requires %d points for Veteran status and %d points for Elite status.",
                        vehicleVeterancyPoints, str, vehicleVeteranRequirement, vehicleEliteRequirement);
            else
                message.Format("You currently have %.2f vehicle veterancy points. A %s cannot be promoted",
                               vehicleVeterancyPoints, str);
            delete[] str;
        } else
        ...
        if (Purchase_Item(obj, (int) ceil(cost))) {
            recieveVeterancyPoints(outstandingPoints +
                                   0.1f); // Add a bit more to "top it up" and avoid rounding errors leaving them just below the promotion boundary
            Send_Message_Player(obj, DP88_RGB_GENERAL_MSG, StringClass::getFormattedString(
                    "Your unit has been boosted to the next level at a cost of %.0f credits", cost));
        } else {
            Send_Message_Player(obj, DP88_RGB_GENERAL_MSG, StringClass::getFormattedString(
                    "You cannot afford to boost your unit at this time, you need at least %.0f credits", cost));
        }
    }
}
...
void dp88_veterancyUnit::createChevrons() {
    ...
    if (obj->As_SoldierGameObj()) {
        ...
        GameObject * promotionChevron = Create_Object_Attach_To_Object(obj,
                                                                       (2 == currentLevel) ? "chev_inf_elite_promotion"
                                                                                           : "chev_inf_veteran_promotion",
                                                                       "Worldbox");
        ...
        chevron = Create_Object_Attach_To_Object(obj, (2 == currentLevel) ? "chev_inf_elite" : "chev_inf_veteran",
                                                 "Worldbox");
    } else if (obj->As_VehicleGameObj()) {
        // Create promoted chevron and record it's ID
        GameObject * promotionChevron = Create_Object_Attach_To_Object(obj,
                                                                       (2 == currentLevel) ? "chev_veh_elite_promotion"
                                                                                           : "chev_veh_veteran_promotion",
                                                                       "Worldbox");
        promotionChevronObjId = Commands->Get_ID(promotionChevron);
        ...
        // Now create the main chevron
        chevron = Create_Object_Attach_To_Object(obj, (2 == currentLevel) ? "chev_veh_elite" : "chev_veh_veteran",
                                                 "chevBone");
    }
...
void dp88_veterancyUnit::recieveVeterancyPoints(float points) {
    ...
    if (obj->As_SoldierGameObj() && Get_Vehicle(obj) == NULL) {
        ...
        else if (currentLevel < 1 && infantryVeterancyPoints >= infantryVeteranRequirement &&
                 infantryVeteranRequirement > 0)
            promoteToVeteran();
    }
...
void dp88_veterancyUnit::promoteToVeteran() {
    ...
    else if (obj->As_VehicleGameObj() && Get_Vehicle_Driver(obj) != NULL &&
             Get_Player_ID(Get_Vehicle_Driver(obj)) >= 0) {
...
void dp88_veterancyUnit::promoteToElite() {
    ...
    if (weaponPowerup[0] != '\0' && Is_Valid_Preset(weaponPowerup)) {
        ...
        if (Get_Current_Weapon(obj) == NULL ||
            ...
            || (oldLevel == 1 &&
                (hasVeteranWeaponPowerup &&
                 strcmp(Get_Current_Weapon(obj), Get_Powerup_Weapon(Get_Parameter("veteran_weaponPowerup"))) == 0)
        ...
        if (obj->As_SoldierGameObj()) {
            if (selectWeapon) {
                ...
                Remove_Weapon(obj,
                              (hasVeteranWeaponPowerup) ? Get_Powerup_Weapon(Get_Parameter("veteran_weaponPowerup"))
                                                        : rookieWeapon);
        }
    }
...
void dp88_veterancyGrantPoints::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (CUSTOM_EVENT_POWERUP_GRANTED == type || CUSTOM_VETERANCY_GRANT_POINTS_DELAY == type) {
        ...
        dp88_veterancyUnit *veterancy_script = (dp88_veterancyUnit *) Find_Script_On_Object(sender,
                                                                                            "dp88_veterancyUnit");
        if (veterancy_script) {
