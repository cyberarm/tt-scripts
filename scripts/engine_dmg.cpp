...
ArmorWarheadManager::Get_Armor_Type(const char *)
...
ArmorWarheadManager::Get_Warhead_Type(const char *)
...
ArmorWarheadManager::Get_Special_Damage_Type( WarheadType
type )
...
RENEGADE_FUNCTION
        WarheadType
ArmorWarheadManager::Get_Special_Damage_Warhead( SpecialDamageType
type )
AT3(0x00687E80,0x00687720,0x005195A0);

RENEGADE_FUNCTION
float    ArmorWarheadManager::Get_Special_Damage_Duration(SpecialDamageType
type )
...
RENEGADE_FUNCTION
bool    ArmorWarheadManager::Is_Skin_Impervious(SpecialDamageType
type,
ArmorType skin
)
...
Damage_All_Buildings_By_Team(int Team, float Damage, const char *Warhead, GameObject *Damager) {
...
Repair_All_Buildings_By_Team(int Team, int ConstructionYardID, float Health) {
    SLNode<BuildingGameObj> *x = GameObjManager::BuildingGameObjList.Head();
    while (x) {
        GameObject * o = x->Data();
        if (o) {
...
Repair_All_Static_Vehicles_By_Team(int Team, int type) {
    SLNode<VehicleGameObj> *x = GameObjManager::VehicleGameObjList.Head();
    while (x) {
        VehicleGameObj *o = x->Data();
        if (o) {
            if (Get_Object_Type(o) == Team && o->Peek_Physical_Object() &&
                o->Peek_Physical_Object()->As_DecorationPhysClass()) {
...
o->Get_Defense_Object()->
Set_Health_Max(health);
Commands->
Set_Health(obj, health
);
}
...
o->Get_Defense_Object()->
Set_Shield_Strength_Max(shieldstrength);
Commands->
Set_Shield_Strength(obj, shieldstrength
);
}
...
return
ArmorWarheadManager::Get_Armor_Name(o
->Get_Defense_Object()->Get_Shield_Type());
}
    ...
    while (x) {
        ...
        if (obj) {
            ...
            if ((Vector3::Distance_Squared(ObjPosition, TestPosition) <= Distance * Distance) &&
                (Commands->Get_ID(obj) != Commands->Get_ID(Host))) {
...
Damage_All_Objects_Area(float Damage, const char *Warhead, const Vector3 &Position, float Distance, GameObject *Host,
    ...
    while (x) {
        ...
        if (o2) {
            ...
            ObjPosition.Z = 0;
            if ((Vector3::Distance_Squared(ObjPosition, TestPosition) <= Distance * Distance) &&
                (Commands->Get_ID(o2) != Commands->Get_ID(Host))) {
...
Damage_All_Objects_Area_By_Team(float Damage, const char *Warhead, const Vector3 &Position, float Distance,
                                GameObject *Damager, bool Soldiers, bool Vehicles, int Team) {
...
o->Get_Defense_Object()->
Set_Death_Points(points);
}

void SCRIPTS_API

Repair_All_Buildings_By_Team_Radius(int Team, int ConstructionYardID, float Health, float Radius) {
...
Repair_All_Turrets_By_Team(int team, float health) {
    SLNode<VehicleGameObj> *x = GameObjManager::VehicleGameObjList.Head();
    while (x) {
        GameObject * o = x->Data();
        if (o) {
...
{
if (
IsInsideZone(Zone, o2
))
{
Commands->
Apply_Damage(o2, Damage, Warhead, Damager
);
}
}
x = x->Next();
...
Ranged_Variable_Percent_Vehicle_Damage(float EnemyPercentage, float FriendPercentage, bool ForceFriendly,
                                       const char *Warhead, Vector3 Location, float DamageRadius, GameObject *Damager) {
