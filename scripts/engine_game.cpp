...
REF_DEF2(bool, CombatManager::IAmServer,

0x00855EC8, 0x008550B0);
...
BaseControllerClass::Find_Base(int Team)
...
VehicleGameObj *BaseControllerClass::Get_Harvester_Vehicle() {
    ...
    return refinery && refinery->As_RefineryGameObj() ? refinery->As_RefineryGameObj()->Get_Harvester_Vehicle()
                                                      : nullptr;
}
...
BaseControllerClass::Find_Building(unsigned int Type)

AT2(0x006EF6D0,0x006EEC90);
RENEGADE_FUNCTION
        cGameDataSkirmish

SCRIPTS_API *The_Skirmish_Game()

AT2(0x00477CC0,0x00477390);
RENEGADE_FUNCTION
        cGameDataCnc

SCRIPTS_API *The_Cnc_Game()

AT2(0x00477CD0,0x004773A0);
RENEGADE_FUNCTION
        cGameDataSinglePlayer

SCRIPTS_API *The_Single_Player_Game()
...
void BaseControllerClass::Check_Vehicle_Factory() {
    if (CombatManager::I_Am_Server()) {
        for (int i = 0; i < BuildingList.Count(); ++i) {
            ...
            if (building->As_VehicleFactoryGameObj() || building->As_AirFactoryGameObj() ||
                building->As_NavalFactoryGameObj()) {
...
REF_DEF2(int, cGameData::HostedGameNumber,

0x0082F120, 0x0082E308);
REF_DEF2(WideStringClass, cGameData::WinText,
0x0082F108, 0x0082E2F0);
REF_DEF2(Vector3, PrivateMessageColor,
0x0082FC20, 0x0082EE08);
REF_DEF2(Vector3, PublicMessageColor,
0x0082FBA0, 0x0082ED88);
REF_DEF2(Vector3, GDIHouseColor,
0x0085CAA8, 0x0085BC90);
REF_DEF2(Vector3, NodHouseColor,
0x0085CA98, 0x0085BC80);

REF_DEF2(bool, IsMapVTOL,

0x0085E31C, 0x0085D4F4);

REF_DEF2(bool, IsListed,

0x0082FD94, 0x0082EF7C);

REF_DEF2(unsigned int, GameMode,

0x0081F5F0, 0x0081E7D8);

void SCRIPTS_API

Power_Base(int team, bool powered) {
...
Set_Can_Generate_Soldiers(int team, bool cangenerate) {
    BaseControllerClass * c = BaseControllerClass::Find_Base(team);
    if (c) {
...
Set_Can_Generate_Vehicles(int team, bool cangenerate) {
    BaseControllerClass * c = BaseControllerClass::Find_Base(team);
    if (c) {
...
Beacon_Destroyed_Base(int team, bool destroyed) {
    BaseControllerClass * c = BaseControllerClass::Find_Base(team);
    if (c) {
...
Enable_Base_Radar(int team, bool enable) {
    BaseControllerClass * c = BaseControllerClass::Find_Base(team);
    if (c) {
...
Find_Building_By_Type(int team, int type) {
    BaseControllerClass * c = BaseControllerClass::Find_Base(team);
    if (c) {
...
Create_Building(const char *preset, const Vector3 &Position) {
...
Can_Generate_Vehicles(int team) {
    BaseControllerClass * c = BaseControllerClass::Find_Base(team);
    if (c) {
...
Can_Generate_Soldiers(int team) {
...
Get_Building_Count_Team(int Team) {
...
Find_Building_By_Class_ID(int Team, unsigned long Class_ID) {
    SLNode<BuildingGameObj> *x = GameObjManager::BuildingGameObjList.Head();
    while (x) {
        GameObject * o = x->Data();
        if (o) {
...
Find_Construction_Yard(int Team) {
...
Find_Building_By_Team(int Team) {
    SLNode<BuildingGameObj> *x = GameObjManager::BuildingGameObjList.Head();
    while (x) {
        GameObject * o = x->Data();
        if (o) {
...
Find_Building_By_Preset(int Team, const char *Preset_Name) {
    SLNode<BuildingGameObj> *x = GameObjManager::BuildingGameObjList.Head();
    while (x) {
        GameObject * o = x->Data();
        if (o) {
...
Get_Private_Message_Color(unsigned int *red, unsigned int *green, unsigned int *blue) {
...
Get_Public_Message_Color(unsigned int *red, unsigned int *green, unsigned int *blue) {
...
Get_Harvester_Preset_ID(int Team) {
...
bool SCRIPTS_API
Is_Harvester_Preset(GameObject
*obj)
{
...
SimpleGameObj::Get_Definition(void) const {
    return (const SimpleGameObjDef &) BaseGameObj::Get_Definition();
}

bool SCRIPTS_API

Vehicle_Preset_Is_Air(int preset) {
...
Vehicle_Preset_Is_Naval(int preset) {
    DefinitionClass *def = DefinitionMgrClass::Find_Definition(preset, true);
    if (!def) {
        return false;
    }
...
Seconds_To_Hms(float secs, int &hours, int &minutes, int &seconds) {
