...
void SCRIPTS_API
Strip_Path_From_Filename(StringClass
& target,
const char *fileName
)
{
...
SCRIPTS_API REF_DEF2(FileFactoryClass *, _TheFileFactory,

0x00809E74, 0x0080904C);
#else
FileFactoryClass *_TheFileFactory;
#endif
FileClass SCRIPTS_API
*

Get_Data_File(const char *file) {
    return _TheFileFactory->Get_File(file);
}

void SCRIPTS_API
Close_Data_File(FileClass
*file)
{
_TheFileFactory->
Return_File(file);
...
#ifndef TTLE_EXPORTS
#ifndef DDBEDIT
INIClass SCRIPTS_API
*

Get_INI(char const *filename) {
    INIClass * ini = 0;
...
RENEGADE_FUNCTION
        WideStringClass
&
INIClass::Get_Wide_String(WideStringClass
&,char const*,char const*,wchar_t const*) const
AT2(0x005E06E0,0x005DFF80);
#endif

int Read_Line(Straw & straw, char * line, int
lineSize,
bool &isLast
)
{
...
StringClass &
INIClass::Get_String(StringClass &string, const char *section, const char *entry, const char *defaultvalue) const {
...
StringClass &
INIClass::Get_String_Advanced(StringClass &string, const char *section, const char *entry, const char *defaultvalue,
                              bool updateIfNotFound) const {
    const char *value = defaultvalue;
    if (!section || !entry) {
        string = "";
    }
...
int
INIClass::Get_String(char const *section, char const *entry, char const *defaultvalue, char *result, int size) const {
...
int
INIClass::Get_String_Advanced(char const *section, char const *entry, char const *defaultvalue, char *result, int size,
                              bool updateIfNotFound) const {
    if (!result || size <= 1 || !section || !entry)
        return 0;
    INIEntry *Entry = Find_Entry(section, entry);
...
Get_Registry_Int(const char *entry, int defaultvalue) {
    unsigned int value;
    HKEY key;
