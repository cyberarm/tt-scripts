...
Get_Data_File(const char *file); //Open a file using the mix file opening logic
void SCRIPTS_API
Close_Data_File(FileClass
*file); //Close a file that was opened with Get_Data_File
INIClass SCRIPTS_API
*

Get_INI(char const *filename); //Open an INI file (reading using the normal mix file opening logic) and read stuff from it
void SCRIPTS_API
Release_INI(INIClass
*ini); //Close an INI file opened with Get_INI
unsigned int SCRIPTS_API

Get_Registry_Int(const char *entry, int defaultvalue); //Get an int value from the renegade registry key
#ifndef TTLE_EXPORTS
#ifndef WWCONFIG
