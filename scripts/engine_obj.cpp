...
SCRIPTS_API const AirStripGameObjDef
&
...
SCRIPTS_API const ArmedGameObjDef
&

ArmedGameObj::Get_Definition(void) const {
    return (const ArmedGameObjDef &)BaseGameObj::Get_Definition();
}

SCRIPTS_API const BeaconGameObjDef
&
...
SCRIPTS_API const C4GameObjDef
&

C4GameObj::Get_Definition(void) const {
...
CinematicGameObj::Get_Definition(void) const {
...
DamageZoneGameObj::Get_Definition(void) const {
...
SCRIPTS_API const SAMSiteGameObjDef
&

SAMSiteGameObj::Get_Definition(void) const {
    return (const SAMSiteGameObjDef &)BaseGameObj::Get_Definition();
}

SCRIPTS_API const SoldierGameObjDef
&

SoldierGameObj::Get_Definition(void) const {
...
SCRIPTS_API const TransitionGameObjDef
&

TransitionGameObj::Get_Definition(void) const {
...
SCRIPTS_API const ScriptZoneGameObjDef
&

ScriptZoneGameObj::Get_Definition(void) const {
...
SCRIPTS_API REF_DEF3(SList<BuildingGameObj>, GameObjManager::BuildingGameObjList,

0x00856FE8, 0x008561D0, 0x0085BED0);

SCRIPTS_API REF_DEF3(SList<BaseGameObj>, GameObjManager::GameObjList,

0x00856FF8, 0x008561E0, 0x0085BEE0);

SCRIPTS_API REF_DEF3(SList<SoldierGameObj>, GameObjManager::StarGameObjList,

0x00856FC8, 0x008561B0, 0x0085BEB0);

SCRIPTS_API REF_DEF3(SList<SmartGameObj>, GameObjManager::SmartGameObjList,

0x00856FD8, 0x008561C0, 0x0085BEC0);

SCRIPTS_API const DamageableGameObjDef
&

DamageableGameObj::Get_Definition(void) const {
...
SCRIPTS_API GameObjObserverClass
*

ScriptableGameObj::Find_Observer(const char *Name) {
...
SCRIPTS_API const ScriptableGameObjDef
&

ScriptableGameObj::Get_Definition(void) const {
    return (const ScriptableGameObjDef &) BaseGameObj::Get_Definition();
}

SCRIPTS_API const BuildingGameObjDef
&
...
SCRIPTS_API const RefineryGameObjDef
&
...
GameObject SCRIPTS_API
*

Find_Smart_Object_By_Team(int Team) {
...
Find_Non_Player_Object_By_Team(int Team) {
    SLNode<SmartGameObj> *x = GameObjManager::SmartGameObjList.Head();
    while (x) {
        GameObject * o = x->Data();
        if (o) {
...
Find_Object_By_Preset(int Team, const char *Preset_Name) {
    SLNode<BaseGameObj> *x = GameObjManager::GameObjList.Head();
    while (x) {
        BaseGameObj *o = x->Data();
        ScriptableGameObj * o2 = 0;
...
Find_Closest_Non_Building_Object_By_Team(int Team, Vector3 position) {
...
Find_Closest_Preset_By_Team(int Team, const Vector3 &pos, const char *Preset) {
    float closestdist_sq = FLT_MAX;
    GameObject * closest = 0;
    SLNode<BaseGameObj> *x = GameObjManager::GameObjList.Head();
    while (x) {
...
Find_Random_Preset_By_Team(int Team, const char *Preset) {
...
Get_Object_Count(int Team, const char *Preset) {
    int count = 0;
    SLNode<SmartGameObj> *x = GameObjManager::SmartGameObjList.Head();
    while (x) {
        GameObject * o = x->Data();
...
SimpleDynVecClass<GameObject *> SCRIPTS_API
*

Get_All_Objects_By_Preset(int Team, const char *Preset) {
...
Find_Closest_Building(const Vector3 &position) {
...
Find_Random_Preset(const char *preset, int min, int max) {
...
Send_Custom_To_Team_Buildings(int Team, GameObject *sender, int type, int param, float delay) {
    if (!sender) {
        return;
    }
...
Send_Custom_To_Team_Preset(int Team, const char *PresetName, GameObject *sender, int type, int param, float delay) {
    if (!sender) {
        return;
    }
    SLNode<BaseGameObj> *x = GameObjManager::GameObjList.Head();
...
Send_Custom_All_Objects_Area(int type, const Vector3 &Position, float Distance, GameObject *sender, int team) {
    if (!sender) {
        return;
    }
...
Send_Custom_All_Objects(int type, GameObject *sender, int team) {
    if (!sender) {
        return;
    }
    SLNode<BaseGameObj> *x = GameObjManager::GameObjList.Head();
...
void SCRIPTS_API
Send_Custom_Event_To_Object(GameObject
*sender,
const char *Preset,
int type,
int param,
...
{
Commands->
Send_Custom_Event(sender, o2, type, param, delay
);
}
}
x = x->Next();
...
Is_Unit_In_Range(const char *preset, float range, Vector3 location, int team, bool empty) {
    SLNode<SmartGameObj> *x = GameObjManager::SmartGameObjList.Head();
    while (x) {
        BaseGameObj *o = x->Data();
        ScriptableGameObj * o2 = 0;
...
bool SCRIPTS_API
Get_Is_Powerup_Persistant(GameObject
*obj)
{
if (obj->
...
bool SCRIPTS_API
Get_Powerup_Always_Allow_Grant(GameObject
*obj)
{
if (obj->
...
int SCRIPTS_API
Get_Powerup_Grant_Sound(GameObject
*obj)
{
if (obj->
...
Matrix3D SCRIPTS_API
Get_Transform( GameObject
* obj )
{
...
GameObject SCRIPTS_API
*
Get_Closest_Armed_Object_To_Object( GameObject
* obj,
int team
)
{
...
o2->As_PhysicalGameObj()->As_ArmedGameObj())
{
Vector3 pos = Commands->Get_Position(o2);
float dist_sq = Vector3::Distance_Squared(pos, position);
if (dist_sq<closestdist_sq)
...
VehicleGameObj::Get_Occupant_Count(void) {
    int count = 0;
...
SCRIPTS_API AnimControlClass
*

PhysicalGameObj::Get_Anim_Control() {
    return AnimControl;
}
...
SCRIPTS_API const PowerUpGameObjDef
&

PowerUpGameObj::Get_Definition(void) const {
    return (const PowerUpGameObjDef &)BaseGameObj::Get_Definition();
}

SCRIPTS_API const PhysicalGameObjDef
&

PhysicalGameObj::Get_Definition(void) const {
    return (const PhysicalGameObjDef &) BaseGameObj::Get_Definition();
}

SCRIPTS_API bool
Verify_Preset_Limit(int team, int instance_count, uint32 *preset_ids, int preset_id_count, GameObject *ignore) {
...
void SCRIPTS_API
Set_Powerup_Always_Allow_Grant(GameObject
*obj,
bool Grant
)
{
...
((PowerUpGameObjDef&)o3->

Get_Definition()

).
AlwaysAllowGrant = Grant;
}

void SCRIPTS_API

PhysicalGameObj::Set_Collision_Group(int group) {
    PhysObj->Set_Collision_Group((Collision_Group_Type) group);
}

void SCRIPTS_API

VehicleGameObj::Set_Immovable(bool b) {
...
SCRIPTS_API const SmartGameObjDef
&

SmartGameObj::Get_Definition(void) const {
