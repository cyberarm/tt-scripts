...
Create_Library_Object(const char *preset)
...
{
return
ScriptZoneGameObj::Find_Closest_Zone(location, (ZoneConstants::ZoneType)
type);
}
...
factory->As_BuildingGameObj()->As_VehicleFactoryGameObj())
{
return factory->As_BuildingGameObj()->As_VehicleFactoryGameObj()->
...
factory->As_BuildingGameObj()->As_AirFactoryGameObj())
{
return factory->As_BuildingGameObj()->As_AirFactoryGameObj()->
...
factory->As_BuildingGameObj()->As_NavalFactoryGameObj())
{
return factory->As_BuildingGameObj()->As_NavalFactoryGameObj()->
...
Get_Vehicle_Definition_Mode(const char *preset) {
...
Get_Vehicle_Definition_Mode_By_ID(unsigned long ID) {
...
Create_Zone(const char *preset, OBBoxClass &box) {
...
SCRIPTS_API const VehicleGameObjDef
&

VehicleGameObj::Get_Definition(void) const {
    return (const VehicleGameObjDef &) BaseGameObj::Get_Definition();
}

// 0x006802B0, 0x0067FB50, 0x, 0x0810D93A
SCRIPTS_API SmartGameObj
*

GameObjManager::Find_SmartGameObj(int networkId) {
    if (!networkId)
        return 0;
    for (SLNode<SmartGameObj> *node = GameObjManager::SmartGameObjList.Head(); node; node = node->Next()) {
        SmartGameObj *object = (SmartGameObj * )
        node->Data();
...
SCRIPTS_API bool SmartGameObj::Is_Obj_Visible(PhysicalGameObj *object) {
    ...
    PhysRayCollisionTestClass collisionTest(LineSegClass(lookPosition, bullseyePosition), &castResult,
...
SCRIPTS_API bool SmartGameObj::Is_Splash_Possible(PhysicalGameObj *object) {
    ...
    CastResultStruct castResult;
    PhysRayCollisionTestClass collisionTest(LineSegClass(bullseyePosition, lookPosition), &castResult,
                                            BULLET_COLLISION_GROUP);
...
{
return obj->As_SoldierGameObj()->Get_Definition().
...
int Return = 0;
for (
SLNode<SoldierGameObj> *z = GameObjManager::StarGameObjList.Head();
z;
z = z->Next()
)
{
...
{
Commands->
Set_Facing(obj, Commands
->
Get_Facing(host)
);
Commands->
Attach_To_Object_Bone(obj, host, (NULL
!= bone) ? bone : "origin");
}
...
return vehicle->As_VehicleGameObj()->
Find_Seat(occupant
->
...
REF_DEF2(float, SmartGameObj::GlobalSightRangeScale,

0x00811E64, 0x0081103C);
