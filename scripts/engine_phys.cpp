...
return o->Peek_Physical_Object()->Peek_Model()->
...
Create_Effect_All_Of_Preset(const char *object, const char *preset, float ZAdjust, bool ZSet) {
    SLNode<SmartGameObj> *x = GameObjManager::SmartGameObjList.Head();
    while (x) {
...
PhysicsSceneClass::Get_Instance()->
Collect_Objects(BoundingBox,
false, true, &list);
for (
...
|| !obj->As_PhysicalGameObj()->Peek_Model())
{
return;
}
