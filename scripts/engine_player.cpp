...
Find_Player(const WideStringClass &playerName)
...
Get_Player_Name_By_ID(int PlayerID) {
...
Change_Team_By_ID(int PlayerID, int Team) {
    cPlayer * x = Find_Player(PlayerID);
    if (!x) {
        return;
    }
...
Get_Kill_To_Death_Ratio(int PlayerID) {
    cPlayer * x = Find_Player(PlayerID);
    if (!x) {
        return 0;
    }
...
Get_Player_Color(int PlayerID, unsigned int *red, unsigned int *green, unsigned int *blue) {
    Vector3 v;
    cPlayer * x = Find_Player(PlayerID);
    if (!x) {
        return;
...
Set_Money(int PlayerID, float amount) {
    cPlayer * x = Find_Player(PlayerID);
    if (!x) {
        return;
    }
...
Set_Score(int PlayerID, float amount) {
    cPlayer * x = Find_Player(PlayerID);
    if (!x) {
        return;
    }
...
Set_Ladder_Points(int PlayerID, int amount) {
    cPlayer * x = Find_Player(PlayerID);
    if (!x) {
        return;
    }
...
Set_Rung(int PlayerID, int amount) {
    cPlayer * x = Find_Player(PlayerID);
    if (!x) {
        return;
    }
...
Tally_Team_Size(int team) {
    return TallyTeamSize(team);
}

float SCRIPTS_API

Get_Team_Score(int team) {
...
Get_Team_Color(unsigned int team, unsigned int *red, unsigned int *green, unsigned int *blue) {
...
Commands->
Give_Points(obj, (Score
*-1),false);
if (Score < 0)
{
Commands->
Give_Money(obj, Score,
false);
}
Commands->
Give_Money(obj, (Credits
*-1),false);
if (!Commands->
Get_Player_Type(obj)
)
...
Get_Part_Name(const char *name1) {
    SLNode<SoldierGameObj> *x = GameObjManager::StarGameObjList.Head();
    int count = 0;
...
Get_Team_Player_Count(int Team) {
    int Total = 0;
    SLNode<SoldierGameObj> *x = GameObjManager::StarGameObjList.Head();
    while (x) {
        GameObject * o = x->Data();
