...
SCRIPTS_API void Remove_Script(GameObject * obj,
const char *script
)
{
if (!obj)
...
SCRIPTS_API void Remove_Duplicate_Script(GameObject * obj,
const char *script
)
{
if (!obj)
...
SCRIPTS_API bool Is_Script_Attached(GameObject * obj,
const char *script
)
{
if (!obj)
...
return obj->
Find_Observer(script)
!= 0;
}

SCRIPTS_API void Attach_Script_Once(GameObject * obj,
const char *script,
const char *params
)
{
...
if (!
Is_Script_Attached(obj, script
))
{
Commands->
Attach_Script(obj, script, params
);
}
}
...
SCRIPTS_API void Attach_Script_Is_Preset(GameObject * obj,
const char *preset,
const char *script,
const char *params,
int team
...
SCRIPTS_API void Attach_Script_Is_Type(GameObject * obj, unsigned long
type,
const char *script,
const char *params,
int team
...
Find_Building_With_Script(int Team, int Type, const char *Script, GameObject *Caller) {
    SLNode<BuildingGameObj> *x = GameObjManager::BuildingGameObjList.Head();
    while (x) {
        GameObject * o = x->Data();
        if (o) {
            if ((Type == -1 || Building_Type(o) == Type) && (Get_Object_Type(o) == Team || Team == 2) &&
                !Is_Building_Dead(o) && Is_Script_Attached(o, Script) && o != Caller) {
                return o;
            }
        }
        x = x->Next();
...
Find_Object_With_Script(const char *script) {
    SLNode<BaseGameObj> *x = GameObjManager::GameObjList.Head();
    while (x) {
        BaseGameObj *o = x->Data();
        ScriptableGameObj * o2 = 0;
...
Find_Closest_Object_With_Script(const char *script, Vector3 pos) {
...
Find_All_Objects_With_Script_By_Distance(const char *script, SList<GameObject> &objects, Vector3 position) {
    ...
    for (SLNode<BaseGameObj> *objNode = GameObjManager::GameObjList.Head();
...
{
Commands->
Send_Custom_Event(sender, o2, message, param, delay
);
}
}
x = x->Next();
...
Get_ID ( o2 )
&&
Vector3::Distance_Squared ( Commands
->
Get_Position ( sender ), Commands
->
Get_Position ( o2 )
) <=
range *range
...
return (ScriptImpClass *)obj->
Find_Observer(script);
}

void SCRIPTS_API
Attach_Script_Occupants(GameObject
*obj,
const char *script,
const char *params
)
...
_vsnprintf_s(formattedParams, length
+1, length, params, vargs);

Commands->
Attach_Script(pObj, script, formattedParams
);
...
void SCRIPTS_API
Attach_Script_Once_V ( GameObject
* pObj,
const char *script,
const char *params, ...
)
{
if ( !pObj ||
Is_Script_Attached(pObj, script
))
return;
