...
public:
    ...
    SHARED_API int _cdecl

    Format(const wchar_t *format, ...);

    SHARED_API int _cdecl

    Format_Args(const wchar_t *format, const va_list &arg_list);
    ...
    SHARED_API WideStringClass

    Substring(int start, int length) const;
...
SCRIPTS_API const wchar_t *
wcsistr(const wchar_t *str, const wchar_t *substr); //like strstr but case insenstive and for wchar_t
#endif
