...
SCRIPTS_API _Enable_Stealth_Player
Enable_Stealth_Player;
SCRIPTS_API _Set_Fog_Enable_Player
Set_Fog_Enable_Player;
SCRIPTS_API _Set_Fog_Range_Player
Set_Fog_Range_Player;
SCRIPTS_API _Set_Background_Music_Player
Set_Background_Music_Player;
SCRIPTS_API _Fade_Background_Music_Player
Fade_Background_Music_Player;
SCRIPTS_API _Stop_Background_Music_Player
Stop_Background_Music_Player;
SCRIPTS_API _Enable_Radar_Player
Enable_Radar_Player;
SCRIPTS_API _Display_GDI_Player_Terminal_Player
Display_GDI_Player_Terminal_Player;
SCRIPTS_API _Display_NOD_Player_Terminal_Player
Display_NOD_Player_Terminal_Player;
SCRIPTS_API _Set_Screen_Fade_Color_Player
Set_Screen_Fade_Color_Player;
SCRIPTS_API _Set_Screen_Fade_Opacity_Player
Set_Screen_Fade_Opacity_Player;
SCRIPTS_API _Force_Camera_Look_Player
Force_Camera_Look_Player;
SCRIPTS_API _Enable_HUD_Player
Enable_HUD_Player;
SCRIPTS_API _Create_Sound_Player
Create_Sound_Player;
SCRIPTS_API _Create_2D_Sound_Player
Create_2D_Sound_Player;
SCRIPTS_API _Create_2D_WAV_Sound_Player
Create_2D_WAV_Sound_Player;
SCRIPTS_API _Create_3D_WAV_Sound_At_Bone_Player
Create_3D_WAV_Sound_At_Bone_Player;
SCRIPTS_API _Create_3D_Sound_At_Bone_Player
...
SCRIPTS_API Get_IP_Addressx
Get_IP_Address;
SCRIPTS_API Get_IP_Portx
Get_IP_Port;
SCRIPTS_API Get_IPx
Get_IP;
SCRIPTS_API Get_Bandwidthx
Get_Bandwidth;
SCRIPTS_API Get_Pingx
Get_Ping;
SCRIPTS_API Get_Kbitsx
Get_Kbits;
SCRIPTS_API sfop
...
SCRIPTS_API gtc
Get_Team_Credits;
SCRIPTS_API addConnectionAcceptanceFilterType
...
SCRIPTS_API _Stop_Sound_Player
Stop_Sound_Player;
SCRIPTS_API _Stop_Sound_Team
Stop_Sound_Team;
SCRIPTS_API ssa
...
Get_Enlisted_Purchase_Items(int team) {
    DynamicVectorClass<int> list;
    TeamPurchaseSettingsDefClass * t = TeamPurchaseSettingsDefClass::Get_Definition(
            (TeamPurchaseSettingsDefClass::TEAM) PTTEAM(team));
    for (int i = 0; i < 4; i++) {
...
Get_Purchase_Items(int team, int defType) {
    DynamicVectorClass<int> list;
    PurchaseSettingsDefClass * purchaseSettings = PurchaseSettingsDefClass::Find_Definition(
            (PurchaseSettingsDefClass::TYPE) defType, (PurchaseSettingsDefClass::TEAM) PTTEAM(team));
    if (purchaseSettings) {
        for (int i = 0; i < 10; i++) {
...
SCRIPTS_API bool Is_Preset_Purchasable_In_List(int id, int team, PurchaseSettingsDefClass::TYPE def) {
    PurchaseSettingsDefClass * purchaseSettings = PurchaseSettingsDefClass::Find_Definition(def,
                                                                                            (PurchaseSettingsDefClass::TEAM) PTTEAM(
                                                                                                    team));
    if (purchaseSettings && !purchaseSettings->Get_Page_Hidden() && !purchaseSettings->Get_Page_Busy() &&
        !purchaseSettings->Get_Page_Disabled()) {
        for (int i = 0; i < 10; i++) {
            if ((purchaseSettings->Get_Definition(i) == id) ||
                (purchaseSettings->Get_Alt_Definition(i, 0) == id) ||
                (purchaseSettings->Get_Alt_Definition(i, 1) == id) ||
                (purchaseSettings->Get_Alt_Definition(i, 2) == id)) {
                ...
                return !purchaseSettings->Get_Disabled(i) && !purchaseSettings->Get_Busy(i) &&
                       !purchaseSettings->Get_Hidden(i);
            }
        }
    }
    return false;
...
SCRIPTS_API bool Is_Infantry_Purchaseable(const char *preset, int team) {
    ...
    BaseControllerClass *baseController = BaseControllerClass::Find_Base(
            team); // This does not require a PurchaseSettingsDefClass team
...
SCRIPTS_API bool Is_Vehicle_Purchaseable(const char *preset, int team) {
    ...
    if (!baseController || (baseController->Can_Generate_Vehicles() && Can_Build_Ground(team))) {
        if (Is_Preset_Purchasable_In_List(id, team, PurchaseSettingsDefClass::TYPE_VEHICLES) &&
            !(Get_Vehicle_Definition_Mode(preset) == VEHICLE_TYPE_FLYING && !Is_Map_Flying()))
            return true;
...
SCRIPTS_API void Send_Message_With_Obj_Color(GameObject * obj,
const char *Msg
)
{
unsigned int Red = 0, Blue = 0, Green = 0;
...
SCRIPTS_API void
Send_Translated_Message_Team(unsigned long ID, int team, int red, int green, int blue, bool bPlaySound) {
...
SCRIPTS_API void Set_Occupants_Fade(GameObject * obj, float
red,
float green,
float blue,
float opacity
...
{
Set_Screen_Fade_Color_Player(o
->
Get_Occupant(i), red, green, blue,
0);
Set_Screen_Fade_Opacity_Player(o
->
Get_Occupant(i), opacity,
0);
}
}
}
...
SCRIPTS_API void SendShaderParamPlayer(GameObject * player,
...
SCRIPTS_API void SendShaderParamObjPlayer(GameObject * player,
const char *parameter,
const char *value, GameObject
* obj)
{
...
Create_Vehicle(const char *Preset_Name, float Delay, GameObject *Owner, int Team) {
    ...
    if (vf && ((!vf->Is_Available()))) {
        if (((!Vehicle_Preset_Is_Air(preset)) || (!vf->Get_Definition().Get_Build_Ground_Only())) &&
            !Vehicle_Preset_Is_Naval(preset)) {
            return false;
        }
    }
...
void SuperweaponGameObj::Launch()
...
bool PhysicsSceneClass::Cast_AABox(class PhysAABoxCollisionTestClass &, bool)
...
bool PhysicsSceneClass::Cast_OBBox(class PhysOBBoxCollisionTestClass &, bool)
...
SCRIPTS_API REF_DEF3(PhysicsSceneClass *, PhysicsSceneClass::TheScene,

0x00855444, 0x0085462C, 0x00872BCC);
...
*
TeamPurchaseSettingsDefClass::Get_Definition(TeamPurchaseSettingsDefClass::TEAM
team)
AT2(0x006F3430,0x006F29F0);
RENEGADE_FUNCTION
        PurchaseSettingsDefClass
*
PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE
type,
...
bool SoldierGameObj::Is_In_Elevator()
...
cPlayer::Set_Player_Type(int type)
...
SCRIPTS_API Get_Color_For_Team(int teamId)
