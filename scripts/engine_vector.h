...
public:
    ...
    explicit VectorClass(int size = 0, T const *array = 0) : Vector(0), VectorMax(size), IsValid(true),
                                                             IsAllocated(false) {
        if (size) {
            if (array) {
...
public:
    ...
    DynamicVectorClass(const DynamicVectorClass<T> &vector) : VectorClass<T>(vector), GrowthStep(vector.GrowthStep),
...
template<typename T>
T *Binary_Search(T *list, int count, T &var) {
    T *list2 = list;
    int pos = count;
    while (pos > 0) {
        T *list3 = &list2[pos / 2];
