...
const char *Get_Translated_Preset_Name_Part(GameObject * obj) {
    ...
    if (obj->As_BuildingGameObj() || (obj->As_PhysicalGameObj() && obj->As_PhysicalGameObj()->Peek_Physical_Object() &&
                                      obj->As_PhysicalGameObj()->Peek_Physical_Object()->As_DecorationPhysClass()) ||
        Is_Harvester_Preset(obj)) {
...
if (
auto killerVehicle = Get_Vehicle(killer)
)
{
...
auto killedObj = obj;
if (
auto killedVehicle = Get_Vehicle(obj)
)
{
...
SSGMGameLog::Log_Gamelog("KILLED;%s;%d;%s;%d;%d;%d;%d;%d;%s;%d;%d;%d;%d;%s;%s;%s;%ls;%ls", objectType, Commands->
Get_ID(obj), Commands
->
Get_Preset_Name(obj),
int(victimpos
...
->
Get_Facing(killer)
), killerWeapon, str1, str2,
Get_Wide_Player_Name(killer), Get_Wide_Player_Name(killedObj)
);
delete[]
str1;
delete[]
...
SCRIPTS_API void Print_C4(GameObject * Player) {
    ...
    StringClass msg;
    msg.Format("ppage %d Remote: %d  - Proximity: %d - Total: %d - Limit: %d", Get_Player_ID(Player), Remote, Prox,
               Remote + Prox, Get_Mine_Limit());
    Console_Input(msg);
}
...
bool SSGMGameManager::RadioHook(int PlayerType, int PlayerID, int AnnouncementID, int IconID,
                                AnnouncementEnum AnnouncementType) {
    for (int i = 0; i < RegisteredEvents[EVENT_RADIO_HOOK].Count(); i++) {
        if (!RegisteredEvents[EVENT_RADIO_HOOK][i]->OnRadioCommand(PlayerType, PlayerID, AnnouncementID, IconID,
                                                                   AnnouncementType)) {
            return false;
        }
    }
...
RENEGADE_FUNCTION
void Destroy_Pending_Scripts() AT2(0x006A83D0, 0x006A7C70);
            ...
            if (DisableExtraWeapons) {
                ...
                for (int i = 0; i < observers.Count(); i++) {
                    if (!_stricmp(observers[i]->Get_Name(), "M00_GrantPowerup_Created")) {
                        deleteobservers.Add((ScriptImpClass * )
                        observers[i]);
                    }
                }
    ...
    bool
    SSGMGameManager::StockDamageHook(PhysicalGameObj *damager, PhysicalGameObj *target, float damage, uint warheadId) {
    ...
    bool
    SSGMGameManager::TtDamageHook(PhysicalGameObj *damager, PhysicalGameObj *target, const AmmoDefinitionClass *ammo,
                                  const char *bone) {
    ...
    void SSGMGameManager::LoadLevelHook() {
        ...
        while (x) {
            ...
            if (o) {
                if (DisablePowerPlants && o->Get_Definition().Get_Type() == BuildingConstants::TYPE_POWER_PLANT) {
                ...
                } else if (DisableSoldierFactories &&
                           o->Get_Definition().Get_Type() == BuildingConstants::TYPE_SOLDIER_FACTORY) {
                    Commands->Apply_Damage(o, 99999, "Death", 0);
                } else if (DisableVehicleFactories &&
                           o->Get_Definition().Get_Type() == BuildingConstants::TYPE_VEHICLE_FACTORY) {
                    Commands->Apply_Damage(o, 99999, "Death", 0);
                    ...
                    Commands->Apply_Damage(o, 99999, "Death", 0);
                } else if (DisableConstructionYards &&
                           o->Get_Definition().Get_Type() == BuildingConstants::TYPE_CONYARD) {
                    Commands->Apply_Damage(o, 99999, "Death", 0);
                } else if (DisableBaseDefenses &&
                           o->Get_Definition().Get_Type() == BuildingConstants::TYPE_BASE_DEFENSE) {
                    Commands->Apply_Damage(o, 99999, "Death", 0);
                } else if (DisableTechCenters &&
                           o->Get_Definition().Get_Type() == BuildingConstants::TYPE_TECH_CENTER) {
                    Commands->Apply_Damage(o, 99999, "Death", 0);
                } else if (DisableNavalFactories &&
                           o->Get_Definition().Get_Type() == BuildingConstants::TYPE_NAVAL_FACTORY) {
                    Commands->Apply_Damage(o, 99999, "Death", 0);
        ...
        SSGMGameLog::Log_Gamelog("CONFIG;%u;%ls", The_Game()->Get_Time_Limit_Minutes(),
                                 The_Game()->Get_Game_Title().Peek_Buffer());
        Vector3 loc;
        loc.X = 0.0f;
        loc.Y = 0.0f;
        loc.Z = 0.0f;
    ...
    void SSGM_Log_Scores::Timer_Expired(GameObject *obj, int number) {
        if (number == 1) {
            ...
            for (int j = 1; j <= x; j++) {
                ...
                if (player) {
                    SSGMGameLog::Log_Gamelog("SCORE;%d;%d;%d", Commands->Get_ID(player),
                                             (int) Commands->Get_Points(player), (int) Commands->Get_Money(player));
                }
            }
        }
    }
    ...
    void SSGMGameManager::GameOverHook() {
        ...
        str.Format("Current game on map %s has ended. Game was won by %s by %s.", The_Game()->Get_Map_Name(), str2,
                   WinType);
        delete[] str2;
        SSGMGameLog::Log_Message(str, "_GENERAL");
    ...
    int SSGMGameManager::PowerupPurchaseHook(BaseControllerClass *base, GameObject *purchaser, unsigned int cost,
        ...
        for (int i = 0; i < RegisteredEvents[EVENT_POWERUP_PURCHASE_HOOK].Count(); i++) {
            int ret = RegisteredEvents[EVENT_POWERUP_PURCHASE_HOOK][i]->OnPowerupPurchase(base, purchaser, cost, preset,
                                                                                          data);
            if (ret == -2) {
                fp = true;
            } else if (ret != -1) {
    ...
    int SSGMGameManager::VehiclePurchaseHook(BaseControllerClass *base, GameObject *purchaser, unsigned int cost,
                                             unsigned int preset, const char *data) {
        ...
        for (int i = 0; i < RegisteredEvents[EVENT_VEHICLE_PURCHASE_HOOK].Count(); i++) {
            int ret = RegisteredEvents[EVENT_VEHICLE_PURCHASE_HOOK][i]->OnVehiclePurchase(base, purchaser, cost, preset,
                                                                                          data);
            if (ret == -2) {
                fp = true;
            } else if (ret != -1) {
    ...
    int SSGMGameManager::CharacterPurchaseHook(BaseControllerClass *base, GameObject *purchaser, unsigned int cost,
                                               unsigned int preset, const char *data) {
        SSGM_Soldier *script = (SSGM_Soldier *) Find_Script_On_Object(purchaser, "SSGM_Soldier");
        ...
        for (int i = 0; i < RegisteredEvents[EVENT_CHARACTER_PURCHASE_HOOK].Count(); i++) {
            int ret = RegisteredEvents[EVENT_CHARACTER_PURCHASE_HOOK][i]->OnCharacterPurchase(base, purchaser, cost,
                                                                                              preset, data);
            if (ret == -2) {
                fp = true;
            } else if (ret != -1) {
    ...
    void SSGM_Log_Mine::Created(GameObject *obj) {
        ...
        SSGMGameLog::Log_Gamelog("CREATED;OBJECT;%d;%s;%d;%d;%d;%d;%d;%d;%d;%d", Commands->Get_ID(obj),
                                 Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z),
                                 int(Commands->Get_Facing(obj)), int(Commands->Get_Max_Health(obj)),
                                 int(Commands->Get_Max_Shield_Strength(obj)), Get_Object_Type(obj),
                                 Commands->Get_ID(Get_C4_Planter(obj)));
    }
    ...
    void SSGM_Log_Mine::Destroyed(GameObject *obj) {
        ...
        if (!WasDisarmed && WasDetonated) {
            ...
            str.Format("%s %s has detonated (Owner: %ls - Attached to: %s)", str4, str5,
                       Get_Wide_Player_Name(Get_C4_Planter(obj)), str6);
            delete[] str4;
            delete[] str5;
            delete[] str6;
            SSGMGameLog::Log_Message(str, "_C4");
            SSGMGameLog::Log_Gamelog("DETONATED;OBJECT;%d;%s;%d;%d;%d", Commands->Get_ID(obj),
                                     Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z));
        }
        SSGMGameLog::Log_Gamelog("DESTROYED;OBJECT;%d;%s;%d;%d;%d", Commands->Get_ID(obj),
                                 Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z));
    }
    ...
    void SSGM_Log_Mine::Damaged(GameObject *obj, GameObject *damager, float amount) {
        ...
        if (amount != 0) {
            ...
            SSGMGameLog::Log_Gamelog("DAMAGED;OBJECT;%d;%s;%d;%d;%d;%d;%d;%s;%d;%d;%d;%d;%f;%d;%d",
    ...
    void SSGM_Log_Mine::Killed(GameObject *obj, GameObject *killer) {
        ...
        const char *str2 = Get_Translated_Preset_Name_Ex(killer);
        SSGMGameLog::Log_Gamelog("KILLED;OBJECT;%d;%s;%d;%d;%d;%d;%d;%s;%d;%d;%d;%d;%s;%s;%s", Commands->Get_ID(obj),
        ...
        const char *str5 = Get_Translated_Preset_Name_Ini(obj);
        const char *str6 = Get_Translated_Preset_Name_Ini(Get_C4_Attached(obj));
        str3.Format("%s %s has been disarmed by %ls (Owner: %ls - Attached to: %s)", str4, str5,
                    Get_Wide_Player_Name(killer), Get_Wide_Player_Name(Get_C4_Planter(obj)), str6);
        delete[] str4;
        delete[] str5;
        delete[] str6;
    ...
    void SSGM_Log_Beacon::Created(GameObject *obj) {
        ...
        SSGMGameLog::Log_Gamelog("CREATED;OBJECT;%d;%s;%d;%d;%d;%d;%d;%d;%d;%d", Commands->Get_ID(obj),
                                 Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z),
                                 int(Commands->Get_Facing(obj)), int(Commands->Get_Max_Health(obj)),
                                 int(Commands->Get_Max_Shield_Strength(obj)), Commands->Get_Player_Type(obj),
                                 Commands->Get_ID(Get_Beacon_Planter(obj)));
    }
    ...
    void SSGM_Powerup::Custom(GameObject *obj, int message, int param, GameObject *sender) {
        if (message == CUSTOM_EVENT_POWERUP_GRANTED) {
            if (strstr(Commands->Get_Preset_Name(obj), "CnC_POW_MineRemote_02")) {
                ...
                if (!SBHCanPickupDropWeapons) {
                    if (strcmp(Commands->Get_Preset_Name(sender), "CnC_Nod_FlameThrower_2SF") == 0) {
                       ...
                       (strstr(Commands->Get_Preset_Name(obj), "POW_Chaingun_Player_Nod"))) {
                ...
                Create_2D_WAV_Sound_Player(sender, "m00pwlr_aqob0004i1evag_snd.wav");
                       ...
                       (strstr(Commands->Get_Preset_Name(obj), "CnC_POW_VoltAutoRifle_Player_Nod"))) {
                ...
                if (!SBHCanPickupDropWeapons) {
                    if (strcmp(Commands->Get_Preset_Name(sender), "CnC_Nod_FlameThrower_2SF") == 0) {
                       ...
                       (strstr(Commands->Get_Preset_Name(obj), "POW_SniperRifle_Player_Nod"))) {
                ...
                if (!SBHCanPickupDropWeapons) {
                    if (strcmp(Commands->Get_Preset_Name(sender), "CnC_Nod_FlameThrower_2SF") == 0) {
                       ...
                       (strstr(Commands->Get_Preset_Name(obj), "POW_Armor_100"))) {
            ...
            } else if ((strstr(Commands->Get_Preset_Name(obj), "CnC_POW_Health_Max")) ||
                       (strstr(Commands->Get_Preset_Name(obj), "POW_Health_025")) ||
                       (strstr(Commands->Get_Preset_Name(obj), "POW_Health_050")) ||
                       (strstr(Commands->Get_Preset_Name(obj), "POW_Health_100"))) {
        ...
        if (strstr(Commands->Get_Preset_Name(obj), "CnC_GDI_MiniGunner_1Off")) {
            ...
            if (RandomNum == 1 || RandomNum == 2) {
                ...
                Destroy_Script();
                   ...
                   (strstr(Commands->Get_Preset_Name(obj), "CnC_Sydney_PowerSuit_ALT2"))) {
            if (RandomNum == 1 || RandomNum == 2) {
                BackpackScript = Commands->Create_Object("POW_Tissue_Nanites", DeathPlace);
                Set_Powerup_Always_Allow_Grant(BackpackScript, true);
            ...
            if (RandomNum == 1 || RandomNum == 2) {
                ...
                Destroy_Script();
                   ...
                   (strstr(Commands->Get_Preset_Name(obj), "CnC_Nod_MiniGunner_3Boss_ALT2"))) {
            if (RandomNum == 1 || RandomNum == 2) {
                BackpackScript = Commands->Create_Object("POW_Tissue_Nanites", DeathPlace);
                Set_Powerup_Always_Allow_Grant(BackpackScript, true);
            ...
            if (RandomNum == 1 || RandomNum == 2) {
                ...
                Destroy_Script();
                   ...
                   (strstr(Commands->Get_Preset_Name(obj), "CnC_Ignatio_Mobius_ALT2"))) {
            if (RandomNum == 1 || RandomNum == 2) {
                BackpackScript = Commands->Create_Object("POW_Tissue_Nanites", DeathPlace);
                Set_Powerup_Always_Allow_Grant(BackpackScript, true);
                ...
                Destroy_Script();
                   ...
                   (strstr(Commands->Get_Preset_Name(obj), "CnC_Nod_FlameThrower_3Boss_ALT2"))) {
            if (RandomNum == 1 || RandomNum == 2) {
                BackpackScript = Commands->Create_Object("POW_Tissue_Nanites", DeathPlace);
                Set_Powerup_Always_Allow_Grant(BackpackScript, true);
