...
void SSGMGameLog::Think() {
    ...
    for (int i = 0; i < Connections.Count(); ++i) {
        int chars_read = recv(Connections[i]->socket, Connections[i]->cbuffer + Connections[i]->cbufferFilled,
                              _countof(Connections[i]->cbuffer) - 1 - Connections[i]->cbufferFilled, 0);
        if (chars_read == 0 || (chars_read == SOCKET_ERROR && WSAGetLastError() !=
                                                              WSAEWOULDBLOCK)) // Graceful close or error. Note that if the buffer is full, 0 bytes are read, and this is interpreted as a graceful close too!
        {
            closesocket(Connections[i]->socket);
        ...
        } else if (chars_read > 0) // Data received
        {
            ...
            for (;;) {
                ...
                if (!lineEndPos) break;
                *lineEndPos = '\0';
                if (lineEndPos - lineStartPos > 0 && *(lineEndPos - 1) == '\r')
                    *(lineEndPos - 1) = '\0'; // Remove trailing \r if CRLF line endings are used.
