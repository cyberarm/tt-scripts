...
void SSGM_Soldier::Created(GameObject *obj) {
    ...
    if (Commands->Is_A_Star(obj)) {
        ...
        if (strcmp(Commands->Get_Preset_Name(obj), "GDI_Engineer_2SF") ==
            0)//uberwire crate grants this hotwire character
        {
    ...
    SSGMGameLog::Log_Gamelog("CREATED;SOLDIER;%d;%s;%d;%d;%d;%d;%d;%d;%d;%s", Commands->Get_ID(obj),
                             Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z),
                             int(Commands->Get_Facing(obj)), int(Commands->Get_Max_Health(obj)),
                             int(Commands->Get_Max_Shield_Strength(obj)), Commands->Get_Player_Type(obj), str);
    delete[] str;
}
...
void SSGM_Soldier::Destroyed(GameObject *obj) {
    Vector3 pos = Commands->Get_Position(obj);
    SSGMGameLog::Log_Gamelog("DESTROYED;SOLDIER;%d;%s;%d;%d;%d;%d", Commands->Get_ID(obj),
                             Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z),
...
void SSGM_Soldier::Damaged(GameObject *obj, GameObject *damager, float amount) {
    if (amount != 0) {
        ...
        SSGMGameLog::Log_Gamelog("DAMAGED;SOLDIER;%d;%s;%d;%d;%d;%d;%d;%s;%d;%d;%d;%d;%f;%d;%d;%d",
...
void SSGM_Soldier::Killed(GameObject *obj, GameObject *killer) {
    ...
    if (weapons) {
        GameObject * pup = Commands->Create_Object((*weapons)[Commands->Get_Random_Int(0, weapons->Count())],
