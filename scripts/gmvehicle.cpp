        ...
        if ((!scr->OwnerID) || (!Commands->Find_Object(scr->OwnerID))) {
            if (Get_Vehicle_Driver(veh) == obj) {
                ...
                if (VehicleLockIcons) {
                    ...
                    if (!veh->As_VehicleGameObj()->Is_Stealth_Enabled() &&
                        !Find_Script_On_Object(veh, "SH_UndergroundVehicle")) {
...
SCRIPTS_API void Lock_Vehicle(GameObject * obj) {
    ...
    if (boundveh) {
        if (!boundveh->Get_Owner()) {
            ...
            if (!boundveh->As_VehicleGameObj()->Is_Stealth_Enabled() &&
                !Find_Script_On_Object(boundveh, "SH_UndergroundVehicle")) {
                if (scr->IconModelID) {
...
SCRIPTS_API void Unlock_Vehicle(GameObject * obj, bool
silent)
{
VehicleGameObj *boundveh = Find_My_Vehicle(obj);
if (boundveh)
...
SSGM_Vehicle *scr = (SSGM_Vehicle *) Find_Script_On_Object(boundveh, "SSGM_Vehicle");
if (!boundveh->As_VehicleGameObj()->Is_Stealth_Enabled() && !
Find_Script_On_Object(boundveh,
"SH_UndergroundVehicle"))
{
if (scr->IconModelID)
{
Commands->
...
{
StringClass str;
str.Format("ppage %d Your vehicle has been unlocked.",
Get_Player_ID(obj)
);
Console_Input(str);
}
...
{
StringClass str;
str.Format("ppage %d Your vehicle is not locked.",
Get_Player_ID(obj)
);
Console_Input(str);
}
...
{
StringClass str;
str.Format("ppage %d You have no vehicle bound to you.",
Get_Player_ID(obj)
);
Console_Input(str);
}
...
SCRIPTS_API void Unbind_Vehicle(GameObject * obj, bool
silent)
{
VehicleGameObj *boundveh = Find_My_Vehicle(obj);
if (boundveh)
...
{
StringClass str;
str.Format("ppage %d Your vehicle has been unbound.",
Get_Player_ID(obj)
);
Console_Input(str);
}
...
void SSGM_Vehicle::Created(GameObject *obj) {
    ...
    if (Get_Vehicle_Mode(obj) == VEHICLE_TYPE_TURRET) {
        ...
        SSGMGameLog::Log_Gamelog("CREATED;BUILDING;%d;%s;%d;%d;%d;%d;%f;%f;%d", Commands->Get_ID(obj),
                                 Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z),
                                 int(Commands->Get_Facing(obj)), Commands->Get_Max_Health(obj),
                                 Commands->Get_Max_Shield_Strength(obj), Get_Object_Type(obj));
    } else {
        Vector3 pos = Commands->Get_Position(obj);
        SSGMGameLog::Log_Gamelog("CREATED;VEHICLE;%d;%s;%d;%d;%d;%d;%d;%d;%d", Commands->Get_ID(obj),
                                 Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z),
...
void SSGM_Vehicle::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == CUSTOM_EVENT_VEHICLE_ENTERED) {
        if (VehicleOwnership) {
            ...
            if (OwnerID) {
                if (Commands->Find_Object(OwnerID)) {
                    if (Commands->Find_Object(OwnerID)->As_DamageableGameObj()->Get_Player_Type() !=
                        sender->As_DamageableGameObj()->Get_Player_Type()) {
                        StringClass str;
                        str.Format("ppage %d The enemy has stolen your vehicle.",
                                   Get_Player_ID(Commands->Find_Object(OwnerID)));
                        Console_Input(str);
                    ...
                    if (Commands->Find_Object(OwnerID) != sender) {
                        if (Get_Vehicle_Occupant(obj, 0) == sender) {
                            ...
                            str.Format("ppage %d Player %s has entered your vehicle.",
                                       Get_Player_ID(Commands->Find_Object(OwnerID)), str2);
                            delete[] str2;
                            Console_Input(str);
                        }
                    }
        ...
        SSGMGameLog::Log_Gamelog("ENTER;%d;%s;%d;%d;%d;%d;%s;%d;%d;%d", Commands->Get_ID(obj),
                                 Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z),
                                 Commands->Get_ID(sender), Commands->Get_Preset_Name(sender), int(senderpos.Y),
                                 int(senderpos.X), int(senderpos.Z));
    } else if (type == CUSTOM_EVENT_VEHICLE_EXITED) {
        Vector3 pos = Commands->Get_Position(obj);
        Vector3 senderpos = Commands->Get_Position(sender);
        SSGMGameLog::Log_Gamelog("EXIT;%d;%s;%d;%d;%d;%d;%s;%d;%d;%d", Commands->Get_ID(obj),
                                 Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z),
                                 Commands->Get_ID(sender), Commands->Get_Preset_Name(sender), int(senderpos.Y),
                                 int(senderpos.X), int(senderpos.Z));
    }
...
void SSGM_Vehicle::Destroyed(GameObject *obj) {
    ...
    if (Get_Vehicle_Mode(obj) == VEHICLE_TYPE_TURRET) {
        Vector3 pos = Commands->Get_Position(obj);
        SSGMGameLog::Log_Gamelog("DESTROYED;BUILDING;%d;%s;%d;%d;%d", Commands->Get_ID(obj),
                                 Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z));
    } else {
        Vector3 pos = Commands->Get_Position(obj);
        SSGMGameLog::Log_Gamelog("DESTROYED;VEHICLE;%d;%s;%d;%d;%d", Commands->Get_ID(obj),
                                 Commands->Get_Preset_Name(obj), int(pos.Y), int(pos.X), int(pos.Z));
    }
}
...
void SSGM_Vehicle::Damaged(GameObject *obj, GameObject *damager, float amount) {
    if (amount != 0) {
        if (Get_Vehicle_Mode(obj) == VEHICLE_TYPE_TURRET) {
            ...
            SSGMGameLog::Log_Gamelog("DAMAGED;BUILDING;%d;%s;%d;%d;%d;%d;%d;%s;%d;%d;%d;%d;%f;%d;%d;%d",
                                     ...
                                     int(Commands->Get_Points(obj)));
        } else {
            Vector3 victimpos = Commands->Get_Position(obj);
            Vector3 damagerpos = Commands->Get_Position(damager);
            SSGMGameLog::Log_Gamelog("DAMAGED;VEHICLE;%d;%s;%d;%d;%d;%d;%d;%s;%d;%d;%d;%d;%f;%d;%d;%d",
                                     Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), int(victimpos.Y),
                                     int(victimpos.X), int(victimpos.Z), int(Commands->Get_Facing(obj)),
                                     Commands->Get_ID(damager), Commands->Get_Preset_Name(damager), int(damagerpos.Y),
                                     int(damagerpos.X), int(damagerpos.Z), int(Commands->Get_Facing(damager)), amount,
                                     int(Commands->Get_Health(obj)), int(Commands->Get_Shield_Strength(obj)),
                                     int(Commands->Get_Points(damager)));
        }
    }
    if (ShowExtraMessages) {
        ...
        if (triggerrepaired) {
            if ((strcmp(Commands->Get_Preset_Name(obj), "CnC_Nod_Harvester") == 0) ||
                (strcmp(Commands->Get_Preset_Name(obj), "CnC_GDI_Harvester") == 0)) {
                if (amount < 0.0f) {
...
void SSGM_Vehicle_Wreckage::Damaged(GameObject *obj, GameObject *damager, float amount) {
    if (amount < 0) {
        if (Commands->Get_Max_Health(obj) == Commands->Get_Health(obj) &&
            Commands->Get_Max_Shield_Strength(obj) == Commands->Get_Shield_Strength(obj)) {
            Vector3 pos = Commands->Get_Position(obj);
            pos.Z += 3;
            float facing = Commands->Get_Facing(obj);
