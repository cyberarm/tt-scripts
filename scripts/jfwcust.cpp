...
void JFW_Created_Send_Custom::Created(GameObject *obj) {
    Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("ID")), Get_Int_Parameter("Message"), 0,
                                0);
    Destroy_Script();
}
...
void JFW_Set_Animation_On_Custom::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == Get_Int_Parameter("Message")) {
        ...
        Commands->Set_Animation(obj, Get_Parameter("Animation"), Get_Bool_Parameter("Loop"), subobject, firstframe,
...
void JFW_Enable_Disable_Animation_On_Custom::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == Get_Int_Parameter("Enable")) {
        if (!enabled) {
            ...
            Commands->Set_Animation(obj, Get_Parameter("Animation"), true, subobject, firstframe, -1,
                                    Get_Bool_Parameter("Blended"));
        }
    }
    ...
    if (!trigger) {
        if ((custom1) && (custom2) && (custom3) && (custom4) && (custom5) && (custom6) && (custom7) && (custom8)) {
            ...
            Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("ID")),
                                        Get_Int_Parameter("Message"), Get_Int_Parameter("Param"), 0);
        }
    }
}
...
void JFW_Custom_Multiple_Send_Custom::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (enabled) {
        ...
        if (times_recieved == Get_Int_Parameter("RecieveCount")) {
            enabled = false;
            Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("SendID")),
                                        Get_Int_Parameter("SendMessage"), Get_Int_Parameter("SendParam"), 0);
        }
    }
}
...
void JFW_Create_Destroy_Object_On_Custom_Animate::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == Get_Int_Parameter("CreateMessage")) {
        if (!ObjectID) {
            ...
            Commands->Set_Animation(object, Get_Parameter("Animation"), Get_Bool_Parameter("Loop"), subobject,
                                    firstframe, Get_Float_Parameter("LastFrame"), Get_Bool_Parameter("Blended"));
        }
    }
    if (type == Get_Int_Parameter("DestroyMessage")) {
        if (ObjectID) {
...
void JFW_Create_Destroy_Object_On_Custom_Animate_2::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    ...
    if (type == Get_Int_Parameter("CreateMessage")) {
        if (!ObjectID) {
            ...
            Commands->Set_Animation(object, Get_Parameter("CreateAnimation"), Get_Bool_Parameter("CreateLoop"),
                                    subobject, firstframe, Get_Float_Parameter("CreateLastFrame"),
                                    Get_Bool_Parameter("CreateBlended"));
        }
    }
    if (type == Get_Int_Parameter("DestroyMessage")) {
        if (ObjectID) {
            ...
            Commands->Set_Animation(Commands->Find_Object(ObjectID), Get_Parameter("DestroyAnimation"), false,
                                    subobject, firstframe, Get_Float_Parameter("DestroyLastFrame"),
                                    Get_Bool_Parameter("DestroyBlended"));
            Commands->Start_Timer(obj, this, timertime, timernumber);
        }
    }
}
...
void JFW_Damaged_Send_Custom_Amount::Damaged(GameObject *obj, GameObject *damager, float amount) {
    if (amount < Get_Float_Parameter("Damage")) {
        Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("ID")), Get_Int_Parameter("Message1"),
                                    0, 0);
    } else {
        Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("ID")), Get_Int_Parameter("Message2"),
                                    0, 0);
    }
}
...
void JFW_Custom_Send_Ordered_Customs::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == Get_Int_Parameter("WaitCustom")) {
        Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter(CurrentCustom)),
                                    Get_Int_Parameter(CurrentCustom + 8), 0, 0);
        CurrentCustom++;
        if (CurrentCustom > 7) {
            CurrentCustom = 0;
        }
...
void JFW_Custom_Send_Custom_Multiple_IDs::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == Get_Int_Parameter("WaitCustom")) {
        for (int CurrentCustom = 0; CurrentCustom <= 7; CurrentCustom++) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter(CurrentCustom)),
                                        Get_Int_Parameter("SendCustom"), 0, 0);
        }
    }
}
...
void JFW_Wind_Custom::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (Get_Int_Parameter("Message") == type) {
        if (Get_Float_Parameter("Delay") > 0)
            ...
            Commands->Set_Wind(Get_Float_Parameter("Wind_Heading"), Get_Float_Parameter("Wind_Speed"),
                               Get_Float_Parameter("Wind_Variability"), 1.0f);
    }
}
...
void JFW_Lightning_Custom::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (Get_Int_Parameter("Message") == type) {
        if (Get_Float_Parameter("Delay") > 0)
            ...
            Commands->Set_Lightning(Get_Float_Parameter("Lightning_Intensity"), Get_Float_Parameter("Start_Distance"),
                                    Get_Float_Parameter("End_Distance"), Get_Float_Parameter("Lightning_Heading"),
...
void JFW_Precipitation_Custom::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (Get_Int_Parameter("Message") == type) {
        if (Get_Float_Parameter("Delay") > 0)
            ...
            if (!_stricmp(Get_Parameter("Type"), "Snow"))
                Commands->Set_Snow(Get_Float_Parameter("Density"), Get_Float_Parameter("Density"), true);
            else if (!_stricmp(Get_Parameter("Type"), "Ash"))
                Commands->Set_Ash(Get_Float_Parameter("Density"), Get_Float_Parameter("Density"), true);
            else if (!_stricmp(Get_Parameter("Type"), "Rain"))
                Commands->Set_Rain(Get_Float_Parameter("Density"), Get_Float_Parameter("Density"), true);
            else {
                Commands->Set_Snow(0.0f, 0.0f, true);
                Commands->Set_Ash(0.0f, 0.0f, true);
                Commands->Set_Rain(0.0f, 0.0f, true);
...
void JFW_Send_Custom_Distance_Objects_Timer::Timer_Expired(GameObject *obj, int number) {
    if (number == Get_Int_Parameter("TimerNum")) {
        Send_Custom_All_Objects_Area(Get_Int_Parameter("Message"), Commands->Get_Position(obj),
                                     Get_Float_Parameter("Distance"), obj, Get_Int_Parameter("Player_Type"));
        Commands->Start_Timer(obj, this, Get_Float_Parameter("Time"), Get_Int_Parameter("TimerNum"));
    }
}
...
void JFW_Send_Message_On_Key::KeyHook() {
    Send_Message_Player(Owner(), Get_Int_Parameter("Red"), Get_Int_Parameter("Green"), Get_Int_Parameter("Blue"),
...
ScriptRegistrant<JFW_Send_Custom_Distance_Objects_Timer> JFW_Send_Custom_Distance_Objects_Timer_Registrant(
        "JFW_Send_Custom_Distance_Objects_Timer", "Distance:float,Message:int,Time:float,TimerNum:int,Player_Type:int");
ScriptRegistrant<JFW_Send_Custom_Distance_Objects_Custom> JFW_Send_Custom_Distance_Objects_Custom_Registrant(
        "JFW_Send_Custom_Distance_Objects_Custom", "Distance:float,Message:int,Player_Type:int,ListenMessage:int");
ScriptRegistrant<JFW_Send_Custom_All_Objects_Timer> JFW_Send_Custom_All_Objects_Timer_Registrant(
        "JFW_Send_Custom_All_Objects_Timer", "Message:int,Time:float,TimerNum:int,Player_Type:int");
ScriptRegistrant<JFW_Send_Custom_All_Objects_Custom> JFW_Send_Custom_All_Objects_Custom_Registrant(
        "JFW_Send_Custom_All_Objects_Custom", "Message:int,Player_Type:int,ListenMessage:int");
ScriptRegistrant<JFW_Send_Message_Preset> JFW_Send_Message_Preset_Registrant("JFW_Send_Message_Preset",
                                                                             "Preset:string,CreateMessage:int,DestroyMessage:int");
ScriptRegistrant<JFW_Destroy_Create_Object_On_Custom> JFW_Destroy_Create_Object_On_Custom_Registrant(
        "JFW_Destroy_Create_Object_On_Custom", "Message:int,ID:int,Preset:string");
ScriptRegistrant<JFW_Fog_Custom> JFW_Fog_Custom_Registrant("JFW_Fog_Custom",
                                                           "Fog_Enable:int,Fog_Start_Distance=0.000:float,Fog_End_Distance=0.000:float,Delay=0.000:float,Message:int");
ScriptRegistrant<JFW_War_Blitz_Custom> JFW_War_Blitz_Custom_Registrant("JFW_War_Blitz_Custom",
                                                                       "War_Blitz_Intensity=0.000:float,Start_Distance=0.000:float,End_Distance=1.000:float,War_Blitz_Heading=0.000:float,War_Blitz_Distribution=1.000:float,Delay=0.000:float,Message:int");
ScriptRegistrant<JFW_Lightning_Custom> JFW_Lightning_Custom_Registrant("JFW_Lightning_Custom",
                                                                       "Lightning_Intensity=0.000:float,Start_Distance=0.000:float,End_Distance=1.000:float,Lightning_Heading=0.000:float,Lightning_Distribution=1.000:float,Delay=0.000:float,Message:int");
ScriptRegistrant<JFW_Wind_Custom> JFW_Wind_Custom_Registrant("JFW_Wind_Custom",
                                                             "Wind_Heading=0.000:float,Wind_Speed=0.000:float,Wind_Variability=0.000:float,Delay=0.000:float,Message:int");
ScriptRegistrant<JFW_Precipitation_Custom> JFW_Precipitation_Custom_Registrant("JFW_Precipitation_Custom",
                                                                               "Type=Rain:string,Density=0.000:float,Delay=0.000:float,Reset_Precipitation=0:int,Message:int");
ScriptRegistrant<JFW_Clouds_Custom> JFW_Clouds_Custom_Registrant("JFW_Clouds_Custom",
                                                                 "Cloud_Cover=0.000:float,Cloud_Gloominess=0.000:float,Delay=0.000:float,Message:int");
ScriptRegistrant<JFW_Enable_Stealth_On_Custom> JFW_Enable_Stealth_On_Custom_Registrant("JFW_Enable_Stealth_On_Custom",
                                                                                       "Message:int,Enable:int");
ScriptRegistrant<JFW_Console_Message_On_Custom> JFW_Console_Message_On_Custom_Registrant(
        "JFW_Console_Message_On_Custom", "Message:int,Output:string");
ScriptRegistrant<JFW_Console_Input_On_Custom> JFW_Console_Input_On_Custom_Registrant("JFW_Console_Input_On_Custom",
                                                                                     "Message:int,Input:string");
ScriptRegistrant<JFW_Invulnerable_On_Custom_3> JFW_Invulnerable_On_Custom_3_Registrant("JFW_Invulnerable_On_Custom_3",
                                                                                       "Message:int,InvulnerableArmour:string,NormalArmour:string");
ScriptRegistrant<JFW_Change_Team_On_Custom> JFW_Change_Team_On_Custom_Registrant("JFW_Change_Team_On_Custom",
                                                                                 "Message:int");
ScriptRegistrant<JFW_Enable_Team_Radar_On_Custom> JFW_Enable_Team_Radar_On_Custom_Registrant(
        "JFW_Enable_Team_Radar_On_Custom", "Player_Type:int,Message:int,Enable:int");
ScriptRegistrant<JFW_2D_Sound_Team_Custom> JFW_2D_Sound_Team_Custom_Registrant("JFW_2D_Sound_Team_Custom",
                                                                               "Message:int,Sound:string,Team:int");
ScriptRegistrant<JFW_3D_Sound_Team_Custom> JFW_3D_Sound_Team_Custom_Registrant("JFW_3D_Sound_Team_Custom",
                                                                               "Message:int,Sound:string,Position:vector3,Team:int");
ScriptRegistrant<JFW_Custom_Send_Ordered_Customs> JFW_Custom_Send_Ordered_Customs_Registrant(
        "JFW_Custom_Send_Ordered_Customs",
        "ID1:int,ID2:int,ID3:int,ID4:int,ID5:int,ID6:int,ID7:int,ID8:int,Custom1:int,Custom2:int,Custom3:int,Custom4:int,Custom5:int,Custom6:int,Custom7:int,Custom8:int,WaitCustom:int");
ScriptRegistrant<JFW_Custom_Send_Customs> JFW_Custom_Send_Customs_Registrant("JFW_Custom_Send_Customs",
                                                                             "ID1:int,ID2:int,ID3:int,ID4:int,ID5:int,ID6:int,ID7:int,ID8:int,Custom1:int,Custom2:int,Custom3:int,Custom4:int,Custom5:int,Custom6:int,Custom7:int,Custom8:int,WaitCustom:int");
ScriptRegistrant<JFW_Custom_Send_Custom_Multiple_IDs> JFW_Custom_Send_Custom_Multiple_IDs_Registrant(
        "JFW_Custom_Send_Custom_Multiple_IDs",
        "ID1:int,ID2:int,ID3:int,ID4:int,ID5:int,ID6:int,ID7:int,ID8:int,WaitCustom:int,SendCustom:int");
ScriptRegistrant<JFW_Create_Destroy_Object_On_Custom_Death> JFW_Create_Destroy_Object_On_Custom_Death_Registrant(
...
ScriptRegistrant<JFW_Create_Destroy_Object_On_Custom_Death_Bone> JFW_Create_Destroy_Object_On_Custom_Death_Bone_Registrant(
        "JFW_Create_Destroy_Object_On_Custom_Death_Bone",
        "Location:vector3,Facing:float,Preset:string,CreateMessage:int,DestroyMessage:int,BoneName:string");
ScriptRegistrant<JFW_Custom_Destroy_All_Buildings> JFW_Custom_Destroy_All_Buildings_Registrant(
        "JFW_Custom_Destroy_All_Buildings", "Player_Type:int");
ScriptRegistrant<JFW_Timer_Custom_On_Off> JFW_Timer_Custom_On_Off_Registrant("JFW_Timer_Custom_On_Off",
                                                                             "Time:float,TimerNum:int,Repeat:int,ID:int,Message:int,Param:int,EnableMessage:int,DisableMessage:int");
ScriptRegistrant<JFW_Create_Vehicle_On_Custom> JFW_Create_Vehicle_On_Custom_Registrant("JFW_Create_Vehicle_On_Custom",
                                                                                       "Preset_Name:string,Delay:float,Player_Type:int");
ScriptRegistrant<JFW_Set_Model_On_Custom_Sender> JFW_Set_Model_On_Custom_Sender_Registrant(
        "JFW_Set_Model_On_Custom_Sender", "Message:int,Model:string");
ScriptRegistrant<JFW_Death_Send_Custom_ID> JFW_Death_Send_Custom_ID_Registrant("JFW_Death_Send_Custom_ID",
                                                                               "ID:int,Message:int");
ScriptRegistrant<JFW_Death_Send_Custom_Driver_ID> JFW_Death_Send_Custom_Driver_ID_Registrant(
        "JFW_Death_Send_Custom_Driver_ID", "ID:int,Message:int");
ScriptRegistrant<JFW_Repair_On_Custom> JFW_Repair_On_Custom_Registrant("JFW_Repair_On_Custom",
                                                                       "Message:int,Health:float");
ScriptRegistrant<JFW_Teleport_Custom> JFW_Teleport_Custom_Registrant("JFW_Teleport_Custom",
                                                                     "Location:vector3,Object_ID=0:int,Message:int");
ScriptRegistrant<JFW_Teleport_Custom_Random> JFW_Teleport_Custom_Random_Registrant("JFW_Teleport_Custom_Random",
                                                                                   "Location1:vector3,Object_ID1=0:int,Location2:vector3,Object_ID2=0:int,Location3:vector3,Object_ID3=0:int,Location4:vector3,Object_ID4=0:int,Location5:vector3,Object_ID5=0:int,Message:int");
ScriptRegistrant<JFW_Teleport_Custom_Timer> JFW_Teleport_Custom_Timer_Registrant("JFW_Teleport_Custom_Timer",
                                                                                 "Location:vector3,Object_ID=0:int,Message:int,Time:float,TimerNum:int");
ScriptRegistrant<JFW_Blow_Up_Location_On_Custom> JFW_Blow_Location_Up_On_Custom_Registrant(
        "JFW_Blow_Location_Up_On_Custom", "Explosion:string,Message:int,Location:vector3");
ScriptRegistrant<JFW_3D_Sound_Custom> JFW_3D_Sound_Custom_Registrant("JFW_3D_Sound_Custom",
                                                                     "Message:int,Preset:string,Position:vector3");
ScriptRegistrant<JFW_Created_Send_Custom> JFW_Created_Send_Custom_Registrant("JFW_Created_Send_Custom",
                                                                             "ID:int,Message:int");
ScriptRegistrant<JFW_Damaged_Send_Custom> JFW_Damaged_Send_Custom_Registrant("JFW_Damaged_Send_Custom",
                                                                             "ID:int,Message:int");
ScriptRegistrant<JFW_Enemy_Seen_Send_Custom> JFW_Enemy_Seen_Send_Custom_Registrant("JFW_Enemy_Seen_Send_Custom",
                                                                                   "ID:int,Message:int");
ScriptRegistrant<JFW_Create_Object_Custom> JFW_Create_Object_Custom_Registrant("JFW_Create_Object_Custom",
                                                                               "Object:string,Location:vector3,Facing:float,Message:int");
ScriptRegistrant<JFW_Enable_Spawner_On_Custom> JFW_Enable_Spawner_On_Custom_Registrant("JFW_Enable_Spawner_On_Custom",
                                                                                       "ID:int,Enable:int,Message:int");
ScriptRegistrant<JFW_Set_Animation_On_Custom> JFW_Set_Animation_On_Custom_Registrant("JFW_Set_Animation_On_Custom",
                                                                                     "Animation:string,Loop:int,Subobject:string,FirstFrame:float,LastFrame:float,Blended:int,Message:int");
ScriptRegistrant<JFW_Set_Model_Animation_On_Custom> JFW_Set_Model_Animation_On_Custom_Registrant(
        "JFW_Set_Model_Animation_On_Custom",
        "Animation:string,Loop:int,Subobject:string,FirstFrame:float,LastFrame:float,Blended:int,Model:string,Message:int");
ScriptRegistrant<JFW_Set_Model_On_Custom> JFW_Set_Model_On_Custom_Registrant("JFW_Set_Model_On_Custom",
                                                                             "Model:string,Message:int");
ScriptRegistrant<JFW_Enable_Disable_Animation_On_Custom> JFW_Enable_Disable_Animation_On_Custom_Registrant(
        "JFW_Enable_Disable_Animation_On_Custom",
        "Animation:string,Subobject:string,FirstFrame:float,LastFrame:float,Blended:int,Enable:int,Disable:int");
ScriptRegistrant<JFW_Custom_Toggle_Visible> JFW_Custom_Toggle_Visible_Registrant("JFW_Custom_Toggle_Visible",
                                                                                 "Message:int,Visible:int");
ScriptRegistrant<JFW_Customs_Send_Custom> JFW_Customs_Send_Custom_Registrant("JFW_Customs_Send_Custom",
                                                                             "Message1:int,Message2:int,Message3:int,Message4:int,Message5:int,Message6:int,Message7:int,Message8:int,Message:int,Param:int,ID:int");
ScriptRegistrant<JFW_Custom_Send_Custom> JFW_Custom_Send_Custom_Registrant("JFW_Custom_Send_Custom",
                                                                           "RecieveMessage:int,SendMessage:int,SendParam:int,SendID:int");
ScriptRegistrant<JFW_Custom_Multiple_Send_Custom> JFW_Custom_Multiple_Send_Custom_Registrant(
        "JFW_Custom_Multiple_Send_Custom",
        "RecieveMessage:int,RecieveCount:int,SendMessage:int,SendParam:int,SendID:int");
ScriptRegistrant<JFW_Custom_Multiple_Send_Custom_2> JFW_Custom_Multiple_Send_Custom_2_Registrant(
        "JFW_Custom_Multiple_Send_Custom_2",
        "RecieveMessage:int,RecieveCount:int,SendMessage:int,SendParam:int,SendID:int");
ScriptRegistrant<JFW_Create_Destroy_Object_On_Custom> JFW_Create_Destroy_Object_On_Custom_Registrant(
        "JFW_Create_Destroy_Object_On_Custom",
        "Location:vector3,Facing:float,Preset:string,CreateMessage:int,DestroyMessage:int");
ScriptRegistrant<JFW_Create_Destroy_Object_On_Custom_Animate> JFW_Create_Destroy_Object_On_Custom_Animate_Registrant(
        "JFW_Create_Destroy_Object_On_Custom_Animate",
        "Location:vector3,Facing:float,Preset:string,CreateMessage:int,DestroyMessage:int,Animation:string,Loop:int,Subobject:string,FirstFrame:float,LastFrame:float,Blended:int");
ScriptRegistrant<JFW_Create_Destroy_Object_On_Custom_Animate_2> JFW_Create_Destroy_Object_On_Custom_Animate_2_Registrant(
...
ScriptRegistrant<JFW_Damaged_Send_Custom_Amount> JFW_Damaged_Send_Custom_Amount_Registrant(
        "JFW_Damaged_Send_Custom_Amount", "ID:int,Message1:int,Message2:int,Damage:float");
ScriptRegistrant<JFW_Invulnerable_On_Custom> JFW_Invulnerable_On_Custom_Registrant("JFW_Invulnerable_On_Custom",
                                                                                   "Message:int");
ScriptRegistrant<JFW_Invulnerable_On_Custom_2> JFW_Invulnerable_On_Custom_2_Registrant("JFW_Invulnerable_On_Custom_2",
                                                                                       "Message:int,InvulnerableArmour:string,NormalArmour:string");
ScriptRegistrant<JFW_Custom_Send_Custom_Parameter> JFW_Custom_Send_Custom_Parameter_Registrant(
        "JFW_Custom_Send_Custom_Parameter", "ID:int,BaseMessage:int,Limit:int");
ScriptRegistrant<JFW_Create_Destroy_Object_On_Custom_Bone> JFW_Create_Destroy_Object_On_Custom_Bone_Registrant(
        "JFW_Create_Destroy_Object_On_Custom_Bone",
        "Location:vector3,Facing:float,Preset:string,CreateMessage:int,DestroyMessage:int,BoneName:string");
ScriptRegistrant<JFW_Timer_Custom> JFW_Timer_Custom_Registrant("JFW_Timer_Custom",
                                                               "Time:float,TimerNum:int,Repeat:int,ID:int,Message:int,Param:int");
ScriptRegistrant<JFW_2D_Sound_Custom> JFW_2D_Sound_Custom_Registrant("JFW_2D_Sound_Custom",
                                                                     "Message:int,Preset:string");
ScriptRegistrant<JFW_3D_Sound_Custom_2> JFW_3D_Sound_Custom_2_Registrant("JFW_3D_Sound_Custom_2",
                                                                         "Message:int,Preset:string");
ScriptRegistrant<JFW_3D_Sound_Custom_Bone> JFW_3D_Sound_Custom_Bone_Registrant("JFW_3D_Sound_Custom_Bone",
                                                                               "Message:int,Preset:string,Bone:string");
ScriptRegistrant<JFW_Random_Custom> JFW_Random_Custom_Registrant("JFW_Random_Custom",
                                                                 "Message:int,ID:int,TimerNum:int,Time_Min:float,Time_Max:float,Repeat:int");
ScriptRegistrant<JFW_Custom_Damage_Object> JFW_Custom_Damage_Object_Registrant("JFW_Custom_Damage_Object",
                                                                               "ID:int,Message:int,Damage:float");
ScriptRegistrant<JFW_Blow_Up_On_Custom> JFW_Blow_Up_On_Custom_Registrant("JFW_Blow_Up_On_Custom",
                                                                         "Explosion:string,Message:int");
ScriptRegistrant<JFW_Blow_Up_Obj_On_Custom> JFW_Blow_Obj_Up_On_Custom_Registrant("JFW_Blow_Obj_Up_On_Custom",
                                                                                 "Explosion:string,Message:int");
...
ScriptRegistrant<JFW_Custom_Destroy_Object> JFW_Custom_Destroy_Object_Registrant("JFW_Custom_Destroy_Object",
                                                                                 "Message:int,ID:int");
ScriptRegistrant<JFW_Custom_Objectives_Dlg> JFW_Custom_Objectives_Dlg_Registrant("JFW_Custom_Objectives_Dlg",
                                                                                 "Message:int,File:string");
ScriptRegistrant<JFW_Reflect_Custom> JFW_Reflect_Custom_Registrant("JFW_Reflect_Custom", "");
ScriptRegistrant<JFW_Reflect_Custom_Delay> JFW_Reflect_Custom_Delay_Registrant("JFW_Reflect_Custom_Delay",
                                                                               "Delay:float");
ScriptRegistrant<JFW_Death_Send_Custom> JFW_Death_Send_Custom_Registrant("JFW_Death_Send_Custom",
                                                                         "ID:int,Message:int,Param:int");
ScriptRegistrant<JFW_Custom_Destroy_Building> JFW_Custom_Destroy_Building_Registrant("JFW_Custom_Destroy_Building",
                                                                                     "Message:int,ID:int");
ScriptRegistrant<JFW_Shader_Custom_Zone_Entry> JFW_Shader_Custom_Zone_Entry_Registrant("JFW_Shader_Custom_Zone_Entry",
                                                                                       "Player_Type:int,Number:int,Number2:float");
ScriptRegistrant<JFW_Shader_Custom_Zone_Exit> JFW_Shader_Custom_Zone_Exit_Registrant("JFW_Shader_Custom_Zone_Exit",
                                                                                     "Player_Type:int,Number:int,Number2:float");
ScriptRegistrant<JFW_Shader_Custom_Vehicle_Entry> JFW_Shader_Custom_Vehicle_Entry_Registrant(
        "JFW_Shader_Custom_Vehicle_Entry", "Number:int,Number2:float");
ScriptRegistrant<JFW_Shader_Custom_Vehicle_Exit> JFW_Shader_Custom_Vehicle_Exit_Registrant(
        "JFW_Shader_Custom_Vehicle_Exit", "Number:int,Number2:float");
ScriptRegistrant<JFW_Shader_Custom_Create> JFW_Shader_Custom_Create_Registrant("JFW_Shader_Custom_Create",
                                                                               "Number:int,Number2:float");
ScriptRegistrant<JFW_Shader_Custom_Poke> JFW_Shader_Custom_Poke_Registrant("JFW_Shader_Custom_Poke",
                                                                           "Number:int,Number2:float");
ScriptRegistrant<JFW_Kill_All_Buildings_Customs> JFW_Kill_All_Buildings_Customs_Registrant(
        "JFW_Kill_All_Buildings_Customs", "Message:int,Count:int,Player_Type:int");
ScriptRegistrant<JFW_Send_Self_Custom_On_Key> JFW_Send_Self_Custom_On_Key_Registrant("JFW_Send_Self_Custom_On_Key",
                                                                                     "Key=Key:string,Message=0:int,Once=1:int");
ScriptRegistrant<JFW_Send_Message_On_Key> JFW_Send_Message_On_Key_Registrant("JFW_Send_Message_On_Key",
                                                                             "Key:string,Message:string,Red:int,Green:int,Blue:int");
ScriptRegistrant<JFW_Custom_Send_Random_Custom> JFW_Custom_Send_Random_Custom_Registrant(
        "JFW_Custom_Send_Random_Custom", "RecieveMessage:int,SendMessage:int,SendParam:int,SendID:int,Percentage:int");
ScriptRegistrant<JFW_Startup_Custom_Self> JFW_Startup_Custom_Self_Registrant("JFW_Startup_Custom_Self",
                                                                             "Message:int,Param=0:int,Delay=0:int");
ScriptRegistrant<JFW_Set_Skin_Custom> JFW_Set_Skin_Custom_Registrant("JFW_Set_Skin_Custom",
                                                                     "Message:int,Armour:string");
ScriptRegistrant<JFW_Set_Armor_Custom> JFW_Set_Armor_Custom_Registrant("JFW_Set_Armor_Custom",
                                                                       "Message:int,Armour:string");
ScriptRegistrant<JFW_Send_Custom_On_Damage> JFW_Send_Custom_On_Damage_Registrant("JFW_Send_Custom_On_Damage",
