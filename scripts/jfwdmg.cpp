...
void JFW_Engineer_Repair::Sound_Heard(GameObject *obj, const CombatSound &sound) {
    ...
    if ((sound.sound == 40012) && (!active) &&
        (Commands->Get_Player_Type(sound.Creator) == Commands->Get_Player_Type(obj))) {
...
void JFW_Building_Damage::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == Get_Int_Parameter("Message")) {
        Ranged_Damage_To_Buildings_Team(Get_Int_Parameter("Player_Type"), Get_Float_Parameter("Damage"),
                                        Get_Parameter("Warhead"), Commands->Get_Position(sender),
                                        Get_Float_Parameter("Distance"), Commands->Find_Object(param));
    }
}

void JFW_Building_Damage_Scale::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == Get_Int_Parameter("Message")) {
        Ranged_Scale_Damage_To_Buildings_Team(Get_Int_Parameter("Player_Type"), Get_Float_Parameter("Damage"),
                                              Get_Parameter("Warhead"), Commands->Get_Position(sender),
                                              Get_Float_Parameter("Distance"), Commands->Find_Object(param));
    }
}
...
void JFW_Building_Damage_Percentage::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == Get_Int_Parameter("Message")) {
        Ranged_Percentage_Damage_To_Buildings_Team(Get_Int_Parameter("Player_Type"), Get_Float_Parameter("Percentage"),
                                                   Get_Parameter("Warhead"), Commands->Get_Position(sender),
                                                   Get_Float_Parameter("Distance"), Commands->Find_Object(param));
    }
}
...
void JFW_Animation_Frame_Damage::Damaged(GameObject *obj, GameObject *damager, float amount) {
    if ((Commands->Get_Health(obj) <= Get_Float_Parameter("MaxHealth")) &&
        (Commands->Get_Health(obj) >= Get_Float_Parameter("MinHealth"))) {
        if (enabled) {
            enabled = false;
...
void JFW_Kill_Message::Killed(GameObject *obj, GameObject *killer) {
    if (killer) {
        Commands->Send_Custom_Event(obj, Find_Object_With_Script("JFW_Kill_Message_Display"),
                                    Get_Int_Parameter("Message"), Commands->Get_ID(killer), 0);
    } else {
        Commands->Send_Custom_Event(obj, Find_Object_With_Script("JFW_Kill_Message_Display"),
...
int JFW_Beacon_Building::PowerupPurchaseHook(BaseControllerClass *base, GameObject *purchaser, unsigned int cost,
                                             unsigned int preset, const char *data) {
    if (((GameObject *) data)->As_BuildingGameObj() && ((GameObject *) data)->As_BuildingGameObj()->Is_Destroyed() &&
        ((GameObject *) data)->As_BuildingGameObj()->Get_Player_Type() == Commands->Get_Player_Type(purchaser)) {
        return 3;
    }
    return -1;
}
...
ScriptRegistrant<JFW_Invulnerable_On_Create> JFW_Invulnerable_On_Create_Registrant("JFW_Invulnerable_On_Create",
                                                                                   "Time:float,TimerNum:int,InvulnerableArmour:string,NormalArmour:string");
ScriptRegistrant<JFW_Destroy_Self_Timer> JFW_Destroy_Self_Timer_Registrant("JFW_Destroy_Self_Timer",
                                                                           "Time:float,TimerNum:int");
ScriptRegistrant<JFW_Change_Model_Health> JFW_Change_Model_Health_Registrant("JFW_Change_Model_Health",
                                                                             "Model:string,Health:float");
ScriptRegistrant<JFW_Change_Model_Health2> JFW_Change_Model_Health2_Registrant("JFW_Change_Model_Health2",
                                                                               "Model:string,Health:float");
ScriptRegistrant<JFW_Damage_Animation> JFW_Damage_Animation_Registrant("JFW_Damage_Animation",
                                                                       "Animation:string,Time:float,TimerNum:int");
ScriptRegistrant<JFW_Spawn_Object_Death_Weapon> JFW_Spawn_Object_Death_Weapon_Registrant(
        "JFW_Spawn_Object_Death_Weapon", "Drop_Object:string,Drop_Height:float,Weapon:string");
ScriptRegistrant<JFW_Spawn_Object_Death_Team> JFW_Spawn_Object_Death_Team_Registrant("JFW_Spawn_Object_Death_Team",
                                                                                     "Drop_Object_GDI:string,Drop_Object_Nod:string,Drop_Height:float");
ScriptRegistrant<JFW_Building_Damage_Percentage> JFW_Building_Damage_Percentage_Registrant(
        "JFW_Building_Damage_Percentage", "Message:int,Player_Type:int,Percentage:float,Warhead:string,Distance:float");
ScriptRegistrant<JFW_Animation_Frame_Damage> JFW_Animation_Frame_Damage_Registrant("JFW_Animation_Frame_Damage",
                                                                                   "Animation:string,Frame:int,MinHealth:float,MaxHealth:float");
ScriptRegistrant<JFW_Damage_Do_Damage> JFW_Damage_Do_Damage_Registrant("JFW_Damage_Do_Damage",
                                                                       "Health:float,Damage:float,Warhead:string,Time:float,TimerNum:int");
ScriptRegistrant<JFW_Death_Destroy_Object_Delay> JFW_Death_Destroy_Object_Delay_Registrant(
        "JFW_Death_Destroy_Object_Delay", "ID:int,Time:float,TimerNum:int");
ScriptRegistrant<JFW_Set_Health_On_Custom> JFW_Set_Health_On_Custom_Registrant("JFW_Set_Health_On_Custom",
                                                                               "Custom:int,Value:float");
ScriptRegistrant<JFW_Add_Health_On_Custom> JFW_Add_Health_On_Custom_Registrant("JFW_Add_Health_On_Custom",
                                                                               "Custom:int,Value:float");
ScriptRegistrant<JFW_Set_Max_Health_On_Custom> JFW_Set_Max_Health_On_Custom_Registrant("JFW_Set_Max_Health_On_Custom",
                                                                                       "Custom:int,Value:float");
ScriptRegistrant<JFW_Add_Max_Health_On_Custom> JFW_Add_Max_Health_On_Custom_Registrant("JFW_Add_Max_Health_On_Custom",
                                                                                       "Custom:int,Value:float");
ScriptRegistrant<JFW_Set_Shield_Strength_On_Custom> JFW_Set_Shield_Strength_On_Custom_Registrant(
        "JFW_Set_Shield_Strength_On_Custom", "Custom:int,Value:float");
ScriptRegistrant<JFW_Add_Shield_Strength_On_Custom> JFW_Add_Shield_Strength_On_Custom_Registrant(
        "JFW_Add_Shield_Strength_On_Custom", "Custom:int,Value:float");
ScriptRegistrant<JFW_Set_Max_Shield_Strength_On_Custom> JFW_Set_Max_Shield_Strength_On_Custom_Registrant(
        "JFW_Set_Max_Shield_Strength_On_Custom", "Custom:int,Value:float");
ScriptRegistrant<JFW_Add_Max_Shield_Strength_On_Custom> JFW_Add_Max_Shield_Strength_On_Custom_Registrant(
        "JFW_Add_Max_Shield_Strength_On_Custom", "Custom:int,Value:float");
ScriptRegistrant<JFW_Set_Health_On_Custom_Sender> JFW_Set_Health_On_Custom_Sender_Registrant(
        "JFW_Set_Health_On_Custom_Sender", "Custom:int,Value:float");
ScriptRegistrant<JFW_Add_Health_On_Custom_Sender> JFW_Add_Health_On_Custom_Sender_Registrant(
        "JFW_Add_Health_On_Custom_Sender", "Custom:int,Value:float");
ScriptRegistrant<JFW_Set_Max_Health_On_Custom_Sender> JFW_Set_Max_Health_On_Custom_Sender_Registrant(
        "JFW_Set_Max_Health_On_Custom_Sender", "Custom:int,Value:float");
ScriptRegistrant<JFW_Add_Max_Health_On_Custom_Sender> JFW_Add_Max_Health_On_Custom_Sender_Registrant(
        "JFW_Add_Max_Health_On_Custom_Sender", "Custom:int,Value:float");
ScriptRegistrant<JFW_Set_Shield_Strength_On_Custom_Sender> JFW_Set_Shield_Strength_On_Custom_Sender_Registrant(
        "JFW_Set_Shield_Strength_On_Custom_Sender", "Custom:int,Value:float");
ScriptRegistrant<JFW_Add_Shield_Strength_On_Custom_Sender> JFW_Add_Shield_Strength_On_Custom_Sender_Registrant(
        "JFW_Add_Shield_Strength_On_Custom_Sender", "Custom:int,Value:float");
ScriptRegistrant<JFW_Set_Max_Shield_Strength_On_Custom_Sender> JFW_Set_Max_Shield_Strength_On_Custom_Sender_Registrant(
        "JFW_Set_Max_Shield_Strength_On_Custom_Sender", "Custom:int,Value:float");
ScriptRegistrant<JFW_Add_Max_Shield_Strength_On_Custom_Sender> JFW_Add_Max_Shield_Strength_On_Custom_Sender_Registrant(
        "JFW_Add_Max_Shield_Strength_On_Custom_Sender", "Custom:int,Value:float");
ScriptRegistrant<JFW_Regenerate_Health_Conditional> JFW_Regenerate_Health_Conditional_Registrant(
        "JFW_Regenerate_Health_Conditional", "EnableMessage:int,DisableMessage:int,Amount:int");
ScriptRegistrant<JFW_Building_Damage> JFW_Building_Damage_Registrant("JFW_Building_Damage",
                                                                     "Message:int,Player_Type:int,Damage:float,Warhead:string,Distance:float");
ScriptRegistrant<JFW_Building_Damage_Scale> JFW_Building_Damage_Scale_Registrant("JFW_Building_Damage_Scale",
                                                                                 "Message:int,Player_Type:int,Damage:float,Warhead:string,Distance:float");
ScriptRegistrant<JFW_Blow_Up_On_Death_Driver> JFW_Blow_Up_On_Death_Driver_Registrant("JFW_Blow_Up_On_Death_Driver",
                                                                                     "Explosion:string");
ScriptRegistrant<JFW_Random_DriverDeath> JFW_Random_DriverDeath_Registrant("JFW_Random_DriverDeath", "Percentage:int");
ScriptRegistrant<JFW_Armour_Regen> JFW_Armour_Regen_Registrant("JFW_Armour_Regen",
                                                               "Time:float,TimerNum:int,Points:float");
ScriptRegistrant<JFW_Health_Regen> JFW_Health_Regen_Registrant("JFW_Health_Regen",
                                                               "Time:float,TimerNum:int,Points:float");
ScriptRegistrant<JFW_Give_Points_On_Death> JFW_Give_Points_On_Death_Registrant("JFW_Give_Points_On_Death",
                                                                               "Points:float,Player_Type:int,Whole_Team:int");
ScriptRegistrant<JFW_Death_Destroy_Object> JFW_Death_Destroy_Object_Registrant("JFW_Death_Destroy_Object", "ID:int");
ScriptRegistrant<JFW_Invulnerability_Timer> JFW_Invulnerability_Timer_Registrant("JFW_Invulnerability_Timer",
                                                                                 "Invulnerability_Time:float,Invulnerability_TimerNum:int");
ScriptRegistrant<JFW_Spawn_Object_Death_Enable> JFW_Spawn_Object_Death_Enable_Registrant(
        "JFW_Spawn_Object_Death_Enable", "Drop_Object:string,Drop_Height:float,EnableMessage:int,DisableMessage:int");
ScriptRegistrant<JFW_Spawn_Object_Death_Enable_Random> JFW_Spawn_Object_Death_Enable_Random_Registrant(
        "JFW_Spawn_Object_Death_Enable_Random",
        "Drop_Object:string,Drop_Height:float,EnableMessage:int,DisableMessage:int,Probobility:int");
ScriptRegistrant<JFW_Spawn_Object_Death_Random> JFW_Spawn_Object_Death_Random_Registrant(
        "JFW_Spawn_Object_Death_Random", "Drop_Object:string,Drop_Height:float,Probobility:int");
...
ScriptRegistrant<JFW_Spawn_Object_Death> JFW_Spawn_Object_Death_Registrant("JFW_Spawn_Object_Death",
                                                                           "Drop_Object:string,Drop_Height:float");
ScriptRegistrant<JFW_Timer_Destroy_Building> JFW_Timer_Destroy_Building_Registrant("JFW_Timer_Destroy_Building",
                                                                                   "Time:float,TimerNum:int");
ScriptRegistrant<JFW_Pilot_Repair> JFW_Pilot_Repair_Registrant("JFW_Pilot_Repair",
                                                               "Time:float,TimerNum:int,Health:float");
ScriptRegistrant<JFW_Damage_Occupants_Death> JFW_Damage_Occupants_Death_Registrant("JFW_Damage_Occupants_Death",
                                                                                   "Damage:float,Warhead:string");
ScriptRegistrant<JFW_Engineer_Target_2> JFW_Engineer_Target_2_Registrant("JFW_Engineer_Target_2", "");
ScriptRegistrant<JFW_Engineer_Repair_2> JFW_Engineer_Repair_2_Registrant("JFW_Engineer_Repair_2",
                                                                         "Repair_Priority=96:int,Target_ID:int");
ScriptRegistrant<JFW_Kill_Message_Display> JFW_Kill_Message_Display_Registrant("JFW_Kill_Message_Display",
                                                                               "ConfigFile:string,Message:int");
...
ScriptRegistrant<JFW_Timer_Destroy_Object> JFW_Timer_Destroy_Object_Registrant("JFW_Timer_Destroy_Object",
                                                                               "Time:float,TimerNum:int,Amount:float,Warhead:string");
ScriptRegistrant<JFW_Beacon_Building> JFW_Beacon_Building_Registrant("JFW_Beacon_Building", "");
ScriptRegistrant<JFW_Spawn_Object_Repair> JFW_Spawn_Object_Repair_Registrant("JFW_Spawn_Object_Repair",
                                                                             "Object:string");
