...
void JFW_Team_DM_Zone::Exited(GameObject *obj, GameObject *exiter) {
    ...
    if (!triggered) {
        char param[100];
        sprintf(param, "%d,%d,%d", Get_Int_Parameter("Controller_ID"), Get_Int_Parameter("Deactivate_Message"),
    ...
    if ((type == Refinery_Message) && (HasOre)) {
        ...
        if (team == 0) {
            if (cash == 700) {
                ...
                sprintf(DumpcMsg,
                        "cmsgt 0 9,193,18 %s dumped Ore, funding the Soviet Warmachine with 700 additional credits.",
                        c);
                delete[] c;
                HasOre = false;
                cash = 0;
                ...
                const char *c = Get_Player_Name_By_ID(PlayerID);
                sprintf(DumpcMsg,
                        "cmsgt 0 9,193,18 %s dumped Mixed Ore/Gems, funding the Soviet Warmachine with 1050 additional credits.",
                        c);
                delete[] c;
                HasOre = false;
                cash = 0;
                ...
                const char *c = Get_Player_Name_By_ID(PlayerID);
                sprintf(DumpcMsg,
                        "cmsgt 0 9,193,18 %s dumped Gems, funding the Soviet Warmachine with 1400 additional credits.",
                        c);
                delete[] c;
                HasOre = false;
                cash = 0;
            ...
            if (cash == 700) {
                ...
                const char *c = Get_Player_Name_By_ID(PlayerID);
                sprintf(DumpcMsg,
                        "cmsgt 1 9,193,18 %s dumped Ore, funding the Allied Forces with 700 additional credits.", c);
                delete[] c;
                HasOre = false;
                cash = 0;
            } else if (cash == 1050) {
                ...
                const char *c = Get_Player_Name_By_ID(PlayerID);
                sprintf(DumpcMsg,
                        "cmsgt 1 9,193,18 %s dumped Mixed Ore/Gems, funding the Allied Forces with 1050 additional credits.",
                        c);
                delete[] c;
                HasOre = false;
                cash = 0;
                ...
                const char *c = Get_Player_Name_By_ID(PlayerID);
                sprintf(DumpcMsg,
                        "cmsgt 1 9,193,18 %s dumped Gems, funding the Allied Forces with 1400 additional credits.", c);
                delete[] c;
                HasOre = false;
                cash = 0;
            }
    ...
    if ((type == Refinery_Message) && (HasOre)) {
        ...
        if (team == 0) {
            if (cash == 700) {
                ...
                const char *c = Get_Player_Name_By_ID(PlayerID);
                sprintf(DumpcMsg,
                        "cmsgt 0 9,193,18 %s dumped Tiberium, funding the Nod Warmachine with 700 additional credits.",
                        c);
                delete[] c;
                HasOre = false;
                cash = 0;
                ...
                const char *c = Get_Player_Name_By_ID(PlayerID);
                sprintf(DumpcMsg,
                        "cmsgt 0 9,193,18 %s dumped Mixed Tiberium/Blue Tiberium, funding the Nod Warmachine with 1050 additional credits.",
                        c);
                delete[] c;
                HasOre = false;
                cash = 0;
                ...
                const char *c = Get_Player_Name_By_ID(PlayerID);
                sprintf(DumpcMsg,
                        "cmsgt 0 9,193,18 %s dumped Blue Tiberium, funding the Nod Warmachine with 1400 additional credits.",
                        c);
                delete[] c;
                HasOre = false;
                cash = 0;
            ...
            if (cash == 700) {
                ...
                const char *c = Get_Player_Name_By_ID(PlayerID);
                sprintf(DumpcMsg,
                        "cmsgt 1 9,193,18 %s dumped Tiberium, funding the GDI Forces with 700 additional credits.", c);
                delete[] c;
                HasOre = false;
                cash = 0;
            } else if (cash == 1050) {
                ...
                const char *c = Get_Player_Name_By_ID(PlayerID);
                sprintf(DumpcMsg,
                        "cmsgt 1 9,193,18 %s dumped Mixed Tiberium/Blue Tiberium, funding the GDI Forces with 1050 additional credits.",
                        c);
                delete[] c;
                HasOre = false;
                cash = 0;
                ...
                const char *c = Get_Player_Name_By_ID(PlayerID);
                sprintf(DumpcMsg,
                        "cmsgt 1 9,193,18 %s dumped Blue Tiberium, funding the GDI Forces with 1400 additional credits.",
                        c);
                delete[] c;
                HasOre = false;
                cash = 0;
...
void JFW_Iron_Curtain_Zone::Entered(GameObject *obj, GameObject *enterer) {
    ...
    if (enterer->As_VehicleGameObj()) {
        ...
        Commands->Send_Custom_Event(obj, object, 10000 + Get_Int_Parameter("Animation2Slot"),
                                    Get_Int_Parameter("BuildingID"), 0);
...
void JFW_Iron_Curtain_Zone::Killed(GameObject *obj, GameObject *killer) {
    if (enabled) {
        ...
        Commands->Set_Facing(object, facing);
        Commands->Attach_Script(object, "Test_Cinematic", scriptname);
        Commands->Send_Custom_Event(obj, object, 10000 + Get_Int_Parameter("Animation3Slot"),
                                    Get_Int_Parameter("BuildingID"), 0);
    }
}
...
void JFW_Iron_Curtain_Zone::Timer_Expired(GameObject *obj, int number) {
    ...
    if (num == number) {
        ...
        Commands->Set_Facing(object, facing);
        Commands->Attach_Script(object, "Test_Cinematic", scriptname);
        Commands->Send_Custom_Event(obj, object, 10000 + Get_Int_Parameter("Animation1Slot"),
                                    Get_Int_Parameter("BuildingID"), 0);
        enabled = true;
    }
}
...
void JFW_Carryable_Vehicle::Damaged(GameObject *obj, GameObject *damager, float amount) {
    ...
    if ((onlyempty == 1 && !Get_Vehicle_Occupant_Count(obj)) || (!onlyempty)) {
        Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("Control_ID")), 9876,
                                    Commands->Get_ID(damager), 0);
    }
}

void JFW_Carryable_Vehicle::Killed(GameObject *obj, GameObject *killer) {
    Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("Control_ID")), 9875,
                                Commands->Get_ID(killer), 0);
}
...
void JFW_Carry_Control::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == 9877 && param == 6) {
        ...
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && !carryall6 &&
            Get_Int_Parameter("Max_Carryalls") >= 6) {
            carryall6 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && !carryall7 &&
            Get_Int_Parameter("Max_Carryalls") >= 7) {
            carryall7 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && !carryall8 &&
            Get_Int_Parameter("Max_Carryalls") >= 8) {
            carryall8 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            !carryall9 && Get_Int_Parameter("Max_Carryalls") >= 9) {
            carryall9 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            carryall9 && !carryall10 && Get_Int_Parameter("Max_Carryalls") >= 10) {
            carryall10 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            carryall9 && carryall10 && !carryall11 && Get_Int_Parameter("Max_Carryalls") >= 11) {
            carryall11 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            carryall9 && carryall10 && carryall11 && !carryall12 && Get_Int_Parameter("Max_Carryalls") >= 12) {
            carryall12 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            carryall9 && carryall10 && carryall11 && carryall12 && !carryall13 &&
            Get_Int_Parameter("Max_Carryalls") >= 13) {
            carryall13 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            carryall9 && carryall10 && carryall11 && carryall12 && carryall13 && !carryall14 &&
            Get_Int_Parameter("Max_Carryalls") >= 14) {
            carryall14 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            carryall9 && carryall10 && carryall11 && carryall12 && carryall13 && carryall14 && !carryall15 &&
            Get_Int_Parameter("Max_Carryalls") >= 15) {
            carryall15 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            carryall9 && carryall10 && carryall11 && carryall12 && carryall13 && carryall14 && carryall15 &&
            !carryall16 && Get_Int_Parameter("Max_Carryalls") >= 16) {
            carryall16 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            carryall9 && carryall10 && carryall11 && carryall12 && carryall13 && carryall14 && carryall15 &&
            carryall16 && !carryall17 && Get_Int_Parameter("Max_Carryalls") >= 17) {
            carryall17 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            carryall9 && carryall10 && carryall11 && carryall12 && carryall13 && carryall14 && carryall15 &&
            carryall16 && carryall17 && !carryall18 && Get_Int_Parameter("Max_Carryalls") >= 18) {
            carryall18 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            carryall9 && carryall10 && carryall11 && carryall12 && carryall13 && carryall14 && carryall15 &&
            carryall16 && carryall17 && carryall18 && !carryall19 && Get_Int_Parameter("Max_Carryalls") >= 19) {
            carryall19 = Commands->Get_ID(sender);
            return;
        }
        if (carryall1 && carryall2 && carryall3 && carryall4 && carryall5 && carryall6 && carryall7 && carryall8 &&
            carryall9 && carryall10 && carryall11 && carryall12 && carryall13 && carryall14 && carryall15 &&
            carryall16 && carryall17 && carryall18 && carryall19 && !carryall20 &&
            Get_Int_Parameter("Max_Carryalls") >= 20) {
...
void JFW_Sensor_Array::Timer_Expired(GameObject *obj, int number) {
    if (number == Get_Int_Parameter("TimerNum")) {
        ...
        Create_Effect_All_Stealthed_Objects_Area(Commands->Get_Position(obj), Get_Float_Parameter("Distance"),
                                                 Get_Parameter("Indicator"), v, Get_Int_Parameter("Player_Type"));
        Commands->Start_Timer(obj, this, Get_Float_Parameter("Time"), Get_Int_Parameter("TimerNum"));
    }
}
...
void JFW_Conyard_Radius::Timer_Expired(GameObject *obj, int number) {
    if (!disabled) {
        Repair_All_Buildings_By_Team_Radius(Get_Object_Type(obj), Commands->Get_ID(obj), Get_Float_Parameter("Health"),
...
void JFW_Chrono_Harvester_Attached::Created(GameObject *obj) {
    End_Timer = false;
    Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("LogicID")), 100003, Commands->Get_ID(obj),
                                0);
...
void
JFW_Chrono_Harvester_Attached::Action_Complete(GameObject *obj, int action_id, ActionCompleteReason complete_reason) {
    if (action_id == 1) {
        if (!complete_reason) {
...
void JFW_Chrono_Harvester_Attached::Killed(GameObject *obj, GameObject *killer) {
    ...
    Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("LogicID")), 100001, 0,
    ...
    if (number == Get_Int_Parameter("TimerNum")) {
        if (Is_Base_Powered(Get_Object_Type(obj))) {
            Vector3 v = Commands->Get_Position(
                    Find_Object_By_Preset(2, Get_Definition_Name(Get_Int_Parameter("CenterID"))));
            if (Is_Unit_In_Range(Get_Parameter("Preset"), Get_Float_Parameter("Range"), v, PTTEAM(Get_Object_Type(obj)),
                                 false)) {
                if (radaron) {
...
void JFW_Sonar_Pulse::Timer_Expired(GameObject *obj, int number) {
    if (number == 1) {
        ...
        Create_Effect_All_Of_Preset(Get_Parameter("Indicator"), Get_Parameter("Sub_Preset_1"),
                                    Get_Float_Parameter("ZAdjust"), false);
        Create_Effect_All_Of_Preset(Get_Parameter("Indicator"), Get_Parameter("Sub_Preset_2"),
                                    Get_Float_Parameter("ZAdjust"), false);
        Create_Effect_All_Of_Preset(Get_Parameter("WaterIndicator"), Get_Parameter("Sub_Preset_1"),
                                    Get_Float_Parameter("WaterZ"), true);
        Create_Effect_All_Of_Preset(Get_Parameter("WaterIndicator"), Get_Parameter("Sub_Preset_2"),
                                    Get_Float_Parameter("WaterZ"), true);
...
void JFW_Message_Send_Zone::Entered(GameObject *obj, GameObject *enterer) {
    ...
    Commands->Create_2D_Sound(Get_Parameter("Sound"));
    Send_Message(Get_Int_Parameter("Red"), Get_Int_Parameter("Green"), Get_Int_Parameter("Blue"),
...
void JFW_Message_Send_Zone_Team::Entered(GameObject *obj, GameObject *enterer) {
    ...
    Send_Message_Team(team, Get_Int_Parameter("Red"), Get_Int_Parameter("Green"), Get_Int_Parameter("Blue"),
                      Get_Parameter("Message"));
    if (Get_Int_Parameter("Delete")) {
        Destroy_Script();
    }
...
void JFW_Message_Send_Zone_Player::Entered(GameObject *obj, GameObject *enterer) {
    ...
    Send_Message_Player(enterer, Get_Int_Parameter("Red"), Get_Int_Parameter("Green"), Get_Int_Parameter("Blue"),
                        Get_Parameter("Message"));
    if (Get_Int_Parameter("Delete")) {
        Destroy_Script();
    }
...
void JFW_Message_Send_Powerup::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == CUSTOM_EVENT_POWERUP_GRANTED) {
        ...
        Send_Message_Player(sender, Get_Int_Parameter("Red"), Get_Int_Parameter("Green"), Get_Int_Parameter("Blue"),
                            Get_Parameter("Display_Message"));
    }
}
...
void JFW_Hijacker_Vehicle::Killed(GameObject *obj, GameObject *killer) {
    if (HijackerID) {
        Commands->Apply_Damage(Commands->Find_Object(HijackerID), Get_Float_Parameter("Damage"),
                               Get_Parameter("Warhead"), 0);
    }
}
            ...
            if (!damager->As_DamageableGameObj()->Is_Teammate(obj->As_DamageableGameObj())) {
                if (Get_Vehicle_Occupant_Count(obj) == 1) {
                    ...
                    if (o && o->As_SmartGameObj() && o->As_SmartGameObj()->Is_Control_Enabled()) {
                        if (Commands->Get_Distance(Commands->Get_Position(obj), Commands->Get_Position(damager)) <=
                            Get_Float_Parameter("Distance")) {
                            Commands->Create_Sound(Get_Parameter("Sound"), Commands->Get_Position(damager), damager);
                            Force_Occupants_Exit(obj);
                            Commands->Start_Timer(obj, this, 0.5, 1);
                            HijackerID = Commands->Get_ID(damager);
...
ScriptRegistrant<JFW_Domination_Controler_End_Game> JFW_Domination_Controler_End_Game_Registrant(
        "JFW_Domination_Controler_End_Game",
        "NeutralMessage:int,OwnedMessage:int,Time:float,TimerNum:int,PointsToGive:float,PointsToTake:float,GDIObjectID:int,NodObjectID:int,ZoneCount:int");
ScriptRegistrant<JFW_Carryable_Vehicle> JFW_Carryable_Vehicle_Registrant("JFW_Carryable_Vehicle",
                                                                         "Only_Empty:int,Control_ID:int");
...
ScriptRegistrant<JFW_Sensor_Array> JFW_Sensor_Array_Registrant("JFW_Sensor_Array",
                                                               "Distance:float,Indicator:string,IndicatorZOffset:float,Time:float,TimerNum:int,Player_Type:int");
ScriptRegistrant<JFW_Team_DM_Controller_2> JFW_Team_DM_Controller_2_Registrant("JFW_Team_DM_Controller_2",
                                                                               "Deaths_To_Loose:int,Death_Message:int,Team:int");
ScriptRegistrant<JFW_Vehicle_Thief> JFW_Vehicle_Thief_Registrant("JFW_Vehicle_Thief", "Weapon:string,Player_Type:int");
ScriptRegistrant<JFW_Vehicle_Thief_2> JFW_Vehicle_Thief_2_Registrant("JFW_Vehicle_Thief_2",
                                                                     "Weapon:string,Player_Type:int,Sound:string");
ScriptRegistrant<JFW_Conyard> JFW_Conyard_Registrant("JFW_Conyard",
                                                     "Time:float,TimerNum:int,DisableCustom:int,Health:float");
ScriptRegistrant<JFW_Flying_Infantry> JFW_Flying_Infantry_Registrant("JFW_Flying_Infantry", "");
ScriptRegistrant<JFW_Flying_Infantry_Custom> JFW_Flying_Infantry_Custom_Registrant("JFW_Flying_Infantry_Custom",
                                                                                   "Message:int");
ScriptRegistrant<JFW_Flying_Infantry_Powerup> JFW_Flying_Infantry_Powerup_Registrant("JFW_Flying_Infantry_Powerup", "");
ScriptRegistrant<JFW_Flying_Infantry_Enter> JFW_Flying_Infantry_Enter_Registrant("JFW_Flying_Infantry_Enter",
                                                                                 "Player_Type:int");
ScriptRegistrant<JFW_Flying_Infantry_Exit> JFW_Flying_Infantry_Exit_Registrant("JFW_Flying_Infantry_Exit",
                                                                               "Player_Type:int");
ScriptRegistrant<JFW_Drone_Damage> JFW_Drone_Damage_Registrant("JFW_Drone_Damage",
                                                               "RepairStop:int,Preset:string,Amount:int,Custom:int");
ScriptRegistrant<JFW_Conyard_Vehicle_Buildings> JFW_Conyard_Vehicle_Buildings_Registrant(
        "JFW_Conyard_Vehicle_Buildings", "Time:float,TimerNum:int,DisableCustom:int,Health:float,Message:int");
ScriptRegistrant<JFW_Domination_Zone> JFW_Domination_Zone_Registrant("JFW_Domination_Zone",
                                                                     "Controler_ID:int,NeutralMessage:int,OwnedMessage:int,Nod_Model:string,GDI_Model:string,Neutral_Model:string,Capture_Delay_Time:float,TimerNum:int");
ScriptRegistrant<JFW_Domination_Controler> JFW_Domination_Controler_Registrant("JFW_Domination_Controler",
                                                                               "NeutralMessage:int,OwnedMessage:int,Time:float,TimerNum:int,PointsToGive:float,PointsToTake:float,GDIObjectID:int,NodObjectID:int");
ScriptRegistrant<JFW_Iron_Curtain_Zone> JFW_Iron_Curtain_Zone_Registrant("JFW_Iron_Curtain_Zone",
                                                                         "Time:float,TimerNum:int,Animation1:string,Animation1Loc:vector3,Animation1Facing:float,Animation1Slot:int,Animation2:string,Animation2Loc:vector3,Animation2Facing:float,Animation2Slot:int,Player_Type:int,Animation3:string,Animation3Loc:vector3,Animation3Facing:float,Animation3Slot:int,BuildingID:int,Invunlerability_Time:float,Invunlerability_TimerNum:int,Animation1Msg:int,Animation2Msg:int,Animation3Msg:int,OffID:int,OnID:int,CinematicPowerOff:int");
ScriptRegistrant<JFW_Chrono_Controler_Zone> JFW_Chrono_Controler_Zone_Registrant("JFW_Chrono_Controler_Zone",
                                                                                 "Spawner_Time:float,Spawner_TimerNum:int,Spawner_Preset:string,Spawner_Location:vector3,Chrono_Time:float,Chrono_TimerNum:int,Message:int,Animation1:string,Animation1Loc:vector3,Animation1Facing:float,Animation1Slot:int,Animation2:string,Animation2Loc:vector3,Animation2Facing:float,Animation2Slot:int,Animation3:string,Animation3Loc:vector3,Animation3Facing:float,Animation3Slot:int,BuildingID:int,ZAdjust:float,Animation1Msg:int,Animation2Msg:int,Animation3Msg:int,OffID:int,OnID:int,CinematicPowerOff:int,Activate_Time:float,Activate_TimerNum:int,DeathMessage:int");
ScriptRegistrant<JFW_Assault_Powerplant_Controller> JFW_Assault_Powerplant_Controller_Registrant(
        "JFW_Assault_Powerplant_Controller",
        "Time:float,TimerNum:int,Player_Type:int,Power_Off:int,Power_On:int,Building1:int,Building2:int,Building3:int,Building4:int,Building5:int,Building6:int,Building7:int,Building8:int,Building9:int,Building10:int,Building11:int,Building12:int,Building13:int,Building14:int,Building15:int");
ScriptRegistrant<JFW_Resource_Collector> JFW_Resource_Collector_Registrant("JFW_Resource_Collector",
                                                                           "Collected_Mesasge:int,Refinery_Message:int,Cash_Limit:int,Cash_Type:int,Entire_Team:int");
ScriptRegistrant<JFW_Resource_Collector_2> JFW_Resource_Collector_2_Registrant("JFW_Resource_Collector_2",
                                                                               "Collected_Mesasge:int,Refinery_Message:int,Cash_Limit:int,Cash_Type:int,Entire_Team:int");
ScriptRegistrant<JFW_Resource_Collector_3> JFW_Resource_Collector_3_Registrant("JFW_Resource_Collector_3",
                                                                               "Collected_Mesasge:int,Refinery_Message:int,Cash_Limit:int,Cash_Type:int,Entire_Team:int");
ScriptRegistrant<JFW_Resource_Refinery> JFW_Resource_Refinery_Registrant("JFW_Resource_Refinery",
                                                                         "Refinery_Message:int");
ScriptRegistrant<JFW_Resource_Field> JFW_Resource_Field_Registrant("JFW_Resource_Field",
                                                                   "Cash:int,Time:float,TimerNum:int,Collected_Preset:string,Collected_Preset_Distance:float,Collected_Message:int,Nod_Preset:string,GDI_Preset:string,StartHarvest:int,StopHarvest:int");
ScriptRegistrant<JFW_Team_DM_Controller> JFW_Team_DM_Controller_Registrant("JFW_Team_DM_Controller",
                                                                           "Deaths_To_Loose:int,Death_Message:int,Building_To_Destroy1:int,Building_To_Destroy2:int,Building_To_Destroy3:int,Building_To_Destroy4:int,Building_To_Destroy5:int");
ScriptRegistrant<JFW_Team_DM_Zone> JFW_Team_DM_Zone_Registrant("JFW_Team_DM_Zone",
                                                               "Player_Type:int,Controller_ID:int,Deactivate_Message:int,Death_Message:int");
ScriptRegistrant<JFW_Team_DM_Attatched> JFW_Team_DM_Attatched_Registrant("JFW_Team_DM_Attatched",
                                                                         "Controller_ID:int,Deactivate_Message:int,Death_Message:int");
ScriptRegistrant<JFW_Conyard_Radius> JFW_Conyard_Radius_Registrant("JFW_Conyard_Radius",
                                                                   "Time:float,TimerNum:int,DisableCustom:int,Health:float,Radius:float");
ScriptRegistrant<JFW_Chrono_Harvester_Logic> JFW_Chrono_Harvester_Logic_Registrant("JFW_Chrono_Harvester_Logic",
                                                                                   "Harvester_Preset:string,Explosion_Preset:string");
ScriptRegistrant<JFW_Chrono_Harvester_Attached> JFW_Chrono_Harvester_Attached_Registrant(
        "JFW_Chrono_Harvester_Attached",
        "LogicID=0:int,Number_Of_Paths=0:int,Waypath1_Dock2Field=0:int,Waypath1_Field2Dock=0:int,Waypath1_Tib_Zone:vector3,Waypath2_Dock2Field=0:int,Waypath2_Field2Dock=0:int,Waypath2_Tib_Zone:vector3,Waypath3_Dock2Field=0:int,Waypath3_Field2Dock=0:int,Waypath3_Tib_Zone:vector3,Harvest_Animation:string,Dock_Animation:string,Tiberium_Idle_Time=45.00:float,Dock_Location:vector3,Dock_Idle_Time=10.00:float,Dock_Credits=400.00:float,Harvester_Create_Idle_Time=5.00:float,Crystal_Preset:string,Chrono_Location:vector3");
ScriptRegistrant<JFW_Chrono_Harvester_Kill> JFW_Chrono_Harvester_Kill_Registrant("JFW_Chrono_Harvester_Kill",
                                                                                 "LogicID=0:int");
ScriptRegistrant<JFW_Chrono_Harvester_Spawn_Kill> JFW_Chrono_Harvester_Spawn_Kill_Registrant(
        "JFW_Chrono_Harvester_Spawn_Kill", "LogicID=0:int");
ScriptRegistrant<JFW_Conyard_Turrets> JFW_Conyard_Turrets_Registrant("JFW_Conyard_Turrets",
                                                                     "Time:float,TimerNum:int,DisableCustom:int,Health:float");
ScriptRegistrant<JFW_Radar_Jammer> JFW_Radar_Jammer_Registrant("JFW_Radar_Jammer",
                                                               "Time:float,TimerNum:int,DisableCustom:int,Preset:string,CenterID:int,Range:float,StartSound1:string,StopSound1:string,StartSound2:string,StopSound2:string");
ScriptRegistrant<JFW_Radar_Jammer_Sound> JFW_Radar_Jammer_Sound_Registrant("JFW_Radar_Jammer_Sound",
                                                                           "Time:float,TimerNum:int,DisableCustom:int,Preset:string,CenterID:int,Range:float,Sound:string");
ScriptRegistrant<JFW_Sonar_Pulse> JFW_Sonar_Pulse_Registrant("JFW_Sonar_Pulse",
                                                             "Spy_Script:string,Sub_Preset_1:string,Sub_Preset_2:string,Indicator:string,ZAdjust:float,WaterIndicator:string,WaterZ:float,Sound:string,Time:float,ResetTime:float");
ScriptRegistrant<JFW_Global_Stealth_Controller> JFW_Global_Stealth_Controller_Registrant(
        "JFW_Global_Stealth_Controller", "Update_Delay=2.0:float,Timer_Number=687:int");
ScriptRegistrant<JFW_Stealth_Generator_Building> JFW_Stealth_Generator_Building_Registrant(
        "JFW_Stealth_Generator_Building",
        "Timer_Number=876:int,Stealth_Range=50:int,Stealth_Controller:string,Team=1:int,SizePreset:string");
ScriptRegistrant<JFW_Stealth_Generator_Vehicle> JFW_Stealth_Generator_Vehicle_Registrant(
        "JFW_Stealth_Generator_Vehicle", "Stealth_Range=50:int,Stealth_Controller:string,SizePreset:string");
ScriptRegistrant<JFW_Stealth_Generator_Vehicle_Deploy> JFW_Stealth_Generator_Vehicle_Deploy_Registrant(
        "JFW_Stealth_Generator_Vehicle_Deploy", "Stealth_Range=50:int,Stealth_Controller:string,SizePreset:string");
ScriptRegistrant<JFW_Stealth_Generator_Ignored> JFW_Stealth_Generator_Ignored_Registrant(
...
ScriptRegistrant<JFW_Low_Power_Sound> JFW_Low_Power_Sound_Registrant("JFW_Low_Power_Sound",
                                                                     "DownSound:string,UpSound:string");
ScriptRegistrant<JFW_Message_Send_Zone> JFW_Message_Send_Zone_Registrant("JFW_Message_Send_Zone",
                                                                         "Player_Type:int,Message:string,Red:int,Blue:int,Green:int,Sound:string,Delete:int");
ScriptRegistrant<JFW_Message_Send_Zone_Team> JFW_Message_Send_Zone_Team_Registrant("JFW_Message_Send_Zone_Team",
                                                                                   "Player_Type:int,Message:string,Red:int,Blue:int,Green:int,Sound:string,Delete:int");
ScriptRegistrant<JFW_Message_Send_Zone_Player> JFW_Message_Send_Zone_Player_Registrant("JFW_Message_Send_Zone_Player",
                                                                                       "Player_Type:int,Message:string,Red:int,Blue:int,Green:int,Sound:string,Delete:int");
ScriptRegistrant<JFW_Message_Send_Death> JFW_Message_Send_Death_Registrant("JFW_Message_Send_Death",
                                                                           "Message:string,Red:int,Blue:int,Green:int,Sound:string");
ScriptRegistrant<JFW_Message_Send_Death_Team> JFW_Message_Send_Death_Team_Registrant("JFW_Message_Send_Death_Team",
                                                                                     "Player_Type:int,Message:string,Red:int,Blue:int,Green:int,Sound:string");
ScriptRegistrant<JFW_Message_Send_Custom> JFW_Message_Send_Custom_Registrant("JFW_Message_Send_Custom",
                                                                             "Display_Message:string,Red:int,Blue:int,Green:int,Sound:string,Message:int,Delete:int");
ScriptRegistrant<JFW_Message_Send_Powerup> JFW_Message_Send_Powerup_Registrant("JFW_Message_Send_Powerup",
                                                                               "Display_Message:string,Red:int,Blue:int,Green:int,Sound:string");
ScriptRegistrant<JFW_Message_Send_Custom_Team> JFW_Message_Send_Custom_Team_Registrant("JFW_Message_Send_Custom_Team",
                                                                                       "Player_Type:int,Display_Message:string,Red:int,Blue:int,Green:int,Sound:string,Message:int,Delete:int");
ScriptRegistrant<JFW_Spy_Switch_Team> JFW_Spy_Switch_Team_Registrant("JFW_Spy_Switch_Team", "");
ScriptRegistrant<JFW_Spy_Vehicle_Ignore> JFW_Spy_Vehicle_Ignore_Registrant("JFW_Spy_Vehicle_Ignore",
                                                                           "Spy_Script:string");
ScriptRegistrant<JFW_Spy_Vehicle_Ignore_New> JFW_Spy_Vehicle_Ignore_New_Registrant("JFW_Spy_Vehicle_Ignore_New", "");
ScriptRegistrant<JFW_Message_Send_Zone_Player_Vehicle> JFW_Message_Send_Zone_Player_Vehicle_Registrant(
...
ScriptRegistrant<JFW_Submarine> JFW_Submarine_Registrant("JFW_Submarine",
                                                         "Message:int,Submerge_Armor:string,Surface_Armor:string,Block_Weapon:int,Ping_Sound:string,Surface_Sound:string,Ping_Time:float,Surface_Z_Offset:float,Dive_Z_Offset:float,Powerup:string,Weapon:string");
ScriptRegistrant<JFW_Reaper_Web> JFW_Reaper_Web_Registrant("JFW_Reaper_Web",
                                                           "Warhead:string,Time:float,WebModel:string,HumanAnimation:string");
ScriptRegistrant<JFW_Toxic_Grenade> JFW_Toxic_Grenade_Registrant("JFW_Toxic_Grenade",
                                                                 "Weapon:string,Time:float,Warhead:string,Damage:float");
ScriptRegistrant<JFW_Hijacker_Vehicle> JFW_Hijacker_Vehicle_Registrant("JFW_Hijacker_Vehicle",
                                                                       "Weapon:string,Warhead:string,Damage:float,Sound:string");
ScriptRegistrant<JFW_Hijacker_Vehicle_2> JFW_Hijacker_Vehicle_2_Registrant("JFW_Hijacker_Vehicle_2",
                                                                           "Warhead:string,Sound:string,Disable_Transitions:int,Single_Hijack:int,Distance:float,Weapon:string");
ScriptRegistrant<JFW_Building_Preset_Disable> JFW_Building_Preset_Disable_Registrant("JFW_Building_Preset_Disable",
                                                                                     "Disable_Custom:int,Enable_Custom:int");
