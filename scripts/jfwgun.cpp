...
void JFW_Building_Gun::Created(GameObject *obj) {
    ...
    if (object) {
        ...
        sprintf(params, "%s,%s,%s,%s", Get_Parameter("Disable_Custom"), Get_Parameter("Visible"),
...
void JFW_Building_Gun_No_Aircraft::Created(GameObject *obj) {
    ...
    if (object) {
        ...
        sprintf(params, "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s", Get_Parameter("Disable_Custom"),
                Get_Parameter("Visible"), Get_Parameter("Min_Range"), Get_Parameter("Max_Range"),
                Get_Parameter("Preset1"), Get_Parameter("Preset2"), Get_Parameter("Preset3"), Get_Parameter("Preset4"),
                Get_Parameter("Preset5"), Get_Parameter("Preset6"), Get_Parameter("Preset7"), Get_Parameter("Preset8"));
...
void JFW_Building_Gun_Weapon_No_Aircraft::Enemy_Seen(GameObject *obj, GameObject *enemy) {
    ...
    float maxrange;
    float range;
    if ((_stricmp(Commands->Get_Preset_Name(enemy), Get_Parameter("Preset1"))) &&
        (_stricmp(Commands->Get_Preset_Name(enemy), Get_Parameter("Preset2"))) &&
        (_stricmp(Commands->Get_Preset_Name(enemy), Get_Parameter("Preset3"))) &&
        (_stricmp(Commands->Get_Preset_Name(enemy), Get_Parameter("Preset4"))) &&
        (_stricmp(Commands->Get_Preset_Name(enemy), Get_Parameter("Preset5"))) &&
        (_stricmp(Commands->Get_Preset_Name(enemy), Get_Parameter("Preset6"))) &&
        (_stricmp(Commands->Get_Preset_Name(enemy), Get_Parameter("Preset7"))) &&
        (_stricmp(Commands->Get_Preset_Name(enemy), Get_Parameter("Preset8")))) {
        minrange = Get_Float_Parameter("Min_Range");
        maxrange = Get_Float_Parameter("Max_Range");
        Vector3 pos, pos2;
        pos = Commands->Get_Position(obj);
...
void JFW_Building_Gun_Animated::Created(GameObject *obj) {
    ...
    if (object) {
        Commands->Start_Timer(obj, this, 1.0, 1);
        weaponid = Commands->Get_ID(object);
        sprintf(params, "%s,%s,%s,%s,%s,%s,%s,%s", Get_Parameter("Disable_Custom"), Get_Parameter("Visible"),
                Get_Parameter("Min_Range"), Get_Parameter("Max_Range"), Get_Parameter("EffectPreset"),
                Get_Parameter("EffectAnimation"), Get_Parameter("EffectTime"), Get_Parameter("EffectFrame"));
...
void JFW_Building_Gun_Animated_Weapon::Enemy_Seen(GameObject *obj, GameObject *enemy) {
    ...
    if ((Commands->Get_Distance(pos, pos2) > minrange) && (range < maxrange)) {
        ...
        if (!lastframe) {
            lastframe = true;
            Commands->Set_Animation(Commands->Find_Object(effectid), Get_Parameter("EffectAnimation"), false, 0, 0,
                                    Get_Float_Parameter("EffectFrame"), false);
...
void JFW_Building_Gun_Animated_Weapon::Timer_Expired(GameObject *obj, int number) {
    if (number == 1) {
        ...
        if (lastframe) {
            lastframe = false;
            Commands->Set_Animation(Commands->Find_Object(effectid), Get_Parameter("EffectAnimation"), false, 0,
                                    Get_Float_Parameter("EffectFrame"), 0, false);
...
void JFW_Building_Gun_Animated_No_Aircraft::Created(GameObject *obj) {
    ...
    if (object) {
        ...
        sprintf(params, "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s", Get_Parameter("Disable_Custom"),
                ...
                Get_Parameter("Preset1"), Get_Parameter("Preset2"), Get_Parameter("Preset3"), Get_Parameter("Preset4"),
                Get_Parameter("Preset5"), Get_Parameter("Preset6"), Get_Parameter("Preset7"), Get_Parameter("Preset8"),
                Get_Parameter("EffectPreset"), Get_Parameter("EffectAnimation"), Get_Parameter("EffectTime"),
                Get_Parameter("EffectFrame"));
...
void JFW_Building_Gun_Animated_Weapon_No_Aircraft_Secondary::Custom(GameObject *obj, int type, int param,
                                                                    GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
...
void JFW_Building_Gun_Animated_Weapon_Aircraft_Only_Secondary::Custom(GameObject *obj, int type, int param,
                                                                      GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
...
void JFW_Building_Gun_Animated_Sound::Created(GameObject *obj) {
    ...
    if (object) {
        Commands->Start_Timer(obj, this, 1.0, 1);
        weaponid = Commands->Get_ID(object);
        sprintf(params, "%s,%s,%s,%s,%s,%s,%s,%s,%s", Get_Parameter("Disable_Custom"), Get_Parameter("Visible"),
                Get_Parameter("Min_Range"), Get_Parameter("Max_Range"), Get_Parameter("EffectPreset"),
                Get_Parameter("EffectAnimation"), Get_Parameter("EffectTime"), Get_Parameter("EffectFrame"),
                Get_Parameter("Sound"));
...
void JFW_Building_Gun_Animated_Sound_No_Aircraft::Created(GameObject *obj) {
    ...
    if (object) {
        ...
        sprintf(params, "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s", Get_Parameter("Disable_Custom"),
                ...
                Get_Parameter("Preset5"), Get_Parameter("Preset6"), Get_Parameter("Preset7"), Get_Parameter("Preset8"),
                Get_Parameter("EffectPreset"), Get_Parameter("EffectAnimation"), Get_Parameter("EffectTime"),
                Get_Parameter("EffectFrame"), Get_Parameter("Sound"));
...
void
JFW_Building_Gun_Animated_Sound_Weapon_No_Aircraft::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
        Commands->Enable_Enemy_Seen(obj, false);
...
void
JFW_Building_Gun_Animated_Sound_Weapon_Aircraft_Only::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
        Commands->Enable_Enemy_Seen(obj, false);
...
void
JFW_Building_Gun_Animated_Sound_Weapon_Secondary::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
        Commands->Enable_Enemy_Seen(obj, false);
...
void JFW_Building_Gun_Animated_Sound_Weapon_No_Aircraft_Secondary::Custom(GameObject *obj, int type, int param,
                                                                          GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
...
void JFW_Building_Gun_Animated_Sound_Weapon_Aircraft_Only_Secondary::Custom(GameObject *obj, int type, int param,
                                                                            GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
...
void
JFW_Building_Gun_Animated_Sound_Weapon_VTOL_Only::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
        Commands->Enable_Enemy_Seen(obj, false);
...
void JFW_Building_Gun_Animated_Sound_Weapon_No_VTOL_Secondary::Custom(GameObject *obj, int type, int param,
                                                                      GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
...
void JFW_Building_Gun_Animated_Sound_Weapon_VTOL_Only_Secondary::Custom(GameObject *obj, int type, int param,
                                                                        GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
...
void
JFW_Building_Gun_Animated_Weapon_No_VTOL_Secondary::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
        Commands->Enable_Enemy_Seen(obj, false);
...
void
JFW_Building_Gun_Animated_Weapon_VTOL_Only_Secondary::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    int msg;
    msg = Get_Int_Parameter("Disable_Custom");
    if (type == msg) {
        Commands->Enable_Enemy_Seen(obj, false);
...
ScriptRegistrant<JFW_Building_Gun> JFW_Building_Gun_Registrant("JFW_Building_Gun",
                                                               "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_Weapon> JFW_Building_Gun_Weapon_Registrant("JFW_Building_Gun_Weapon",
                                                                             "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_No_Aircraft> JFW_Building_Gun_No_Aircraft_Registrant("JFW_Building_Gun_No_Aircraft",
...
ScriptRegistrant<JFW_Building_Gun_Aircraft_Only> JFW_Building_Gun_Aircraft_Only_Registrant(
        "JFW_Building_Gun_Aircraft_Only",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string");
ScriptRegistrant<JFW_Building_Gun_Weapon_No_Aircraft> JFW_Building_Gun_Weapon_No_Aircraft_Registrant(
...
ScriptRegistrant<JFW_Building_Gun_Weapon_Aircraft_Only> JFW_Building_Gun_Weapon_Aircraft_Only_Registrant(
        "JFW_Building_Gun_Weapon_Aircraft_Only",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string");
ScriptRegistrant<JFW_Building_Gun_Secondary> JFW_Building_Gun_Secondary_Registrant("JFW_Building_Gun_Secondary",
                                                                                   "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_Weapon_Secondary> JFW_Building_Gun_Weapon_Secondary_Registrant(
        "JFW_Building_Gun_Weapon_Secondary", "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_No_Aircraft_Secondary> JFW_Building_Gun_No_Aircraft_Secondary_Registrant(
        "JFW_Building_Gun_No_Aircraft_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string");
ScriptRegistrant<JFW_Building_Gun_Aircraft_Only_Secondary> JFW_Building_Gun_Aircraft_Only_Secondary_Registrant(
        "JFW_Building_Gun_Aircraft_Only_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string");
ScriptRegistrant<JFW_Building_Gun_Weapon_No_Aircraft_Secondary> JFW_Building_Gun_Weapon_No_Aircraft_Secondary_Registrant(
        "JFW_Building_Gun_Weapon_No_Aircraft_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string");
ScriptRegistrant<JFW_Building_Gun_Weapon_Aircraft_Only_Secondary> JFW_Building_Gun_Weapon_Aircraft_Only_Secondary_Registrant(
        "JFW_Building_Gun_Weapon_Aircraft_Only_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string");
ScriptRegistrant<JFW_Building_Gun_Animated> JFW_Building_Gun_Animated_Registrant("JFW_Building_Gun_Animated",
...
ScriptRegistrant<JFW_Building_Gun_Animated_Weapon> JFW_Building_Gun_Animated_Weapon_Registrant(
...
ScriptRegistrant<JFW_Building_Gun_Animated_No_Aircraft> JFW_Building_Gun_Animated_No_Aircraft_Registrant(
        "JFW_Building_Gun_Animated_No_Aircraft",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Aircraft_Only> JFW_Building_Gun_Animated_Aircraft_Only_Registrant(
        "JFW_Building_Gun_Animated_Aircraft_Only",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Weapon_No_Aircraft> JFW_Building_Gun_Animated_Weapon_No_Aircraft_Registrant(
        "JFW_Building_Gun_Animated_Weapon_No_Aircraft",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Weapon_Aircraft_Only> JFW_Building_Gun_Animated_Weapon_Aircraft_Only_Registrant(
        "JFW_Building_Gun_Animated_Weapon_Aircraft_Only",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Secondary> JFW_Building_Gun_Animated_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Weapon_Secondary> JFW_Building_Gun_Animated_Weapon_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Weapon_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_No_Aircraft_Secondary> JFW_Building_Gun_Animated_No_Aircraft_Secondary_Registrant(
        "JFW_Building_Gun_Animated_No_Aircraft_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Aircraft_Only_Secondary> JFW_Building_Gun_Animated_Aircraft_Only_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Aircraft_Only_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Weapon_No_Aircraft_Secondary> JFW_Building_Gun_Animated_Weapon_No_Aircraft_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Weapon_No_Aircraft_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Weapon_Aircraft_Only_Secondary> JFW_Building_Gun_Animated_Weapon_Aircraft_Only_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Weapon_Aircraft_Only_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_No_VTOL> JFW_Building_Gun_No_VTOL_Registrant("JFW_Building_Gun_No_VTOL",
                                                                               "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_Weapon_No_VTOL> JFW_Building_Gun_Weapon_No_VTOL_Registrant(
        "JFW_Building_Gun_Weapon_No_VTOL", "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_No_VTOL_Secondary> JFW_Building_Gun_No_VTOL_Secondary_Registrant(
        "JFW_Building_Gun_No_VTOL_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_Weapon_No_VTOL_Secondary> JFW_Building_Gun_Weapon_No_VTOL_Secondary_Registrant(
        "JFW_Building_Gun_Weapon_No_VTOL_Secondary", "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_Animated_No_VTOL> JFW_Building_Gun_Animated_No_VTOL_Registrant(
        "JFW_Building_Gun_Animated_No_VTOL",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Weapon_No_VTOL> JFW_Building_Gun_Animated_Weapon_No_VTOL_Registrant(
        "JFW_Building_Gun_Animated_Weapon_No_VTOL",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_No_VTOL_Secondary> JFW_Building_Gun_Animated_No_VTOL_Secondary_Registrant(
        "JFW_Building_Gun_Animated_No_VTOL_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Weapon_No_VTOL_Secondary> JFW_Building_Gun_Animated_Weapon_No_VTOL_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Weapon_No_VTOL_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_VTOL_Only> JFW_Building_Gun_VTOL_Only_Registrant("JFW_Building_Gun_VTOL_Only",
                                                                                   "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_Weapon_VTOL_Only> JFW_Building_Gun_Weapon_VTOL_Only_Registrant(
        "JFW_Building_Gun_Weapon_VTOL_Only", "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_VTOL_Only_Secondary> JFW_Building_Gun_VTOL_Only_Secondary_Registrant(
        "JFW_Building_Gun_VTOL_Only_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_Weapon_VTOL_Only_Secondary> JFW_Building_Gun_Weapon_VTOL_Only_Secondary_Registrant(
        "JFW_Building_Gun_Weapon_VTOL_Only_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float");
ScriptRegistrant<JFW_Building_Gun_Animated_VTOL_Only> JFW_Building_Gun_Animated_VTOL_Only_Registrant(
        "JFW_Building_Gun_Animated_VTOL_Only",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Weapon_VTOL_Only> JFW_Building_Gun_Animated_Weapon_VTOL_Only_Registrant(
        "JFW_Building_Gun_Animated_Weapon_VTOL_Only",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_VTOL_Only_Secondary> JFW_Building_Gun_Animated_VTOL_Only_Secondary_Registrant(
        "JFW_Building_Gun_Animated_VTOL_Only_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Weapon_VTOL_Only_Secondary> JFW_Building_Gun_Animated_Weapon_VTOL_Only_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Weapon_VTOL_Only_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_No_VTOL> JFW_Building_Gun_Animated_Sound_No_VTOL_Registrant(
...
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Weapon_No_VTOL> JFW_Building_Gun_Animated_Sound_Weapon_No_VTOL_Registrant(
...
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_No_VTOL_Secondary> JFW_Building_Gun_Animated_Sound_No_VTOL_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Sound_No_VTOL_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Weapon_No_VTOL_Secondary> JFW_Building_Gun_Animated_Sound_Weapon_No_VTOL_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Sound_Weapon_No_VTOL_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_VTOL_Only> JFW_Building_Gun_Animated_Sound_VTOL_Only_Registrant(
        "JFW_Building_Gun_Animated_Sound_VTOL_Only",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Weapon_VTOL_Only> JFW_Building_Gun_Animated_Sound_Weapon_VTOL_Only_Registrant(
        "JFW_Building_Gun_Animated_Sound_Weapon_VTOL_Only",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_VTOL_Only_Secondary> JFW_Building_Gun_Animated_Sound_VTOL_Only_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Sound_VTOL_Only_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Weapon_VTOL_Only_Secondary> JFW_Building_Gun_Animated_Sound_Weapon_VTOL_Only_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Sound_Weapon_VTOL_Only_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound> JFW_Building_Gun_Animated_Sound_Registrant(
        "JFW_Building_Gun_Animated_Sound",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Weapon> JFW_Building_Gun_Animated_Sound_Weapon_Registrant(
        "JFW_Building_Gun_Animated_Sound_Weapon",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_No_Aircraft> JFW_Building_Gun_Animated_Sound_No_Aircraft_Registrant(
...
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Aircraft_Only> JFW_Building_Gun_Animated_Sound_Aircraft_Only_Registrant(
        "JFW_Building_Gun_Animated_Sound_Aircraft_Only",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Weapon_No_Aircraft> JFW_Building_Gun_Animated_Sound_Weapon_No_Aircraft_Registrant(
        "JFW_Building_Gun_Animated_Sound_Weapon_No_Aircraft",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Weapon_Aircraft_Only> JFW_Building_Gun_Animated_Sound_Weapon_Aircraft_Only_Registrant(
        "JFW_Building_Gun_Animated_Sound_Weapon_Aircraft_Only",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Secondary> JFW_Building_Gun_Animated_Sound_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Sound_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Weapon_Secondary> JFW_Building_Gun_Animated_Sound_Weapon_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Sound_Weapon_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_No_Aircraft_Secondary> JFW_Building_Gun_Animated_Sound_No_Aircraft_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Sound_No_Aircraft_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Aircraft_Only_Secondary> JFW_Building_Gun_Animated_Sound_Aircraft_Only_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Sound_Aircraft_Only_Secondary",
        "Disable_Custom:int,Positon:vector3,Weapon:string,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Weapon_No_Aircraft_Secondary> JFW_Building_Gun_Animated_Sound_Weapon_No_Aircraft_Secondary_Registrant(
        "JFW_Building_Gun_Animated_Sound_Weapon_No_Aircraft_Secondary",
        "Disable_Custom:int,Visible:int,Min_Range:float,Max_Range:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string,EffectPreset:string,EffectAnimation:string,EffectTime:float,EffectFrame:float,Sound:string");
ScriptRegistrant<JFW_Building_Gun_Animated_Sound_Weapon_Aircraft_Only_Secondary> JFW_Building_Gun_Animated_Sound_Weapon_Aircraft_Only_Secondary_Registrant(
