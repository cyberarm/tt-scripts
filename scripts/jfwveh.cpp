...
void JFW_Turret_Spawn_3::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == CUSTOM_EVENT_VEHICLE_ENTERED) {
        if (!m_bHasDriver) {
            m_bHasDriver = true;
            Commands->Set_Player_Type(Commands->Find_Object(turID), Commands->Get_Player_Type(sender));
            Commands->Send_Custom_Event(obj, Commands->Find_Object(turID), Get_Int_Parameter("Driver_Entered_Custom"),
                                        0, 0);
        }
    }
    if (type == CUSTOM_EVENT_VEHICLE_EXITED) {
        if (m_bHasDriver && obj->As_VehicleGameObj() && obj->As_VehicleGameObj()->Get_Occupant(0) == NULL) {
            m_bHasDriver = false;
            Commands->Send_Custom_Event(obj, Commands->Find_Object(turID), Get_Int_Parameter("Driver_Exited_Custom"), 0,
                                        0);
        }
    }
}
...
void JFW_Drive_To_Player::Created(GameObject *obj) {
    ...
    params.Set_Movement(Commands->Get_A_Star(Commands->Get_Position(obj)), Get_Float_Parameter("Speed"),
                        Get_Float_Parameter("Arrive_Distance"));
...
void JFW_Visible_People_In_Vechicle::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == CUSTOM_EVENT_VEHICLE_ENTERED) {
        if (!modelid1) {
            ...
            Commands->Set_Animation(object, Get_Parameter("Animation1"), false, subobject, firstframe,
                                    Get_Float_Parameter("LastFrame1"), Get_Bool_Parameter("Blended1"));
        } else {
            if (!modelid2) {
                ...
                Commands->Set_Animation(object, Get_Parameter("Animation2"), false, subobject, firstframe,
                                        Get_Float_Parameter("LastFrame2"), Get_Bool_Parameter("Blended2"));
            }
        }
    }
    if (type == CUSTOM_EVENT_VEHICLE_EXITED) {
...
void JFW_Vechicle_Animation_2::Timer_Expired(GameObject *obj, int number) {
    ...
    if ((position.X != xpos) || (position.Y != ypos) || (position.Z != zpos)) {
        ...
        if (!enabled) {
            ...
            Commands->Set_Animation(obj, Get_Parameter("UpAnimation"), false, subobject, firstframe,
        ...
        Commands->Set_Animation(obj, Get_Parameter("DownAnimation"), false, subobject, firstframe,
...
void JFW_Vehicle_Model_Preset::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if ((!_stricmp(Commands->Get_Preset_Name(sender), Get_Parameter("Preset"))) &&
        (type == CUSTOM_EVENT_VEHICLE_ENTERED)) {
        Commands->Set_Model(obj, Get_Parameter("Model"));
    }
}
...
void JFW_Aircraft_Refill_Animation::Entered(GameObject *obj, GameObject *enterer) {
    if (Commands->Get_Player_Type(enterer) == Get_Int_Parameter("TeamSelection")) {
        if (!active) {
            ...
            if (vobj) {
                if (Get_Vehicle_Mode(vobj) == VEHICLE_TYPE_FLYING) {
                    if (Get_Vehicle_Driver(vobj) && vobj->As_VehicleGameObj()->Get_Weapon() &&
                        !vobj->As_VehicleGameObj()->Get_Weapon()->Is_Ammo_Maxed() &&
                        vobj->As_VehicleGameObj()->Get_Weapon()->Get_Definition()->MaxInventoryRounds != 0) {
                        GameObject * anim = Find_Nearest_Preset(Commands->Get_Position(obj),
                                                                Get_Parameter("AnimPreset"));
                        if (anim->As_PhysicalGameObj()) {
                            Commands->Set_Animation(anim, Get_Parameter("Animation"), true, 0, 0, -1, false);
                        }
                        Commands->Start_Timer(obj, this, Get_Float_Parameter("Reload_Interval"),
                                              Commands->Get_ID(vobj));
                        active = true;
                    }
                }
            }
...
void JFW_Aircraft_Refill_Animation::Timer_Expired(GameObject *obj, int number) {
    ...
    pos2 = Commands->Get_Position(object);
    distance = Commands->Get_Distance(pos1, pos2);
    if (distance <= 10.0 && object->As_VehicleGameObj() && Get_Vehicle_Mode(object) == VEHICLE_TYPE_FLYING &&
        Get_Vehicle_Driver(object) && object->As_VehicleGameObj()->Get_Weapon() &&
        !object->As_VehicleGameObj()->Get_Weapon()->Is_Ammo_Maxed() &&
        object->As_VehicleGameObj()->Get_Weapon()->Get_Definition()->MaxInventoryRounds != 0) {
        Commands->Give_PowerUp(object, Get_Parameter("Reload_Powerup"), false);
        Commands->Start_Timer(obj, this, Get_Float_Parameter("Reload_Interval"), number);
        active = true;
    } else {
...
void
JFW_Per_Preset_Visible_Multiple_People_In_Vehicle::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == CUSTOM_EVENT_VEHICLE_ENTERED) {
...
void
JFW_Per_Preset_Visible_Multiple_People_In_Vehicle_2::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == CUSTOM_EVENT_VEHICLE_ENTERED) {
        int seat = Get_Occupant_Seat(obj, sender);
...
ScriptRegistrant<JFW_Vehicle_Model_Team> JFW_Vehicle_Model_Team_Registrant("JFW_Vehicle_Model_Team",
                                                                           "Model:string,Player_Type:int");
ScriptRegistrant<JFW_Vehicle_Model_Preset> JFW_Vehicle_Model_Preset_Registrant("JFW_Vehicle_Model_Preset",
                                                                               "Model:string,Preset:string");
ScriptRegistrant<JFW_Vehicle_Model_Weapon> JFW_Vehicle_Model_Weapon_Registrant("JFW_Vehicle_Model_Weapon",
                                                                               "Model:string,CharWeapon:string");
ScriptRegistrant<JFW_Vechicle_Animation_Trigger> JFW_Vechicle_Animation_Trigger_Registrant(
        "JFW_Vechicle_Animation_Trigger",
        "Animation:string,Subobject:string,FirstFrame:float,LastFrame:float,Blended:int,Time:float,TimerNum:int,UpAnimation:string,UpSubobject:string,UpFirstFrame:float,UpLastFrame:float,UpBlended:int,DownAnimation:string,DownSubobject:string,DownFirstFrame:float,DownLastFrame:float,DownBlended:int,UpTrigger:int,DownTrigger:int");
ScriptRegistrant<JFW_Vechicle_Animation_2> JFW_Vechicle_Animation_2_Registrant("JFW_Vechicle_Animation_2",
                                                                               "Animation:string,Subobject:string,FirstFrame:float,LastFrame:float,Blended:int,Time:float,TimerNum:int,UpAnimation:string,UpSubobject:string,UpFirstFrame:float,UpLastFrame:float,UpBlended:int,DownAnimation:string,DownSubobject:string,DownFirstFrame:float,DownLastFrame:float,DownBlended:int");
ScriptRegistrant<JFW_Turret_Spawn_2> JFW_Turret_Spawn_2_Registrant("JFW_Turret_Spawn_2",
                                                                   "Turret_Preset:string,Bone_Name=Tur_Mount:string,Explosion_Preset:string,Explosion_Bone:string");
ScriptRegistrant<JFW_Turret_Spawn_3> JFW_Turret_Spawn_3_Registrant("JFW_Turret_Spawn_3",
                                                                   "Turret_Preset:string,Bone_Name=Tur_Mount:string,Explosion_Preset:string,Explosion_Bone:string,Driver_Entered_Custom:int,Driver_Exited_Custom:int");
ScriptRegistrant<JFW_Drive_To_Player> JFW_Drive_To_Player_Registrant("JFW_Drive_To_Player",
                                                                     "Speed:float,Arrive_Distance:float");
ScriptRegistrant<JFW_Vechicle_Animation> JFW_Vechicle_Animation_Registrant("JFW_Vechicle_Animation",
                                                                           "Animation:string,Subobject:string,FirstFrame:float,LastFrame:float,Blended:int,Time:float,TimerNum:int");
ScriptRegistrant<JFW_Visible_Person_In_Vechicle> JFW_Visible_Person_In_Vechicle_Registrant(
        "JFW_Visible_Person_In_Vechicle",
        "BoneName:string,ModelName:string,Animation:string,SubObject:string,FirstFrame:float,LastFrame:float,Blended:int");
ScriptRegistrant<JFW_Visible_People_In_Vechicle> JFW_Visible_People_In_Vechicle_Registrant(
...
ScriptRegistrant<JFW_Per_Preset_Visible_Person_In_Vechicle> JFW_Per_Preset_Visible_Person_In_Vechicle_Registrant(
        "JFW_Per_Preset_Visible_Person_In_Vechicle", "BoneName:string,Message:int");
ScriptRegistrant<JFW_Per_Preset_Visible_People_In_Vechicle> JFW_Per_Preset_Visible_People_In_Vechicle_Registrant(
        "JFW_Per_Preset_Visible_People_In_Vechicle", "BoneName1:string,BoneName2:string,Message1:int,Message2:int");
ScriptRegistrant<JFW_Visible_Person_Settings> JFW_Visible_Person_Settings_Registrant("JFW_Visible_Person_Settings",
                                                                                     "Message:int,ModelName:string,Animation:string,SubObject:string,FirstFrame:float,LastFrame:float,Blended:int");
ScriptRegistrant<JFW_Visible_Person_Settings_2> JFW_Visible_Person_Settings_Registrant_2(
        "JFW_Visible_Person_Settings_2", "Message:int,ModelName:string,Animation:string,Frame:float");
ScriptRegistrant<JFW_Aircraft_Fuel> JFW_Aircraft_Fuel_Registrant("JFW_Aircraft_Fuel",
                                                                 "Time:float,TimerNum:int,Explosion:string,Refuel_Message:int");
ScriptRegistrant<JFW_Vehicle_Block_Preset> JFW_Vehicle_Block_Preset_Registrant("JFW_Vehicle_Block_Preset",
                                                                               "Preset:string");
ScriptRegistrant<JFW_Vehicle_Extra> JFW_Vehicle_Extra_Registrant("JFW_Vehicle_Extra",
                                                                 "Extra_Preset:string,Bone_Name:string");
ScriptRegistrant<JFW_Vehicle_Extra_2> JFW_Vehicle_Extra_2_Registrant("JFW_Vehicle_Extra_2",
                                                                     "Extra_Preset:string,Bone_Name:string");
ScriptRegistrant<JFW_Vehicle_Extra_Attach> JFW_Vehicle_Extra_Attach_Registrant("JFW_Vehicle_Extra_Attach",
                                                                               "Main_Object:int");
ScriptRegistrant<JFW_Vehicle_Lock> JFW_Vehicle_Lock_Registrant("JFW_Vehicle_Lock",
                                                               "Message:int,Spy_Script:string,Time:float");
ScriptRegistrant<JFW_Vehicle_Effect_Animation> JFW_Vehicle_Effect_Animation_Registrant("JFW_Vehicle_Effect_Animation",
                                                                                       "Animation:string,Subobject:string,FirstFrame:float,LastFrame:float,Blended:int");
ScriptRegistrant<JFW_Vehicle_Regen_2> JFW_Vechicle_Regen_2_Registrant("JFW_Vechicle_Regen_2",
                                                                      "Health:float,Time:float");
ScriptRegistrant<JFW_Vehicle_Regen_3> JFW_Vechicle_Regen_3_Registrant("JFW_Vechicle_Regen_3",
                                                                      "Health:float,Time:float,Warhead:string");
ScriptRegistrant<JFW_Aircraft_Refill> JFW_Aircraft_Refill_Registrant("JFW_Aircraft_Refill",
                                                                     "TeamSelection:int,Reload_Interval:float,Reload_Powerup:string");
ScriptRegistrant<JFW_Aircraft_Refill_Animation> JFW_Aircraft_Refill_Animation_Registrant(
...
ScriptRegistrant<JFW_Vehicle_Reinforcement> JFW_Vehicle_Reinforcement_Registrant("JFW_Vehicle_Reinforcement",
                                                                                 "Vehicle:string,Time:float,FactoryID:int,Waypathid:int,Speed:float");
ScriptRegistrant<JFW_Empty_Vehicle_Timer> JFW_Empty_Vehicle_Timer_Registrant("JFW_Empty_Vehicle_Timer",
                                                                             "Time:float,TimerNum:int");
ScriptRegistrant<JFW_Vehicle_Visible_Weapon> JFW_Vehicle_Visible_Weapon_Registrant("JFW_Vehicle_Visible_Weapon",
                                                                                   "Animation:string");
ScriptRegistrant<JFW_Per_Preset_Visible_Multiple_People_In_Vehicle> JFW_Per_Preset_Visible_Multiple_People_In_Vehicle_Registrant(
        "JFW_Per_Preset_Visible_Multiple_People_In_Vehicle", "Message:int");
ScriptRegistrant<JFW_Per_Preset_Visible_Multiple_People_In_Vehicle_2> JFW_Per_Preset_Visible_Multiple_People_In_Vehicle_2_Registrant(
        "JFW_Per_Preset_Visible_Multiple_People_In_Vehicle_2", "Message:int,FirstSeat:int,LastSeat:int");
