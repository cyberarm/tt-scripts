...
void JFW_Repair_Zone::Timer_Expired(GameObject *obj, int number) {
    ...
    if (distance <= Get_Float_Parameter("Repair_Distance")) {
        ...
        if (Shield_Strength < Max_Shield_Strength) {
            Commands->Set_Shield_Strength(Commands->Find_Object(number),
                                          Shield_Strength + Get_Float_Parameter("Repair_Health"));
            repair = true;
        }
        if (repair && Commands->Find_Object(number)->As_VehicleGameObj()) {
            Commands->Find_Object(
                    number)->As_VehicleGameObj()->Damage_Meshes_Update(); //repair was done, update damage meshes
        }
...
void JFW_Repair_Zone_Aircraft_Only::Entered(GameObject *obj, GameObject *enterer) {
    ...
    if ((!_stricmp(Commands->Get_Preset_Name(enterer), Get_Parameter("Preset1"))) ||
        (!_stricmp(Commands->Get_Preset_Name(enterer), Get_Parameter("Preset2"))) ||
        (!_stricmp(Commands->Get_Preset_Name(enterer), Get_Parameter("Preset3"))) ||
        (!_stricmp(Commands->Get_Preset_Name(enterer), Get_Parameter("Preset4"))) ||
        (!_stricmp(Commands->Get_Preset_Name(enterer), Get_Parameter("Preset5"))) ||
        (!_stricmp(Commands->Get_Preset_Name(enterer), Get_Parameter("Preset6"))) ||
        (!_stricmp(Commands->Get_Preset_Name(enterer), Get_Parameter("Preset7"))) ||
        (!_stricmp(Commands->Get_Preset_Name(enterer), Get_Parameter("Preset8")))) {
        if (enterer->As_VehicleGameObj() && enterer->As_VehicleGameObj()->Get_Definition().Can_Repair()) {
            Commands->Start_Timer(obj, this, 1.0, Commands->Get_ID(enterer));
        }
    }
...
void JFW_Heal_Zone::Timer_Expired(GameObject *obj, int number) {
    ...
    if (distance <= Get_Float_Parameter("Heal_Distance")) {
        ...
        if (Shield_Strength < Max_Shield_Strength) {
            Commands->Set_Shield_Strength(Commands->Find_Object(number),
                                          Shield_Strength + Get_Float_Parameter("Heal_Health"));
        }
        Commands->Start_Timer(obj, this, 1.0, number);
    }
}
...
void JFW_Zone_Animation::Entered(GameObject *obj, GameObject *enterer) {
    ...
    Commands->Set_Animation(Commands->Find_Object(ID), Get_Parameter("UpAnimation"), false, subobject, firstframe,
                            Get_Float_Parameter("UpLastFrame"), Get_Bool_Parameter("UpBlended"));
}
...
void JFW_Zone_Animation::Animation_Complete(GameObject *obj, const char *animation_name) {
    if (!_stricmp(animation_name, Get_Parameter("UpAnimation")) ||
        !_stricmp(animation_name, Get_Parameter("Animation"))) {
        ...
        Commands->Set_Animation(Commands->Find_Object(ID), Get_Parameter("Animation"), false, subobject, firstframe,
...
void JFW_Zone_Animation::Exited(GameObject *obj, GameObject *exiter) {
    ...
    Commands->Set_Animation(Commands->Find_Object(ID), Get_Parameter("DownAnimation"), false, subobject, firstframe,
                            Get_Float_Parameter("DownLastFrame"), Get_Bool_Parameter("DownBlended"));
    enabled = false;
}
...
void JFW_Group_Purchase_Zone::Entered(GameObject *obj, GameObject *enterer) {
    if (enabled) {
        ...
        if (cash >= Get_Float_Parameter("TotalCash")) {
            cash = 0;
            enabled = false;
            Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("ID")),
...
void JFW_Zone_Timer::Timer_Expired(GameObject *obj, int number) {
    if (count >= Get_Int_Parameter("Count")) {
        enabled = false;
        Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("ID")), Get_Int_Parameter("Message"),
                                    0, 0);
    }
}
...
void JFW_Gate_Zone_2::Timer_Expired(GameObject *obj, int number) {
    ...
    int TimerNum = Get_Int_Parameter("TimerNum");
    if (TimerNum == number) {
        Commands->Set_Animation(Commands->Find_Object(ID), Animation, false, 0,
                                Get_Animation_Frame(Commands->Find_Object(ID)), 0, false);
    }
}
...
void JFW_Fog_Zone::Entered(GameObject *obj, GameObject *enterer) {
    if (Get_Int_Parameter("OnEnter") > 0) {
        if (Get_Float_Parameter("Delay") > 0) {
            ...
            if (fogenable) {
                Commands->Set_Fog_Range(Get_Float_Parameter("Fog_Start_Distance"),
...
void JFW_Fog_Zone::Timer_Expired(GameObject *obj, int number) {
    if (number == 1) {
        ...
        Commands->Set_Fog_Enable(fogenable);
        if (fogenable) {
            Commands->Set_Fog_Range(Get_Float_Parameter("Fog_Start_Distance"), Get_Float_Parameter("Fog_End_Distance"),
                                    1.0f);
        }
    }
}
...
void JFW_War_Blitz_Zone::Entered(GameObject *obj, GameObject *enterer) {
    if (Get_Int_Parameter("OnEnter") > 0) {
        if (Get_Float_Parameter("Delay") > 0)
            ...
            Commands->Set_War_Blitz(Get_Float_Parameter("War_Blitz_Intensity"), Get_Float_Parameter("Start_Distance"),
                                    Get_Float_Parameter("End_Distance"), Get_Float_Parameter("War_Blitz_Heading"),
...
void JFW_Vehicle_Zone_Animation::Entered(GameObject *obj, GameObject *enterer) {
    if (enterer->As_VehicleGameObj()) {
        if (!refcount) {
            ...
            Commands->Set_Animation(Commands->Find_Object(ID), Get_Parameter("UpAnimation"), false, subobject,
                                    firstframe, Get_Float_Parameter("UpLastFrame"), Get_Bool_Parameter("UpBlended"));
        }
        refcount++;
    }
}
...
void JFW_Vehicle_Zone_Animation::Exited(GameObject *obj, GameObject *exiter) {
    if (exiter->As_VehicleGameObj()) {
        ...
        if (!refcount) {
            ...
            Commands->Set_Animation(Commands->Find_Object(ID), Get_Parameter("DownAnimation"), false, subobject,
                                    firstframe, Get_Float_Parameter("DownLastFrame"),
                                    Get_Bool_Parameter("DownBlended"));
        }
    }
}
...
void JFW_Radar_Spy_Zone::Entered(GameObject *obj, GameObject *enterer) {
    ...
    if (Find_Com_Center(Commands->Get_Player_Type(enterer)) &&
        (Is_Building_Dead(Find_Com_Center(Commands->Get_Player_Type(enterer))) &&
         (Is_Radar_Enabled(PTTEAM(Commands->Get_Player_Type(enterer)))))) {
...
void JFW_Repair_Zone_2::Timer_Expired(GameObject *obj, int number) {
    ...
    if (distance <= Get_Float_Parameter("Repair_Distance")) {
        if (Get_Vehicle_Driver(Commands->Find_Object(number))) {
            ...
            float cost = (costDivisor == 0) ? 0 :
                         (Get_Cost(Commands->Get_Preset_Name(Commands->Find_Object(number))) / 2) / costDivisor;
            if (Commands->Get_Money(Get_Vehicle_Driver(Commands->Find_Object(number))) >= cost) {
                ...
                if (Health < Max_Health) {
                    Commands->Set_Health(Commands->Find_Object(number),
                                         Health + Max_Health / Get_Float_Parameter("Repair_Health"));
                    cost_scale = 0.5f;
                }
                if (Shield_Strength < Max_Shield_Strength) {
                    Commands->Set_Shield_Strength(Commands->Find_Object(number), Shield_Strength + Max_Shield_Strength /
                                                                                                   Get_Float_Parameter(
                                                                                                           "Repair_Health"));
                    cost_scale += 0.5f;
                }
...
void JFW_Repair_Zone_No_Boats::Entered(GameObject *obj, GameObject *enterer) {
    ...
    if (enterer->As_VehicleGameObj() && enterer->As_VehicleGameObj()->Get_Definition().Can_Repair() &&
        Get_Vehicle_Driver(enterer) && Get_Vehicle_Mode(enterer) != VEHICLE_TYPE_BOAT &&
...
void JFW_Repair_Zone_No_Boats::Timer_Expired(GameObject *obj, int number) {
    ...
    if (distance <= Get_Float_Parameter("Repair_Distance")) {
        if (Get_Vehicle_Driver(Commands->Find_Object(number))) {
            float cost = (Get_Cost(Commands->Get_Preset_Name(Commands->Find_Object(number))) / 2) /
                         Get_Float_Parameter("Credits");
            if (Commands->Get_Money(Get_Vehicle_Driver(Commands->Find_Object(number))) >= cost) {
                float Max_Health = Commands->Get_Max_Health(Commands->Find_Object(number));
                float Health = Commands->Get_Health(Commands->Find_Object(number));
                float Max_Shield_Strength = Commands->Get_Max_Shield_Strength(Commands->Find_Object(number));
...
void JFW_Repair_Zone_Boats::Entered(GameObject *obj, GameObject *enterer) {
    ...
    if (enterer->As_VehicleGameObj() && enterer->As_VehicleGameObj()->Get_Definition().Can_Repair() &&
        Get_Vehicle_Driver(enterer) &&
        (Get_Vehicle_Mode(enterer) == VEHICLE_TYPE_BOAT || Get_Vehicle_Mode(enterer) == VEHICLE_TYPE_SUB)) {
        Create_2D_Sound_Player(Get_Vehicle_Driver(enterer), Get_Parameter("Sound"));
        InstallHook("Repair", Get_Vehicle_Driver(enterer));
        ID = Commands->Get_ID(enterer);
    }
...
void JFW_Sell_Zone::KeyHook() {
    ...
    if (Commands->Find_Object(ID)->As_VehicleGameObj()) {
        if (Get_Vehicle_Driver(Commands->Find_Object(ID))) {
            ...
            Commands->Send_Custom_Event(Owner(), Commands->Find_Object(ID), CUSTOM_TRANSITION_VTOL_LAND_ZONE, 1,
                                        0);    // Compatibility with dp88_Aircraft_LandingZone_Aircraft
...
void JFW_Infantry_Force_Composition_Zone::Created(GameObject *obj) {
    ...
    defs[0] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_CLASSES,
                                                        PurchaseSettingsDefClass::TEAM_GDI);
    defs[1] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_CLASSES,
                                                        PurchaseSettingsDefClass::TEAM_NOD);
    defs[2] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_SECRET_CLASSES,
                                                        PurchaseSettingsDefClass::TEAM_GDI);
    defs[3] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_SECRET_CLASSES,
                                                        PurchaseSettingsDefClass::TEAM_NOD);
...
void JFW_Vehicle_Force_Composition_Zone::Created(GameObject *obj) {
    ...
    defs[0] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_VEHICLES,
                                                        PurchaseSettingsDefClass::TEAM_GDI);
    defs[1] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_VEHICLES,
                                                        PurchaseSettingsDefClass::TEAM_NOD);
    defs[2] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_SECRET_VEHICLES,
                                                        PurchaseSettingsDefClass::TEAM_GDI);
    defs[3] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_SECRET_VEHICLES,
                                                        PurchaseSettingsDefClass::TEAM_NOD);
    defs[4] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_AIR,
                                                        PurchaseSettingsDefClass::TEAM_GDI);
    defs[5] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_NAVAL,
                                                        PurchaseSettingsDefClass::TEAM_NOD);
    defs[6] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_AIR,
                                                        PurchaseSettingsDefClass::TEAM_GDI);
    defs[7] = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_NAVAL,
                                                        PurchaseSettingsDefClass::TEAM_NOD);
...
void JFW_Power_Spy_Zone::Entered(GameObject *obj, GameObject *enterer) {
    ...
    if (Is_Base_Powered(PTTEAM(team)) && !reset &&
        Find_Building_By_Type(PTTEAM(team), BuildingConstants::TYPE_POWER_PLANT) &&
        (!Is_Building_Dead(Find_Building_By_Type(PTTEAM(team), BuildingConstants::TYPE_POWER_PLANT)))) {
        Create_2D_Sound_Team(Get_Parameter("Sound"), team);
...
void JFW_Power_Spy_Zone::Timer_Expired(GameObject *obj, int number) {
    if ((number == 1) && Find_Building_By_Type(PTTEAM(team), BuildingConstants::TYPE_POWER_PLANT) &&
        (!Is_Building_Dead(Find_Building_By_Type(PTTEAM(team), BuildingConstants::TYPE_POWER_PLANT)))) {
...
void JFW_Conyard_Spy_Zone::Entered(GameObject *obj, GameObject *enterer) {
    ...
    team = Commands->Get_Player_Type(enterer);
    if (!cydisabled && !reset && Find_Building_By_Type(PTTEAM(team), BuildingConstants::TYPE_CONYARD) &&
        (!Is_Building_Dead(Find_Building_By_Type(PTTEAM(team), BuildingConstants::TYPE_CONYARD)))) {
        Create_2D_Sound_Team(Get_Parameter("Sound"), team);
        cydisabled = true;
        Commands->Send_Custom_Event(obj, Find_Building_By_Type(PTTEAM(team), BuildingConstants::TYPE_CONYARD), 3000,
                                    320023, 0);
        Commands->Start_Timer(obj, this, Get_Float_Parameter("Power_Time"), 1);
    }
}
...
void JFW_Conyard_Spy_Zone::Timer_Expired(GameObject *obj, int number) {
    if ((number == 1) && Find_Building_By_Type(PTTEAM(team), BuildingConstants::TYPE_CONYARD) &&
        (!Is_Building_Dead(Find_Building_By_Type(PTTEAM(team), BuildingConstants::TYPE_CONYARD)))) {
        Create_2D_Sound_Team(Get_Parameter("Sound2"), PTTEAM(team));
        cydisabled = false;
        Commands->Send_Custom_Event(obj, Find_Building_By_Type(PTTEAM(team), BuildingConstants::TYPE_CONYARD), 3000,
...
void JFW_Conyard_Spy_Zone_2::Entered(GameObject *obj, GameObject *enterer) {
    ...
    team = Commands->Get_Player_Type(enterer);
    if (!cydisabled && !reset && Find_Construction_Yard(PTTEAM(team)) &&
        (!Is_Building_Dead(Find_Construction_Yard(PTTEAM(team))))) {
        Create_2D_Sound_Team(Get_Parameter("Sound"), team);
        cydisabled = true;
...
void JFW_Conyard_Spy_Zone_2::Timer_Expired(GameObject *obj, int number) {
    if ((number == 1) && Find_Construction_Yard(PTTEAM(team)) &&
        (!Is_Building_Dead(Find_Construction_Yard(PTTEAM(team))))) {
        Create_2D_Sound_Team(Get_Parameter("Sound2"), PTTEAM(team));
        cydisabled = false;
        ConstructionYardGameObj *obj2 = (ConstructionYardGameObj *) Find_Construction_Yard(PTTEAM(team));
...
ScriptRegistrant<JFW_Look_At_Location_Entry> JFW_Look_At_Location_Entry_Registrant("JFW_Look_At_Location_Entry",
                                                                                   "Player_Type:int,Location:vector3");
ScriptRegistrant<JFW_Look_At_Object_Entry> JFW_Look_At_Object_Entry_Registrant("JFW_Look_At_Object_Entry",
                                                                               "Player_Type:int,ObjectID:int");
ScriptRegistrant<JFW_Zone_PCT> JFW_Zone_PCT_Registrant("JFW_Zone_PCT", "Player_Type:int");
ScriptRegistrant<JFW_3D_Sound_Team_Zone> JFW_3D_Sound_Team_Zone_Registrant("JFW_3D_Sound_Team_Zone",
                                                                           "Player_Type:int,Sound:string");
ScriptRegistrant<JFW_3D_Sound_Player_Zone> JFW_3D_Sound_Player_Zone_Registrant("JFW_3D_Sound_Player_Zone",
                                                                               "Player_Type:int,Sound:string");
ScriptRegistrant<JFW_2D_Sound_Team_Zone> JFW_2D_Sound_Team_Zone_Registrant("JFW_2D_Sound_Team_Zone",
                                                                           "Player_Type:int,Sound:string");
ScriptRegistrant<JFW_2D_Sound_Player_Zone> JFW_2D_Sound_Player_Zone_Registrant("JFW_2D_Sound_Player_Zone",
                                                                               "Player_Type:int,Sound:string");
ScriptRegistrant<JFW_Disable_Radar_Zone> JFW_Disable_Radar_Zone_Registrant("JFW_Disable_Radar_Zone", "Player_Type:int");
ScriptRegistrant<JFW_Zone_Send_Custom_Multiple> JFW_Zone_Send_Custom_Multiple_Registrant(
        "JFW_Zone_Send_Custom_Multiple", "Player_Type:int,ID:int,Message:int,Count:int");
ScriptRegistrant<JFW_Zone_Send_Custom_Enter> JFW_Zone_Send_Custom_Enter_Registrant("JFW_Zone_Send_Custom_Enter",
                                                                                   "Player_Type:int,Message:int");
ScriptRegistrant<JFW_Zone_Send_Custom_Exit> JFW_Zone_Send_Custom_Exit_Registrant("JFW_Zone_Send_Custom_Exit",
                                                                                 "Player_Type:int,Message:int");
ScriptRegistrant<JFW_Vehicle_Zone_Animation> JFW_Vehicle_Zone_Animation_Registrant("JFW_Vehicle_Zone_Animation",
                                                                                   "Animation:string,Subobject:string,FirstFrame:float,LastFrame:float,Blended:int,UpAnimation:string,UpSubobject:string,UpFirstFrame:float,UpLastFrame:float,UpBlended:int,DownAnimation:string,DownSubobject:string,DownFirstFrame:float,DownLastFrame:float,DownBlended:int,Model:string");
ScriptRegistrant<JFW_Zone_Money_Preset> JFW_Zone_Money_Preset_Registrant("JFW_Zone_Money_Preset",
                                                                         "Preset:string,Money:float,Player_Type:int,ID:int");
ScriptRegistrant<JFW_Fog_Zone> JFW_Fog_Zone_Registrant("JFW_Fog_Zone",
                                                       "Fog_Enable:int,Fog_Start_Distance=0.000:float,Fog_End_Distance=0.000:float,Delay=0.000:float,OnEnter=0:int,OnExit=0:int");
ScriptRegistrant<JFW_War_Blitz_Zone> JFW_War_Blitz_Zone_Registrant("JFW_War_Blitz_Zone",
                                                                   "War_Blitz_Intensity=0.000:float,Start_Distance=0.000:float,End_Distance=1.000:float,War_Blitz_Heading=0.000:float,War_Blitz_Distribution=1.000:float,Delay=0.000:float,OnEnter=0:int,OnExit=0:int");
ScriptRegistrant<JFW_Create_Destroy_Object_On_Enter> JFW_Create_Destroy_Object_On_Enter_Registrant(
        "JFW_Create_Destroy_Object_On_Enter", "Location:vector3,Facing:float,Preset:string,Player_Type:int");
ScriptRegistrant<JFW_Apply_Damage_On_Enter_Timer> JFW_Apply_Damage_On_Enter_Timer_Registrant(
        "JFW_Apply_Damage_On_Enter_Timer", "Warhead:string,Damage:float,Time:float,Distance:float,Player_Type:int");
ScriptRegistrant<JFW_Blow_Up_On_Enter_Random> JFW_Blow_Up_On_Enter_Random_Registrant("JFW_Blow_Up_On_Enter_Random",
                                                                                     "Explosion:string,Probobility:int");
ScriptRegistrant<JFW_Blow_Up_On_Enter> JFW_Blow_Up_On_Enter_Registrant("JFW_Blow_Up_On_Enter", "Explosion:string");
ScriptRegistrant<JFW_Apply_Damage_On_Enter> JFW_Apply_Damage_On_Enter_Registrant("JFW_Apply_Damage_On_Enter",
                                                                                 "Warhead:string,Damage:float");
ScriptRegistrant<JFW_Zone_Character> JFW_Zone_Character_Registrant("JFW_Zone_Character",
                                                                   "Preset_Name:string,Player_Type:int");
ScriptRegistrant<JFW_Zone_Character_Swap> JFW_Zone_Character_Swap_Registrant("JFW_Zone_Character_Swap",
                                                                             "Preset_Name:string,Old_Preset_Name:string,Player_Type:int");
ScriptRegistrant<JFW_Heavy_Vehicle_Damage_Zone> JFW_Heavy_Vehicle_Damage_Zone_Registrant(
        "JFW_Heavy_Vehicle_Damage_Zone",
        "Warhead:string,Damage:float,Time:float,Distance:float,Player_Type:int,Mass:float");
ScriptRegistrant<JFW_Light_Vehicle_Damage_Zone> JFW_Light_Vehicle_Damage_Zone_Registrant(
...
ScriptRegistrant<JFW_Stealth_Zone> JFW_Stealth_Zone_Registrant("JFW_Stealth_Zone",
                                                               "Player_Type:int,Stealth1:string,Stealth2:string,Stealth3:string,Stealth4:string");
ScriptRegistrant<JFW_Teleport_Zone_Team> JFW_Teleport_Zone_Team_Registrant("JFW_Teleport_Zone_Team",
                                                                           "Location:vector3,Object_ID=0:int,Player_Type:int");
ScriptRegistrant<JFW_Character_Buy> JFW_Character_Buy_Registrant("JFW_Character_Buy",
                                                                 "Preset_Name:string,Cost:int,Player_Type:int");
ScriptRegistrant<JFW_Preset_Buy> JFW_Preset_Buy_Registrant("JFW_Preset_Buy",
                                                           "Preset_Name:string,Cost:int,location:vector3,Player_Type:int");
ScriptRegistrant<JFW_Weapon_Buy> JFW_Weapon_Buy_Registrant("JFW_Weapon_Buy",
                                                           "Weapon_Name:string,Cost:int,Player_Type:int");
ScriptRegistrant<JFW_3D_Sound_Zone> JFW_3D_Sound_Zone_Registrant("JFW_3D_Sound_Zone", "Player_Type:int,Sound:string");
ScriptRegistrant<JFW_Bounce_Zone_Entry> JFW_Bounce_Zone_Entry_Registrant("JFW_Bounce_Zone_Entry",
                                                                         "Player_Type:int,Amount:float");
ScriptRegistrant<JFW_Grant_Key_Zone_Entry> JFW_Grant_Key_Zone_Entry_Registrant("JFW_Grant_Key_Zone_Entry",
                                                                               "Player_Type:int,Key:int");
ScriptRegistrant<JFW_Zone_Send_Custom_Enable> JFW_Zone_Send_Custom_Enable_Registrant("JFW_Zone_Send_Custom_Enable",
                                                                                     "ID:int,EnterMessage:int,EnterParam:int,ExitMessage:int,ExitParam:int,Player_Type:int,Enable_Message:int,Disable_Message:int");
ScriptRegistrant<JFW_Zone_Send_Custom_Preset> JFW_Zone_Send_Custom_Preset_Registrant("JFW_Zone_Send_Custom_Preset",
                                                                                     "ID:int,EnterMessage:int,EnterParam:int,ExitMessage:int,ExitParam:int,Player_Type:int,Preset_Name:string");
ScriptRegistrant<JFW_Zone_Send_Custom_Enable_Enter> JFW_Zone_Send_Custom_Enable_Enter_Registrant(
        "JFW_Zone_Send_Custom_Enable_Enter",
        "EnterMessage:int,EnterParam:int,ExitMessage:int,ExitParam:int,Player_Type:int,Enable_Message:int,Disable_Message:int");
ScriptRegistrant<JFW_Zone_Send_Custom_Preset_Enter> JFW_Zone_Send_Custom_Preset_Enter_Registrant(
        "JFW_Zone_Send_Custom_Preset_Enter",
        "EnterMessage:int,EnterParam:int,ExitMessage:int,ExitParam:int,Player_Type:int,Preset_Name:string");
ScriptRegistrant<JFW_Send_Custom_Zone_Enter> JFW_Send_Custom_Zone_Enter_Registrant("JFW_Send_Custom_Zone_Enter",
                                                                                   "EnterMessage:int,EnterParam:int,ExitMessage:int,ExitParam:int,Team_ID:int");
ScriptRegistrant<JFW_Gate_Zone> JFW_Gate_Zone_Registrant("JFW_Gate_Zone",
                                                         "Player_Type:int,Time:float,TimerNum:int,ID:int,Animation:string");
ScriptRegistrant<JFW_Repair_Zone> JFW_Repair_Zone_Registrant("JFW_Repair_Zone",
                                                             "Player_Type:int,Repair_Distance:float,Repair_Health:float");
ScriptRegistrant<JFW_Repair_Zone_Aircraft_Only> JFW_Repair_Zone_Aircraft_Only_Registrant(
        "JFW_Repair_Zone_Aircraft_Only",
        "Player_Type:int,Repair_Distance:float,Repair_Health:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string");
ScriptRegistrant<JFW_Repair_Zone_No_Aircraft> JFW_Repair_Zone_No_Aircraft_Registrant("JFW_Repair_Zone_No_Aircraft",
                                                                                     "Player_Type:int,Repair_Distance:float,Repair_Health:float,Preset1:string,Preset2:string,Preset3:string,Preset4:string,Preset5:string,Preset6:string,Preset7:string,Preset8:string ");
ScriptRegistrant<JFW_Zone_Send_Custom_Not_Preset> JFW_Zone_Send_Custom_Not_Preset_Registrant(
        "JFW_Zone_Send_Custom_Not_Preset",
        "ID:int,EnterMessage:int,EnterParam:int,ExitMessage:int,ExitParam:int,Player_Type:int,Preset_Name:string");
ScriptRegistrant<JFW_Zone_Send_Custom_Keycard> JFW_Zone_Send_Custom_Keycard_Registrant("JFW_Zone_Send_Custom_Keycard",
                                                                                       "ID:int,EnterMessage:int,EnterParam:int,ExitMessage:int,ExitParam:int,Player_Type:int,Keycard_Number:int");
ScriptRegistrant<JFW_Heal_Zone> JFW_Heal_Zone_Registrant("JFW_Heal_Zone",
                                                         "Player_Type:int,Heal_Distance:float,Heal_Health:float");
ScriptRegistrant<JFW_Heal_Zone_2> JFW_Heal_Zone_2_Registrant("JFW_Heal_Zone_2",
                                                             "Player_Type:int,Heal_Distance:float,Heal_Health:float,Preset_Name:string");
ScriptRegistrant<JFW_Disable_Transitions_Zone> JFW_Disable_Transitions_Zone_Registrant("JFW_Disable_Transitions_Zone",
                                                                                       "");
ScriptRegistrant<JFW_Bounce_Zone_Entry_All_Directions> JFW_Bounce_Zone_Entry_All_Directions_Registrant(
        "JFW_Bounce_Zone_Entry_All_Directions", "Player_Type:int,ZAmount:float,Amount:float");
ScriptRegistrant<JFW_Zone_Send_Custom_Cost> JFW_Zone_Send_Custom_Cost_Registrant("JFW_Zone_Send_Custom_Cost",
                                                                                 "ID:int,Message:int,Param:int,Player_Type:int,Cost:int");
ScriptRegistrant<JFW_Vechicle_Damage_Zone> JFW_Vechicle_Damage_Zone_Registrant("JFW_Vechicle_Damage_Zone",
                                                                               "Warhead:string,Damage:float,Time:float,Distance:float,Player_Type:int");
ScriptRegistrant<JFW_Refill_Buy> JFW_Refill_Buy_Registrant("JFW_Refill_Buy", "Cost:int,Player_Type:int");
ScriptRegistrant<JFW_Vehicle_Buy> JFW_Vehicle_Buy_Registrant("JFW_Vehicle_Buy",
                                                             "Preset_Name:string,Cost:int,Player_Type:int");
ScriptRegistrant<JFW_Powerup_Buy> JFW_Powerup_Buy_Registrant("JFW_Powerup_Buy",
                                                             "Preset_Name:string,Cost:int,Player_Type:int");
ScriptRegistrant<JFW_Repair_Zone_No_VTOL> JFW_Repair_Zone_No_VTOL_Registrant("JFW_Repair_Zone_No_VTOL",
                                                                             "Player_Type:int,Repair_Distance:float,Repair_Health:float");
ScriptRegistrant<JFW_Repair_Zone_VTOL_Only> JFW_Repair_Zone_VTOL_Only_Registrant("JFW_Repair_Zone_VTOL_Only",
                                                                                 "Player_Type:int,Repair_Distance:float,Repair_Health:float");
ScriptRegistrant<JFW_Zone_Vehicle> JFW_Zone_Vehicle_Registrant("JFW_Zone_Vehicle",
                                                               "Preset_Name:string,Player_Type:int");
ScriptRegistrant<JFW_Zone_Powerup> JFW_Zone_Powerup_Registrant("JFW_Zone_Powerup",
                                                               "Preset_Name:string,Player_Type:int");
ScriptRegistrant<JFW_Zone_Refill> JFW_Zone_Refill_Registrant("JFW_Zone_Refill", "Player_Type:int");
ScriptRegistrant<JFW_Zone_Send_Custom_Team> JFW_Zone_Send_Custom_Team_Registrant("JFW_Zone_Send_Custom_Team",
                                                                                 "GDIID:int,GDIEnterMessage:int,GDIEnterParam:int,GDIExitMessage:int,GDIExitParam:int,NodID:int,NodEnterMessage:int,NodEnterParam:int,NodExitMessage:int,NodExitParam:int");
ScriptRegistrant<JFW_Zone_Animation> JFW_Zone_Animation_Registrant("JFW_Zone_Animation",
                                                                   "Animation:string,Subobject:string,FirstFrame:float,LastFrame:float,Blended:int,UpAnimation:string,UpSubobject:string,UpFirstFrame:float,UpLastFrame:float,UpBlended:int,DownAnimation:string,DownSubobject:string,DownFirstFrame:float,DownLastFrame:float,DownBlended:int,Model:string");
ScriptRegistrant<JFW_Group_Purchase_Zone> JFW_Group_Purchase_Zone_Registrant("JFW_Group_Purchase_Zone",
                                                                             "CashPerPerson:float,TotalCash:float,ID:int,Custom:int");
ScriptRegistrant<JFW_Zone_Timer> JFW_Zone_Timer_Registrant("JFW_Zone_Timer",
                                                           "Count:int,ID:int,Custom:int,Time:float,TimerNum:int");
ScriptRegistrant<JFW_Gate_Zone_2> JFW_Gate_Zone_2_Registrant("JFW_Gate_Zone_2",
                                                             "Player_Type:int,Time:float,TimerNum:int,ID:int,Animation:string");
...
ScriptRegistrant<JFW_Send_Driver_Custom_On_Enter> JFW_Send_Driver_Custom_On_Enter_Registrant(
        "JFW_Send_Driver_Custom_On_Enter", "EnterMessage:int,ExitMessage:int");
ScriptRegistrant<JFW_Teleport_Zone_Enable> JFW_Teleport_Zone_Enable_Registrant("JFW_Teleport_Zone_Enable",
                                                                               "Location:vector3,Object_ID=0:int,Player_Type:int,EnableCustom:int,DisableCustom:int");
ScriptRegistrant<JFW_Radar_Spy_Zone> JFW_Radar_Spy_Zone_Registrant("JFW_Radar_Spy_Zone",
                                                                   "Spy_Script:string,Sound:string");
ScriptRegistrant<JFW_Radar_Spy_Zone_New> JFW_Radar_Spy_Zone_New_Registrant("JFW_Radar_Spy_Zone_New", "Sound:string");
ScriptRegistrant<JFW_2D_Sound_Zone_Team> JFW_2D_Sound_Zone_Team_Registrant("JFW_2D_Sound_Zone_Team",
                                                                           "Player_Type:int,Sound:string,Time:float,TimerNum:int,Sound_Player_Type:int");
ScriptRegistrant<JFW_Repair_Zone_2> JFW_Repair_Zone_2_Registrant("JFW_Repair_Zone_2",
                                                                 "Player_Type:int,Repair_Distance:float,Repair_Health:float,Sound:string,Credits:float,RepairSound:string,RepairStopSound:string,WaitForKeyhook=1:int");
ScriptRegistrant<JFW_Repair_Zone_No_Boats> JFW_Repair_Zone_No_Boats_Registrant("JFW_Repair_Zone_No_Boats",
                                                                               "Player_Type:int,Repair_Distance:float,Repair_Health:float,Sound:string,Credits:float,RepairSound:string,RepairStopSound:string");
ScriptRegistrant<JFW_Repair_Zone_Boats> JFW_Repair_Zone_Boats_Registrant("JFW_Repair_Zone_Boats",
                                                                         "Player_Type:int,Repair_Distance:float,Repair_Health:float,Sound:string,Credits:float,RepairSound:string,RepairStopSound:string");
ScriptRegistrant<JFW_Infantry_Force_Composition_Zone> JFW_Infantry_Force_Composition_Zone_Registrant(
        "JFW_Infantry_Force_Composition_Zone", "Spy_Script:string,Sound:string,ResetTime:float");
ScriptRegistrant<JFW_Vehicle_Force_Composition_Zone> JFW_Vehicle_Force_Composition_Zone_Registrant(
        "JFW_Vehicle_Force_Composition_Zone", "Spy_Script:string,Sound:string,ResetTime:float");
ScriptRegistrant<JFW_Infantry_Force_Composition_Zone_New> JFW_Infantry_Force_Composition_Zone_New_Registrant(
        "JFW_Infantry_Force_Composition_Zone_New", "Sound:string,ResetTime:float");
ScriptRegistrant<JFW_Vehicle_Force_Composition_Zone_New> JFW_Vehicle_Force_Composition_Zone_New_Registrant(
        "JFW_Vehicle_Force_Composition_Zone_New", "Sound:string,ResetTime:float");
ScriptRegistrant<JFW_Sell_Zone> JFW_Sell_Zone_Registrant("JFW_Sell_Zone",
                                                         "Player_Type:int,Sound:string,SellSound:string");
ScriptRegistrant<JFW_Resize_Zone> JFW_Resize_Zone_Registrant("JFW_Resize_Zone", "Size:vector3,ZRotate:float");
ScriptRegistrant<JFW_Cash_Spy_Zone> JFW_Cash_Spy_Zone_Registrant("JFW_Cash_Spy_Zone",
                                                                 "Spy_Script:string,Sound:string,ResetTime:float");
ScriptRegistrant<JFW_Power_Spy_Zone> JFW_Power_Spy_Zone_Registrant("JFW_Power_Spy_Zone",
                                                                   "Spy_Script:string,Power_Time:float,Reset_Time:float,Sound:string,Sound2:string");
ScriptRegistrant<JFW_Cash_Spy_Zone_New> JFW_Cash_Spy_Zone_New_Registrant("JFW_Cash_Spy_Zone_New",
                                                                         "Sound:string,ResetTime:float");
ScriptRegistrant<JFW_Power_Spy_Zone_New> JFW_Power_Spy_Zone_New_Registrant("JFW_Power_Spy_Zone_New",
                                                                           "Power_Time:float,Reset_Time:float,Sound:string,Sound2:string");
ScriptRegistrant<JFW_Blow_Up_On_Enter_Delay> JFW_Blow_Up_On_Enter_Delay_Registrant("JFW_Blow_Up_On_Enter_Delay",
                                                                                   "Player_Type:int,Time:float,TimerNum:int,Explosion:string");
ScriptRegistrant<JFW_Conyard_Spy_Zone> JFW_Conyard_Spy_Zone_Registrant("JFW_Conyard_Spy_Zone",
                                                                       "Spy_Script:string,Power_Time:float,Reset_Time:float,Sound:string,Sound2:string");
ScriptRegistrant<JFW_Conyard_Spy_Zone_2> JFW_Conyard_Spy_Zone_2_Registrant("JFW_Conyard_Spy_Zone_2",
                                                                           "Spy_Script:string,Power_Time:float,Reset_Time:float,Sound:string,Sound2:string");
ScriptRegistrant<JFW_Conyard_Spy_Zone_2_New> JFW_Conyard_Spy_Zone_2_New_Registrant("JFW_Conyard_Spy_Zone_2_New",
                                                                                   "Power_Time:float,Reset_Time:float,Sound:string,Sound2:string");
ScriptRegistrant<JFW_Custom_Spawn_Zone> JFW_Custom_Spawn_Zone_Registrant("JFW_Custom_Spawn_Zone",
                                                                         "Position:vector3,Size:vector3,ZRotate:float,SpawnCustom:int,DestroyCustom:int,Preset:string");
ScriptRegistrant<JFW_Spawn_Zone_Created> JFW_Spawn_Zone_Created_Registrant("JFW_Spawn_Zone_Created",
                                                                           "Offset:vector3,Size:vector3,ZRotate:float,Preset:string");
...
ScriptRegistrant<JFW_Destroy_Vehicle_Zone_Marker> JFW_Destroy_Vehicle_Zone_Marker_Registrant(
