...
public:
    ...
    void DestroyLeastSignificant() {
        ...
        while (Current) {
            ...
            if (Object) {
                ...
                if (Star) {
                    float Temp = JmgUtility::SimpleDistance(Commands->Get_Position(Object),
    ...
    int countInRangeTargetingId(Vector3 pos, float range, int targetId) {
        ...
        SimpleObjectNode *current = SimpleObjectNodeList;
        while (current) {
            if (current->ID && current->enemyId == targetId && current->obj &&
                JmgUtility::SimpleFlatDistance(Commands->Get_Position(current->obj), pos) <= range &&
                Commands->Get_Health(current->obj))
                inRangeCount++;
            current = current->next;
        }
        return inRangeCount;
...
struct PlayerEmulatedSound {
    enum logicalSoundType {
        stNone = 0, stOldWeapon = 1, stFootsteps = 2, stVehicle = 3, stGunshot = 4, stBulletHit = 5, stNoticeMe = 6
    };
    float Range;
...
private:
    ...
    void AddNodesToLookupTable(BearHunterBearPositionNode *currentNode) {
        ...
        while (Current) {
            if (currentNode->pos.X < Current->pos.X &&
                (!Prev || currentNode->pos.X > Prev->pos.X)) {// Add an xNode before
                if (!Prev)
                    BearHunterBearPositionNodeGrouping = currentNode;
                else
            ...
            if (currentNode->pos.X > Current->pos.X &&
                (!Current->xNodes || currentNode->pos.X < Current->xNodes->pos.X)) {// Add an xNode after
                currentNode->xNodes = Current->xNodes;
                Current->xNodes = currentNode;
                return;
            }
            if (Current->pos.X == currentNode->pos.X) {//add a Y node
                ...
                while (YNode) {
                    if (currentNode->pos.Y < YNode->pos.Y &&
                        (!yPrev || currentNode->pos.Y > yPrev->pos.Y)) {// add node before
                        if (!yPrev)
                            Current->yNodes = currentNode;
                        else
                    ...
                    if (currentNode->pos.Y > YNode->pos.Y &&
                        (!YNode->yNodes || currentNode->pos.Y < YNode->yNodes->pos.Y)) {// add node after
...
public:
    ...
    Vector3 LookupNearestPosition(Vector2 pos) {
        if (BearHunterBearPositionNodeGrouping) {
            BearHunterBearPositionNode *table = BearHunterBearPositionNodeGrouping;
            while (table) {
                if (pos.X < table->pos.X ||
                    (pos.X >= table->pos.X && (!table->xNodes || pos.X < table->xNodes->pos.X))) {
                    BearHunterBearPositionNode *y = table->yNodes;
                    while (y) {
            ...
            char debugmsg[220];
            sprintf(debugmsg,
                    "msg BearHunterGameControl::LookupNearestPosition ERROR: Could not find pos for location <%.2f,%.2f>",
                    pos.X, pos.Y);
...
public:
    ...
    static enum GameState {
        LeaveBase = 0, HuntBears = 1, MutationStart = 2, FallBack = 3, DefendPrez = 4, SecureBase = 5, BossArrive = 6
    } gameState;
...
Vector3 JMG_Bear_Hunter_Game_Control::engineerSpawnLocations[5] = {Vector3(-263.045f, 150.827f, 0.839f),
                                                                   Vector3(-266.169f, 150.874f, 0.839f),
                                                                   Vector3(-198.472f, -262.617f, 0.251f),
                                                                   Vector3(-198.408f, -259.017f, 0.259f),
                                                                   Vector3(-203.714f, -260.658f, 0.216f)};
...
#define BHHighScoreListCount 124
    ...
    struct BHScoreNode {
        ...
        BHScoreNode(const char *PlayerName) {
            ...
            startedRound =
                    JMG_Bear_Hunter_Game_Control::gameState >= JMG_Bear_Hunter_Game_Control::HuntBears ? true : false;
...
public:
    ...
    void SaveData() {
        ...
        while (Current) {
            ...
            sprintf(EncryptString,
                    ...
                    Current->PlayTime, Current->PreGameTime, Current->IdleTime, Current->RoundsPlayed,
                    Current->RoundsCompleted, Current->RoundsQuit, Current->RoundsWon, Current->RoundsLost,
                    Current->MostKillsInARound, Current->MostDeathsInARound,
                    Current->MostBonusObjectivesCompletedInARound, Current->Deaths, Current->Kills,
                    Current->VehicleKills, Current->KilledSelf, Current->KilledPlayers, Current->KilledPresident,
                    Current->KilledTurrets, Current->KilledBears, Current->KilledBlackBears, Current->KilledMutantBears,
                    Current->KilledMutantDeer, Current->KilledMutantCats, Current->KilledMutantCatsB,
                    Current->KilledMutantCatsR, Current->KilledMutantRabbits, Current->ObjectiveActivatedAlarm,
                    Current->ObjectiveTurretTruck, Current->ObjectiveTurretTruckAlarm,
                    Current->ObjectiveOilRigsActivated, Current->ObjectiveOilRigsRepaired,
                    Current->ObjectiveEngineersSaved, Current->ObjectiveWeaponsFound, Current->ObjectiveWeaponsReturned,
                    Current->ObjectivePlasmaRifleReturned, Current->BonusObjectivesCompleted,
                    Current->PickedupHealthPowerups, Current->PickedupArmorPowerups, Current->PickedupCashPowerups,
                    Current->PickedupAmmoPowerups, Current->PickedupHealthTotal, Current->PickedupArmorTotal,
                    Current->PickedupCashTotal, Current->PickedupAmmoTotal, Current->PickedupTotalPowerups,
                    Current->PickedupTotalPowerupsInARound, Current->KilledHumanAi, Current->VehiclesDestroyed,
                    Current->VehiclesLost, Current->JazzsLost, Current->CleasansLost, Current->TrucksLost,
                    Current->TanksLost, Current->TurretTruckLost, Current->C4VestPowerups, Current->ActivatedCommTower,
                    Current->PlayedGamesWithDefenseTurrets, Current->PlayedGamesWithGuardianHelicopter,
                    Current->TimesDrown, Current->TimesFallen, Current->KillsWithSentryTurret,
                    Current->KilledSentryTurrets, Current->SentryTurretsPlaced,
                    Current->SentryTurretsLost, Current->PickedUpMedicalNeedle, Current->ReturnedMedicalNeedle,
                    Current->RepairedSubstation, Current->SubstationOnLineAtEnd, Current->SubstationNotDamaged,
                    Current->GiantDeerKilled, Current->SurvivedAlarm, Current->WolfKilled, Current->MutantDogKilled,
                    Current->BlueDeerKilled, Current->CheatedRounds, Current->NeverInjured, Current->MooseKilled,
                    Current->MooseKilled, Current->EatenByRabbit, Current->EatenByRabbit, Current->PickedUpDeerStatue,
                    Current->DroppedDeerStatue, Current->ReturnedDeerStatue, Current->TinyDeerKilled,
                    Current->MutantSquirrelsKilled,
                    Current->WildDeerKilled, Current->WildSquirrelsKilled, Current->ArmoredCarsLost,
                    Current->WarriorsLost, Current->TimeOnFoot, Current->TimeInAJazzs, Current->TimeInACleasans,
                    Current->TimeInASecurityTruck, Current->TimeInArmoredCars, Current->TimeInAUDVs,
                    Current->TimeInGatlingTanks, Current->TimeInIFVs, Current->FriendlyTinyDeerKilled,
                    ...
                    Current->LastPlayTime.lTime, Current->SupportReceivedInfantryHp,
                    Current->SupportReceivedVehicleHp, Current->SupportReceivedAmmo, Current->SupportGrantedHpInfantry,
                    Current->SupportGrantedHpVehicle, Current->SupportGrantedAmmo,
                    Current->SupportReceivedHpInfantrySelf, Current->SupportReceivedHpVehicleSelf,
                    Current->SupportReceivedAmmoSelf, Current->PettedCougars, Current->KilledCougars,
                    Current->KilledMutantCougars, Current->FailedPettingCougars, Current->KilledFriendlyCougars,
                    Current->DiedTryingToEscapeBase, Current->RescuedCows, Current->KilledCows, Current->LostCows,
                    Current->CompletedCowObjective, Current->KilledMice,
                    Current->CompledMiceObjective, Current->KilledPortablePumpJacks, Current->PortablePumpJacksPlaced,
                    Current->PortablePumpJacksLost, Current->PumpJackMoney, Current->MobilePumpJackMoney, Current->Rank,
                    Current->KilledTurkey, Current->ReturnedTurkey);
            ...
            fprintf(SaveScores, "\n%s",
                    JmgUtility::Rp2Encrypt(EncryptString, Current->PlayerName[1], Current->PlayerName[0]));
            if (Current->next)
                fprintf(SaveScores, "\n");
            Current = Current->next;
        }
    ...
    void LoadData() {
        ...
        if (LoadScores) {
            while (!feof(LoadScores)) {
                ...
                sprintf(decryptedString, "%s",
                        JmgUtility::Rp2Decrypt(DecryptString, Current->PlayerName[0], Current->PlayerName[1]));
                sprintf(decryptedString2, "%s",
                        JmgUtility::Rp2Decrypt(DecryptString2, Current->PlayerName[1], Current->PlayerName[0]));
                bool match = true;
                ...
                if (stringLength == strlen(decryptedString2))
                    for (unsigned int x = 0; x < stringLength; x++)
                        if (decryptedString[x] != decryptedString2[x]) {
                            match = false;
                            char noMatchMsg[220];
                            sprintf(noMatchMsg, "msg Error: Cou%s%syp%say%st%sr %s", "ld ", "Not Decr", "t Pl", "er Da",
                ...
                if (match)
                    sscanf(decryptedString,
                           ...
                           &Current->MostKillsInARound, &Current->MostDeathsInARound,
                           &Current->MostBonusObjectivesCompletedInARound, &Current->Deaths, &Current->Kills,
                           &Current->VehicleKills, &Current->KilledSelf, &Current->KilledPlayers,
                           &Current->KilledPresident, &Current->KilledTurrets, &Current->KilledBears,
                           &Current->KilledBlackBears, &Current->KilledMutantBears, &Current->KilledMutantDeer,
                           &Current->KilledMutantCats, &Current->KilledMutantCatsB,
                           &Current->KilledMutantCatsR, &Current->KilledMutantRabbits,
                           &Current->ObjectiveActivatedAlarm, &Current->ObjectiveTurretTruck,
                           &Current->ObjectiveTurretTruckAlarm, &Current->ObjectiveOilRigsActivated,
                           &Current->ObjectiveOilRigsRepaired, &Current->ObjectiveEngineersSaved,
                           &Current->ObjectiveWeaponsFound, &Current->ObjectiveWeaponsReturned,
                           &Current->ObjectivePlasmaRifleReturned, &Current->BonusObjectivesCompleted,
                           &Current->PickedupHealthPowerups, &Current->PickedupArmorPowerups,
                           &Current->PickedupCashPowerups, &Current->PickedupAmmoPowerups,
                           &Current->PickedupHealthTotal, &Current->PickedupArmorTotal,
                           &Current->PickedupCashTotal, &Current->PickedupAmmoTotal, &Current->PickedupTotalPowerups,
                           &Current->PickedupTotalPowerupsInARound, &Current->KilledHumanAi,
                           &Current->VehiclesDestroyed, &Current->VehiclesLost, &Current->JazzsLost,
                           &Current->CleasansLost, &Current->TrucksLost, &Current->TanksLost, &Current->TurretTruckLost,
                           &Current->C4VestPowerups, &Current->ActivatedCommTower,
                           &Current->PlayedGamesWithDefenseTurrets, &Current->PlayedGamesWithGuardianHelicopter,
                           &Current->TimesDrown, &Current->TimesFallen, &Current->KillsWithSentryTurret,
                           &Current->KilledSentryTurrets, &Current->SentryTurretsPlaced,
                           &Current->SentryTurretsLost, &Current->PickedUpMedicalNeedle,
                           &Current->ReturnedMedicalNeedle, &Current->RepairedSubstation,
                           &Current->SubstationOnLineAtEnd, &Current->SubstationNotDamaged, &Current->GiantDeerKilled,
                           &Current->SurvivedAlarm, &Current->WolfKilled, &Current->MutantDogKilled,
                           &Current->BlueDeerKilled, &Current->CheatedRounds, &Current->NeverInjured,
                           &Current->MooseKilled, &Current->MooseKilled, &Current->EatenByRabbit,
                           &Current->EatenByRabbit, &Current->PickedUpDeerStatue, &Current->DroppedDeerStatue,
                           &Current->ReturnedDeerStatue, &Current->TinyDeerKilled, &Current->MutantSquirrelsKilled,
                           &Current->WildDeerKilled, &Current->WildSquirrelsKilled, &Current->ArmoredCarsLost,
                           &Current->WarriorsLost, &Current->TimeOnFoot, &Current->TimeInAJazzs,
                           &Current->TimeInACleasans, &Current->TimeInASecurityTruck, &Current->TimeInArmoredCars,
                           &Current->TimeInAUDVs, &Current->TimeInGatlingTanks, &Current->TimeInIFVs,
                           &Current->FriendlyTinyDeerKilled, &Current->LastPlayTime.day, &Current->LastPlayTime.month,
                           &Current->LastPlayTime.year, &Current->LastPlayTime.second, &Current->LastPlayTime.minute,
                           &Current->LastPlayTime.hour, &Current->LastPlayTime.lTime,
                           &Current->SupportReceivedInfantryHp,
                           &Current->SupportReceivedVehicleHp, &Current->SupportReceivedAmmo,
                           &Current->SupportGrantedHpInfantry, &Current->SupportGrantedHpVehicle,
                           &Current->SupportGrantedAmmo, &Current->SupportReceivedHpInfantrySelf,
                           &Current->SupportReceivedHpVehicleSelf, &Current->SupportReceivedAmmoSelf,
                           &Current->PettedCougars, &Current->KilledCougars, &Current->KilledMutantCougars,
                           &Current->FailedPettingCougars, &Current->KilledFriendlyCougars,
                           &Current->DiedTryingToEscapeBase, &Current->RescuedCows, &Current->KilledCows,
                           &Current->LostCows, &Current->CompletedCowObjective, &Current->KilledMice,
                           &Current->CompledMiceObjective,
                           &Current->KilledPortablePumpJacks, &Current->PortablePumpJacksPlaced,
                           &Current->PortablePumpJacksLost, &Current->PumpJackMoney, &Current->MobilePumpJackMoney,
                           &Current->Rank, &Current->KilledTurkey, &Current->ReturnedTurkey);
            }
            fclose(LoadScores);
        }
    }
    ...
    inline BHScoreNode *
    EveluateHighestScore(unsigned long highValue, unsigned long currentValue, BHScoreNode *High, BHScoreNode *Current) {
    ...
    BHScoreNode *CompareHighScoreNodes(BHScoreNode *High, BHScoreNode *Current, int CompVal) {
        ...
        switch (CompVal) {
            ...
            case 10:
                return EveluateHighestScore(High->MostBonusObjectivesCompletedInARound,
                                            Current->MostBonusObjectivesCompletedInARound, High, Current);
            case 11:
            ...
            case 26:
                return EveluateHighestScore(High->ObjectiveActivatedAlarm, Current->ObjectiveActivatedAlarm, High,
                                            Current);
            case 27:
                return EveluateHighestScore(High->ObjectiveTurretTruck, Current->ObjectiveTurretTruck, High, Current);
            case 28:
                return EveluateHighestScore(High->ObjectiveTurretTruckAlarm, Current->ObjectiveTurretTruckAlarm, High,
                                            Current);
            case 29:
                return EveluateHighestScore(High->ObjectiveOilRigsActivated, Current->ObjectiveOilRigsActivated, High,
                                            Current);
            case 30:
                return EveluateHighestScore(High->ObjectiveOilRigsRepaired, Current->ObjectiveOilRigsRepaired, High,
                                            Current);
            case 31:
                return EveluateHighestScore(High->ObjectiveEngineersSaved, Current->ObjectiveEngineersSaved, High,
                                            Current);
            case 32:
                return EveluateHighestScore(High->ObjectiveWeaponsFound, Current->ObjectiveWeaponsFound, High, Current);
            case 33:
                return EveluateHighestScore(High->ObjectiveWeaponsReturned, Current->ObjectiveWeaponsReturned, High,
                                            Current);
            case 34:
                return EveluateHighestScore(High->ObjectivePlasmaRifleReturned, Current->ObjectivePlasmaRifleReturned,
                                            High, Current);
            case 35:
                return EveluateHighestScore(High->BonusObjectivesCompleted, Current->BonusObjectivesCompleted, High,
                                            Current);
            case 36:
                return EveluateHighestScore(High->PickedupHealthPowerups, Current->PickedupHealthPowerups, High,
                                            Current);
            case 37:
            ...
            case 45:
                return EveluateHighestScore(High->PickedupTotalPowerupsInARound, Current->PickedupTotalPowerupsInARound,
                                            High, Current);
            case 46:
            ...
            case 56:
                return EveluateHighestScore(High->PlayedGamesWithDefenseTurrets, Current->PlayedGamesWithDefenseTurrets,
                                            High, Current);
            case 57:
                return EveluateHighestScore(High->PlayedGamesWithGuardianHelicopter,
                                            Current->PlayedGamesWithGuardianHelicopter, High, Current);
            case 58:
            ...
            case 95:
                return EveluateHighestScore(High->FriendlyTinyDeerKilled, Current->FriendlyTinyDeerKilled, High,
                                            Current);
            case 96:
                return EveluateHighestScore(High->SupportReceivedInfantryHp, Current->SupportReceivedInfantryHp, High,
                                            Current);
            case 97:
                return EveluateHighestScore(High->SupportReceivedVehicleHp, Current->SupportReceivedVehicleHp, High,
                                            Current);
            case 98:
                return EveluateHighestScore(High->SupportReceivedAmmo, Current->SupportReceivedAmmo, High, Current);
            case 99:
                return EveluateHighestScore(High->SupportGrantedHpInfantry, Current->SupportGrantedHpInfantry, High,
                                            Current);
            case 100:
                return EveluateHighestScore(High->SupportGrantedHpVehicle, Current->SupportGrantedHpVehicle, High,
                                            Current);
            case 101:
                return EveluateHighestScore(High->SupportGrantedAmmo, Current->SupportGrantedAmmo, High, Current);
            case 102:
                return EveluateHighestScore(High->SupportReceivedHpInfantrySelf, Current->SupportReceivedHpInfantrySelf,
                                            High, Current);
            case 103:
                return EveluateHighestScore(High->SupportReceivedHpVehicleSelf, Current->SupportReceivedHpVehicleSelf,
                                            High, Current);
            case 104:
                return EveluateHighestScore(High->SupportReceivedAmmoSelf, Current->SupportReceivedAmmoSelf, High,
                                            Current);
            case 105:
            ...
            case 110:
                return EveluateHighestScore(High->DiedTryingToEscapeBase, Current->DiedTryingToEscapeBase, High,
                                            Current);
            case 111:
            ...
            case 117:
                return EveluateHighestScore(High->KilledPortablePumpJacks, Current->KilledPortablePumpJacks, High,
                                            Current);
            case 118:
                return EveluateHighestScore(High->PortablePumpJacksPlaced, Current->PortablePumpJacksPlaced, High,
                                            Current);
            case 119:
    ...
    bool ReturnHighScoreNodeValue(BHScoreNode *Node, int CompVal) {
        ...
        switch (CompVal) {
            ...
            case 1:
                return Node->PreGameTime ? true : false;
            case 2:
                return Node->IdleTime ? true : false;
            case 3:
                return Node->RoundsPlayed ? true : false;
            case 4:
                return Node->RoundsCompleted ? true : false;
            case 5:
                return Node->RoundsQuit ? true : false;
            case 6:
                return Node->RoundsWon ? true : false;
            case 7:
                return Node->RoundsLost ? true : false;
            case 8:
                return Node->MostKillsInARound ? true : false;
            case 9:
                return Node->MostDeathsInARound ? true : false;
            case 10:
                return Node->MostBonusObjectivesCompletedInARound ? true : false;
            case 11:
                return Node->Deaths ? true : false;
            case 12:
                return Node->Kills ? true : false;
            case 13:
                return Node->VehicleKills ? true : false;
            case 14:
                return Node->KilledSelf ? true : false;
            case 15:
                return Node->KilledPlayers ? true : false;
            case 16:
                return Node->KilledPresident ? true : false;
            case 17:
                return Node->KilledTurrets ? true : false;
            case 18:
                return Node->KilledBears ? true : false;
            case 19:
                return Node->KilledBlackBears ? true : false;
            case 20:
                return Node->KilledMutantBears ? true : false;
            case 21:
                return Node->KilledMutantDeer ? true : false;
            case 22:
                return Node->KilledMutantCats ? true : false;
            case 23:
                return Node->KilledMutantCatsB ? true : false;
            case 24:
                return Node->KilledMutantCatsR ? true : false;
            case 25:
                return Node->KilledMutantRabbits ? true : false;
            case 26:
                return Node->ObjectiveActivatedAlarm ? true : false;
            case 27:
                return Node->ObjectiveTurretTruck ? true : false;
            case 28:
                return Node->ObjectiveTurretTruckAlarm ? true : false;
            case 29:
                return Node->ObjectiveOilRigsActivated ? true : false;
            case 30:
                return Node->ObjectiveOilRigsRepaired ? true : false;
            case 31:
                return Node->ObjectiveEngineersSaved ? true : false;
            case 32:
                return Node->ObjectiveWeaponsFound ? true : false;
            case 33:
                return Node->ObjectiveWeaponsReturned ? true : false;
            case 34:
                return Node->ObjectivePlasmaRifleReturned ? true : false;
            case 35:
                return Node->BonusObjectivesCompleted ? true : false;
            case 36:
                return Node->PickedupHealthPowerups ? true : false;
            case 37:
                return Node->PickedupArmorPowerups ? true : false;
            case 38:
                return Node->PickedupCashPowerups ? true : false;
            case 39:
                return Node->PickedupAmmoPowerups ? true : false;
            case 40:
                return Node->PickedupHealthTotal ? true : false;
            case 41:
                return Node->PickedupArmorTotal ? true : false;
            case 42:
                return Node->PickedupCashTotal ? true : false;
            case 43:
                return Node->PickedupAmmoTotal ? true : false;
            case 44:
                return Node->PickedupTotalPowerups ? true : false;
            case 45:
                return Node->PickedupTotalPowerupsInARound ? true : false;
            case 46:
                return Node->KilledHumanAi ? true : false;
            case 47:
                return Node->VehiclesDestroyed ? true : false;
            case 48:
                return Node->VehiclesLost ? true : false;
            case 49:
                return Node->JazzsLost ? true : false;
            case 50:
                return Node->CleasansLost ? true : false;
            case 51:
                return Node->TrucksLost ? true : false;
            case 52:
                return Node->TanksLost ? true : false;
            case 53:
                return Node->TurretTruckLost ? true : false;
            case 54:
                return Node->C4VestPowerups ? true : false;
            case 55:
                return Node->ActivatedCommTower ? true : false;
            case 56:
                return Node->PlayedGamesWithDefenseTurrets ? true : false;
            case 57:
                return Node->PlayedGamesWithGuardianHelicopter ? true : false;
            case 58:
                return Node->TimesDrown ? true : false;
            case 59:
                return Node->TimesFallen ? true : false;
            case 60:
                return Node->KillsWithSentryTurret ? true : false;
            case 61:
                return Node->KilledSentryTurrets ? true : false;
            case 62:
                return Node->SentryTurretsPlaced ? true : false;
            case 63:
                return Node->SentryTurretsLost ? true : false;
            case 64:
                return Node->PickedUpMedicalNeedle ? true : false;
            case 65:
                return Node->ReturnedMedicalNeedle ? true : false;
            case 66:
                return Node->RepairedSubstation ? true : false;
            case 67:
                return Node->SubstationOnLineAtEnd ? true : false;
            case 68:
                return Node->RepairedSubstation ? true : false;
            case 69:
                return Node->GiantDeerKilled ? true : false;
            case 70:
                return Node->SurvivedAlarm ? true : false;
            case 71:
                return Node->WolfKilled ? true : false;
            case 72:
                return Node->MutantDogKilled ? true : false;
            case 73:
                return Node->BlueDeerKilled ? true : false;
            case 74:
                return Node->CheatedRounds ? true : false;
            case 75:
                return Node->NeverInjured ? true : false;
            case 76:
                return Node->MooseKilled ? true : false;
            case 77:
                return Node->EatenByRabbit ? true : false;
            case 78:
                return Node->PickedUpDeerStatue ? true : false;
            case 79:
                return Node->DroppedDeerStatue ? true : false;
            case 80:
                return Node->ReturnedDeerStatue ? true : false;
            case 81:
                return Node->TinyDeerKilled ? true : false;
            case 82:
                return Node->MutantSquirrelsKilled ? true : false;
            case 83:
                return Node->WildDeerKilled ? true : false;
            case 84:
                return Node->WildSquirrelsKilled ? true : false;
            case 85:
                return Node->ArmoredCarsLost ? true : false;
            case 86:
                return Node->WarriorsLost ? true : false;
            case 87:
                return Node->TimeOnFoot ? true : false;
            case 88:
                return Node->TimeInAJazzs ? true : false;
            case 89:
                return Node->TimeInACleasans ? true : false;
            case 90:
                return Node->TimeInASecurityTruck ? true : false;
            case 91:
                return Node->TimeInArmoredCars ? true : false;
            case 92:
                return Node->TimeInAUDVs ? true : false;
            case 93:
                return Node->TimeInGatlingTanks ? true : false;
            case 94:
                return Node->TimeInIFVs ? true : false;
            case 95:
                return Node->FriendlyTinyDeerKilled ? true : false;
            case 96:
                return Node->SupportReceivedInfantryHp ? true : false;
            case 97:
                return Node->SupportReceivedVehicleHp ? true : false;
            case 98:
                return Node->SupportReceivedAmmo ? true : false;
            case 99:
                return Node->SupportGrantedHpInfantry ? true : false;
            case 100:
                return Node->SupportGrantedHpVehicle ? true : false;
            case 101:
                return Node->SupportGrantedAmmo ? true : false;
            case 102:
                return Node->SupportReceivedHpInfantrySelf ? true : false;
            case 103:
                return Node->SupportReceivedHpVehicleSelf ? true : false;
            case 104:
                return Node->SupportReceivedAmmoSelf ? true : false;
            case 105:
                return Node->PettedCougars ? true : false;
            case 106:
                return Node->KilledCougars ? true : false;
            case 107:
                return Node->KilledMutantCougars ? true : false;
            case 108:
                return Node->FailedPettingCougars ? true : false;
            case 109:
                return Node->KilledFriendlyCougars ? true : false;
            case 110:
                return Node->DiedTryingToEscapeBase ? true : false;
            case 111:
                return Node->RescuedCows ? true : false;
            case 112:
                return Node->KilledCows ? true : false;
            case 113:
                return Node->LostCows ? true : false;
            case 114:
                return Node->CompletedCowObjective ? true : false;
            case 115:
                return Node->KilledMice ? true : false;
            case 116:
                return Node->CompledMiceObjective ? true : false;
            case 117:
                return Node->KilledPortablePumpJacks ? true : false;
            case 118:
                return Node->PortablePumpJacksPlaced ? true : false;
            case 119:
                return Node->PortablePumpJacksLost ? true : false;
            case 120:
                return Node->PumpJackMoney ? true : false;
            case 121:
                return Node->MobilePumpJackMoney ? true : false;
            case 122:
                return Node->KilledTurkey ? true : false;
            case 123:
                return Node->ReturnedTurkey ? true : false;
            default:
                Console_Input("msg SCORE SYSTEM ERROR: Out of bounds!");
                return false;
        }
    }
    ...
    char *ReturnScore(BHScoreNode *High, int CompVal) {
        ...
        switch (CompVal) {
            case 0:
                sprintf(RetChar, "Server Record: %s has played bear hunter for %s mintues.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PlayTime / 60.0f));
                return RetChar;
            case 1:
                sprintf(RetChar,
                        "Server Record: %s has dicked around for %s minutes before finally leaving the base to go hunting.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->PreGameTime / 60.0f));
                return RetChar;
            case 2:
                sprintf(RetChar, "Server Record: %s has been idle for %s minutes.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->IdleTime / 60.0f));
                return RetChar;
            case 3:
                sprintf(RetChar, "Server Record: %s has joined %s rounds of bear hunter.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->RoundsPlayed));
                return RetChar;
            case 4:
                sprintf(RetChar, "Server Record: %s has completed %s rounds of bear hunter.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->RoundsCompleted));
                return RetChar;
            case 5:
                sprintf(RetChar, "Server Record: %s has quit %s rounds of bear hunter.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->RoundsQuit));
                return RetChar;
            case 6:
                sprintf(RetChar, "Server Record: %s has won %s rounds of bear hunter.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->RoundsWon));
                return RetChar;
            case 7:
                sprintf(RetChar, "Server Record: %s has lost %s rounds of bear hunter.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->RoundsLost));
                return RetChar;
            case 8:
                sprintf(RetChar, "Server Record: %s has the most kills in a round: %s.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->MostKillsInARound));
                return RetChar;
            case 9:
                sprintf(RetChar, "Server Record: %s has the most deaths in a round: %s.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->MostDeathsInARound));
                return RetChar;
            case 10:
                sprintf(RetChar, "Server Record: %s has completed the most objectives in a round: %s.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->MostBonusObjectivesCompletedInARound));
                return RetChar;
            case 11:
                sprintf(RetChar, "Server Record: %s has %s deaths.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->Deaths));
                return RetChar;
            case 12:
                sprintf(RetChar, "Server Record: %s has %s kills.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->Kills));
                return RetChar;
            case 13:
                sprintf(RetChar, "Server Record: %s has %s vehicle kills.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->VehicleKills));
                return RetChar;
            case 14:
                sprintf(RetChar, "Server Record: %s has comitted suicide %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledSelf));
                return RetChar;
            case 15:
                sprintf(RetChar, "Server Record: %s has killed %s other players.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledPlayers));
                return RetChar;
            case 16:
                sprintf(RetChar, "Server Record: %s has killed the President of Corporate America %s times.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->KilledPresident));
                return RetChar;
            case 17:
                sprintf(RetChar, "Server Record: %s has killed %s shredder turrets.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledTurrets));
                return RetChar;
            case 18:
                sprintf(RetChar, "Server Record: %s has killed %s brown bears.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledBears));
                return RetChar;
            case 19:
                sprintf(RetChar, "Server Record: %s has killed %s black bears.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledBlackBears));
                return RetChar;
            case 20:
                sprintf(RetChar, "Server Record: %s has killed %s mutant bears.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledMutantBears));
                return RetChar;
            case 21:
                sprintf(RetChar, "Server Record: %s has killed %s mutant deer.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledMutantDeer));
                return RetChar;
            case 22:
                sprintf(RetChar, "Server Record: %s has killed %s mutant cats.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledMutantCats));
                return RetChar;
            case 23:
                sprintf(RetChar, "Server Record: %s has killed %s mutant ion cats.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledMutantCatsB));
                return RetChar;
            case 24:
                sprintf(RetChar, "Server Record: %s has killed %s mutant nuclear cats.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledMutantCatsR));
                return RetChar;
            case 25:
                sprintf(RetChar, "Server Record: %s has killed the boss rabbit %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledMutantRabbits));
                return RetChar;
            case 26:
                sprintf(RetChar, "Server Record: %s has activated the alarm %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ObjectiveActivatedAlarm));
                return RetChar;
            case 27:
                sprintf(RetChar, "Server Record: %s has returned the security truck %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ObjectiveTurretTruck));
                return RetChar;
            case 28:
                sprintf(RetChar, "Server Record: %s has deactivated the security alarm %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ObjectiveTurretTruckAlarm));
                return RetChar;
            case 29:
                sprintf(RetChar, "Server Record: %s has activated %s pump jacks.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ObjectiveOilRigsActivated));
                return RetChar;
            case 30:
                sprintf(RetChar, "Server Record: %s has repaired %s pump jacks.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ObjectiveOilRigsRepaired));
                return RetChar;
            case 31:
                sprintf(RetChar, "Server Record: %s has saved %s engineers.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ObjectiveEngineersSaved));
                return RetChar;
            case 32:
                sprintf(RetChar, "Server Record: %s has found %s weapon containers.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ObjectiveWeaponsFound));
                return RetChar;
            case 33:
                sprintf(RetChar, "Server Record: %s has returned %s weapon containers.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ObjectiveWeaponsReturned));
                return RetChar;
            case 34:
                sprintf(RetChar, "Server Record: %s has retrived the plasma rifle weapon container %s times.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->ObjectivePlasmaRifleReturned));
                return RetChar;
            case 35:
                sprintf(RetChar, "Server Record: %s has completed %s bonus objectives.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->BonusObjectivesCompleted));
                return RetChar;
            case 36:
                sprintf(RetChar, "Server Record: %s has acquired %s health powerups.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PickedupHealthPowerups));
                return RetChar;
            case 37:
                sprintf(RetChar, "Server Record: %s has acquired %s armor powerups.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PickedupArmorPowerups));
                return RetChar;
            case 38:
                sprintf(RetChar, "Server Record: %s has acquired %s cash powerups.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PickedupCashPowerups));
                return RetChar;
            case 39:
                sprintf(RetChar, "Server Record: %s has acquired %s ammo powerups.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PickedupAmmoPowerups));
                return RetChar;
            case 40:
                sprintf(RetChar, "Server Record: %s has acquired %s health from health powerups.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PickedupHealthTotal));
                return RetChar;
            case 41:
                sprintf(RetChar, "Server Record: %s has acquired %s armor from armor powerups.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PickedupArmorTotal));
                return RetChar;
            case 42:
                sprintf(RetChar, "Server Record: %s has acquired $%s.00 dollars from cash powerups.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PickedupCashTotal));
                return RetChar;
            case 43:
                sprintf(RetChar, "Server Record: %s has acquired %s clips of ammo from ammo powerups.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->PickedupAmmoTotal));
                return RetChar;
            case 44:
                sprintf(RetChar, "Server Record: %s has acquired %s powerups.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PickedupTotalPowerups));
                return RetChar;
            case 45:
                sprintf(RetChar, "Server Record: %s has acquired %s powerups in a round.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PickedupTotalPowerupsInARound));
                return RetChar;
            case 46:
                sprintf(RetChar, "Server Record: %s has killed %s friendly AI.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledHumanAi));
                return RetChar;
            case 47:
                sprintf(RetChar, "Server Record: %s has destroyed %s vehicles.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->VehiclesDestroyed));
                return RetChar;
            case 48:
                sprintf(RetChar, "Server Record: %s has lost %s vehicles.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->VehiclesLost));
                return RetChar;
            case 49:
                sprintf(RetChar, "Server Record: %s has lost %s Punda Jazzs.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->JazzsLost));
                return RetChar;
            case 50:
                sprintf(RetChar, "Server Record: %s has lost %s Cleasan Primera.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->CleasansLost));
                return RetChar;
            case 51:
                sprintf(RetChar, "Server Record: %s has lost %s Urban Defense Vehicles.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TrucksLost));
                return RetChar;
            case 52:
                sprintf(RetChar, "Server Record: %s has lost %s Gattling Tanks.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TanksLost));
                return RetChar;
            case 53:
                sprintf(RetChar, "Server Record: %s has lost %s Cargo Trucks.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TurretTruckLost));
                return RetChar;
            case 54:
                sprintf(RetChar, "Server Record: %s has acquired %s C4 Vests.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->C4VestPowerups));
                return RetChar;
            case 55:
                sprintf(RetChar, "Server Record: %s activated the transmitter %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ActivatedCommTower));
                return RetChar;
            case 56:
                sprintf(RetChar,
                        "Server Record: %s played in %s games where the security truck has been returned to base.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->PlayedGamesWithDefenseTurrets));
                return RetChar;
            case 57:
                sprintf(RetChar, "Server Record: %s has played %s games with assistance from Comanche 4573.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->PlayedGamesWithGuardianHelicopter));
                return RetChar;
            case 58:
                sprintf(RetChar, "Server Record: %s has gone too far from the shore %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TimesDrown));
                return RetChar;
            case 59:
                sprintf(RetChar, "Server Record: %s has fell to their death %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TimesFallen));
                return RetChar;
            case 60:
                sprintf(RetChar, "Server Record: %s has got %s kills with a sentry turret.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KillsWithSentryTurret));
                return RetChar;
            case 61:
                sprintf(RetChar, "Server Record: %s has destroyed %s sentry turrets.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledSentryTurrets));
                return RetChar;
            case 62:
                sprintf(RetChar, "Server Record: %s has placed %s sentry turrets.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->SentryTurretsPlaced));
                return RetChar;
            case 63:
                sprintf(RetChar, "Server Record: %s has lost %s sentry turrets.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->SentryTurretsLost));
                return RetChar;
            case 64:
                sprintf(RetChar, "Server Record: %s has picked up the medical needle %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PickedUpMedicalNeedle));
                return RetChar;
            case 65:
                sprintf(RetChar, "Server Record: %s has returned the medical needle %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ReturnedMedicalNeedle));
                return RetChar;
            case 66:
                sprintf(RetChar, "Server Record: %s has repaired the Power Substation %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->RepairedSubstation));
                return RetChar;
            case 67:
                sprintf(RetChar, "Server Record: %s has finished %s rounds with the substation online.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SubstationOnLineAtEnd));
                return RetChar;
            case 68:
                sprintf(RetChar, "Server Record: %s has played %s rounds in which the substation was never damaged.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SubstationNotDamaged));
                return RetChar;
            case 69:
                sprintf(RetChar, "Server Record: %s has killed %s Genitically Enhanced Deer.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->GiantDeerKilled));
                return RetChar;
            case 70:
                sprintf(RetChar,
                        "Server Record: %s has successfully held the cabin %s times for the duration of the EAS.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SurvivedAlarm));
                return RetChar;
            case 71:
                sprintf(RetChar, "Server Record: %s has killed %s Wolfs.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->WolfKilled));
                return RetChar;
            case 72:
                sprintf(RetChar, "Server Record: %s has killed %s Mutant Dogs.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->MutantDogKilled));
                return RetChar;
            case 73:
                sprintf(RetChar, "Server Record: %s has killed %s Karma Deer.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->BlueDeerKilled));
                return RetChar;
            case 74:
                sprintf(RetChar, "Server Record: %s has been in %s games that were won unbelievably well.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->CheatedRounds));
                return RetChar;
            case 75:
                sprintf(RetChar,
                        "Server Record: %s has been in %s games without the president getting hurt by mutants.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->NeverInjured));
                return RetChar;
            case 76:
                sprintf(RetChar, "Server Record: %s has killed %s Moose.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->MooseKilled));
                return RetChar;
            case 77:
                sprintf(RetChar, "Server Record: %s has been eaten by the rabbit %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->EatenByRabbit));
                return RetChar;
            case 78:
                sprintf(RetChar, "Server Record: %s has picked up the golden deer statue %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PickedUpDeerStatue));
                return RetChar;
            case 79:
                sprintf(RetChar, "Server Record: %s has dropped up the golden deer statue %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->DroppedDeerStatue));
                return RetChar;
            case 80:
                sprintf(RetChar, "Server Record: %s has returned up the golden deer statue %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ReturnedDeerStatue));
                return RetChar;
            case 81:
                sprintf(RetChar, "Server Record: %s has killed %s tiny deer.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TinyDeerKilled));
                return RetChar;
            case 82:
                sprintf(RetChar, "Server Record: %s has killed %s mutant squirrels.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->MutantSquirrelsKilled));
                return RetChar;
            case 83:
                sprintf(RetChar, "Server Record: %s has killed %s wild deer.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->WildDeerKilled));
                return RetChar;
            case 84:
                sprintf(RetChar, "Server Record: %s has killed %s wild squirrels.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->WildSquirrelsKilled));
                return RetChar;
            case 85:
                sprintf(RetChar, "Server Record: %s has lost %s Armored Cars.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->ArmoredCarsLost));
                return RetChar;
            case 86:
                sprintf(RetChar, "Server Record: %s has lost %s Warrior Infantry Fighting Vehicles.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->WarriorsLost));
                return RetChar;
            case 87:
                sprintf(RetChar, "Server Record: %s has spent %s minutes on foot.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TimeOnFoot / 60.0f));
                return RetChar;
            case 88:
                sprintf(RetChar, "Server Record: %s has spent %s minutes in Punda Jazzes.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TimeInAJazzs / 60.0f));
                return RetChar;
            case 89:
                sprintf(RetChar, "Server Record: %s has spent %s minutes in Cleasan Primeras.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TimeInACleasans / 60.0f));
                return RetChar;
            case 90:
                sprintf(RetChar, "Server Record: %s has spent %s minutes in Turret Trucks.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TimeInASecurityTruck / 60.0f));
                return RetChar;
            case 91:
                sprintf(RetChar, "Server Record: %s has spent %s minutes in Armored Cars.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TimeInArmoredCars / 60.0f));
                return RetChar;
            case 92:
                sprintf(RetChar, "Server Record: %s has spent %s minutes in Urban Defense Vehicles.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TimeInAUDVs / 60.0f));
                return RetChar;
            case 93:
                sprintf(RetChar, "Server Record: %s has spent %s minutes in Gatling Tanks.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->TimeInGatlingTanks / 60.0f));
                return RetChar;
            case 94:
                sprintf(RetChar, "Server Record: %s has spent %s minutes in Warrior Infantry Fighting Vehicles.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->TimeInIFVs / 60.0f));
                return RetChar;
            case 95:
                sprintf(RetChar, "Server Record: %s has killed %s friendly tiny deer.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->FriendlyTinyDeerKilled));
                return RetChar;
            case 96:
                sprintf(RetChar,
                        "Server Record: %s has received %s points of HP to infantry from Armored Supply Trucks.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SupportReceivedInfantryHp));
                return RetChar;
            case 97:
                sprintf(RetChar,
                        "Server Record: %s has received %s points of HP to vehicles from Armored Supply Trucks.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SupportReceivedVehicleHp));
                return RetChar;
            case 98:
                sprintf(RetChar, "Server Record: %s has received %s points of ammo from Armored Supply Trucks.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SupportReceivedAmmo));
                return RetChar;
            case 99:
                sprintf(RetChar, "Server Record: %s has given %s points of HP to infantry from Armored Supply Trucks.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SupportGrantedHpInfantry));
                return RetChar;
            case 100:
                sprintf(RetChar, "Server Record: %s has given %s points of HP to vehicles from Armored Supply Trucks.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SupportGrantedHpVehicle));
                return RetChar;
            case 101:
                sprintf(RetChar, "Server Record: %s has given %s points of ammo from Armored Supply Trucks.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SupportGrantedAmmo));
                return RetChar;
            case 102:
                sprintf(RetChar,
                        "Server Record: %s has given %s points of HP to infantry from Armored Supply Trucks to theirself.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SupportReceivedHpInfantrySelf));
                return RetChar;
            case 103:
                sprintf(RetChar,
                        "Server Record: %s has given %s points of HP to vehicles from Armored Supply Trucks to theirself.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SupportReceivedHpVehicleSelf));
                return RetChar;
            case 104:
                sprintf(RetChar,
                        "Server Record: %s has given %s points of ammo from Armored Supply Trucks to theirself.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->SupportReceivedAmmoSelf));
                return RetChar;
            case 105:
                sprintf(RetChar, "Server Record: %s has petted %s cougars.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PettedCougars));
                return RetChar;
            case 106:
                sprintf(RetChar, "Server Record: %s has killed %s wild cougars.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledCougars));
                return RetChar;
            case 107:
                sprintf(RetChar, "Server Record: %s has killed %s mutant cougars.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledMutantCougars));
                return RetChar;
            case 108:
                sprintf(RetChar, "Server Record: %s has killed %s cougars instead of petting them.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->FailedPettingCougars));
                return RetChar;
            case 109:
                sprintf(RetChar, "Server Record: %s has killed %s friendly cougars.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledFriendlyCougars));
                return RetChar;
            case 110:
                sprintf(RetChar,
                        "Server Record: %s has died trying to escape the base %s times when the boss was lurking about.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->DiedTryingToEscapeBase));
                return RetChar;
            case 111:
                sprintf(RetChar, "Server Record: %s has rescued %s cows.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->RescuedCows));
                return RetChar;
            case 112:
                sprintf(RetChar, "Server Record: %s has killed %s cows.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledCows));
                return RetChar;
            case 113:
                sprintf(RetChar, "Server Record: %s has lost %s cows.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->LostCows));
                return RetChar;
            case 114:
                sprintf(RetChar, "Server Record: %s has completed the cow objective %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->CompletedCowObjective));
                return RetChar;
            case 115:
                sprintf(RetChar, "Server Record: %s has killed %s mice.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledMice));
                return RetChar;
            case 116:
                sprintf(RetChar, "Server Record: %s has completed the pest objective %s times.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->CompledMiceObjective));
                return RetChar;
            case 117:
                sprintf(RetChar, "Server Record: %s has destroyed %s Portable Pumpjacks.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledPortablePumpJacks));
                return RetChar;
            case 118:
                sprintf(RetChar, "Server Record: %s has placed %s Portable Pumpjacks.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PortablePumpJacksPlaced));
                return RetChar;
            case 119:
                sprintf(RetChar, "Server Record: %s has lost %s Portable Pumpjacks.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PortablePumpJacksLost));
                return RetChar;
            case 120:
                sprintf(RetChar, "Server Record: %s has made $%s.00 from Pumpjacks.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->PumpJackMoney));
                return RetChar;
            case 121:
                sprintf(RetChar, "Server Record: %s has made $%s.00 from Portable Pumpjacks.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->MobilePumpJackMoney));
                return RetChar;
            case 122:
                sprintf(RetChar, "Server Record: %s has killed %s turkeys.", High->PlayerName,
                        JmgUtility::formatDigitGrouping(High->KilledTurkey));
                return RetChar;
            case 123:
                sprintf(RetChar, "Server Record: %s has made $%s.00 from bringing turkeys back to base.",
                        High->PlayerName, JmgUtility::formatDigitGrouping(High->ReturnedTurkey));
                return RetChar;
            default:
                sprintf(RetChar, "Server Record ERROR: Record index out of bounds!");
                return RetChar;
        }
    }
    ...
    void EndGameUpdatePlayerStats(bool won) {
        for (SLNode<cPlayer> *PlayerIter = Get_Player_List()->Head(); (PlayerIter !=
                                                                       NULL); PlayerIter = PlayerIter->Next()) {
                ...
                if (pobj->idleDelay > 60) {
                    ...
                    if (pVehicle) {
                        switch (Commands->Get_Preset_ID(pVehicle)) {
                            ...
                            case 1000001327:
                            case 1000001325:
                                pobj->TimeInACleasans++;
                                break;
                            case 1000001310:
                                pobj->TimeInAUDVs++;
                                break;
                            case 1000001306:
                                pobj->TimeInGatlingTanks++;
                                break;
                            case 1000001174:
                                pobj->TimeInASecurityTruck++;
                                break;
                            case 1000001811:
                                pobj->TimeInArmoredCars++;
                                break;
                            case 1000003594:
                                pobj->TimeInIFVs++;
                                break;
...
class JMG_Bear_Hunter_Animal_Control : public ScriptImpClass {
    ...
    void Create_Powerup(GameObject *powerup, GameObject *obj, float random, const char *powerup1, const char *powerup2,
                        const char *powerup3);
};
...
public:
    ...
    void PurchaseSoldier(GameObject *obj, float *cash) {
        ...
        if (*cash > 1250)
            while (current) {
                if (current->requiredPurchaseState == -1 ||
                    current->requiredPurchaseState == JMG_Bear_Hunter_Game_Control::gameState)
                    if (*cash >= current->cost && (!best || (current->rating > best->rating ||
                                                             (current->rating == best->rating &&
                                                              Commands->Get_Random(0.0f, 1.0f) < 0.5f))))
                        best = current;
                current = current->next;
            }
        else {
...
class JMG_Bear_Hunter_Player_Assist_AI : public ScriptImpClass {
    ...
    void SetAIState(GameObject *obj, AIStates newState, Vector3 pos, float distance, float speed,
                    Rp2SimplePositionSystem::SimplePositionNode *newNode = NULL);
...
class JMG_Bear_Hunter_Vehicle_Purchase_System : public ScriptImpClass {
    ...
    void AirDropFunction(GameObject *obj, Vector3 pos, float facing, int deathZoneId, int waypath1, int waypath2,
                         int waypath3);
};
...
class JMG_Airdrop_Attached_Object_On_Create : public ScriptImpClass {
    ...
    GameObject *
    CreateAObject(GameObject *obj, GameObject *Object, const char *Preset, const char *Model, const char *Animation,
                  const Vector3 &Pos, float Facing, GameObject *GAttachTo, const char *Bone, float DeathFrame,
                  float LastFrame);
};
...
JMG_Bear_Hunter_Oil_Rig::PumpjackNode JMG_Bear_Hunter_Oil_Rig::pumpJacks[7] = {
        JMG_Bear_Hunter_Oil_Rig::PumpjackNode(0)};
...
class JMG_Bear_Hunter_Guardian_Aircraft : public ScriptImpClass {
    enum {
        SECONDARY_RELOADING, SECONDARY_READY, SECONDARY_SWITCHING, SECONDARY_ACTIVE
    } secondaryState;
...
public:
    ...
    struct BearHunterWolfHivemindNode {
        ...
        BearHunterWolfHivemindNode(int id, const char *presetName, int maxPackSize, Vector3 territoryCenterSpot,
                                   float territoryRange, int wanderTime, int randomWanderTime, float respawnTime,
                                   float respawnTimeRandom, float listenerScale, bool hearsGlobalSounds) {
            this->id = id;
            ...
            currentWanderTime =
                    wanderTime + (randomWanderTime ? Commands->Get_Random_Int(-randomWanderTime, randomWanderTime) : 0);
...
public:
    ...
    bool addNode(int id, const char *presetName, int maxPackSize, Vector3 territoryCenterSpot, float territoryRange,
                 int wanderTime, int randomWanderTime, float respawnTime, float respawnTimeRandom, float listenerScale,
                 bool hearsGlobalSounds) {
        BearHunterWolfHivemindNode *current = BearHunterWolfHivemindNodeList;
        if (!BearHunterWolfHivemindNodeList) {
            BearHunterWolfHivemindNodeList = new BearHunterWolfHivemindNode(id, presetName, maxPackSize,
                                                                            territoryCenterSpot, territoryRange,
                                                                            wanderTime, randomWanderTime, respawnTime,
                                                                            respawnTimeRandom, listenerScale,
                                                                            hearsGlobalSounds);
            return true;
        }
        while (current) {
            if (current->id == id)
                return false;
            if (!current->next) {
                current->next = new BearHunterWolfHivemindNode(id, presetName, maxPackSize, territoryCenterSpot,
                                                               territoryRange, wanderTime, randomWanderTime,
                                                               respawnTime, respawnTimeRandom, listenerScale,
                                                               hearsGlobalSounds);
                return true;
            }
            current = current->next;
        ...
        while (current) {
            ...
            if (current->currentWanderTime) {
                ...
                if (!current->currentWanderTime) {
                    ...
                    current->currentWanderTime = current->wanderTime +
                                                 (current->randomWanderTime ? Commands->Get_Random_Int(
                                                         -current->randomWanderTime, current->randomWanderTime) : 0);
                }
            }
            if (current->currentPackSize < current->maxPackSize) {
                ...
                if (Get_Random_Pathfind_Spot(createLocation, 25.0f, &createLocation)) {
                    ...
                    MoveablePhysClass *mphys = packMember->As_PhysicalGameObj()
                                               ? packMember->As_PhysicalGameObj()->Peek_Physical_Object()->As_MoveablePhysClass()
                                               : NULL;
...
class JMG_Bear_Hunter_Armored_Car_Controller : public ScriptImpClass {
    ...
    void DecideScoreVehicleOrInfantry(GameObject *obj, GameObject *owner, BearHunterScoreSystem::BHScoreNode *ownerNode,
                                      float grantHp);

    void GrantVehicleRepairScore(GameObject *obj, GameObject *owner, BearHunterScoreSystem::BHScoreNode *ownerNode,
                                 float grantHp);

    void GrantInfantryRepairScore(GameObject *obj, GameObject *owner, BearHunterScoreSystem::BHScoreNode *ownerNode,
                                  float grantHp);
};
