...
void MetroidGame::WinMap() {
    ...
    for (int x = 1; x < 128; x++) {
        ...
        if (!Player)
            continue;
        if (JmgUtility::SimpleDistance(Commands->Get_Position(Player), Vector3(-1750.0f, -350.0f, 0.0f)) <=
            62500.0f) {// Move player out of snow area
...
void JMG_Metroid_Gibs_On_Enter::Entered(GameObject *obj, GameObject *enter) {
    ...
    if (Commands->Is_A_Star(enter)) {
        char DeathMSG[512];
        sprintf(DeathMSG, "%s killed %s by %s.", Get_Player_Name(enter), JmgUtility::JMG_Get_Units_Sex(enter),
                Get_Parameter("DeathMessage"));
...
void JMG_Metroid_Game_Control::Created(GameObject *obj) {
    ...
    MetroidObjectiveSystemControl.Add_Objective(1, NewObjectiveSystem::Primary, NewObjectiveSystem::Pending, 12781, "",
                                                12781, Commands->Get_Position(Commands->Find_Object(100126)));
    MetroidRoomObjectives.Add_Objective(1, NewObjectiveSystem::Primary, NewObjectiveSystem::Pending, 0, "", 0,
                                        Commands->Get_Position(Commands->Find_Object(100126)));
    JMG_Metroid_Game_Control::currentBriefingId = 12766;
    for (int x = 1; x < 128; x++) {
        if (!Get_GameObj(x))
            continue;
        JMG_Metroid_Game_Control::ParseBriefingString(
                Get_Translated_String(JMG_Metroid_Game_Control::currentBriefingId), x);
    }
    ...
    GameObject * ShieldWall = Commands->Create_Object("Simple_Sydney_SandM_Wall",
                                                      Vector3(344.817f, -435.808f, -0.887f));
...
void JMG_Metroid_Game_Control::Timer_Expired(GameObject *obj, int number) {
    if (1 == number) {
        ...
        if (lastPlayerCount != tempPlayerCount) {
            ...
            JMG_Metroid_Game_Control::EnemyShieldRegenStartTime = (int) JmgUtility::MathClamp(
                    5.5f - tempPlayerCount * 0.5f, 1.0f, 5.0f);
        }
        IncreasePlayerTimeScores();
        if (gameRemainingTime) {
            gameRemainingTime--;
            char timeMsg[220];
            if ((gameRemainingTime >= 300 && !(gameRemainingTime % 300)) ||
                (!(gameRemainingTime % 60) && gameRemainingTime > 0 && gameRemainingTime < 300)) {
                JmgUtility::MessageAllPlayers(127, 127, 255, "===============================================");
                sprintf(timeMsg, "The map has been won! The round will end in %d minute%s", gameRemainingTime / 60,
                        gameRemainingTime / 60 == 1 ? "!" : "s!");
    ...
    for (int y = 1; y < 128; y++) {
        ...
        if (MetroidPSSControl.PlayerSettings[y]->InTrainingRoom) {
            ...
            switch (MetroidPSSControl.PlayerSettings[y]->ArmorMode) {
                case 0:
                    pobj->PlayTimeAsClass1++;
                    break;
                case 1:
                    pobj->PlayTimeAsClass2++;
                    break;
                case 2:
                    pobj->PlayTimeAsClass3++;
                    break;
                case 3:
                    pobj->PlayTimeAsClass4++;
                    break;
                case 4:
                    pobj->PlayTimeAsClass5++;
                    break;
                case 5:
                    pobj->PlayTimeAsClass6++;
                    break;
                case 6:
                    pobj->PlayTimeAsClass7++;
                    break;
                case 7:
                    pobj->PlayTimeAsClass8++;
                    break;
                case 8:
                    pobj->PlayTimeAsClass9++;
                    break;
                case 9:
                    pobj->PlayTimeAsClass10++;
                    break;
            }
        }
    }
...
void JMG_Metroid_Game_Control::ParseBriefingString(const char *text, int playerId) {
    ...
    for (int x = 0; x < length; x++) {
        if ((Count > 150 && TextString[x] == ' ') || Count == 220) {// String getting too long, start a new one.
...
void JMG_Metroid_Character_Control::Created(GameObject *obj) {
    ...
    if (!mPlayerData.players[PlayerID]->destroyTime ||
        difftime(clock(), mPlayerData.players[PlayerID]->destroyTime) > 500) {
        if (Commands->Get_Preset_ID(obj) == 81930243 || Commands->Get_Preset_ID(obj) == 81930257) {
            JMG_Metroid_Game_Control::ParseBriefingString(
                    Get_Translated_String(JMG_Metroid_Game_Control::currentBriefingId), PlayerID);
            MetroidObjectiveSystemControl.Display_All_Objectives(obj);
            int objectiveId = MetroidObjectiveSystemControl.Get_First_Pending_Objective_Of_Priority(
                    NewObjectiveSystem::Primary);
    ...
    if (PlayerID)
        switch (Commands->Get_Preset_ID(obj)) {
            ...
            case 81930106:
                ...
                break;
            case 81930082:
                MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode = 9;
                break;
            case 81930243:
            case 81930257:
            case 81930103:
                break;
            default:
                sprintf(KeySetup, "msg JMG_Metroid_Character_Control ERROR: Undefined soldier type %d!",
                        Commands->Get_Preset_ID(obj));
        ...
        if (JmgUtility::SimpleDistance(Commands->Get_Position(obj), Vector3(550.0f, -425.0f, 7.5f)) > 22500.0f) {
            if (MetroidPSSControl.PlayerSettings[PlayerID]->InTrainingRoom) {
                ...
                switch (MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode) {
                    case 0:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[PlayerID]->SpawnedAsCharacter0++;
                        break;
                    case 1:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[PlayerID]->SpawnedAsCharacter1++;
                        break;
                    case 2:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[PlayerID]->SpawnedAsCharacter2++;
                        break;
                    case 3:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[PlayerID]->SpawnedAsCharacter3++;
                        break;
                    case 4:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[PlayerID]->SpawnedAsCharacter4++;
                        break;
                    case 5:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[PlayerID]->SpawnedAsCharacter5++;
                        break;
                    case 6:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[PlayerID]->SpawnedAsCharacter6++;
                        break;
                    case 7:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[PlayerID]->SpawnedAsCharacter7++;
                        break;
                    case 8:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[PlayerID]->SpawnedAsCharacter8++;
                        break;
                    case 9:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[PlayerID]->SpawnedAsCharacter9++;
                        break;
                }
            }
...
void JMG_Metroid_Character_Control::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    ...
    if (message == 2342346) {
        ...
        switch (MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode) {
            ...
            case 5:
                ...
                break;
            case 9:
                Change_Character(obj, "Cheat_Soldier");
                break;
        }
        //Change_Character(obj,"Cheat_Soldier");//TODO: remove
    }
    if (message == 940860501 && MetroidPSSControl.PlayerSettings[PlayerID]->InStartRoom &&
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode != 0) {
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode = 0;
        Change_Character(obj, "Marine_super");
    ...
    if (message == 940860502 && MetroidPSSControl.PlayerSettings[PlayerID]->InStartRoom &&
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode != 1) {
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode = 1;
        Change_Character(obj, "Ally_soldier");
    ...
    if (message == 940860503 && MetroidPSSControl.PlayerSettings[PlayerID]->InStartRoom &&
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode != 2) {
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode = 2;
        Change_Character(obj, "FirePower_Mode");
    ...
    if (message == 940860504 && MetroidPSSControl.PlayerSettings[PlayerID]->InStartRoom &&
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode != 3) {
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode = 3;
        Change_Character(obj, "Support_Mode");
    ...
    if (message == 940860505 && MetroidPSSControl.PlayerSettings[PlayerID]->InStartRoom &&
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode != 4) {
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode = 4;
        Change_Character(obj, "Defense_Mode");
    ...
    if (message == 940860506 && MetroidPSSControl.PlayerSettings[PlayerID]->InStartRoom &&
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode != 5) {
        MetroidPSSControl.PlayerSettings[PlayerID]->ArmorMode = 5;
        Change_Character(obj, "Supply_Mode");
...
void JMG_Metroid_Character_Control::Killed(GameObject *obj, GameObject *damager) {
    MetroidScoreSystem::MetroidHighScoresNode *pobj = MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
            obj)];
    pobj->Deaths++;
    if (Commands->Is_A_Star(damager) && damager != obj) {
        MetroidScoreSystem::MetroidHighScoresNode *dobj = MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                damager)];
        dobj->KilledPlayers++;
        dobj->Kills++;
    }
    if (obj == damager && damager)
        ...
        switch (Commands->Get_Preset_ID(damager)) {
            ...
            case 81930092:
            case 81930103:
                pobj->DeathFromPlayers++;
                break;
            case 1000000808:
                pobj->DeathFromMiniBoss++;
                break;
            case 82080053:
                pobj->DeathFromDefenseCameras++;
                break;
            case 81930093:
            case 1000000107:
            case 1000000399:
            ...
            case 81930088:
            case 1000000249:
                pobj->DeathFromRobots++;
                break;
            case 81930094:
            case 81930097:
            case 81930098:
                pobj->DeathFromTraitors++;
                break;
            case 82080078:
            case 82080079:
                pobj->DeathFromAttackCameras++;
                break;
            case 82080037:
            case 82080129:
            case 82080141:
            ...
            case 82080124:
            case 82080128:
                pobj->DeathFromGunEmplacements++;
                break;
            case 82080097:
                pobj->DeathFromShredderTurret++;
                break;
            case 82080095:
                pobj->DeathFromBansheeBoss++;
                break;
            case 82080048:
                pobj->DeathFromMineBoss++;
                break;
            case 82080043:
                pobj->DeathFromMineBossSquish++;
                break;
        }
...
void JMG_Metroid_New_Ambush_Cover_Point::Created(GameObject *obj) {
    AmbushPointSystemControl.NewCoverPoint(obj, Get_Int_Parameter("ID"), Get_Int_Parameter("GroupID"),
                                           Get_Int_Parameter("Crouch") ? true : false,
                                           Get_Int_Parameter("ExcellentHidingSpot") ? true : false);
}
...
void JMG_Metroid_Move_To_Random_Ambush_Spot::Created(GameObject *obj) {
    ...
    if (Get_Int_Parameter("BeginOnSpawn"))
        ...
        Commands->Start_Timer(obj, this, Get_Float_Parameter("Delay") +
                                         (Get_Float_Parameter("RandomDelay") ? Commands->Get_Random(
                                                 -Get_Float_Parameter("RandomDelay"),
                                                 Get_Float_Parameter("RandomDelay")) : 0.0f), 2);
...
void JMG_Metroid_Move_To_Random_Ambush_Spot::Timer_Expired(GameObject *obj, int number) {
    if (1 == number) {
        ...
        if (Get_Int_Parameter("FindVantagePoint") == 2 && EnemySeen && LastEnemyID != currentEnemyID && AmbushPoint &&
            !AmbushPoint->Crouch &&
            JmgUtility::SimpleDistance(AmbushPoint->coverPos, Commands->Get_Position(obj)) < 9.0f) {
...
bool JMG_Metroid_Move_To_Random_Ambush_Spot::SelectRandomAmbushSpot(GameObject *obj) {
    ...
    if (Get_Int_Parameter("FindHidingSpots"))
        Temp = AmbushPointSystemControl.GetGoodHidingSpot(&FaceLocation, Get_Int_Parameter("GroupID"),
                                                          Get_Int_Parameter("MaxGuards"));
        ...
        if (!AStar)
            return false;
        Temp = AmbushPointSystemControl.FindBestCoverPoint(MyPos, &FaceLocation, Get_Int_Parameter("GroupID"),
                                                           Get_Float_Parameter("MaxDistance"),
                                                           Get_Float_Parameter("MaxTargetDistance"),
                                                           Get_Int_Parameter("MaxGuards"));
    } else
        Temp = AmbushPointSystemControl.GetRandomCoverPoint(&FaceLocation, StartPos, Get_Int_Parameter("GroupID"),
                                                            Get_Int_Parameter("MaxGuards"),
                                                            Get_Float_Parameter("MaxDistance"));
    if (Temp == AmbushPoint)
        return false;
    ...
    params.Set_Movement(movePos,
                        (Get_Float_Parameter("SpeedMultiplier") ? Get_Float_Parameter("SpeedMultiplier") : 1.0f),
                        Get_Int_Parameter("MaxGuards") == 1 ? 0.0f : 1.0f, false);
    params.MovePathfind = true;
...
void JMG_Metroid_Move_To_Random_Ambush_Spot::FaceAmbushLocation(GameObject *obj) {
    if (CurrentAmbushPoint && !EnemySeen &&
        JmgUtility::SimpleDistance(CurrentAmbushPoint->coverPos, Commands->Get_Position(obj)) < 4.0f) {
        faceReset = 5;
        Commands->Action_Reset(obj, 100.0f);
        ActionParamsStruct params;
        params.Set_Basic(this, 10, 1);
        params.Set_Face_Location(
                CurrentAmbushPoint ? CurrentAmbushPoint->GetRandomAmbushPoint() : Vector3(0.0f, 0.0f, 0.0f), 10.0f);
        Commands->Action_Face_Location(obj, params);
        /*ActionParamsStruct params;
...
void JMG_Metroid_Mine_Tower_Control::Timer_Expired(GameObject *obj, int number) {
    if (number == 1) {
        Commands->Static_Anim_Phys_Goto_Frame(456473, PendingActivation == 3 ? 999.0f : 0.0f,
                                              "MineElevator.MineElevator");
        Commands->Static_Anim_Phys_Goto_Frame(458415, PendingActivation == 3 ? 999.0f : 0.0f,
                                              "MineElevatorKZ.MineElevatorKZ");
        Commands->Start_Timer(obj, this, 60.0f, 1);
    }
}
...
void JMG_Metroid_Mine_Tower_Control::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == 223424) {
        for (int x = 0; x < 3; x++) {
            if (MetroidObjectiveSystemControl.Get_Objective_Status(30 + x) == NewObjectiveSystem::NotDefined) {
                MetroidObjectiveSystemControl.Add_Objective(30 + x, NewObjectiveSystem::Primary,
                                                            NewObjectiveSystem::Pending, 12810 + x, "", 12810 + x,
                                                            Commands->Get_Position(
                                                                    Commands->Find_Object(LockDownButtonIDs[x])));
    ...
    if (message == 123424) {
        ...
        if (PendingActivation == 3) {
            ...
            for (int x = 1; x < 128; x++) {
                ...
                if (!Player)
                    continue;
                if (JmgUtility::SimpleDistance(Commands->Get_Position(Player), Vector3(1000.0f, 500.0f, 0.0f)) >
                    360000.0f)
                    continue;
    ...
    if (message == 123425) {
        if (MetroidObjectiveSystemControl.Get_Objective_Status(32) == NewObjectiveSystem::NotDefined) {
            MetroidObjectiveSystemControl.Add_Objective(33, NewObjectiveSystem::Primary, NewObjectiveSystem::Pending,
                                                        12813, "", 12813,
                                                        Commands->Get_Position(Commands->Find_Object(406329)));
...
void JMG_Metroid_Send_Custom_On_Poke::Poked(GameObject *obj, GameObject *poker) {
    ...
    if (!Target)
        return;
    Commands->Send_Custom_Event(obj, Target, Get_Int_Parameter("Message"),
                                Get_Int_Parameter("Param") ? Get_Int_Parameter("Param") : JmgUtility::JMG_Get_Player_ID(
                                        poker), 0);
}
...
void JMG_Metroid_AI_Path_Node::Timer_Expired(GameObject *obj, int number) {
    if (number == 847734) {
        ...
        for (int x = 0; x < Astrlen; x++) {// Break the node list up
            if (Get_Parameter("LinkedNodes")[x] == Get_Parameter("Delim")[0]) {// Check for the delims
                if (TempSize) {// Make sure there is a name to convert
                    Temp[TempSize] = '\0';
        ...
        if (TempSize) {// Catch any leftovers
            Temp[TempSize] = '\0';
            MyNode->AddLinkNode(MetroidBossPathControlSystem.Find_AIPathNode_Node(Temp));
            MyNode->NumberOfLinks++;
        }
...
void JMG_Metroid_Mine_Boss::Timer_Expired(GameObject *obj, int number) {
    if (number == 4732548) {
        if (!SoundID) {
            ...
            if (AStar && JmgUtility::SimpleDistance(Vector3(950.0f, 500.0f, -1501.0f), Commands->Get_Position(AStar)) <
                         2250000.0f) {
    ...
    if (number == 4732547) {
        Vector3 TargetSpot = Commands->Get_Bone_Position(obj,
                                                         The_Cnc_Game()->Is_Dedicated() ? "DedicatedZone" : "ZonePos");
...
void JMG_Metroid_Mine_Boss::Action_Complete(GameObject *obj, int action, ActionCompleteReason reason) {
    ...
    if (EnemyID) {
        ...
        if (Enemy) {
            RoadPoint = NULL;
            ChasingID = EnemyID;
            Goto_Location(obj, EnemyPos, false, 5.0, false, true,
                          JmgUtility::MathClamp(Commands->Get_Distance(Commands->Get_Position(obj), EnemyPos) / 250.0f,
                                                1.5f, 5));
            GameObject * Turret = Commands->Find_Object(TurretID);
            if (Turret)
...
void JMG_Metroid_Mine_Boss::Goto_Location(GameObject *obj, const Vector3 &TargetLocation, bool Pathfind,
                                          float ArriveDistance, bool Backward, bool Monitor, float Speed) {
    Commands->Action_Reset(obj, 10);
    ActionParamsStruct params;
...
void JMG_Metroid_Boss_Turret::Enemy_Seen(GameObject *obj, GameObject *seen) {
    ...
    if (TempDist >= MinDistance && TempDist <= MaxDistance &&
        (!SeenID || SeenID == TargetID || TempDist < TargetDistance)) {
        ...
        if (SeenID != TargetID) {
            ...
            params.Set_Attack(seen, Get_Float_Parameter("MaxAttackDistance"), 0.0,
                              MetroidGameControl.BossMode ? false : true);
            params.AttackCheckBlocked = false;
            Commands->Action_Attack(obj, params);
        }
    }
...
void JMG_Metroid_Boss_Turret::Timer_Expired(GameObject *obj, int number) {
    ...
    if (number == 1) {
        ...
        params.Set_Basic(this, 70, 1);
        params.Set_Attack(
                Vector3(10.0f * Commands->Get_Random_Int(-1, 2), 10.0f * Commands->Get_Random_Int(-1, 2), 2.0f), 0.0f,
                0.0f, true);
        Commands->Action_Attack(obj, params);
        Commands->Start_Timer(obj, this, 10.0f, 1);
    }
    if (number == 3) {
        ...
        Vector3 Pos = Commands->Get_Position(obj);
        float Rotation = JmgUtility::MathClampDegrees(
                Commands->Get_Facing(obj) + RotateDirection * Get_Int_Parameter("SwivelTurret")) * PI180;
...
void JMG_Metroid_Boss_Turret::Damaged(GameObject *obj, GameObject *damager, float damage) {
    if (damage > 0 && !damager && obj != damager && !SeenID) {
        ...
        ActionParamsStruct params;
        params.Set_Basic(this, 100, 2);
        params.Set_Attack(damager, Get_Float_Parameter("MaxAttackDistance"), 1.0,
                          MetroidGameControl.BossMode ? false : true);
        params.AttackCheckBlocked = false;
...
void JMG_Metroid_Boss_Grinder_Kill_Zone::Entered(GameObject *obj, GameObject *enter) {
    ...
    if (PID == 82080051 || PID == 82080029 || PID == 82080055 || PID == 82080056 ||
        PID == 82080053) {// Crates or player equipment
    ...
    GameObject * Boss = Commands->Find_Object(MetroidGameControl.BossID);
    if (Boss)
        Commands->Apply_Damage(Boss, -(Commands->Get_Health(enter) + Commands->Get_Shield_Strength(enter) +
                                       (Get_Player_Count() * 2.5f)), "None", enter);
    char splatter[32];
    ...
    if (Commands->Is_A_Star(enter)) {
        ...
        if (MetroidPSSControl.PlayerSettings[JmgUtility::JMG_Get_Player_ID(enter)]->hasDied) {
            MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                    enter)]->DeathFromMineBossResources++;
...
void JMG_Metroid_Crate_Random_Powerup::Killed(GameObject *obj, GameObject *damager) {
    ...
    if (Commands->Get_Random(0.0f, 1.0f) < 0.2f) {
        GameObject * Powerup = Commands->Create_Object(
                Commands->Get_Random(0.0f, 1.0f) < 0.5 ? "POW_Health_050" : "CnC_Ammo_Crate",
...
void JMG_Metroid_Mine_Tower_Elevator::Created(GameObject *obj) {
    ...
    GameObject * LockDownLasers = Commands->Create_Object("Metroid_Lockdown_Lazer_Wall",
                                                          Vector3(1000.0f, 524.157f, 5.011f));
...
void JMG_Metroid_Mine_Tower_Elevator::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    ...
    if (message == 223425) {
        ...
        if (ElevatorInMotion || (ElevatorCurrentFloor != ButtonFloor && Floor != 4) ||
            (ElevatorCurrentFloor == ButtonFloor && Floor == 4)) {
            Commands->Create_3D_WAV_Sound_At_Bone("interface_emblem.wav", sender, "origin");
            return;
        }
        ...
        float MoveTime =
                abs(FloorFrams[ElevatorCurrentFloor] - FloorFrams[Floor == 4 ? ButtonFloor : Floor]) / 30.0f + 2.0f;
        ElevatorInMotion = true;
        Commands->Static_Anim_Phys_Goto_Frame(456474, FloorFrams[Floor == 4 ? ButtonFloor : Floor],
                                              "jg_elevator.jg_elevator");
    ...
    if (message == 406919) {
        if (MetroidObjectiveSystemControl.Get_Objective_Status(29) == NewObjectiveSystem::NotDefined) {
            MetroidObjectiveSystemControl.Add_Objective(29, NewObjectiveSystem::Primary, NewObjectiveSystem::Pending,
                                                        12813, "", 12813, Commands->Get_Position(
                            Commands->Find_Object(LockDownButtonID)));
            JmgUtility::SetHUDHelpText(12813, Vector3(0, 1, 0));
        }
    }
    if (message == 406920) {
        for (int x = 0; x < 3; x++) {
            if (MetroidObjectiveSystemControl.Get_Objective_Status(34 + x) == NewObjectiveSystem::NotDefined) {
                MetroidObjectiveSystemControl.Add_Objective(34 + x, NewObjectiveSystem::Primary,
                                                            NewObjectiveSystem::Pending, 12814 + x, "", 12814 + x,
                                                            Commands->Get_Position(Commands->Find_Object(
                                                                    MetroidGameControl.MineTerminalID[x])));
...
void
JMG_Metroid_Mine_Computer_Console_Disarm_Script::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == 9870923) {// Disarm another one
...
void JMG_Metroid_Teleport_Zone::Entered(GameObject *obj, GameObject *enter) {
    ...
    MoveablePhysClass *mphys = enter->As_PhysicalGameObj()
                               ? enter->As_PhysicalGameObj()->Peek_Physical_Object()->As_MoveablePhysClass() : NULL;
...
void JMG_Metroid_Electrified_Water::Entered(GameObject *obj, GameObject *enter) {
    ...
    if (Commands->Is_A_Star(enter)) {
        ...
        Set_Screen_Fade_Opacity_Player(enter, 0.75f, 0.1f);
        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                enter)]->DeathFromElectrifiedWater++;
    }
...
void JMG_Metroid_Set_Screen_Color_While_In_Zone::Entered(GameObject *obj, GameObject *enter) {
    ...
    Set_Screen_Fade_Color_Player(enter, Get_Vector3_Parameter("ScreenColor").X, Get_Vector3_Parameter("ScreenColor").Y,
                                 Get_Vector3_Parameter("ScreenColor").Z, 0);
...
void JMG_Metroid_Support_Health_Powerup::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == CUSTOM_EVENT_POWERUP_GRANTED) {
        ...
        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                sender)]->PickedUpPlayerPlacedHealthPack++;
    ...
    if (number == 1) {
        ...
        for (int x = 1; x < 128; x++) {
            ...
            if (!SupportModeSystemControl[x].LastFired) {
                ...
                if (!Weapon)
                    continue;
                if (!_stricmp(Weapon, "Weapon_Generator_Beacon") || !_stricmp(Weapon, "Weapon_Medical_Kit_Beacon") ||
                    !_stricmp(Weapon, "Weapon_Defense_Mode_Gun_Beacon") ||
                    !_stricmp(Weapon, "Weapon_Defense_Mode_Telepad") ||
                    !_stricmp(Weapon, "Weapon_Shield_Upgrade_Normal") ||
                    !_stricmp(Weapon, "Weapon_Shield_Upgrade_Energy") ||
                    !_stricmp(Weapon, "Weapon_Shield_Upgrade_Heat") ||
                    !_stricmp(Weapon, "Weapon_Shield_Upgrade_Kinetic") ||
                    !_stricmp(Weapon, "Weapon_Shield_Upgrade_Rad") || !_stricmp(Weapon, "Weapon_Weapons_Spawner"))
                    continue;
                int CurrentClipBullets = Get_Current_Clip_Bullets(Player);
                int MaxClipBullets = Get_Current_Clip_Max_Bullets(Player);
                if (CurrentClipBullets < MaxClipBullets &&
                    currentCount % ((int) ((100.00 / (double) MaxClipBullets) * 100) / 10 | 1) == 0) {
    ...
    if (number == 2) {
        ...
        while (Current) {
            ...
            if (o && obj != o && vampireShield < 500.0f) {
                ...
                if (team != 1 && team != -1 && shield &&
                    JmgUtility::SimpleDistance(Commands->Get_Position(o), mypos) < 225) {
...
void JMG_Metroid_AI_Hunt_Attack::Action_Complete(GameObject *obj, int action, ActionCompleteReason reason) {
    if (Get_Int_Parameter("ReturnToOriginalPosition") &&
        JmgUtility::SimpleDistance(Commands->Get_Position(obj), StartPosition) > 100.0f && !StarInRange) {
...
void JMG_Metroid_AI_Hunt_Attack::GetClosestStar(GameObject *obj) {
    ...
    if (AStar) {
        ...
        if ((MaxRange <= 0 || TempDist < MaxRange) && MaxRange > 100.0f) {
            ...
            return;
        } else if (Get_Int_Parameter("ReturnToOriginalPosition") &&
                   JmgUtility::SimpleDistance(Pos, StartPosition) > 100.0f) {
            MoveToLocation = StartPosition;
            if (InnateEnabled) {
                InnateEnabled = false;
                Commands->Innate_Disable(obj);
...
void JMG_Metroid_Powered_Teleport_Zone::Entered(GameObject *obj, GameObject *enter) {
    if (!JmgUtility::IsTruePhysicalObject(enter) || !Enabled ||
        MetroidPSSControl.PlayerSettings[JmgUtility::JMG_Get_Player_ID(enter)]->IsInTeleport)
        return;
    GameObject * Object = Commands->Find_Object(Get_Int_Parameter("ObjectID"));
...
void JMG_Metroid_Lockdown_Controller::Created(GameObject *obj) {
    ...
    RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].HUDMessageLockdownInitiated = Get_Int_Parameter(
            "HUDMsgLockdownStart");
    RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].HUDMessageLockdownAverted = Get_Int_Parameter(
            "HUDMsgLockdownEnd");
    RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].LockDownInitiatedStringID = Get_Int_Parameter(
            "LockDownStartMsg");
    RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].LockDownAvertedStringID = Get_Int_Parameter(
            "LockDownEndMsg");
    sprintf(RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].LockDownInitiatedSound, "%s",
            Get_Parameter("LockDownStartMsg"));
    sprintf(RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].LockDownAvertedSound, "%s",
            Get_Parameter("LockDownEndMsg"));
    Commands->Start_Timer(obj, this, 1.0f, 1);
}
...
void JMG_Metroid_Lockdown_Lockdown_Send_Custom_Object::Created(GameObject *obj) {
    RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].AddLockdownCustomObject(obj, Get_Int_Parameter(
            "LockdownStartCustom"), Get_Int_Parameter("PreLockdownStartCustom"), Get_Int_Parameter(
            "PostLockdownStartCustom"));
}
...
void JMG_Metroid_Lockdown_Unit_Spawner::Created(GameObject *obj) {
    RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].AddSpawnPoint(obj, Get_Parameter("PresetName"),
                                                                           Get_Int_Parameter("LockDownZoneID"),
                                                                           Get_Int_Parameter("MaxSpawnedAtOnce"),
                                                                           Get_Int_Parameter("MaxTotalSpawned"),
                                                                           Get_Int_Parameter("SpawnFequency"),
                                                                           Get_Int_Parameter("RespawnDelay"),
                                                                           Get_Int_Parameter("RandomRespawnDelay"));
}
...
void JMG_Metroid_Lockdown_Trigger_Zone::Entered(GameObject *obj, GameObject *enter) {
    if (RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].BeginLockDown(Get_Int_Parameter("EnemyArriveDelay"),
                                                                               Get_Int_Parameter("LockDownZoneID")))
        switch (Get_Int_Parameter("LockDownZoneID")) {
            case 0:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                        enter)]->TriggeredLockdown0++;
                break;
            case 1:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                        enter)]->TriggeredLockdown1++;
                break;
            case 2:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                        enter)]->TriggeredLockdown2++;
                break;
            case 3:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                        enter)]->TriggeredLockdown3++;
                break;
            case 4:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                        enter)]->TriggeredLockdown4++;
                break;
            case 5:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                        enter)]->TriggeredLockdown5++;
                break;
            case 6:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                        enter)]->TriggeredLockdown6++;
                break;
            case 7:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                        enter)]->TriggeredLockdown7++;
                break;
            case 8:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                        enter)]->TriggeredLockdown8++;
                break;
            case 9:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                        enter)]->TriggeredLockdown9++;
                break;
        }
}
...
void JMG_Metroid_Lockdown_zComputer_Console_Disarm_Script::Custom(GameObject *obj, int message, int param,
                                                                  GameObject *sender) {
    if (message == 9870924) {// Disarm another one
        Killed(obj, NULL);
    }
    switch (message) {
        case 940860500:
...
void JMG_Metroid_Lockdown_Disable_Lockdown_On_Custom_Count::Custom(GameObject *obj, int message, int param,
                                                                   GameObject *sender) {
    if (message == Get_Int_Parameter("Message")) {
        MessageCount++;
        if (Get_Int_Parameter("Count") <= MessageCount) {
            RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].EndLockDown(
                    Get_Int_Parameter("ResetSpawnersOnComplete") ? true : false,
                    Get_Int_Parameter("LockoutLockdownTime"));
            MessageCount = 0;
        }
    }
}
...
void JMG_Metroid_Lockdown_Disable_Lockdown_On_Death::Killed(GameObject *obj, GameObject *killer) {
    RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].EndLockDown(
            Get_Int_Parameter("ResetSpawnersOnComplete") ? true : false, Get_Int_Parameter("LockoutLockdownTime"));
}

void JMG_Metroid_Lockdown_Trigger_On_Message::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == Get_Int_Parameter("Message")) {
        if (RoomLockdownControl[Get_Int_Parameter("LockDownZoneID")].BeginLockDown(
                Get_Int_Parameter("EnemyArrivalDelay"), Get_Int_Parameter("LockDownZoneID"))) {
            GameObject * player = Commands->Find_Object(param);
            if (player)
                switch (Get_Int_Parameter("LockDownZoneID")) {
                    case 0:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                                player)]->TriggeredLockdown0++;
                        break;
                    case 1:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                                player)]->TriggeredLockdown1++;
                        break;
                    case 2:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                                player)]->TriggeredLockdown2++;
                        break;
                    case 3:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                                player)]->TriggeredLockdown3++;
                        break;
                    case 4:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                                player)]->TriggeredLockdown4++;
                        break;
                    case 5:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                                player)]->TriggeredLockdown5++;
                        break;
                    case 6:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                                player)]->TriggeredLockdown6++;
                        break;
                    case 7:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                                player)]->TriggeredLockdown7++;
                        break;
                    case 8:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                                player)]->TriggeredLockdown8++;
                        break;
                    case 9:
                        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                                player)]->TriggeredLockdown9++;
                        break;
                }
        }
    }
...
void JMG_Metroid_Lockdown_Disable_Lockdown_On_Time_Without_Message::Custom(GameObject *obj, int message, int param,
                                                                           GameObject *sender) {
...
void JMG_Metroid_Defense_Mode::Timer_Expired(GameObject *obj, int number) {
    if (number == 1) {
        ...
        if (!BlockSupportGunRegen && CurrentAmmo < Get_Max_Total_Bullets(obj, "Weapon_Defense_Mode_Gun_Beacon")) {
            Set_Clip_Bullets(obj, "Weapon_Defense_Mode_Gun_Beacon",
                             Get_Total_Bullets(obj, "Weapon_Defense_Mode_Gun_Beacon") + 1);
            LastGunAmmo = Get_Total_Bullets(obj, "Weapon_Defense_Mode_Gun_Beacon");
        }
        ...
        if (!BlockTelepadRegen && CurrentAmmo < Get_Max_Total_Bullets(obj, "Weapon_Defense_Mode_Telepad")) {
            Set_Clip_Bullets(obj, "Weapon_Defense_Mode_Telepad",
                             Get_Total_Bullets(obj, "Weapon_Defense_Mode_Telepad") + 1);
...
void JMG_Metroid_Screen_Fade_Red_On_Damage::ScreenFade(GameObject *obj) {
    ...
    if (MetroidPSSControl.PlayerSettings[PlayerID]->ScreenOpacity) {
        ...
        Set_Screen_Fade_Color_Player(obj, (MetroidPSSControl.PlayerSettings[PlayerID]->ScreenColor.X * Temp +
                                           InjuryRatio * 0.75f),
                                     MetroidPSSControl.PlayerSettings[PlayerID]->ScreenColor.Y * Temp,
                                     MetroidPSSControl.PlayerSettings[PlayerID]->ScreenColor.Z * Temp, 0.1f);
        Set_Screen_Fade_Opacity_Player(obj, JmgUtility::MathClamp(
                ScreenOpacicity + MetroidPSSControl.PlayerSettings[PlayerID]->ScreenOpacity, 0.0f, 0.85f), 0.1f);
    } else {
...
void JMG_Metroid_AI_Move_To_Position::Timer_Expired(GameObject *obj, int number) {
    if (1 == number) {
        ...
        Commands->Get_Action_Params(obj, params);
        params.Set_Basic(this, 10, 100);
        params.Set_Movement(Get_Vector3_Parameter("TargetSpot"), Get_Float_Parameter("Speed"),
                            Get_Float_Parameter("ArriveDistance"), false);
        params.MovePathfind = true;
        Commands->Action_Goto(obj, params);
        params.Set_Basic(this, 100, 10);
    }
...
void JMG_Metroid_ZeroBug_Tracker::Timer_Expired(GameObject *obj, int number) {
    if (number == 1) {
        if (!MetroidPSSControl.PlayerSettings[PlayerID]->hasZeroBug && !Get_Money(PlayerID) && !Get_Score(PlayerID)) {
            ...
            char zeroBug[512];
            Vector3 Pos = Commands->Get_Position(obj);
            sprintf(zeroBug, "msg %s has contracted the ZeroBug! (%.2f %.2f %.2f)", Get_Player_Name(obj), Pos.X, Pos.Y,
                    Pos.Z);
...
void JMG_Metroid_Zone_Damage_Object_While_In_Zone::Entered(GameObject *obj, GameObject *enter) {
    char params[128];
    sprintf(params, "%.3f,%s,%.2f,%d", Get_Float_Parameter("DamageRate"), Get_Parameter("Warhead"),
            Get_Float_Parameter("Delay"), Get_Int_Parameter("Death_Zone_Type"));
...
void JMG_Metroid_Attached_Zone_Damage_Object_While_In_Zone::Timer_Expired(GameObject *obj, int number) {
    if (number == 1) {
        ...
        if (Get_Int_Parameter("Death_Zone_Type") && Commands->Get_Health(obj) &&
            MetroidPSSControl.PlayerSettings[JmgUtility::JMG_Get_Player_ID(obj)]->hasDied) {
            switch (Get_Int_Parameter("Death_Zone_Type")) {
                case 1:
                    MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                            obj)]->DeathFromSpikePit++;
                    break;
                case 2:
                    MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                            obj)]->DeathFromFallingOffTheTrain++;
                    break;
                case 3:
                    MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(obj)]->DeathInLava++;
                    break;
                case 4:
                    MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                            obj)]->DeathInRadioactivePit++;
                    break;
                case 5:
                    MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                            obj)]->DeathFromReactorRadiation++;
                    break;
                case 6:
                    MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(obj)]->DeathInFire++;
                    break;
                case 7:
                    MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                            obj)]->DeathFromMineBossResources++;
                    break;
            }
...
void JMG_Metroid_Spawn_At_Random_Spawn_Point::Timer_Expired(GameObject *obj, int number) {
    if (number == 1) {
        Vector3 pos = MetroidRandomSpawnControl.SelectFurthestSpawnPoint(Commands->Get_Position(obj),
                                                                         Get_Int_Parameter("SpawnGroup"));
...
void JMG_Metroid_Supply_Mode::ClearUpgrades(int playerId, const char *playerName) {
    ...
    for (int x = 1; x < 128; x++) {
        if (!JMG_Metroid_Supply_Mode::ShieldUpgradeGrantedBy[x] ||
            JMG_Metroid_Supply_Mode::ShieldUpgradeGrantedBy[x] != playerId ||
            (x == playerId && JMG_Metroid_Supply_Mode::ShieldUpgradeGrantedBy[playerId] == playerId))
            continue;
        GameObject * player = Get_GameObj(x);
        if (!player)
            continue;
...
void JMG_Metroid_Supply_Shield_Powerup::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == CUSTOM_EVENT_POWERUP_GRANTED) {
        ...
        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                sender)]->PickedUpPlayerShieldUpgrade++;
    }
}
...
void JMG_Metroid_Supply_Weapon_Spawner_Weapon::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == CUSTOM_EVENT_POWERUP_GRANTED) {
        ...
        MetroidScoreControl.MetroidPlayerHighScoreNodes[JmgUtility::JMG_Get_Player_ID(
                sender)]->PickedUpPlayerPlacedWeapon++;
...
void JMG_Metroid_Display_Message_On_Pickup::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == CUSTOM_EVENT_POWERUP_GRANTED) {
        JmgUtility::DisplayChatMessage(sender, (int) Get_Vector3_Parameter("Color[R|G|B]").X,
                                       (int) Get_Vector3_Parameter("Color[R|G|B]").Y,
                                       (int) Get_Vector3_Parameter("Color[R|G|B]").Z, Get_Parameter("PickupMessage"));
    }
}
...
void JMG_Metroid_Boss_HP_Modifier::Created(GameObject *obj) {
    maxShield = JmgUtility::MathClamp((float) Get_Player_Count() / 10.0f, 0.0f, 1.0f) *
                Commands->Get_Max_Shield_Strength(obj);
    ...
    if (number == 1) {
        if (Commands->Get_Health(obj) < Commands->Get_Max_Health(obj)) {
            float health = Commands->Get_Health(obj) +
                           JmgUtility::MathClamp((float) Get_Player_Count() / 10.0f, 0.0f, 1.0f) *
                           Get_Float_Parameter("Points");
...
void JMG_Metroid_Damage_Score_Monitor::Killed(GameObject *obj, GameObject *damager) {
    ...
    switch (Commands->Get_Preset_ID(obj)) {
        case 82080082:
            pobj->KilledSecurityCameras++;
            break;
        case 1000000808:
            pobj->KilledMiniBoss++;
            break;
        case 82080053:
            pobj->KilledDefenseCameras++;
            break;
        case 81930093:
        case 1000000107:
        case 1000000399:
        ...
        case 82080124:
        case 82080128:
            pobj->KilledGunEmplacements++;
            break;
        case 82080097:
            pobj->KilledTurretBoss++;
            break;
        case 82080095:
            pobj->KilledBansheeBoss++;
            break;
        case 82080043:
            pobj->KilledMineBoss++;
            break;
    ...
    switch (JmgUtility::JMG_Get_Current_Weapon_ID(damager)) {
        case 409610067:
            pobj->KillsWithDefenseGun++;
            break;
        case 1000000285:
        case 409610130:
            pobj->KillsWithCheatGun++;
            break;
        case 409610048:
            pobj->KillsWithFlameUpgrade++;
            break;
        case 409610022:
            pobj->KillsWithRoboticFlameUpgrade++;
            break;
        case 1000000416:
            pobj->KillsWithGlitchGun++;
            break;
        case 409610017:
            pobj->KillsWithImperialist++;
            break;
        case 409610020:
            pobj->KillsWithRoboticImperialist++;
            break;
        case 409610021:
            pobj->KillsWithLaserShotgun++;
            break;
        case 409610023:
            pobj->KillsWithRoboticLaserShotgun++;
            break;
        case 409610025:
            pobj->KillsWithMachineGun++;
            break;
        case 409610089:
            pobj->KillsWithRoboticMachineGun++;
            break;
        case 409610016:
            pobj->KillsWithShockCoil++;
            break;
        case 409610024:
            pobj->KillsWithRoboticShockCoil++;
            break;
        case 409610033:
            pobj->KillsWithLaser++;
            break;
        case 409610031:
            pobj->KillsWithRoboticLaser++;
            break;
        case 409610026:
            pobj->KillsWithHyperdeath++;
            break;
        case 409610029:
            pobj->KillsWithSuperHyperdeath++;
            break;
        case 409610119:
            pobj->KillsWithKnife++;
            break;
        case 409610133:
            pobj->KillsWithGrenade++;
            break;
        case 409610134:
            pobj->KillsWithRoboticGrenade++;
            break;
        default:
            pobj->KillsWithUnknownWeapon++;
            break;
    }
...
void JMG_Metroid_Death_Zone_Score_Monitor::Entered(GameObject *obj, GameObject *enter) {
    ...
    switch (Get_Int_Parameter("Death_Zone_Type")) {
        case 1:
            MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->DeathFromSpikePit++;
            break;
        case 2:
            MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->DeathFromFallingOffTheTrain++;
            break;
        case 3:
            MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->DeathInLava++;
            break;
        case 4:
            MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->DeathInRadioactivePit++;
            break;
        case 5:
            MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->DeathFromReactorRadiation++;
            break;
        case 6:
            MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->DeathInFire++;
            break;
        case 7:
            MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->DeathFromMineBossResources++;
            break;
    }
}
...
void JMG_Metroid_Weapon_Pickup_Score_Monitor::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == CUSTOM_EVENT_POWERUP_GRANTED) {
        ...
        switch (pt) {
            case 0:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpPlayerPlacedWeapon++;
                break;
            case 1:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpLaser++;
                break;
            case 2:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpShotgun++;
                break;
            case 3:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpSniper++;
                break;
            case 4:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpFlame++;
                break;
            case 5:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpShockCoil++;
                break;
            case 6:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpMachineGun++;
                break;
            case 7:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpRoboticLaser++;
                break;
            case 8:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpRoboticShotgun++;
                break;
            case 9:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpRoboticSniper++;
                break;
            case 10:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpRoboticFlame++;
                break;
            case 11:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpRoboticShockCoil++;
                break;
            case 12:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpRoboticMachineGun++;
                break;
            case 13:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpGlitchGun++;
                break;
            case 14:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpHealthPack++;
                break;
            case 15:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpShieldUpgrade++;
                break;
            case 16:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpPlayerShieldUpgrade++;
                break;
            case 17:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->PickedUpGrenadeLaunchers++;
                break;
            case 18:
                MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->FoundCheatGun++;
                break;
            case 19:
                if (!MetroidPSSControl.PlayerSettings[playerID]->hasKeycard) {
...
void JMG_Metroid_Zone_Score_Monitor::Entered(GameObject *obj, GameObject *enter) {
    ...
    switch (Get_Int_Parameter("Zone_Type")) {
        case 1:
            MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->GlitchModeFound++;
            break;
        case 2:
            MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->GlitchModeCompleted++;
            break;
        case 3:
            if (!MetroidPSSControl.PlayerSettings[playerID]->openedSecurityDoor && Commands->Has_Key(enter, 2)) {
            ...
            break;
        case 4:
            MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->KilledByRapids++;
            break;
        case 5:
            MetroidScoreControl.MetroidPlayerHighScoreNodes[playerID]->KilledByFall++;
            break;
        case 6:
            if (!MetroidGameControl.deactivatedLaserFence) {
        ...
        case 10:
            ...
            break;
        case 11:
            if (!MetroidGameControl.tunnelEntryLowered) {
...
void JMG_Metroid_Mine_Computer_Console_Script_Special::Timer_Expired(GameObject *obj, int number) {
    ...
    if (number == 2) {
        if (MetroidGameControl.MineTerminalReset[TerminalNumber]) {
            MetroidGameControl.MineTerminalReset[TerminalNumber]--;
            if (!MetroidGameControl.MineTerminalReset[TerminalNumber]) {
                if (MetroidObjectiveSystemControl.Get_Objective_Status(34 + TerminalNumber) ==
                    NewObjectiveSystem::Accomplished) {
                    MetroidObjectiveSystemControl.Remove_Objective(34 + TerminalNumber);
                    MetroidObjectiveSystemControl.Add_Objective(34 + TerminalNumber, NewObjectiveSystem::Primary,
                                                                NewObjectiveSystem::Pending, 12814 + TerminalNumber, "",
                                                                12814 + TerminalNumber, Commands->Get_Position(
                                    Commands->Find_Object(MetroidGameControl.MineTerminalID[TerminalNumber])));
                }
                Commands->Enable_HUD_Pokable_Indicator(obj, true);
                MetroidGameControl.NumberOfDeactivatedTerminals--;
                MetroidGameControl.MineTerminalDeactivated[TerminalNumber] = false;
...
void JMG_Metroid_Objective_Update_On_Enter::Entered(GameObject *obj, GameObject *enter) {
    ...
    if (Get_Int_Parameter("CompleteObjectiveID")) {
        MetroidObjectiveSystemControl.Add_Objective(Get_Int_Parameter("CompleteObjectiveID"),
                                                    NewObjectiveSystem::Primary, NewObjectiveSystem::Hidden, 0, "", 0);
        MetroidObjectiveSystemControl.Set_Objective_Status(Get_Int_Parameter("CompleteObjectiveID"),
                                                           NewObjectiveSystem::Accomplished);
    }
    Commands->Start_Timer(obj, this, Get_Float_Parameter("Delay"), 1);
}
    ...
    if (number == 1) {
        if (Get_Int_Parameter("NewObjectiveID")) {
            ...
            if (!ObjectiveMarker && Get_Int_Parameter("ObjectiveMarkerObjectID")) {
                Console_Input(
                        "msg JMG_Metroid_Objective_Update_On_Enter ERROR: Could not find objective marker object!");
                return;
            }
            if (ObjectiveMarker)
                MetroidObjectiveSystemControl.Add_Objective(Get_Int_Parameter("NewObjectiveID"),
                                                            (NewObjectiveSystem::Priority) Get_Int_Parameter(
                                                                    "NewObjectivePriority"),
                                                            NewObjectiveSystem::Pending,
                                                            Get_Int_Parameter("NewObjectiveStringID"), "",
                                                            Get_Int_Parameter("NewObjectiveStringID"),
                                                            Commands->Get_Position(ObjectiveMarker));
            else
                MetroidObjectiveSystemControl.Add_Objective(Get_Int_Parameter("NewObjectiveID"),
                                                            (NewObjectiveSystem::Priority) Get_Int_Parameter(
                                                                    "NewObjectivePriority"),
...
void JMG_Metroid_Objective_Update_On_Pickup::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (message == CUSTOM_EVENT_POWERUP_GRANTED) {
        ...
        if (Get_Int_Parameter("CompleteObjectiveID")) {
            MetroidObjectiveSystemControl.Add_Objective(Get_Int_Parameter("CompleteObjectiveID"),
                                                        NewObjectiveSystem::Primary, NewObjectiveSystem::Hidden, 0, "",
                                                        0);
            MetroidObjectiveSystemControl.Set_Objective_Status(Get_Int_Parameter("CompleteObjectiveID"),
                                                               NewObjectiveSystem::Accomplished);
        }
        ...
        if (ObjectiveMarker)
            MetroidObjectiveSystemControl.Add_Objective(Get_Int_Parameter("NewObjectiveID"),
                                                        (NewObjectiveSystem::Priority) Get_Int_Parameter(
                                                                "NewObjectivePriority"), NewObjectiveSystem::Pending,
                                                        Get_Int_Parameter("NewObjectiveStringID"), "",
                                                        Get_Int_Parameter("NewObjectiveStringID"),
                                                        Commands->Get_Position(ObjectiveMarker));
        else
...
void JMG_AI_Artillery_Targeting_Fire_At_Custom::Custom(GameObject *obj, int message, int param, GameObject *sender) {
    if (Get_Int_Parameter("TargetCustom") == message) {
        ...
        Vector3 myPos = obj->As_SoldierGameObj() ? Commands->Get_Bone_Position(obj, "gunbone")
                                                 : Commands->Get_Bone_Position(obj,
                                                                               "MuzzleA0"), enemyPos = Commands->Get_Position(
                seen);
        float targetDist = JmgUtility::SimpleDistance(myPos,
                                                      enemyPos), targetFlatDistance = JmgUtility::SimpleFlatDistance(
                myPos, enemyPos), minDistanceSquared =
                Get_Float_Parameter("MinDistance") * Get_Float_Parameter("MinDistance");
        ...
        if (targetDist <= Get_Float_Parameter("UseLowAngleMaxDistance") * Get_Float_Parameter("UseLowAngleMaxDistance"))
            ...
            useHighAngle = false;
        else if (Get_Float_Parameter("UseLowAngleTargetAboveHeight") &&
                 myPos.Z + Get_Float_Parameter("UseLowAngleTargetAboveHeight") < Commands->Get_Position(seen).Z)
            useHighAngle = false;
        ...
        Vector3 pos = obj->As_SoldierGameObj() ? Commands->Get_Bone_Position(obj, "gunbone")
                                               : Commands->Get_Bone_Position(obj, "MuzzleA0");
...
bool JMG_AI_Artillery_Targeting_Fire_At_Custom::CalculateAngle(double *returnAngle, double distance, double height,
                                                               bool highAngle) {
    double velocitySquared = velocity * velocity;
    double calculatedGravity = gravity * 9.8;
    if (distance <= 0.0)
        return false;
    double x = velocitySquared * velocitySquared -
               calculatedGravity * (calculatedGravity * (distance * distance) + 2 * height * velocitySquared);
    if (x < 0)
        return false;
    *returnAngle =
            atan((velocitySquared + (highAngle ? 1 : -1) * sqrt(x)) / (calculatedGravity * distance)) * 180.0 / PI;
...
void JMG_Metroid_AI_Forest_Mini_Boss::Attack_Target(GameObject *obj, GameObject *target) {
    ...
    params.MovePathfind =
            primary && JmgUtility::SimpleDistance(Commands->Get_Position(obj), Commands->Get_Position(target)) <= 225
            ? false : true;
    Commands->Action_Goto(obj, params);
    params.Set_Basic(this, 999, 10);
...
void JMG_Metroid_SpawnRoom_Objective_Update_On_Enter::Entered(GameObject *obj, GameObject *enter) {
    ...
    triggered = true;
    if (Get_Int_Parameter("CompleteObjectiveID")) {
        MetroidRoomObjectives.Add_Objective(Get_Int_Parameter("CompleteObjectiveID"), NewObjectiveSystem::Primary,
                                            NewObjectiveSystem::Hidden, 0, "", 0);
        MetroidRoomObjectives.Set_Objective_Status(Get_Int_Parameter("CompleteObjectiveID"),
                                                   NewObjectiveSystem::Accomplished);
    }
    Commands->Start_Timer(obj, this, Get_Float_Parameter("Delay"), 1);
}
...
void JMG_Metroid_SpawnRoom_Objective_Update_On_Enter::Timer_Expired(GameObject *obj, int number) {
    if (number == 1) {
        if (Get_Int_Parameter("NewObjectiveID")) {
            ...
            if (ObjectiveMarker)
                MetroidRoomObjectives.Add_Objective(Get_Int_Parameter("NewObjectiveID"),
                                                    (NewObjectiveSystem::Priority) Get_Int_Parameter(
                                                            "NewObjectivePriority"), NewObjectiveSystem::Pending, 0, "",
                                                    0, Commands->Get_Position(ObjectiveMarker));
            else
                MetroidRoomObjectives.Add_Objective(Get_Int_Parameter("NewObjectiveID"),
                                                    (NewObjectiveSystem::Priority) Get_Int_Parameter(
                                                            "NewObjectivePriority"), NewObjectiveSystem::Pending, 0, "",
...
void JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_At_Custom::Custom(GameObject *obj, int message, int param,
                                                                          GameObject *sender) {
    if (Get_Int_Parameter("TargetCustom") == message && reloadComplete) {
        ...
        if (targetDist <= Get_Float_Parameter("UseLowAngleMaxDistance") * Get_Float_Parameter("UseLowAngleMaxDistance"))
            ...
            useHighAngle = false;
                 ...
                 Commands->Get_Position(obj).Z + Get_Float_Parameter("UseLowAngleTargetAboveHeight") <
                 Commands->Get_Position(seen).Z)
            useHighAngle = false;
        double height = enemyPos.Z - myPos.Z, angle;
        myPos.Z = enemyPos.Z = 0.0f;
        if (!CalculateAngle(&angle, Commands->Get_Distance(enemyPos, myPos), height, useHighAngle))
...
bool JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_At_Custom::CalculateAngle(double *returnAngle, double distance,
                                                                                  double height, bool highAngle) {
    ...
    *returnAngle = atan((velocitySquared + (highAngle ? 1 : -1) * sqrt(x)) / (calculatedGravity * distance));
    if (*returnAngle * 180.0 / PI < Get_Float_Parameter("MinAngle") ||
        *returnAngle * 180.0 / PI > Get_Float_Parameter("MaxAngle"))
        return false;
    return true;
}

void JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_At_Custom::FireProjectile(GameObject *obj, double zAngle,
                                                                                  double aimAngle) {
    Commands->Create_3D_Sound_At_Bone(Get_Parameter("FireSound"), obj, "MuzzleA0");
    GameObject * projectile = Commands->Create_Object(Get_Parameter("VehicleProjectilePreset"),
                                                      Commands->Get_Bone_Position(obj, "muzzleA0"));
    Commands->Create_Explosion(Get_Parameter("MuzzleFlashExplosion"), Commands->Get_Bone_Position(obj, "muzzleA0"),
                               obj);
    char params[220];
...
void JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_Custom_Player::Custom(GameObject *obj, int message, int param,
                                                                              GameObject *sender) {
    if (Get_Int_Parameter("TargetCustom") == message && reloadComplete) {
        GameObject * seen = Commands->Find_Object(param);
        if (!seen)
        ...
        FireProjectile(obj, zRotation, angle);
        reloadComplete = false;
        Commands->Start_Timer(obj, this, max(Get_Float_Parameter("ReloadTime") -
                                             (Get_Float_Parameter("SubtractReloadPerPlayer") * Get_Player_Count()),
                                             Get_Float_Parameter("MinReloadTime")), 1);
    }
}
...
bool
JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_Custom_Player::CalculateAngle(double *returnAngle, double distance,
                                                                                 double height, bool highAngle) {
    double velocitySquared = Get_Float_Parameter("FireVelocity") * Get_Float_Parameter("FireVelocity");
    double calculatedGravity = Get_Float_Parameter("GravityScale") * 9.8;
    if (distance <= 0.0)
...
void JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_Custom_Player::FireProjectile(GameObject *obj, double zAngle,
                                                                                      double aimAngle) {
    Commands->Create_3D_Sound_At_Bone(Get_Parameter("FireSound"), obj, "MuzzleA0");
    GameObject * projectile = Commands->Create_Object(Get_Parameter("VehicleProjectilePreset"),
                                                      Commands->Get_Bone_Position(obj, "muzzleA0"));
...
ScriptRegistrant<JMG_Metroid_Gibs_On_Enter> JMG_Metroid_Gibs_On_Enter_Registrant("JMG_Metroid_Gibs_On_Enter",
                                                                                 "DeathMessage=falling into a spiky pit:string,PerkID=0:int");
ScriptRegistrant<JMG_Metroid_Increase_Perk_Unlock_On_Enter> JMG_Metroid_Increase_Perk_Unlock_On_Enter_Registrant(
        "JMG_Metroid_Increase_Perk_Unlock_On_Enter", "PerkID:int");
ScriptRegistrant<JMG_Metroid_HUD_Message_On_Enter> JMG_Metroid_HUD_Message_On_Enter_Registrant(
        "JMG_Metroid_HUD_Message_On_Enter", "StringID:int,Delay=0.0:float");
ScriptRegistrant<JMG_Metroid_Game_Initilization> JMG_Metroid_Game_Initilization_Registrant(
        "JMG_Metroid_Game_Initilization", "");
ScriptRegistrant<JMG_Metroid_Game_Control> JMG_Metroid_Game_Control_Registrant("JMG_Metroid_Game_Control", "");
ScriptRegistrant<JMG_Metroid_Character_Control> JMG_Metroid_Character_Control_Registrant(
        "JMG_Metroid_Character_Control", "");
ScriptRegistrant<JMG_Metroid_Hide_Object_On_Death> JMG_Metroid_Hide_Object_On_Death_Registrant(
        "JMG_Metroid_Hide_Object_On_Death", "");
ScriptRegistrant<JMG_Metroid_New_Ambush_Cover_Point> JMG_Metroid_New_Ambush_Cover_Point_Registrant(
        "JMG_Metroid_New_Ambush_Cover_Point", "ID:int,GroupID=0:int,Crouch=0:int,ExcellentHidingSpot=0:int");
ScriptRegistrant<JMG_Metroid_New_Ambush_Attack_Point> JMG_Metroid_New_Ambush_Attack_Point_Registrant(
        "JMG_Metroid_New_Ambush_Attack_Point", "ID:int");
ScriptRegistrant<JMG_Metroid_Move_To_Random_Ambush_Spot> JMG_Metroid_Move_To_Random_Ambush_Spot_Registrant(
        "JMG_Metroid_Move_To_Random_Ambush_Spot",
        "GroupID:int,MaxGuards=0:int,Delay=30.0:float,RandomDelay=10.0:float,MaxDistance=0.0:float,BeginOnSpawn=0:int,FindVantagePoint=0:int,MaxTargetDistance=0.0:float,FindHidingSpots=0:int,SpeedMultiplier=1.0:float");
ScriptRegistrant<JMG_Metroid_Mine_Tower_Control> JMG_Metroid_Mine_Tower_Control_Registrant(
        "JMG_Metroid_Mine_Tower_Control", "");
ScriptRegistrant<JMG_Metroid_Send_Custom_On_Poke> JMG_Metroid_Send_Custom_On_Poke_Registrant(
        "JMG_Metroid_Send_Custom_On_Poke", "ID:int,Message:int,Param:int");
ScriptRegistrant<JMG_Metroid_AI_Path_Node> JMG_Metroid_AI_Path_Node_Registrant("JMG_Metroid_AI_Path_Node",
                                                                               "NodeName:string,LinkedNodes:string,Delim=@:string,MaxDeviation=15.0:float");
ScriptRegistrant<JMG_Metroid_Mine_Boss> JMG_Metroid_Mine_Boss_Registrant("JMG_Metroid_Mine_Boss", "");
ScriptRegistrant<JMG_Metroid_Boss_Turret> JMG_Metroid_Boss_Turret_Registrant("JMG_Metroid_Boss_Turret",
                                                                             "MinAttackDistance=0.0:float,MaxAttackDistance=300.0:float,AttackTimer=11:int,CanTarget[Infantry|Vehicles|Aircraft]=1.0 1.0 1.0:Vector3,SwivelTurret=1:int");
ScriptRegistrant<JMG_Metroid_Boss_Grinder_Kill_Zone> JMG_Metroid_Boss_Grinder_Kill_Zone_Registrant(
        "JMG_Metroid_Boss_Grinder_Kill_Zone", "");
ScriptRegistrant<JMG_Metroid_Dynamic_Powerup_System> JMG_Metroid_Dynamic_Powerup_System_Registrant(
        "JMG_Metroid_Dynamic_Powerup_System", "");
ScriptRegistrant<JMG_Metroid_Crate_Random_Powerup> JMG_Metroid_Crate_Random_Powerup_Registrant(
        "JMG_Metroid_Crate_Random_Powerup", "");
ScriptRegistrant<JMG_Metroid_Mine_Tower_Elevator> JMG_Metroid_Mine_Tower_Elevator_Registrant(
        "JMG_Metroid_Mine_Tower_Elevator", "");
ScriptRegistrant<JMG_Metroid_Text_Dialog> JMG_Metroid_Text_Dialog_Registrant("JMG_Metroid_Text_Dialog", "StringID:int");
ScriptRegistrant<JMG_Metroid_Mine_Computer_Console> JMG_Metroid_Mine_Computer_Console_Registrant(
        "JMG_Metroid_Mine_Computer_Console", "");
ScriptRegistrant<JMG_Metroid_Mine_Computer_Console_Disarm_Script> JMG_Metroid_Mine_Computer_Console_Disarm_Script_Registrant(
        "JMG_Metroid_Mine_Computer_Console_Disarm_Script", "ID:int");
ScriptRegistrant<JMG_Metroid_Reset_Fall_Distance_On_Enter> JMG_Metroid_Reset_Fall_Distance_On_Enter_Registrant(
        "JMG_Metroid_Reset_Fall_Distance_On_Enter", "");
ScriptRegistrant<JMG_Metroid_Recover_Fall_Damage_Once> JMG_Metroid_Recover_Fall_Damage_Once_Registrant(
        "JMG_Metroid_Recover_Fall_Damage_Once", "");
ScriptRegistrant<JMG_Metroid_Teleport_Zone> JMG_Metroid_Teleport_Zone_Registrant("JMG_Metroid_Teleport_Zone",
                                                                                 "ObjectID:int,Position=0.0 0.0 0.0:vector3,MaxDistance=1.5:float");
ScriptRegistrant<JMG_Metroid_Tower_Put_AI_Down_Zone> JMG_Metroid_Tower_Put_AI_Down_Zone_Registrant(
        "JMG_Metroid_Tower_Put_AI_Down_Zone", "");
ScriptRegistrant<JMG_Metroid_Electrified_Water> JMG_Metroid_Electrified_Water_Registrant(
        "JMG_Metroid_Electrified_Water", "");
ScriptRegistrant<JMG_Metroid_Set_Screen_Color_While_In_Zone> JMG_Metroid_Set_Screen_Color_While_In_Zone_Registrant(
        "JMG_Metroid_Set_Screen_Color_While_In_Zone", "ScreenColor=0.28 0.43 0.55:Vector3,Opacity=0.9:float");
ScriptRegistrant<JMG_Metroid_Medic> JMG_Metroid_Medic_Registrant("JMG_Metroid_Medic", "");
ScriptRegistrant<JMG_Metroid_Support_Health_Powerup_Beacon> JMG_Metroid_Support_Health_Powerup_Beacon_Registrant(
        "JMG_Metroid_Support_Health_Powerup_Beacon", "");
ScriptRegistrant<JMG_Metroid_Support_Health_Powerup> JMG_Metroid_Support_Health_Powerup_Registrant(
        "JMG_Metroid_Support_Health_Powerup", "PlayerID:int");
ScriptRegistrant<JMG_Metroid_Support_Generator_Powerup_Beacon> JMG_Metroid_Support_Generator_Powerup_Beacon_Registrant(
        "JMG_Metroid_Support_Generator_Powerup_Beacon", "");
ScriptRegistrant<JMG_Metroid_Support_Generator_Powerup> JMG_Metroid_Support_Generator_Powerup_Registrant(
        "JMG_Metroid_Support_Generator_Powerup", "BeaconID:int");
ScriptRegistrant<JMG_Metroid_AI_Hunt_Attack> JMG_Metroid_AI_Hunt_Attack_Registrant("JMG_Metroid_AI_Hunt_Attack",
                                                                                   "MaxStarSeekDistance=100.0:float,ScanFromSpawnPosition=0:int,ReturnToOriginalPosition=0:int,AllowInnate=1:int");
ScriptRegistrant<JMG_Metroid_Powered_Teleport_Zone> JMG_Metroid_Powered_Teleport_Zone_Registrant(
        "JMG_Metroid_Powered_Teleport_Zone",
        "ObjectID:int,StartsEnabled=0:int,PowerMessage=0:int,VisibleTeleporterID=0:int");
ScriptRegistrant<JMG_Metroid_Send_Custom_On_Zone_Enter> JMG_Metroid_Send_Custom_On_Zone_Enter_Registrant(
        "JMG_Metroid_Send_Custom_On_Zone_Enter", "ID:int,Message:int,Param:int");
ScriptRegistrant<JMG_Metroid_Sabotaged_Crate> JMG_Metroid_Sabotaged_Crate_Registrant("JMG_Metroid_Sabotaged_Crate", "");
ScriptRegistrant<JMG_Metroid_Enable_Spawners_On_Death> JMG_Metroid_Enable_Spawners_On_Death_Registrant(
        "JMG_Metroid_Enable_Spawners_On_Death", "StartID:int,EndID:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Lockdown_Object> JMG_Metroid_Lockdown_Lockdown_Object_Registrant(
        "JMG_Metroid_Lockdown_Lockdown_Object", "LockDownZoneID=0:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Post_Lockdown_Object> JMG_Metroid_Lockdown_Post_Lockdown_Object_Registrant(
        "JMG_Metroid_Lockdown_Post_Lockdown_Object", "LockDownZoneID=0:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Pre_Lockdown_Object> JMG_Metroid_Lockdown_Pre_Lockdown_Object_Registrant(
        "JMG_Metroid_Lockdown_Pre_Lockdown_Object", "LockDownZoneID=0:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Lockdown_Send_Custom_Object> JMG_Metroid_Lockdown_Lockdown_Send_Custom_Object_Registrant(
        "JMG_Metroid_Lockdown_Lockdown_Send_Custom_Object",
        "LockDownZoneID=0:int,PreLockdownStartCustom=0:int,LockdownStartCustom=0:int,PostLockdownStartCustom=0:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Unit_Spawner> JMG_Metroid_Lockdown_Unit_Spawner_Registrant(
        "JMG_Metroid_Lockdown_Unit_Spawner",
        "LockDownZoneID=0:int,PresetName=null:string,MaxSpawnedAtOnce=5:int,MaxTotalSpawned=0:int,SpawnFequency=1:int,RespawnDelay=60:int,RandomRespawnDelay=10:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Controller> JMG_Metroid_Lockdown_Controller_Registrant(
        "JMG_Metroid_Lockdown_Controller",
        "LockDownZoneID=0:int,HUDMsgLockdownStart=19:int,HUDMsgLockdownEnd=18:int,LockDownStartMsg=24:int,LockDownEndMsg=25:int,LockDownStartWavSound=LockDownInitiated.wav:string,LockDownEndWavSound=LockDownAverted.wav:string");
ScriptRegistrant<JMG_Metroid_Lockdown_Light> JMG_Metroid_Lockdown_Light_Registrant("JMG_Metroid_Lockdown_Light",
                                                                                   "LockDownZoneID=0:int");
ScriptRegistrant<JMG_Metroid_Lockdown_zSpawned_Unit> JMG_Metroid_Lockdown_zSpawned_Unit_Registrant(
        "JMG_Metroid_Lockdown_zSpawned_Unit", "LockDownZoneID=0:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Trigger_Zone> JMG_Metroid_Lockdown_Trigger_Zone_Registrant(
        "JMG_Metroid_Lockdown_Trigger_Zone", "LockDownZoneID=0:int,EnemyArriveDelay=5:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Disable_Lockdown_Terminal> JMG_Metroid_Lockdown_Disable_Lockdown_Terminal_Registrant(
        "JMG_Metroid_Lockdown_Disable_Lockdown_Terminal", "LockDownZoneID=0:int");
ScriptRegistrant<JMG_Metroid_Lockdown_zComputer_Console> JMG_Metroid_Lockdown_zComputer_Console_Registrant(
        "JMG_Metroid_Lockdown_zComputer_Console", "LockDownZoneID=0:int");
ScriptRegistrant<JMG_Metroid_Lockdown_zComputer_Console_Disarm_Script> JMG_Metroid_Lockdown_zComputer_Console_Disarm_Script_Registrant(
        "JMG_Metroid_Lockdown_zComputer_Console_Disarm_Script", "ID:int,LockDownZoneID:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Disable_Lockdown_On_Custom_Count> JMG_Metroid_Lockdown_Disable_Lockdown_On_Custom_Count_Registrant(
        "JMG_Metroid_Lockdown_Disable_Lockdown_On_Custom_Count",
        "LockDownZoneID:int,Message:int,Count:int,ResetSpawnersOnComplete=1:int,LockoutLockdownTime=300:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Disable_Lockdown_On_Death> JMG_Metroid_Lockdown_Disable_Lockdown_On_Death_Registrant(
        "JMG_Metroid_Lockdown_Disable_Lockdown_On_Death",
        "LockDownZoneID:int,ResetSpawnersOnComplete=1:int,LockoutLockdownTime=300:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Trigger_On_Message> JMG_Metroid_Lockdown_Trigger_On_Message_Registrant(
        "JMG_Metroid_Lockdown_Trigger_On_Message", "LockDownZoneID=0:int,Message=0:int,EnemyArrivalDelay=0:int");
ScriptRegistrant<JMG_Metroid_Lockdown_Disable_Lockdown_On_Time_Without_Message> JMG_Metroid_Lockdown_Disable_Lockdown_On_Time_Without_Message_Registrant(
        "JMG_Metroid_Lockdown_Disable_Lockdown_On_Time_Without_Message",
        "LockDownZoneID=0:int,ArmMessage=0:int,DisarmTime=1:int,ResetSpawnersOnComplete=1:int,LockoutLockdownTime=300:int");
ScriptRegistrant<JMG_Metroid_Complete_Chapter> JMG_Metroid_Complete_Chapter_Registrant("JMG_Metroid_Complete_Chapter",
                                                                                       "ChapterID:int");
ScriptRegistrant<JMG_Metroid_Defense_Mode> JMG_Metroid_Defense_Mode_Registrant("JMG_Metroid_Defense_Mode", "");
ScriptRegistrant<JMG_Metroid_Defense_Gun_Beacon> JMG_Metroid_Defense_Gun_Beacon_Registrant(
        "JMG_Metroid_Defense_Gun_Beacon", "");
ScriptRegistrant<JMG_Metroid_Defense_Gun_Powerup> JMG_Metroid_Defense_Gun_Powerup_Registrant(
        "JMG_Metroid_Defense_Gun_Powerup", "BeaconID:int,PlayerID:int");
ScriptRegistrant<JMG_Metroid_Defense_Telepad_Beacon> JMG_Metroid_Defense_Telepad_Beacon_Registrant(
        "JMG_Metroid_Defense_Telepad_Beacon", "");
ScriptRegistrant<JMG_Metroid_Defense_Telepad_Powerup> JMG_Metroid_Defense_Telepad_Powerup_Registrant(
        "JMG_Metroid_Defense_Telepad_Powerup", "BeaconID:int,PlayerID:int");
ScriptRegistrant<JMG_Metroid_Defense_Telepad_Zone> JMG_Metroid_Defense_Telepad_Zone_Registrant(
        "JMG_Metroid_Defense_Telepad_Zone", "PlayerID:int,PadID:int");
ScriptRegistrant<JMG_Metroid_Preplaced_Player_Telepad> JMG_Metroid_Preplaced_Player_Telepad_Registrant(
        "JMG_Metroid_Preplaced_Player_Telepad", "");
ScriptRegistrant<JMG_Metroid_Player_Turret_Damage> JMG_Metroid_Player_Turret_Damage_Registrant(
        "JMG_Metroid_Player_Turret_Damage", "");
ScriptRegistrant<JMG_Metroid_Screen_Fade_Red_On_Damage> JMG_Metroid_Screen_Fade_Red_On_Damage_Registrant(
        "JMG_Metroid_Screen_Fade_Red_On_Damage", "");
ScriptRegistrant<JMG_Metroid_Kill_On_Damage_Amounts> JMG_Metroid_Kill_On_Damage_Amounts_Registrant(
        "JMG_Metroid_Kill_On_Damage_Amounts", "Values:string,Delim=@:string");
ScriptRegistrant<JMG_Metroid_Grant_Keycard> JMG_Metroid_Grant_Keycard_Registrant("JMG_Metroid_Grant_Keycard",
                                                                                 "Key=0:int,Grant=1:int");
ScriptRegistrant<JMG_Metroid_AI_Move_To_Position> JMG_Metroid_AI_Move_To_Position_Registrant(
        "JMG_Metroid_AI_Move_To_Position", "TargetSpot:vector3,Speed=1.0:float,ArriveDistance=1.0:float");
ScriptRegistrant<JMG_Metroid_Reset_Screen_Color_On_Enter> JMG_Metroid_Reset_Screen_Color_On_Enter_Registrant(
        "JMG_Metroid_Reset_Screen_Color_On_Enter", "");
ScriptRegistrant<JMG_Metroid_AI_Hunt_Equipment> JMG_Metroid_AI_Hunt_Equipment_Registrant(
        "JMG_Metroid_AI_Hunt_Equipment",
        "MaxEquipmentSeekDistance=250.0:float,ScanFromSpawnPosition=0:int,ReturnToOriginalPosition=0:int,AllowInnate=1:int");
ScriptRegistrant<JMG_Metroid_Base_Defense> JMG_Metroid_Base_Defense_Registrant("JMG_Metroid_Base_Defense",
                                                                               "MinAttackDistance=0.0:float,MaxAttackDistance=300.0:float,AttackTimer=10.0:float");
ScriptRegistrant<JMG_Metroid_Camera_Behavior> JMG_Metroid_Camera_Behavior_Registrant("JMG_Metroid_Camera_Behavior",
                                                                                     "Angle:float,Alarm_ID=0:int,Is_Gun=1:int,Delay=0:float,Alarm_Message=0:int,Alarm_Sound=Beep:string,UpdateRateMultiplier=1.0:float");
ScriptRegistrant<JMG_Metroid_Zone_Create_Explosion> JMG_Metroid_Zone_Create_Explosion_Registrant(
        "JMG_Metroid_Zone_Create_Explosion", "Explosion=null:string");
ScriptRegistrant<JMG_Metroid_Create_Object_Once_On_Enter> JMG_Metroid_Create_Object_Once_On_Enter_Registrant(
        "JMG_Metroid_Create_Object_Once_On_Enter",
        "PresetName=null:string,Position=0.0 0.0 0.0:vector3,Facing=0.0:float");
ScriptRegistrant<JMG_Metroid_Remove_Object_On_Enter> JMG_Metroid_Remove_Object_On_Enter_Registrant(
        "JMG_Metroid_Remove_Object_On_Enter", "ID:int");
ScriptRegistrant<JMG_Metroid_AI_Snow_Mini_Boss> JMG_Metroid_AI_Snow_Mini_Boss_Registrant(
        "JMG_Metroid_AI_Snow_Mini_Boss", "");
ScriptRegistrant<JMG_Metroid_Banshee_Strike_Craft> JMG_Metroid_Banshee_Strike_Craft_Registrant(
        "JMG_Metroid_Banshee_Strike_Craft", "");
ScriptRegistrant<JMG_Metroid_Cheat_Mode_Death_Counter> JMG_Metroid_Cheat_Mode_Death_Counter_Registrant(
        "JMG_Metroid_Cheat_Mode_Death_Counter", "");
ScriptRegistrant<JMG_Metroid_ZeroBug_Tracker> JMG_Metroid_ZeroBug_Tracker_Registrant("JMG_Metroid_ZeroBug_Tracker", "");
ScriptRegistrant<JMG_Metroid_Zone_Damage_Object_While_In_Zone> JMG_Metroid_Zone_Damage_Object_While_In_Zone_Registrant(
        "JMG_Metroid_Zone_Damage_Object_While_In_Zone",
        "DamageRate:float,Warhead:string,Delay=0.1:float,Death_Zone_Type=0:int");
ScriptRegistrant<JMG_Metroid_Attached_Zone_Damage_Object_While_In_Zone> JMG_Metroid_Attached_Zone_Damage_Object_While_In_Zone_Registrant(
        "JMG_Metroid_Attached_Zone_Damage_Object_While_In_Zone",
        "DamageRate:float,Warhead:string,Delay:float,Death_Zone_Type:int");
ScriptRegistrant<JMG_Metroid_Random_Spawn_Point_Node> JMG_Metroid_Random_Spawn_Point_Node_Registrant(
        "JMG_Metroid_Random_Spawn_Point_Node", "SpawnGroup:int");
ScriptRegistrant<JMG_Metroid_Spawn_At_Random_Spawn_Point> JMG_Metroid_Spawn_At_Random_Spawn_Point_Registrant(
        "JMG_Metroid_Spawn_At_Random_Spawn_Point", "SpawnGroup:int");
ScriptRegistrant<JMG_Metroid_Granted_Shield_Powerup> JMG_Metroid_Granted_Shield_Powerup_Registrant(
        "JMG_Metroid_Granted_Shield_Powerup", "GranterPlayerID:int");
ScriptRegistrant<JMG_Metroid_Supply_Mode> JMG_Metroid_Supply_Mode_Registrant("JMG_Metroid_Supply_Mode", "");
ScriptRegistrant<JMG_Metroid_Supply_Shield_Powerup_Beacon> JMG_Metroid_Supply_Shield_Powerup_Beacon_Registrant(
        "JMG_Metroid_Supply_Shield_Powerup_Beacon", "");
ScriptRegistrant<JMG_Metroid_Supply_Shield_Powerup> JMG_Metroid_Supply_Shield_Powerup_Registrant(
        "JMG_Metroid_Supply_Shield_Powerup", "PlayerID:int");
ScriptRegistrant<JMG_Metroid_Supply_Weapon_Spawner_Beacon> JMG_Metroid_Supply_Weapon_Spawner_Beacon_Registrant(
        "JMG_Metroid_Supply_Weapon_Spawner_Beacon", "");
ScriptRegistrant<JMG_Metroid_Supply_Weapon_Spawner_Decoy> JMG_Metroid_Supply_Weapon_Spawner_Decoy_Registrant(
        "JMG_Metroid_Supply_Weapon_Spawner_Decoy", "PlayerID:int,PowerupPreset:string");
ScriptRegistrant<JMG_Metroid_Supply_Weapon_Spawner_Weapon> JMG_Metroid_Supply_Weapon_Spawner_Weapon_Registrant(
        "JMG_Metroid_Supply_Weapon_Spawner_Weapon", "PlayerID:int,PowerupPreset:string");
ScriptRegistrant<JMG_Metroid_Display_Message_On_Pickup> JMG_Metroid_Display_Message_On_Pickup_Registrant(
        "JMG_Metroid_Display_Message_On_Pickup", "PickupMessage=null:string,Color[R|G|B]=255 255 255:vector3");
ScriptRegistrant<JMG_Metroid_Tutorial_Teleport> JMG_Metroid_Tutorial_Teleport_Registrant(
        "JMG_Metroid_Tutorial_Teleport", "");
ScriptRegistrant<JMG_Metroid_Shield_Regen> JMG_Metroid_Shield_Regen_Registrant("JMG_Metroid_Shield_Regen",
                                                                               "Points:float,Delay:float");
ScriptRegistrant<JMG_Metroid_Transform_Into_Cinematic_Object> JMG_Metroid_Transform_Into_Cinematic_Object_Registrant(
        "JMG_Metroid_Transform_Into_Cinematic_Object", "ControlFileName:String");
ScriptRegistrant<JMG_Metroid_Grant_Phone_If_Pkg_Is_Rp2> JMG_Metroid_Grant_Phone_If_Pkg_Is_Rp2_Registrant(
        "JMG_Metroid_Grant_Phone_If_Pkg_Is_Rp2", "");
ScriptRegistrant<JMG_Metroid_Boss_HP_Modifier> JMG_Metroid_Boss_HP_Modifier_Registrant("JMG_Metroid_Boss_HP_Modifier",
                                                                                       "Points:float");
ScriptRegistrant<JMG_Metroid_Set_Is_Rendered_On_Custom> JMG_Metroid_Set_Is_Rendered_On_Custom_Registrant(
        "JMG_Metroid_Set_Is_Rendered_On_Custom", "ShowMessage=-1:int,HideMessage=-1:int,RenderedOnCreate=1:int");
ScriptRegistrant<JMG_Metroid_AI_Shield_Regen> JMG_Metroid_AI_Shield_Regen_Registrant("JMG_Metroid_AI_Shield_Regen", "");
ScriptRegistrant<JMG_Metroid_Invincible_Object> JMG_Metroid_Invincible_Object_Registrant(
        "JMG_Metroid_Invincible_Object", "");
ScriptRegistrant<JMG_Metroid_Damage_Score_Monitor> JMG_Metroid_Damage_Score_Monitor_Registrant(
        "JMG_Metroid_Damage_Score_Monitor", "");
ScriptRegistrant<JMG_Metroid_Death_Zone_Score_Monitor> JMG_Metroid_Death_Zone_Score_Monitor_Registrant(
        "JMG_Metroid_Death_Zone_Score_Monitor", "Death_Zone_Type:int");
ScriptRegistrant<JMG_Metroid_Weapon_Pickup_Score_Monitor> JMG_Metroid_Weapon_Pickup_Score_Monitor_Registrant(
        "JMG_Metroid_Weapon_Pickup_Score_Monitor", "PowerUp_Type:int");
ScriptRegistrant<JMG_Metroid_Count_Jammed_Door_Destruction> JMG_Metroid_Count_Jammed_Door_Destruction_Registrant(
        "JMG_Metroid_Count_Jammed_Door_Destruction", "");
ScriptRegistrant<JMG_Metroid_Zone_Score_Monitor> JMG_Metroid_Zone_Score_Monitor_Registrant(
        "JMG_Metroid_Zone_Score_Monitor", "Zone_Type:int");
ScriptRegistrant<JMG_Metroid_Lockdown_zComputer_Console_Special_Disarm_Script> JMG_Metroid_Lockdown_zComputer_Console_Special_Disarm_Script_Registrant(
        "JMG_Metroid_Lockdown_zComputer_Console_Special_Disarm_Script", "LockDownZoneID:int");
ScriptRegistrant<JMG_Metroid_Mine_Computer_Console_Script_Special> JMG_Metroid_Mine_Computer_Console_Script_Special_Registrant(
        "JMG_Metroid_Mine_Computer_Console_Script_Special", "");
ScriptRegistrant<JMG_Metroid_Poke_Activate_Purchase_Terminal> JMG_Metroid_Poke_Activate_Purchase_Terminal_Registrant(
        "JMG_Metroid_Poke_Activate_Purchase_Terminal", "");
ScriptRegistrant<JMG_Metroid_Update_And_Display_Briefing_On_Enter> JMG_Metroid_Update_And_Display_Briefing_On_Enter_Registrant(
        "JMG_Metroid_Update_And_Display_Briefing_On_Enter", "BriefingId:int");
ScriptRegistrant<JMG_Metroid_Objective_Update_On_Enter> JMG_Metroid_Objective_Update_On_Enter_Registrant(
        "JMG_Metroid_Objective_Update_On_Enter",
        "NewObjectiveID:int,NewObjectiveStringID:int,ObjectiveMarkerObjectID:int,CompleteObjectiveID:int,Delay:float,NewObjectivePriority=1:int");
ScriptRegistrant<JMG_Metroid_Objective_Update_On_Pickup> JMG_Metroid_Objective_Update_On_Pickup_Registrant(
        "JMG_Metroid_Objective_Update_On_Pickup",
        "NewObjectiveID:int,NewObjectiveStringID:int,ObjectiveMarkerObjectID:int,CompleteObjectiveID:int,Delay:float,NewObjectivePriority=1:int");
ScriptRegistrant<JMG_Metroid_Objective_Update_On_Killed> JMG_Metroid_Objective_Update_On_Killed_Registrant(
        "JMG_Metroid_Objective_Update_On_Killed",
        "NewObjectiveID:int,NewObjectiveStringID:int,ObjectiveMarkerObjectID:int,CompleteObjectiveID:int,Delay:float,NewObjectivePriority=1:int");
ScriptRegistrant<JMG_Metroid_Objective_Update_On_Custom> JMG_Metroid_Objective_Update_On_Custom_Registrant(
        "JMG_Metroid_Objective_Update_On_Custom",
        "Custom:int,NewObjectiveID:int,NewObjectiveStringID:int,ObjectiveMarkerObjectID:int,CompleteObjectiveID:int,Delay:float,NewObjectivePriority=1:int");
ScriptRegistrant<JMG_AI_Artillery_Targeting_Fire_At_Custom> JMG_AI_Artillery_Targeting_Fire_At_Custom_Registrant(
        "JMG_AI_Artillery_Targeting_Fire_At_Custom",
        "TargetCustom:int,MinDistance=0.0:float,MaxDistance=999999.0:float,MinAngle=-90.0:float,MaxAngle=90.0:float,UseLowAngleMaxDistance=0.0:float,UseLowAngleWhenAboveMinDistance=1:int,UseLowAngleTargetAboveHeight=9999.9:float");
ScriptRegistrant<JMG_Metroid_AI_Forest_Mini_Boss> JMG_Metroid_AI_Forest_Mini_Boss_Registrant(
        "JMG_Metroid_AI_Forest_Mini_Boss", "");
ScriptRegistrant<JMG_Metroid_SpawnRoom_Objective_Update_On_Enter> JMG_Metroid_SpawnRoom_Objective_Update_On_Enter_Registrant(
        "JMG_Metroid_SpawnRoom_Objective_Update_On_Enter",
        "NewObjectiveID:int,NewObjectiveStringID:int,ObjectiveMarkerObjectID:int,CompleteObjectiveID:int,Delay:float,NewObjectivePriority=1:int");
ScriptRegistrant<JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_At_Custom> JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_At_Custom_Registrant(
...
ScriptRegistrant<JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_Attach> JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_Attach_Registrant(
        "JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_Attach", "LauncherId:int,ExplosionPreset:string");
ScriptRegistrant<JMG_Metroid_Miniboss_Turret> JMG_Metroid_Miniboss_Turret_Registrant("JMG_Metroid_Miniboss_Turret", "");
ScriptRegistrant<JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_Custom_Player> JMG_AI_Artillery_Targeting_Fire_Vehicle_Projectile_Custom_Player_Registrant(
