    ...
    if (type == 9813199) {
        ...
        if (!Is_VTOL(sender)) {
            if (Commands->Get_Distance(Commands->Get_Position(obj), Commands->Get_Position(sender)) <= MaxDis) {
                if (State == 1) {
                    State = 2;
                    Commands->Set_Animation(obj, Get_Parameter("Animation"), false, 0, 0,
    ...
    if (number == 1) {
        if (State == 3) {
            if ((int) (The_Game()->Get_Game_Duration_S() - LastAttack) + 1 >= Get_Int_Parameter("ReturnTime")) {
                Commands->Set_Animation(obj, Get_Parameter("Animation"), false, 0, Get_Float_Parameter("LastFrame"), 0,
                                        false);
                obj->Set_Object_Dirty_Bit(NetworkObjectClass::BIT_RARE, true);
                if (_stricmp(Get_Parameter("Sound"), "0")) {
                    Commands->Create_Sound(Get_Parameter("Sound"), Commands->Get_Position(obj), obj);
        ...
        Commands->Attach_Script(Commands->Create_Object(Get_Parameter("Sensor_Preset"), Commands->Get_Bone_Position(obj,
                                                                                                                    Get_Parameter(
                                                                                                                            "Sensor_Creation_Bone"))),
                                "MDB_Send_Custom_Enemy_Seen", params);
    } else if (number == 3) {
        if (!Commands->Find_Object(LastSeen) ||
            !obj->As_SmartGameObj()->Is_Obj_Visible(Commands->Find_Object(LastSeen)->As_PhysicalGameObj())) {
            Commands->Action_Reset(obj, 100);
        }
    }
}
...
ScriptRegistrant<MDB_Base_Defense_Popup_No_VTOL> MDB_Base_Defense_Popup_No_VTOL_Registrant(
        "MDB_Base_Defense_Popup_No_VTOL",
        "MaxAttackDistance=10.0:float,ReturnTime=10:int,Animation=0:string,LastFrame=-1.0:float,Sensor_Preset=0:string,Sensor_Creation_Bone=0:string,Sound=0:string,Adjust_Aim_For_Infantry=1:int");
ScriptRegistrant<MDB_Base_Defense_Popup_No_VTOL_No_Stealth> MDB_Base_Defense_Popup_No_VTOL_No_Stealth_Registrant(
        "MDB_Base_Defense_Popup_No_VTOL_No_Stealth",
        "MaxAttackDistance=10.0:float,ReturnTime=10:int,Animation=0:string,LastFrame=-1.0:float,Sensor_Preset=0:string,Sensor_Creation_Bone=0:string,Sound=0:string,Adjust_Aim_For_Infantry=1:int,MaxStealthAttackDistance=0.0:float");
ScriptRegistrant<MDB_Base_Defense_Popup_VTOL_Only> MDB_Base_Defense_Popup_VTOL_Only_Registrant(
        "MDB_Base_Defense_Popup_VTOL_Only",
        "MaxAttackDistance=10.0:float,ReturnTime=10:int,Animation=0:string,LastFrame=-1.0:float,Sensor_Preset=0:string,Sensor_Creation_Bone=0:string,Sound=0:string");
ScriptRegistrant<MDB_Base_Defense_Popup> MDB_Base_Defense_Popup_Registrant("MDB_Base_Defense_Popup",
                                                                           "MaxAttackDistance=10.0:float,ReturnTime=10:int,Animation=0:string,LastFrame=-1.0:float,Sensor_Preset=0:string,Sensor_Creation_Bone=0:string,Sound=0:string,Adjust_Aim_For_Infantry=1:int");
ScriptRegistrant<MDB_Base_Defense> MDB_Base_Defense_Registrant("MDB_Base_Defense",
                                                               "MaxAttackDistance=10.0:float,Adjust_Aim_For_Infantry=1:int");
ScriptRegistrant<MDB_Base_Defense_No_VTOL> MDB_Base_Defense_No_VTOL_Registrant("MDB_Base_Defense_No_VTOL",
                                                                               "MaxAttackDistance=10.0:float,Adjust_Aim_For_Infantry=1:int");
ScriptRegistrant<MDB_Base_Defense_VTOL_Only> MDB_Base_Defense_VTOL_Only_Registrant("MDB_Base_Defense_VTOL_Only",
                                                                                   "MaxAttackDistance=10.0:float,Adjust_Aim_For_Infantry=1:int");
