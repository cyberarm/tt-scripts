        ...
        for (int i = 0; i < x; i++) {
            if (Units[i]) {
                if (Units[i]->VehType == VEHTYPE_GROUND) {
                    ...
                    PurchaseSettingsDefClass * p = PurchaseSettingsDefClass::Find_Definition(Units[i]->Type,
...
void ExpVehFacClass::Load(const char *NavalVehicles, const char *NavalCreationPresets, GameObject *script,
                          const char *SupportVehicles) {
    ...
    if (!NavalVehicles) {
        PurchaseSettingsDefClass * p = PurchaseSettingsDefClass::Find_Definition(
                PurchaseSettingsDefClass::TYPE_VEHICLES, (PurchaseSettingsDefClass::TEAM) PTTEAM(Team));
        if (p) {
            unsigned int j;
            for (j = 0; j < 10; j++) {
                if (p->Get_Definition(j)) {
                    UnitInfo *Temp = new UnitInfo(PurchaseSettingsDefClass::TYPE_VEHICLES, j, p->Get_Definition(j),
                                                  false);
                    ...
                    for (k = 0; k < 3; k++) {
                        if (p->Get_Alt_Definition(j, k)) {
                            UnitInfo *Temp2 = new UnitInfo(PurchaseSettingsDefClass::TYPE_VEHICLES, j,
                                                           p->Get_Alt_Definition(j, k), true);
        ...
        p = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_SECRET_VEHICLES,
                                                      (PurchaseSettingsDefClass::TEAM) PTTEAM(Team));
        if (p) {
            for (int j = 0; j < 10; j++) {
                if (p->Get_Definition(j)) {
                    UnitInfo *Temp = new UnitInfo(PurchaseSettingsDefClass::TYPE_SECRET_VEHICLES, j,
                                                  p->Get_Definition(j), false);
                    if (Is_In_Vector(Support, p->Get_Definition(j))) {
                        Temp->Support = true;
                    }
                    if (Get_Vehicle_Definition_Mode_By_ID(p->Get_Definition(j)) == VEHICLE_TYPE_FLYING) {
                    ...
                    for (k = 0; k < 3; k++) {
                        if (p->Get_Alt_Definition(j, k)) {
                            UnitInfo *Temp2 = new UnitInfo(PurchaseSettingsDefClass::TYPE_SECRET_VEHICLES, j,
                                                           p->Get_Alt_Definition(j, k), true);
                            if (Is_In_Vector(Support, p->Get_Alt_Definition(j, k))) {
                                Temp2->Support = true;
                            }
        ...
        p = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_NAVAL,
                                                      (PurchaseSettingsDefClass::TEAM) PTTEAM(Team));
        if (p) {
            for (int j = 0; j < 10; j++) {
                if (p->Get_Definition(j)) {
                    ...
                    for (k = 0; k < 3; k++) {
                        if (p->Get_Alt_Definition(j, k)) {
                            UnitInfo *Temp2 = new UnitInfo(PurchaseSettingsDefClass::TYPE_NAVAL, j,
                                                           p->Get_Alt_Definition(j, k), true);
                            if (Is_In_Vector(Support, p->Get_Alt_Definition(j, k))) {
                                Temp2->Support = true;
                            }
        ...
        p = PurchaseSettingsDefClass::Find_Definition(PurchaseSettingsDefClass::TYPE_AIR,
                                                      (PurchaseSettingsDefClass::TEAM) PTTEAM(Team));
        if (p) {
            for (int j = 0; j < 10; j++) {
                if (p->Get_Definition(j)) {
                    ...
                    for (k = 0; k < 3; k++) {
                        if (p->Get_Alt_Definition(j, k)) {
                            UnitInfo *Temp2 = new UnitInfo(PurchaseSettingsDefClass::TYPE_AIR, j,
                                                           p->Get_Alt_Definition(j, k), true);
                            if (Is_In_Vector(Support, p->Get_Alt_Definition(j, k))) {
                                Temp2->Support = true;
                            }
...
int
ExpVehFacClass::Purchase_Hook(BaseControllerClass *base, GameObject *purchaser, unsigned int cost, unsigned int preset,
                              const char *data) {
    ...
    if (Temp->VehType == VEHTYPE_NAVAL) {
        ...
        if (ExpVehFac[Team]->Is_Naval_Building) {
            ...
            if (ExpVehFac[Team]->Naval_Build_Time[0] > 0.0f) {
                ...
                Commands->Send_Custom_Event(0, ExpVehFac[Team]->Script, 129310, Team,
                                            ExpVehFac[Team]->Naval_Build_Time[0]);
            }
            if (ExpVehFac[Team]->Naval_Build_Time[1] > 0.0f) {
                ExpVehFac[Team]->Set_Naval_Currently_Building(true);
                Commands->Send_Custom_Event(0, ExpVehFac[Team]->Script, 129311, Team,
                                            ExpVehFac[Team]->Naval_Build_Time[1]);
            }
        ...
        if (ExpVehFac[Team]->Is_Flying_Building) {
            ...
            if (ExpVehFac[Team]->Flying_Build_Time[0] > 0.0f) {
                Set_Currently_Building(true, Team);
                Commands->Send_Custom_Event(0, ExpVehFac[Team]->Script, 129310, Team,
                                            ExpVehFac[Team]->Flying_Build_Time[0]);
            }
            if (ExpVehFac[Team]->Flying_Build_Time[1] > 0.0f) {
                ExpVehFac[Team]->Set_Flying_Currently_Building(true);
                Commands->Send_Custom_Event(0, ExpVehFac[Team]->Script, 129312, Team,
                                            ExpVehFac[Team]->Flying_Build_Time[1]);
            }
            ExpVehFac[Team]->LastFlyVehID = preset;
            ExpVehFac[Team]->LastFlyVehOwner = Commands->Get_ID(purchaser);
            Commands->Attach_Script(
                    Commands->Create_Object("Invisible_Object", ExpVehFac[Team]->Get_Flying_Landing_Position()),
                    "Test_Cinematic", Team ? "ExpVehFac_1.txt" : "ExpVehFac_0.txt");
            return 0;
        }
        return 2;
    } else {
        ...
        if (ExpVehFac[Team]->Is_Ground_Building) {
            ...
            Create_Vehicle(Get_Definition_Name(preset), 5.0f * Get_Build_Time_Multiplier(base->Get_Player_Type()),
                           purchaser, base->Get_Player_Type());
            if (ExpVehFac[Team]->Ground_Build_Time[0] > 0.0f) {
                Set_Currently_Building(true, Team);
                Commands->Send_Custom_Event(0, ExpVehFac[Team]->Script, 129310, Team,
                                            ExpVehFac[Team]->Ground_Build_Time[0]);
            }
            if (ExpVehFac[Team]->Ground_Build_Time[1] > 0.0f) {
                ExpVehFac[Team]->Set_Ground_Currently_Building(true);
                Commands->Send_Custom_Event(0, ExpVehFac[Team]->Script, 129313, Team,
                                            ExpVehFac[Team]->Ground_Build_Time[1]);
            }
            return 0;
        }
        return 2;
...
void MDB_ExpVehFac_Naval::Created(GameObject *obj) {
    ...
    if (!ExpVehFac[Team]->Loaded) {
        if (strstr(Get_Parameter("Creation_Preset"), ";")) {
            ExpVehFac[Team]->Load(Get_Parameter("Naval_Vehicles"), Get_Parameter("Creation_Preset"), obj,
                                  Get_Parameter("Support_Vehicles"));
        } else {
...
void MDB_ExpVehFac_Cinematic::Created(GameObject *obj) {
    ...
    GameObject * Veh = Commands->Create_Object(Get_Definition_Name(ExpVehFac[Team]->LastFlyVehID),
...
ScriptRegistrant<MDB_ExpVehFac_Helipad> MDB_ExpVehFac_Helipad_Registrant("MDB_ExpVehFac_Helipad",
                                                                         "Landing_Preset=Landing:string,Build_Time=1.0:float,Flying_Build_Time=1.0:float,Remove_on_Death=0:int,Support_Vehicles=Veh1;Veh2;etc.:string,Sidebar=0:int");
ScriptRegistrant<MDB_ExpVehFac_Naval> MDB_ExpVehFac_Naval_Registrant("MDB_ExpVehFac_Naval",
                                                                     "Creation_Preset=Preset:string,Naval_Vehicles=Veh1;Veh2;etc.:string,Build_Time=1.0:float,Naval_Build_Time=1.0:float,Remove_on_Death=0:int,Support_Vehicles=Veh1;Veh2;etc.:string,Sidebar=0:int");
ScriptRegistrant<MDB_ExpVehFac_Vehicle_Factory> MDB_ExpVehFac_Vehicle_Factory_Registrant(
        "MDB_ExpVehFac_Vehicle_Factory",
        "Build_Time=1.0:float,Ground_Build_Time=1.0:float,Remove_on_Death=0:int,Support_Vehicles=Veh1;Veh2;etc.:string,Sidebar=0:int");
ScriptRegistrant<MDB_ExpVehFac_Cinematic> MDB_ExpVehFac_Cinematic_Registrant("MDB_ExpVehFac_Cinematic",
                                                                             "Team=0:int,Time:float");
ScriptRegistrant<MDB_ExpVehFac_Cinematic_Vehicle> MDB_ExpVehFac_Cinematic_Vehicle_Registrant(
        "MDB_ExpVehFac_Cinematic_Vehicle", "");
