...
inline bool M_Has_Timer(GameObject * obj, ScriptClass * script, int
number)
{
return
Has_Timer(obj, script, number
);
}

// -------------------------------------------------------------------------------------------------

inline void M_Stop_Timer2(GameObject * obj, ScriptClass * script, int
number)
{
...
inline bool M_Test_Raycast_Collision(const Vector3 &pos1, const Vector3 &pos2, const bool checkDynamicObjects,
                                     Vector3 *contactPoint = nullptr, GameObject *compareObj = nullptr) {
    Vector3 newPoint;
    if (!contactPoint)
...
bool Can_Move_To(GameObject * obj, Vector3
position)
{
MoveablePhysClass *phys = obj->As_PhysicalGameObj()
                          ? obj->As_PhysicalGameObj()->Peek_Physical_Object()->As_MoveablePhysClass() : nullptr;
if (
phys &&phys
->
Can_Teleport(Matrix3D(position)
))
return true;
return false;
}
...
float Calculate_Relative_Facing_Diff(GameObject * obj, Vector3
targetLocation)
{
...
bool Get_Random_Pathfind_Spot_Around_Position(GameObject * obj,
const Vector3 &centerPos,
float minDist,
float maxDist, Vector3
*returnPos,
...
if (!
Get_Random_Pathfind_Spot(centerPos, maxDist, &pathfindPos
))
{
// Try to get the closest sector mid
if (!
Get_Closest_Pathfind_Spot(centerPos,
15.0f, &pathfindPos))
return false;
}
else
...
if (
Can_Move_To(obj, *returnPos
) && (!target || !
M_Test_Raycast_Collision(Commands
->
Get_Position(obj)
+ Vector3(0, 0, 1.5f), Commands->
Get_Position(target)
+ Vector3(0, 0, 1.5f), false)))
{
if (obj->
...
bool Get_Random_Pathfind_Spot_Around_Self(GameObject * obj, float
minDist,
float maxDist, Vector3
*returnPosition,
GameObject *target = nullptr
...
return
Get_Random_Pathfind_Spot_Around_Position(obj, pos, minDist, maxDist, returnPosition, target
);
}
...
Order_Points_By_Distance(const DynamicVectorClass<Vector3> &inArray, const Vector3 &compareToPoint, float maxDistance) {
...
float Calculate_Weapon_DPS(WeaponClass *weapon, float damage) {
    ...
    return (clipSize * damage) / ((clipSize / weapon->PrimaryAmmoDefinition->RateOfFire) +
                                  (clipSize != -1 ? weapon->Get_Definition()->ReloadTime() +
                                                    weapon->PrimaryAmmoDefinition->ChargeTime() : 0));
}
...
void Get_Equipped_Weapon_Range_Preferences(GameObject * obj, int & maxAttackRange, int & preferredAttackRange,
                                           int & maxAttackRangeSecondary, int & preferredAttackRangeSecondary) {
    if (obj->As_PhysicalGameObj() && obj->As_PhysicalGameObj()->As_ArmedGameObj()) {
        ...
        if (auto weapon = armedObj->Get_Weapon()) {
            ...
            if (weapon->SecondaryAmmoDefinition) {
                Calculate_Ammo_Preferences(weapon->SecondaryAmmoDefinition, maxAttackRangeSecondary,
                                           preferredAttackRangeSecondary);
            } else {
...
class MS_AI_Marker_Bomber_Aircraft : public ScriptImpClass {
    void Created(GameObject *obj) override {
        if (!obj->As_VehicleGameObj() || Get_Vehicle_Mode(obj) != VEHICLE_TYPE_FLYING) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: Only flying vehicles can use the MS_AI_Bomber_Aircraft script. Destroying script...\n",
...
class MS_AI_Marker_Deployable : public ScriptImpClass {
    void Created(GameObject *obj) override {
        if (!Is_Script_Attached(obj, Get_Deploy_Script_Name())) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: The script '%hs' does not exist on this object. Destroying script...\n",
...
class MS_AI_Marker_No_Jump : public ScriptImpClass {
    void Created(GameObject *obj) override {
        if (!obj->As_SoldierGameObj()) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: Only soldiers can use the MS_AI_Marker_No_Jump script. Destroying script...\n",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
            M_Console_Output(msg.Peek_Buffer());
            Destroy_Script();
            return;
...
class MS_AI_Marker_Refiller_Vehicle : public ScriptImpClass {
    void Created(GameObject *obj) override {
        if (!obj->As_VehicleGameObj()) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: Only vehicles can use the MS_AI_Marker_Refiller_Vehicle script. Destroying script...\n",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
            M_Console_Output(msg.Peek_Buffer());
            Destroy_Script();
            return;
...
class MS_AI_Marker_Vehicle_Repair_Zone : public ScriptImpClass {
    void Created(GameObject *obj) override {
        if (!obj->As_ScriptZoneGameObj()) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: Only script zones can use the MS_AI_Marker_Vehicle_Repair_Zone script. Destroying script...\n",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
            M_Console_Output(msg.Peek_Buffer());
            Destroy_Script();
            return;
...
class MS_AI_Marker_Vehicle_Search_Ignore : public ScriptImpClass {
    void Created(GameObject *obj) override {
        if (!obj->As_VehicleGameObj()) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: Only vehicles can use the MS_AI_Marker_Vehicle_Search_Ignore script. Destroying script...\n",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
            M_Console_Output(msg.Peek_Buffer());
            Destroy_Script();
            return;
...
class MS_AI_Marker_Transport_Vehicle_Override : public ScriptImpClass {
    void Created(GameObject *obj) override {
        if (!obj->As_VehicleGameObj()) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: Only vehicles can use the MS_AI_Marker_Transport_Vehicle_Override script. Destroying script...\n",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
            M_Console_Output(msg.Peek_Buffer());
            Destroy_Script();
            return;
...
private:
    ...
    void Created(GameObject *obj) override {
        ...
        if ((!obj->As_BuildingGameObj() && !obj->As_PhysicalGameObj()) || obj->As_SoldierGameObj() ||
            (obj->As_VehicleGameObj() && !obj->As_VehicleGameObj()->Is_Turret())) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: Only BuildingGameObjects, turret type Vehicles and Objects can use the MS_AI_Building script. Destroying script...\n",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
            M_Console_Output(msg.Peek_Buffer());
            Destroy_Script();
            return;
            ...
            StringClass params;
            params.Format("%d,%d,%d,%d,%d,%d,%d,%s", (int) (Commands->Get_Max_Health(obj) * ((float) (100 -
                                                                                                      (Get_Int_Parameter(
                                                                                                              "DamagePercentToRepair"))) /
                                                                                             100)), 0,
                          CUSTOM_AI_REPAIR_NEEDED, 0, -1, 0, 0, "false");
            Commands->Attach_Script(obj, "MS_Health_Send_Custom", params.Peek_Buffer());
            params.Format("%d,%d,%d,%d,%d,%d,%d,%s", (int) (Commands->Get_Max_Health(obj)), -1, 0, 0, 0,
                          CUSTOM_AI_REPAIR_COMPLETE, 0, "false");
            Commands->Attach_Script(obj, "MS_Health_Send_Custom", params.Peek_Buffer());
        }
    }
    ...
    void Init(GameObject *obj) {
        ...
        if (obj->As_BuildingGameObj()) {
            ...
            if (closestPoly == Vector3()) {
                if (BuildingDebugLocationsDisplayEnabled) {
                    StringClass debug;
                    debug.Format("msg %s: ATTENTION: This building has no polygons (only a building controller)",
        ...
        auto &boundingBox = obj->As_BuildingGameObj() ? obj->As_BuildingGameObj()->Get_Bounds()
                                                      : obj->As_PhysicalGameObj()->Peek_Model()->Get_Bounding_Box();
        ...
        if (customWeakpointPresetName[0] != '\0') {
            if (auto customMct = Find_Nearest_Preset(Commands->Get_Position(obj), customWeakpointPresetName)) {
                ...
                for (int i = 1; i < 4; i++) {
                    ...
                    if (int boneIdx = customMct->As_PhysicalGameObj()->Peek_Model()->Get_Bone_Index(
                            boneName.Peek_Buffer())) {
            ...
            for (int i = 1; i < bldRadius + 5; i += 1) {
                isWithinPathfindGrid = Get_Closest_Pathfind_Spot_Size(bldCenter, (float) i, &mctObjectiveLocation,
                                                                      0.5f);
            ...
            if (!isWithinPathfindGrid) {
                if (BuildingDebugLocationsDisplayEnabled) {
                    StringClass debug;
                    debug.Format("msg %s: WARNING: Unable to find a suitable MCT/weakpoint objective location for bots",
                                 Commands->Get_Preset_Name(obj));
                    M_Console_Input(debug.Peek_Buffer());
                }
        ...
        if (hasWeakpoint) {
            ...
            for (int i = 0; i < 7; i++) {
                ...
                if (isWithinPathfindGrid && Can_Generic_Soldier_Teleport(testPos) &&
                    !M_Test_Raycast_Collision(helpPos, testPos, false)) {
                    isSuitable = true;
                    break;
                }
        ...
        auto skinName = ArmorWarheadManager::Get_Armor_Name(
                obj->As_DamageableGameObj()->Get_Defense_Object()->Get_Skin());
        ...
        centralTargetGround = Commands->Create_Object("Invisible_Object",
                                                      Vector3(boundingBox.Center.X, boundingBox.Center.Y,
                                                              averageGroundHeight));
        ...
        if (!isReachableOnGround) {
            Get_Closest_Pathfind_Spot_Size(boundingBox.Center, bldRadius + 50.0f, &soldierAttackObjectiveLocation,
                                           0.5f);
            Vector3 helper;
            int count = 0;
            while (count < 50 &&
                   !M_Test_Raycast_Collision(soldierAttackObjectiveLocation, boundingBox.Center, false, &helper, obj) &&
                   !M_Test_Raycast_Collision(soldierAttackObjectiveLocation,
                                             boundingBox.Center + Vector3(0, 0, boundingBox.Extent.Z), false, &helper,
                                             obj)) {
                count++;
                helper = soldierAttackObjectiveLocation -
                         (boundingBox.Center - soldierAttackObjectiveLocation).Normalized() * (float) count;
        ...
        params.Format("%d,%d,%d,%d,%d,%d,%d,%s", Get_Object_Type(obj) ? 0 : 1, dp88_AI_Objective::TYPE_OFFENSIVE, 1, 5,
                      0, 0, 0, Commands->Get_Preset_Name(obj));
        ...
        for (int i = 0; i < 4; i++) {
            if (Get_Closest_Pathfind_Spot_Size(
                    Vector3(boundingBox.Center.X, boundingBox.Center.Y, averageGroundHeight) + offsetVectors[i],
                    offsetAmount - 1.0f, &objectivePos, 2.5f)) {
                ...
                Vector3 helper;
                if (M_Test_Raycast_Collision(objectivePos, checkPoint, false, &helper) &&
                    abs(helper.Z - objectivePos.Z) > 2.0f)
                ...
                params.Format("%d,%d,%d,%d,%d,%d,%d,%s", Get_Object_Type(obj) ? 0 : 1,
                              dp88_AI_Objective::TYPE_OFFENSIVE, 10, 0, 5, 5, 5, Commands->Get_Preset_Name(obj));
        ...
        if (BuildingDebugLocationsDisplayEnabled) {
            StringClass debug;
            debug.Format("msg %s targets: %d, veh objectives: %d (bounding box: %.2f/%.2f/%.2f)",
                         Commands->Get_Preset_Name(obj), exteriorTargets.Count(), vehObjectivesCount,
    ...
    void Generate_Targets(GameObject *obj, const AABoxClass &boundingBox) {
        ...
        // Corners next
        Vector3 sg5 = boundingBox.Center +
                      Vector3(boundingBox.Extent.X + sideOffset, boundingBox.Extent.Y + sideOffset, heightStart);
        Vector3 sg6 = boundingBox.Center +
                      Vector3(-boundingBox.Extent.X - sideOffset, -boundingBox.Extent.Y - sideOffset, heightStart);
        Vector3 sg7 = boundingBox.Center +
                      Vector3(boundingBox.Extent.X + sideOffset, -boundingBox.Extent.Y - sideOffset, heightStart);
        Vector3 sg8 = boundingBox.Center +
                      Vector3(-boundingBox.Extent.X - sideOffset, boundingBox.Extent.Y + sideOffset, heightStart);
    ...
    void Place_Target_Pair(GameObject *obj, Vector3 &sg1, Vector3 &sg2, const AABoxClass &boundingBox) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            Vector3 checkPoint =
                    Vector3(point.X, point.Y, boundingBox.Center.Z) + Vector3(0, 0, 2 * -boundingBox.Extent.Z + 2.0f);

            if (M_Test_Raycast_Collision(startingPoint, checkPoint, false, &helper) &&
                abs(helper.Z - startingPoint.Z) > 3.0f) {
    ...
    // TODO: rename
    void Raycast_Target_Pair(GameObject *obj, Vector3 &sg1, Vector3 &sg2, const Vector3 &closestPoly1,
                             const Vector3 &closestPoly2) {
        for (int i = 0; i < 2; i++) {
            ...
            if (M_Test_Raycast_Collision(p1, p2, !obj->As_BuildingGameObj(), &contactPoint, obj)) {
                ...
                Vector3 otherPoint1 =
                        closestPoly - (p1 - closestPoly).Normalized() * Vector3::Distance(p1, closestPoly);
                Vector3 otherPoint2 =
                        buildingCtrlPos - (p1 - buildingCtrlPos).Normalized() * Vector3::Distance(p1, buildingCtrlPos);
    ...
    void Validate_New_Target(GameObject *obj, const Vector3 &newTargetLocation, const Vector3 &raycastedFromLocation) {
        float min_dist = (obj->As_BuildingGameObj() ? (exteriorTargets.Count() >= 8 ? 3.0f : 1.0f) : (
                exteriorTargets.Count() >= 4 ? 1.0f : 0.5f));
        ...
        Vector3 targetWatchLocation =
                exteriorTargetLocation + (raycastedFromLocation - exteriorTargetLocation).Normalized() * 0.2f;
    ...
    void Custom(GameObject *obj, int type, int param, GameObject *sender) override {
        if (type == CUSTOM_AI_REPAIR_NEEDED) {
            if (!objectiveRepairLocationObj && isReachableOnGround) {
                ...
                StringClass params;
                params.Format("%d,%d,%d,%d,%d,%d,%d,%s", Get_Object_Type(obj), dp88_AI_Objective::TYPE_ENGINEERING, 1,
                              5, 0, 0, 0, Commands->Get_Preset_Name(obj));
    ...
    void Damaged(GameObject *obj, GameObject *damager, float amount) override {
        ...
        bool isWithinPathfindGrid = Get_Closest_Pathfind_Spot_Size(Commands->Get_Position(damager), 2.0f, &targetPos,
                                                                   2.0f);
        if (!isWithinPathfindGrid) {
            dp88_customAI::TargetType targetType = dp88_customAI::GetTargetType(damager);
            if (targetType != dp88_customAI::FLYING &&
                !Get_Closest_Pathfind_Spot_Size(Commands->Get_Position(damager), 20.0f, &targetPos, 2.0f)) {
                return;
            }
                ...
                if (!M_Test_Raycast_Collision(enemyLocation, enemyLocation + Vector3(0, 0, -100), false, &helper) ||
                    ...
                    (!Get_Closest_Pathfind_Spot_Size(helper, 10.0f, &targetPos, 2.0f) &&
                     !Get_Closest_Pathfind_Spot_Size(helper, 50.0f, &targetPos, 2.0f))) {
                    return;
                }
            } else {
...
class MS_AI_Behaviour : public dp88_AI_Tank_Offensive, public Pathfind_Help_Implementer {
    ...
    void Post_Created(GameObject *obj) {
        ...
        if (auto deployParamsScript = (MS_AI_Marker_Deployable *) (Find_Script_On_Object(obj,
                                                                                         "MS_AI_Marker_Deployable"))) {
            deployScript = (JFW_Key_Hook_Base * )(
                    Find_Script_On_Object(obj, deployParamsScript->Get_Deploy_Script_Name()));
        ...
        const float neededSightRange = max(max(weaponMaxRange + 5, defSightRange),
                                           100); // +5: Sight range needs to be larger than the weapon max range
    ...
    void Enemy_Seen(GameObject *obj, GameObject *enemy) override {
        ...
        if ((isLimitedAmmoUnit && !totalAmmo) ||
            (obj->As_SoldierGameObj() && Is_Soldier_Busy(obj->As_SoldierGameObj()))) {
            return;
        }
    ...
    bool IsValidTarget(GameObject *obj, GameObject *target) override {
        if (obj->As_VehicleGameObj()) {
            ...
            const bool isAimFree = getPrimary(target) ? (isPrimaryAmmoTurning || isPrimaryAmmoGravityAffected) : (
                    isSecondaryAmmoTurning || isSecondaryAmmoGravityAffected);
    ...
    void AttackTarget(GameObject *obj, GameObject *target) override {
        ...
        const float tempAttackError = (targetType == BUILDING && weapon) ? (weapon->Get_Style() == 0 ? DEG2RAD(200) :
                                                                            attackError / 5) : (attackError *
                                                                                                personality->Get_Random(
                                                                                                        BotPersonality::ATTACK_ERROR_MULTIPLIER));
        // If C4 type, attack center of the c4, because the origin point is the lowlow part and shooting it would miss
        if (target->As_PhysicalGameObj() &&
            (target->As_PhysicalGameObj()->As_C4GameObj() || target->As_PhysicalGameObj()->As_BeaconGameObj())) {
        ...
        params.AttackForceFire =
                currentDistance2 < maxRange ? (m_bTargetPrimaryFire ? isPrimaryAmmoTurning : isSecondaryAmmoTurning)
                                            : false;
        ...
        if (evadePosition != Vector3()) {
            ...
            m_bMovingToTarget = (preferredRange < getDistance(obj, target) || canSquishTarget) &&
                                (!isDeployed || !isDeployedStationary);
            ...
            if (isTurretRotationLimited) {
                ...
                if ((angle < turretTurnMin || angle > turretTurnMax) &&
                    (!(m_bTargetPrimaryFire ? isPrimaryAmmoTurning : isSecondaryAmmoTurning) ||
                     (abs(angle) > DEG2RAD(20) && Vector3::Distance(objPos, targetPos) > radius + 10.0f)))
                    needsRotating = true;
            }
            ...
            if (targetType == REPAIRABLE) {
                ...
                Vector3 pos = isVTOL && isTurretRotationLimited ? (targetPos +
                                                                   (Vector3(objPos.X, objPos.Y, targetPos.Z) -
                                                                    targetPos).Normalized() * (float) preferredRange)
                                                                : Lerp(targetPos, objPos, ratio);
                ...
                if (targetType == BUILDING) {
                    Get_Closest_Pathfind_Spot_Size(pos, min(20.0f, preferredRange), &pos,
                                                   obj->As_SoldierGameObj() ? 0.5f : 5.0f);
                    params.Set_Movement(pos, 1.0f, 1.0f, moveCrouched);
                } else {
                    float teleportZCorrection = obj->As_VehicleGameObj() ?
                                                obj->As_VehicleGameObj()->Peek_Model()->Get_Bounding_Box().Extent.Z * 2
                                                                         : 0;
                    if (canSquishTarget) {
                    ...
                    else if ((!Get_Random_Pathfind_Spot(pos, (preferredRange / 2 - 2.0f), &pos) && !isTargetFlying) ||
                             !Can_Move_To(obj, pos + Vector3(0, 0, teleportZCorrection)) || preferredRange == 0 ||
                             pathfindFailCount > 4) {
                ...
                if (Get_Random_Pathfind_Spot_Around_Position(obj, pos, 0, (float) preferredRange / 2, &pos))
                    ...
                    // Can't reach: continue to move towards objective (if one exists and not there yet)
                         ...
                         m_pCurrentObjective->GetRange() < getDistance(obj, m_pCurrentObjective->GetGameObject()))
                    params.Set_Movement(Commands->Get_Position(m_pCurrentObjective->GetGameObject()), 1.0f,
                                        (float) (m_pCurrentObjective->GetRange()));
            }
            // COPIED ORIGINAL FUNCTION, MODIFIED UNTIL HERE
        }
            // COPIED ORIGINAL FUNCTION, MODIFIED FROM HERE
    ...
    dp88_AI_Objective *ChooseNewObjective(GameObject *obj) override {
        ...
        dp88_AI_Objective *pObjective = dp88_AI_Objective::GetBestObjective(obj, objType, modifier_distance,
                                                                            ignoredObjectives);
    ...
    void Custom(GameObject *obj, int type, int param, GameObject *sender) override {
        if (type == CUSTOM_DEPLOY_STATE_CHANGED && isDeployable) {
            ...
            GoToPatrolPoint(obj);
                   ...
                   type == CUSTOM_AI_ORDER_DEFEND || type == CUSTOM_AI_GO_TO_REFILL ||
                   type == CUSTOM_AI_GO_SWITCH_OBJECTIVE ||
            ...
            switch (type) {
                ...
                case CUSTOM_AI_ORDER_ATTACK: {
                    if (auto taskTarget = overridingTask.Get_Ptr()) {
                        ...
                        if (!M_Test_Raycast_Collision(Commands->Get_Position(obj) + Vector3(0, 0, 1.5f),
                                                      Commands->Get_Position(taskTarget) + Vector3(0, 0, 1.5f), false))
    ...
    void Timer_Expired(GameObject *obj, int number) override {
        ...
        if (number == TIMER_AI_POST_CREATED) {
            ...
            if (!obj->As_SmartGameObj()->Get_Action()->Is_Acting() && state != GOING_TO_RELOAD &&
                state != GOING_TO_REFILL && state != GOING_TO_SWITCH_OBJECTIVE)
    ...
    void Damaged(GameObject *obj, GameObject *damager, float amount) override {
        ...
        if (damager && isDamageDetectionEnabled &&
            !M_Test_Raycast_Collision(Commands->Get_Position(obj) + Vector3(0, 0, 1.5f),
                                      Commands->Get_Position(damager) + Vector3(0, 0, 1.5f), false)) {
    ...
    void Handle_Idle_State(GameObject *obj) {
        ...
        if (nullptr == m_pCurrentObjective && (objectiveType == dp88_AI_Objective::TYPE_DEFENSIVE ||
                                               objectiveType == dp88_AI_Objective::TYPE_OFFENSIVE) &&
    ...
    void Handle_Target_Reaction_Think(GameObject *obj, GameObject *target) {
        ...
        if (obj->As_SoldierGameObj() && obj->As_SoldierGameObj()->Get_Weapon() &&
            obj->As_SoldierGameObj()->Get_Weapon_Bag()->Get_Count() > 2) {
            WeaponClass *weapon = obj->As_SoldierGameObj()->Get_Weapon();
            if (!weapon->Get_Total_Rounds() || objectiveType == dp88_AI_Objective::TYPE_ENGINEERING ||
                thinkCount % 2 == 0) {
                float scaledDamage = Calculate_Average_Scaled_Damage(weapon, target);

                bool shouldChangeWeapon =
                        !weapon->Get_Total_Rounds() || (targetType != REPAIRABLE && scaledDamage <= 0) ||
                        (targetType == REPAIRABLE && (scaledDamage) >= 0);
                ...
                if (shouldChangeWeapon || thinkCount % 4 == 0) {
                    ...
                    bool isC4UsageAllowed = (targetType == BUILDING && m_pCurrentObjective &&
                                             (m_pCurrentObjective->GetRange() + 1) >
                                             getDistance(obj, m_pCurrentObjective->GetGameObject()) &&
                                             distFromTarget < 4) ||
                    ...
                    for (int index = 0; index < weapon_count; index++) {
                        ...
                        if (!weapon || !weapon->Get_Total_Rounds() || (weapon->Get_Style() == 0 && !isC4UsageAllowed) ||
                            (weapon->PrimaryAmmoDefinition->Range <= 15 && distFromTarget >= 20))
                            continue;
                        ...
                        if (targetType != REPAIRABLE || Get_Object_Type(target) != Get_Object_Type(obj) ? (
                                tempDamagePerSec > damagePerSec) : (tempDamagePerSec < damagePerSec)) {
                    ...
                    if (originalWeapon != nextWeapon) {
                        ...
                        m_primary_prefRange =
                                primary_maxRange < BOT_VERY_LONG_WEAPON_RANGE ? (int) (primary_maxRange / 2) :
                                primary_maxRange - 10;
        ...
        if (!gettingAround && !deployOrderOverride) {
            ...
            const bool targetCanSquishMe = isSquishableUnit && target->As_VehicleGameObj() &&
                                           target->As_VehicleGameObj()->Get_Squish_Velocity() < 10;
            ...
            if (!IsValidTarget(obj, target) ||
                ...
                (ShouldPursueTarget(obj, target) &&
                 ((isDeployed && isDeployedStationary) || Should_Deploy(target) || canSquishTarget ||
                  ...
                  (!M_Has_Timer(obj, this, TIMER_AI_UNDER_ATTACK) && !isTurretRotationLimited &&
                   (!hasWeapon || targetType == BUILDING || targetType == REPAIRABLE))
                 )
                )
                    )
                return;
            ...
            if (obj->As_SoldierGameObj() || (state != GOING_TO_REFILL && state != GOING_TO_SWITCH_OBJECTIVE &&
                                             (!isTurretRotationLimited || overridingTask))) {
                ...
                if (targetCanSquishMe && personality->Get_Random(BotPersonality::SQUISH_EVADE_CHANCE)) {
                    ...
                    const float minEvadeDistance =
                            biggerSideHalf + personality->Get_Random(BotPersonality::SQUISH_EVADE_DISTANCE);
                    ...
                    if (minEvadeDistance > distFromTarget) {
						...
						*/
                        ...
                        const float YOffset =
                                obj_box.Extent.X + (isTooClose ? obj_box.Extent.Y * 3.0f : minEvadeDistance) + 3;
                        ...
                        const bool isWithinPathfindGrid1 = Get_Closest_Pathfind_Spot_Size(Vector3(helpPos1), 1.0f,
                                                                                          &helpPos1, 2.0f);
                        const bool isWithinPathfindGrid2 = Get_Closest_Pathfind_Spot_Size(Vector3(helpPos2), 1.0f,
                                                                                          &helpPos2, 2.0f);
                        if (isWithinPathfindGrid1 && isWithinPathfindGrid2) {
                            // Choose the closer one
                            evadePosition = Vector3::Distance_Squared(objPos, helpPos1) <
                                            Vector3::Distance_Squared(objPos, helpPos2) ? helpPos1 : helpPos2;
                        } else {
                ...
                if (!isSquishEvading) {
                    if ((ShouldPursueTarget(obj, target) || M_Has_Timer(obj, this, TIMER_AI_UNDER_ATTACK)) &&
                        (targetType != BUILDING ? inPrefRange : inRange) &&
                        Get_Random_Pathfind_Spot_Around_Self(obj, minDist, maxDist, &pos,
                                                             targetType != BUILDING ? target : nullptr))
                        evadePosition = pos;
                    else if (objectiveObj && (m_pCurrentObjective->GetRange() + 1) <
                                             getDistance(obj, m_pCurrentObjective->GetGameObject())) {
                        minDist = 0.5f;
    ...
    void Handle_Range_Checks(GameObject *obj) {
        if (overridingTask) {
            ...
            if (nullptr == m_pCurrentObjective || (overridingTaskType == 7 && taskObj && (
                    ...
                    (taskObj->As_VehicleGameObj() && !taskObj->As_VehicleGameObj()->Get_Occupant_Count() &&
                     !Is_Harvester(taskObj)))
            )) {
            ...
            } else if (state == GOING_TO_REFILL || state == GOING_TO_SWITCH_OBJECTIVE ||
                       state == GOING_TO_EXIT_VEHICLE_POINT) {
                if (auto objectiveGO = m_pCurrentObjective->GetGameObject()) {
                    if (m_pCurrentObjective->GetRange() + 1 >= getDistance(obj, objectiveGO)) {
                        auto custom = state == GOING_TO_REFILL || state == GOING_TO_SWITCH_OBJECTIVE
                                      ? CUSTOM_AI_ARRIVED_TO_REFILL : CUSTOM_AI_ARRIVED_TO_EXIT_VEHICLE;
                        Commands->Send_Custom_Event(obj, obj, custom, 0, 0.0f);
                    }
                }
            } else if (state == GOING_TO_RALLY_POINT && !arrivedToOverridingTask) {
                if (auto objectiveGO = m_pCurrentObjective->GetGameObject()) {
                    if (m_pCurrentObjective->GetRange() + 10 + (isVTOL ? flyingHeight : 0) >=
                        getDistance(obj, objectiveGO)) {
                        arrivedToOverridingTask = true;
            ...
            if (auto patrolObj = patrolTask.Get_Ptr()) {
                auto objective = (dp88_AI_Objective *) (Find_Script_On_Object(patrolObj, "dp88_AI_Objective"));
                if (objective &&
                    objective->GetRange() + 4 + (isVTOL ? flyingHeight : 0) >= getDistance(obj, patrolObj)) {
    ...
    void Handle_Refill_Checks(GameObject *obj) {
        if (thinkCount % 4 == 0) {
            if (!overridingTask) {
                ...
                // No money
                if (obj->As_VehicleGameObj() &&
                    MS_AccessHelper::Get_Money(obj) <
                    Get_Team_Cost(Commands->Get_Preset_ID(obj), Get_Object_Type(obj)) * 0.25f) {
                    Commands->Send_Custom_Event(obj, obj, CUSTOM_AI_COOP_STOP_SECONDARY_OBJECTIVE, 0, 0.0f);
                } else {
        ...
        if (isLimitedAmmoUnit && state != GOING_TO_REFILL && obj->As_VehicleGameObj() &&
            obj->As_VehicleGameObj()->Get_Weapon()) {
    ...
    void Check_Repairables(GameObject *obj) {
        ...
        for (auto node = GameObjManager::BeaconGameObjList.Head(); node; node = node->Next()) {
            ...
            if (Get_Object_Type(obj) == Get_Object_Type(target) ||
                !Is_Script_Attached(target, "dp88_AI_Marker_Repairable"))
                continue;
        ...
        for (int index = 0; index < AIBuildings.Count(); index++) {
            ...
            if (target && GetTargetType(target) == REPAIRABLE && Get_Object_Type(obj) == Get_Object_Type(target)) {
                ...
                auto script = (dp88_AI_Marker_Repairable *) (Find_Script_On_Object(target,
                                                                                   "dp88_AI_Marker_Repairable"));
                ...
                if (script && getDistance(obj, target) <
                              script->Get_Distance_From_Pathfind() + GetPreferredAttackRange(obj, target) +
                              extraRange) {
    ...
    void Check_Enemy_Buildings(GameObject *obj) {
        ...
        for (int index = 0; index < AIBuildings.Count(); index++) {
            ...
            if (target && Get_Object_Type(obj) != Get_Object_Type(target) && GetTargetType(target) == BUILDING) {
                ...
                auto &boundingBox = target->As_BuildingGameObj() ? target->As_BuildingGameObj()->Get_Bounds()
                                                                 : target->As_PhysicalGameObj()->Peek_Model()->Get_Bounding_Box();
                const int radius = (int) (max(boundingBox.Extent.X, boundingBox.Extent.Y));
                ...
                const bool isAimFree = obj->As_VehicleGameObj() ? (getPrimary(target) ? (isPrimaryAmmoTurning ||
                                                                                         isPrimaryAmmoGravityAffected)
                                                                                      : (isSecondaryAmmoTurning ||
                                                                                         isSecondaryAmmoGravityAffected))
                                                                : true;

                if (!target->As_BuildingGameObj() &&
                    obj->As_SmartGameObj()->Is_Obj_Visible(target->As_PhysicalGameObj()) &&
                    Can_Tilt_Towards_Target(isAimFree, turretTransform, target)) {
                ...
                if (obj->As_SoldierGameObj() && bldScript->objectiveMCTAttackTargetObj) {
                    if (!M_Test_Raycast_Collision(objPos, bldScript->mctObjectiveLocation, false) && (
                            (isLimitedAmmoInfantry && m_pCurrentObjective &&
                             getDistance(obj, m_pCurrentObjective->GetGameObject()) < 3) ||
                ...
                Vector3 checkHitPos = (Vector3::Distance_Squared(objPos, centralTargetPos) < weaponRange * weaponRange)
                                      ? centralTargetPos : (objPos + (centralTargetPos - objPos).Normalized() *
                                                                     (float) weaponRange);
                if (M_Test_Raycast_Collision(objPos, checkHitPos, false, &checkHitPos, target) &&
                    Can_Tilt_Towards_Target(isAimFree, turretTransform, bldScript->centralTargetGround)) {
                ...
                checkHitPos = (Vector3::Distance_Squared(objPos, centralTargetPos) < weaponRange * weaponRange)
                              ? centralTargetPos : (objPos +
                                                    (centralTargetPos - objPos).Normalized() * (float) weaponRange);
                if (M_Test_Raycast_Collision(objPos, checkHitPos, false, &checkHitPos, target) &&
                    Can_Tilt_Towards_Target(isAimFree, turretTransform, bldScript->centralTarget)) {
                ...
                if (distFromBuildingEdge > 100 && bldScript->exteriorTargetWatchpoints.Count() > 12) {
                    ...
                    bldCorners.Add(boundingBox.Center + Vector3(0, boundingBox.Extent.Y + sideOffset, 0));
                    bldCorners.Add(boundingBox.Center +
                                   Vector3(boundingBox.Extent.X + sideOffset, boundingBox.Extent.Y + sideOffset, 0));
                    bldCorners.Add(boundingBox.Center +
                                   Vector3(-boundingBox.Extent.X - sideOffset, -boundingBox.Extent.Y - sideOffset, 0));
                    bldCorners.Add(boundingBox.Center +
                                   Vector3(boundingBox.Extent.X + sideOffset, -boundingBox.Extent.Y - sideOffset, 0));
                ...
                DynamicVectorClass < Vector3 > watchpointDistanceOrder = Order_Points_By_Distance(
                        bldScript->exteriorTargetWatchpoints, objPos, weaponRange - 1);
    ...
    void Notify_Neighbors_Enemy_Seen(GameObject *obj, GameObject *enemy) {
        ...
        for (auto x = list.Head(); x != nullptr; x = x->Next()) {
            ...
            if (Get_Object_Type(obj) != Get_Object_Type(neighbor) ||
                Commands->Get_ID(obj) == Commands->Get_ID(neighbor))
                continue;
    ...
    void On_Notification_Enemy_Seen(GameObject *obj, GameObject *enemy) {
        ...
        if (enemy &&
            !M_Test_Raycast_Collision(Commands->Get_Position(obj) + Vector3(0, 0, 1.5f),
                                      Commands->Get_Position(enemy) + Vector3(0, 0, 1.5f), false)) {
        ...
        if (action_id == ACTION_ID_MOVE_TO_OBJECTIVE) {
            if (obj->As_SoldierGameObj() && state != GOING_TO_VEHICLE &&
                (complete_reason == ACTION_COMPLETE_PATH_BAD_DEST ||
                 complete_reason == ACTION_COMPLETE_PATH_BAD_START) && m_pCurrentObjective &&
                m_pCurrentObjective->GetGameObject()) {
                ...
                if (overridingTask) {
                    ...
                    if (DebugPathfindFailMessagesLiveEnabled || DebugPathfindFailMessagesFileEnabled) {
                        StringClass debug;
                        debug.Format(
                                "%s(%d): cannot reach overriding task: %s (%s-%d) at location: %.2f / %.2f / %.2f (%.2f dist, %.5f prio)",
                                botName.Peek_Buffer(), Get_Object_Type(obj), m_pCurrentObjective->GetDebugTag(),
                                Commands->Get_Preset_Name(objectiveObj), objectiveObj->Get_ID(), objectiveLocation.X,
                                objectiveLocation.Y, objectiveLocation.Z, getDistance(obj, objectiveObj),
                                m_pCurrentObjective->GetPriority(obj, modifier_distance));
                        Print_Debug(debug);
                    }
                } else {
                    ignoredObjectives.Add(objectiveObj->Get_ID());
                    auto objectiveCount = dp88_AI_Objective::CountUnitObjectives(Get_Object_Type(obj), objectiveType,
                                                                                 obj, modifier_distance);
                    if (DebugPathfindFailMessagesLiveEnabled || DebugPathfindFailMessagesFileEnabled) {
                        StringClass debug;
                        debug.Format(
                                "%s(%d): cannot reach: %s (%s-%d) at location: %.2f / %.2f / %.2f  (%.2f dist, %.5f prio) (%d/%d)",
                                botName.Peek_Buffer(), Get_Object_Type(obj), m_pCurrentObjective->GetDebugTag(),
                                Commands->Get_Preset_Name(objectiveObj), objectiveObj->Get_ID(), objectiveLocation.X,
                                objectiveLocation.Y, objectiveLocation.Z, getDistance(obj, objectiveObj),
                                m_pCurrentObjective->GetPriority(obj, modifier_distance), ignoredObjectives.Count(),
                                objectiveCount);
                        Print_Debug(debug);
                    }
                    ...
                    if (objectiveCount && objectiveCount == ignoredObjectives.Count()) {
                        ...
                        if (DebugPathfindFailMessagesLiveEnabled || DebugPathfindFailMessagesFileEnabled) {
                            StringClass debug;
                            debug.Format("%s: out of objectives (%d), suiciding now at location: %.2f / %.2f / %.2f ",
                                         botName.Peek_Buffer(), objectiveCount, botLocation.X, botLocation.Y,
                                         botLocation.Z);
                            Print_Debug(debug);
                        }
...
public:
    ...
    bool Should_Ignore_Blocked_Checking(GameObject *obj) {
        ...
        if (m_target) {
            ...
            if (!m_bMovingToObjective &&
                ...
                (!IsValidTarget(obj, m_target) || !ShouldPursueTarget(obj, m_target) || !hasWeapon ||
                 targetType == BUILDING))
                return true;
        }
        return false;
    ...
    ActionParamsStruct Get_Param_Attack_Modifications(ActionParamsStruct params) override {
        ...
        if (m_target) {
            ...
            params.Set_Attack(m_target, (float) ((m_bTargetPrimaryFire) ? primary_maxRange : secondary_maxRange),
                              attackError, m_bTargetPrimaryFire);
        }
...
public:
    StringClass Get_Range_Corrected_Parameters(GameObject *obj) {
        ...
        if (Get_Int_Parameter("Max_Attack_Range") == -1 || Get_Int_Parameter("Preferred_Attack_Range") == -1 ||
            Get_Int_Parameter("Max_Attack_Range_Secondary") == -1 ||
            Get_Int_Parameter("Preferred_Attack_Range_Secondary") == -1) {
            int maxAttackRange = 0;
            int preferredAttackRange = 0;
            int maxAttackRangeSecondary = 0;
            int preferredAttackRangeSecondary = 0;
            Get_Equipped_Weapon_Range_Preferences(obj, maxAttackRange, preferredAttackRange, maxAttackRangeSecondary,
                                                  preferredAttackRangeSecondary);
            ...
            while ((i < count)) {
                ...
                } else if (i == Get_Parameter_Index("Preferred_Attack_Range") &&
                           Get_Int_Parameter("Preferred_Attack_Range") == -1) {
                    buffer.Format("%s,%d", buffer.Peek_Buffer(), preferredAttackRange);
                } else if (i == Get_Parameter_Index("Max_Attack_Range_Secondary") &&
                           Get_Int_Parameter("Max_Attack_Range_Secondary") == -1) {
                    buffer.Format("%s,%d", buffer.Peek_Buffer(), maxAttackRangeSecondary);
                } else if (i == Get_Parameter_Index("Preferred_Attack_Range_Secondary") &&
                           Get_Int_Parameter("Preferred_Attack_Range_Secondary") == -1) {
...
class MS_AI_Pathfind_Helper : public ScriptImpClass {
    ...
    void Check_For_Block(GameObject *obj) {
        ...
        if (stuckCount > 5) {
            ...
            if (Get_Random_Pathfind_Spot_Around_Position(obj, backRight, 0, 6, &backRight) &&
                Get_Random_Pathfind_Spot_Around_Position(obj, right, 0, 6, &right)) {
                availableListB.Add(backRight);
                availableListS.Add(right);
            }
            if (Get_Random_Pathfind_Spot_Around_Position(obj, backLeft, 0, 6, &backLeft) &&
                Get_Random_Pathfind_Spot_Around_Position(obj, left, 0, 6, &left)) {
    ...
    bool Should_Ignore_Blocked_Checking(GameObject *obj) {
        return gettingAround || !obj->As_SmartGameObj()->Get_Action()->Is_Acting() ||
               (implScript && implScript->Should_Ignore_Blocked_Checking(obj));
    }
};
...
public:
    ...
    void Init_New_Map() {
        ...
        if (extraTargetList.Count() > 0) {
            for (auto x = GameObjManager::GameObjList.Head(); x != nullptr; x = x->Next()) {
                auto data = x->Data();
                if (!data->As_ScriptableGameObj() ||
                    extraTargetList.ID(Commands->Get_Preset_Name(data->As_ScriptableGameObj())) == -1)
                    continue;
    ...
    void Init_Bot_Config() {
        ...
        if (config) {
            ...
            if (config->Get_Bool("RadioCommands", "Override", false)) {
                Radio_Message_Type::BUILDING_NEEDS_REPAIRS = config->Get_Int("RadioCommands", "BUILDING_NEEDS_REPAIRS",
                                                                             Radio_Message_Type::BUILDING_NEEDS_REPAIRS);
                Radio_Message_Type::GET_IN_THE_VEHICLE = config->Get_Int("RadioCommands", "GET_IN_THE_VEHICLE",
                                                                         Radio_Message_Type::GET_IN_THE_VEHICLE);
                Radio_Message_Type::GET_OUT_OF_THE_VEHICLE = config->Get_Int("RadioCommands", "GET_OUT_OF_THE_VEHICLE",
                                                                             Radio_Message_Type::GET_OUT_OF_THE_VEHICLE);
                Radio_Message_Type::DESTROY_THAT_VEHICLE = config->Get_Int("RadioCommands", "DESTROY_THAT_VEHICLE",
                                                                           Radio_Message_Type::DESTROY_THAT_VEHICLE);
                Radio_Message_Type::DONT_GET_IN_MY_WAY = config->Get_Int("RadioCommands", "DONT_GET_IN_MY_WAY",
                                                                         Radio_Message_Type::DONT_GET_IN_MY_WAY);
                Radio_Message_Type::AFFIRMATIVE = config->Get_Int("RadioCommands", "AFFIRMATIVE",
                                                                  Radio_Message_Type::AFFIRMATIVE);
                Radio_Message_Type::NEGATIVE = config->Get_Int("RadioCommands", "NEGATIVE",
                                                               Radio_Message_Type::NEGATIVE);

                Radio_Message_Type::TAKE_THE_POINT = config->Get_Int("RadioCommands", "TAKE_THE_POINT",
                                                                     Radio_Message_Type::TAKE_THE_POINT);
                Radio_Message_Type::MOVE_OUT = config->Get_Int("RadioCommands", "MOVE_OUT",
                                                               Radio_Message_Type::MOVE_OUT);
                Radio_Message_Type::FOLLOW_ME = config->Get_Int("RadioCommands", "FOLLOW_ME",
                                                                Radio_Message_Type::FOLLOW_ME);
                Radio_Message_Type::COVER_ME = config->Get_Int("RadioCommands", "COVER_ME",
                                                               Radio_Message_Type::COVER_ME);
                Radio_Message_Type::DESTROY_IT_NOW = config->Get_Int("RadioCommands", "DESTROY_IT_NOW",
                                                                     Radio_Message_Type::DESTROY_IT_NOW);

                Radio_Message_Type::ATTACK_THE_HARVESTER = config->Get_Int("RadioCommands", "ATTACK_THE_HARVESTER",
                                                                           Radio_Message_Type::ATTACK_THE_HARVESTER);
                Radio_Message_Type::ATTACK_THAT_STRUCTURE = config->Get_Int("RadioCommands", "ATTACK_THAT_STRUCTURE",
                                                                            Radio_Message_Type::ATTACK_THAT_STRUCTURE);
                Radio_Message_Type::DEFEND_THE_BASE = config->Get_Int("RadioCommands", "DEFEND_THE_BASE",
                                                                      Radio_Message_Type::DEFEND_THE_BASE);
                Radio_Message_Type::DEFEND_THE_HARVESTER = config->Get_Int("RadioCommands", "DEFEND_THE_HARVESTER",
                                                                           Radio_Message_Type::DEFEND_THE_HARVESTER);
                Radio_Message_Type::DEFEND_THAT_STRUCTURE = config->Get_Int("RadioCommands", "DEFEND_THAT_STRUCTURE",
                                                                            Radio_Message_Type::DEFEND_THAT_STRUCTURE);
            }
        ...
        if (number == TIMER_AI_COOP_GROUP_WAIT_TEAM_0 || number == TIMER_AI_COOP_GROUP_WAIT_TEAM_1) {
            ...
            for (auto x = GameObjManager::SmartGameObjList.Head(); x != nullptr; x = x->Next()) {
                ...
                if (phys->Get_Radar_Blip_Shape_Type() && object->As_SmartGameObj()->Is_Visible() &&
                    (!object->As_SoldierGameObj() || (!Get_Vehicle(
                            object)/* && (!Commands->Get_Soldiers_Can_Sneak() || !object->As_SoldierGameObj()->Is_Sneaking())*/)
                    )
                        ) {
            ...
            for (int i = 0; i < botControllers.Count(); i++) {
                ...
                for (int j = 0; j < visibleObjects.Count(); j++) {
                    ...
                    if (Get_Object_Type(visibleObject) != Get_Object_Type(botController) &&
                        Get_Object_Type(visibleObject) != SCRIPT_PLAYERTYPE_NEUTRAL) {
                        Commands->Send_Custom_Event(nullptr, botController, CUSTOM_AI_ADD_RADAR_TARGET,
                                                    visibleObject->Get_ID(), 0.1f);
                    }
                }
            }
            ...
            for (int i = 0; i < botControllers.Count(); i++) {
                ...
                for (int j = 0; j < playerObjects.Count(); j++) {
                    ...
                    if (Get_Object_Type(playerObject) == Get_Object_Type(botController) &&
                        Get_Object_Type(playerObject) != -2) {
                        Commands->Send_Custom_Event(nullptr, botController, CUSTOM_AI_ADD_ALLY_TARGET,
                                                    playerObject->Get_ID(), 0.1f);
                    }
                }
            }
        }
    ...
    void Create_Engineer_Unit_List() {
        ...
        if (auto sampleBuilding = Find_Building_By_Team(1) ? Find_Building_By_Team(1) : Find_Building_By_Team(0)) {
            ...
            characterList = Combine_Lists(characterList,
                                          Get_Purchase_Items(1, PurchaseSettingsDefClass::TYPE_SECRET_CLASSES));
            characterList = Combine_Lists(characterList,
                                          Get_Purchase_Items(0, PurchaseSettingsDefClass::TYPE_SECRET_CLASSES));
                ...
                if (auto soldierDef = (SoldierGameObjDef *) Find_Named_Definition(presetName)) {
                    ...
                    for (int j = 0; j < scriptList.Count(); j++) {
                        ...
                        if (strcmp(scriptList[j], "M00_GrantPowerup_Created") == 0) {
                            if (auto powerupDef = (PowerUpGameObjDef *) Find_Named_Definition(
                                    paramList[j].Peek_Buffer()))
                    ...
                    for (int j = 0; j < weaponList.Count(); j++) {
                        if (auto weaponDef = (WeaponDefinitionClass *) Find_Definition(weaponList[j])) {
                            if (auto ammoDef = (AmmoDefinitionClass *) Find_Definition(weaponDef->PrimaryAmmoDefID)) {
                                ...
                                auto multiplier = ArmorWarheadManager::Get_Damage_Multiplier(bldSkin,
                                                                                             ammoDef->Warhead());
            ...
            if (auto previousPoint = Commands->Find_Object(previousID)) {
                ...
                if (nodes.Count() > 0)
                    ...
                    for (int i = 0; i < copiedPoints.Count(); i++) {
                        ...
                        if (copiedPoints[i]->Get_NodeGroup() == patrolScript->Get_NodeGroup() ||
                            (copiedPoints[i]->Get_IsInfantryOnly() && isForVehicle))
                            continue;
    ...
    void Reset_GroupRush(int team) {
        ...
        for (int i = 0; i < rallyPointRegs.Count(); i++) {
            Commands->Send_Custom_Event(nullptr, Commands->Find_Object(rallyPointRegs[i]),
                                        CUSTOM_AI_COOP_GROUPRUSH_COMPLETE, 0, 0.0f);
        }
	...
	*/
    ...
    void Check_Group_Arrived(int team) {
        ...
        if (rallyPointUnitsNotArrived.Count() == 0 && !regsOpen) {
            ...
            for (int i = 0; i < rallyPointRegs.Count(); i++) {
                Commands->Send_Custom_Event(nullptr, Commands->Find_Object(rallyPointRegs[i]),
                                            CUSTOM_AI_COOP_NEXT_RALLYPOINT, nextID, 0.0f);
            }
            rallyPointUnitsNotArrived.Clear();
    ...
    void Check_Explosives() {
        ...
        for (auto node = GameObjManager::BeaconGameObjList.Head(); node; node = node->Next()) {
            ...
            if (!isValidTarget) {
                ...
                ExplosionDefinitionClass * explosion = (ExplosionDefinitionClass *) DefinitionMgrClass::Find_Definition(
                        object->Get_Definition().Get_Explosion_Obj_Def(), true);
                if (!explosion)
                    continue;
            ...
            if (isValidTarget) {
                ...
                if (isVehicleTarget) {
                    params.Format("%d,%d,%d,%d,%d,%d,%d", team, dp88_AI_Objective::TYPE_OFFENSIVE, 10, 0, 1000, 1000,
                                  1000);
                    Commands->Attach_Script(object, "dp88_AI_Objective", params.Peek_Buffer());
                }
                if (BuildingDebugLocationsDisplayEnabled) {
        ...
        for (auto node = GameObjManager::C4GameObjList.Head(); node; node = node->Next()) {
            ...
            if (!bld)
                continue;
            auto &boundingBox = bld->As_BuildingGameObj() ? bld->As_BuildingGameObj()->Get_Bounds()
                                                          : bld->As_PhysicalGameObj()->Peek_Model()->Get_Bounding_Box();
...
class MS_AI_Player_Count_Controller : public ScriptImpClass {
    ...
    void Timer_Expired(GameObject *obj, int number) override {
        if (number == TIMER_AI_CONTROLLER_PLAYERCHECK) {
            ...
            int botCountDiffTeam0 = spawnTeam == 1 ? -spawnedControllersTeam0.Count() :
                                    (spawnTeam == 2 ? maxBotCount / 2 : maxBotCount) + correction -
                                    Get_Team_Player_Count(0) - spawnedControllersTeam0.Count();
            int botCountDiffTeam1 = spawnTeam == 0 ? -spawnedControllersTeam1.Count() :
                                    (spawnTeam == 2 ? maxBotCount / 2 : maxBotCount) + correction -
                                    Get_Team_Player_Count(1) - spawnedControllersTeam1.Count();
    ...
    bool Init_Parameters(GameObject *obj) {
        ...
        if (!valid) {
            ...
            msg.Format("[%d:%hs:%hs] Critical Error: %hs. Destroying script...\n", Commands->Get_ID(obj),
                       Commands->Get_Preset_Name(obj), this->Get_Name(), errorMessage.Peek_Buffer());
            M_Console_Output(msg.Peek_Buffer());
            Destroy_Script();
        }
        return valid;
...
class MS_AI_Player_Controller : public ScriptImpClass {
	...
	*/
    ...
    void Created(GameObject *obj) override {
        ...
        StringClass buff;
        buff.Format("%s %s", botName.Peek_Buffer(),
                    Get_Translated_String(Get_String_ID_By_Desc("IDS_MP_RENEGADE_JOINS")));
    ...
    void Custom(GameObject *obj, int type, int param, GameObject *sender) override {
        ...
        if (type == CUSTOM_AI_SOLDIER_DIED) {
            ...
            if (param == 0) {
                if (MS_AI_Global_Controller::GetInstance().killedRespEnabled &&
                    personality->Get_Random(BotPersonality::CHAT_CHANCE)) {
                ...
                if (MS_AI_Global_Controller::GetInstance().killerRespEnabled &&
                    personality->Get_Random(BotPersonality::CHAT_CHANCE)) {
            ...
            if (param == 0)
                ...
                if (soldierObject && enemyObj &&
                    Vector3::Distance_Squared(Commands->Get_Position(soldierObject), Commands->Get_Position(enemyObj)) <
            ...
            if (param == 0)
                ...
                if (soldierObject && otherObj &&
                    Vector3::Distance_Squared(Commands->Get_Position(soldierObject), Commands->Get_Position(otherObj)) <
                    checkRange * checkRange) {
            ...
            if (coopState == 2 || coopState == 3) {
                ...
                if (playerObj && coopObject) {
                    ...
                    if (Vector3::Distance_Squared(Commands->Get_Position(playerObj), coopFriendPrevPos) < 2 * 2 &&
                        Vector3::Distance_Squared(Commands->Get_Position(playerObj),
                                                  Commands->Get_Position(coopObject)) < checkRange * checkRange) {
            ...
            if (personality->Get_Random(BotPersonality::INVESTIGATE_CHANCE)) {
                ...
                for (int i = 0; i < enemiesOnRadar.Count(); i++) {
                    ...
                    float dist_sq = Vector3::Distance_Squared(Commands->Get_Position(aiObj),
                                                              Commands->Get_Position(enemy));
            ...
            if (!M_Has_Timer(obj, this, TIMER_AI_COOP_PAUSE_INITIATION) &&
                personality->Get_Random(BotPersonality::TEAMWORK_CHANCE)) {
                ...
                for (int i = 0; i < allyPlayersAround.Count(); i++) {
                    ...
                    if ((!aiObj->As_VehicleGameObj() && Get_Vehicle(ally)) ||
                        (aiObj->As_VehicleGameObj() && !Get_Vehicle(ally)) && FreeRandom.Get_Float() < 0.5f)
                        continue;
                ...
                if (chosenTarget) {
                    ...
                    if (FreeRandom.Get_Float() < sympathyData->sympathy) {
                        ...
                        if (personality->Get_Random(BotPersonality::CHAT_CHANCE)) {
                            ...
                            StringClass response = MS_AI_Global_Controller::GetInstance().Get_Chat_Response(
                                    followMeMsgs);
            ...
            auto patrolPointID = MS_AI_Global_Controller::GetInstance().Get_New_Patrol_Point_ID(team, param,
                                                                                                sender->As_VehicleGameObj()
                                                                                                ? true : false);
            if (patrolPointID)
            ...
            } else if (auto zoneObj2 = exitZonePresetName ? Find_Nearest_Preset(Commands->Get_Position(botVeh),
                                                                                exitZonePresetName) : nullptr) {
                Commands->Destroy_Object(tempObjective);
            ...
            if (sender->As_SoldierGameObj()) {
                ...
                Commands->Send_Custom_Event(obj, sender,
                                            isRepairCheck ? CUSTOM_AI_GO_TO_REFILL : CUSTOM_AI_GO_SWITCH_OBJECTIVE,
                                            tempObjective->Get_ID(), 0.0f);
                coopState = 8;
            }
                ...
                SList<GameObject> list;
                Find_All_Objects_With_Script_By_Distance("MS_AI_Marker_Vehicle_Repair_Zone", list,
                                                         Commands->Get_Position(sender));
                for (auto x = list.Head(); x != nullptr; x = x->Next()) {
                    GameObject * zoneObj = x->Data();
                    auto zoneScript = (MS_AI_Marker_Vehicle_Repair_Zone *) (Find_Script_On_Object(zoneObj,
                                                                                                  "MS_AI_Marker_Vehicle_Repair_Zone"));
                    ...
                    const float dist = Vector3::Distance(Commands->Get_Position(zoneObj),
                                                         Commands->Get_Position(sender));
            ...
            if (param == 1 && vehicleObject) {
                ...
                if (vehicle && vehicle->Get_Occupant_Count() && Get_Vehicle_Driver(vehicle) == soldierObject) {
                    ...
                    if (auto spawnedScript = Find_Script_On_Object(soldierObject,
                                                                   "MS_AI_Player_Controller_Spawned_Object")) {
                ...
                if ((vehicle && vehicle->Get_Occupant_Count() < vehicle->Get_Definition().Get_Seat_Count() &&
                    ...
                    if (!vehicle->Get_Driver() ||
                        (isSoldierAlreadyInVehicle && !(Is_Script_Attached(vehicle, "MS_AI_Behaviour"))))
                    ...
                    if (!Is_Script_Attached(vehicle, "MS_AI_Player_Controller_Spawned_Object")) {
                        ...
                        Commands->Attach_Script(vehicle, "MS_AI_Player_Controller_Spawned_Object",
                                                params.Peek_Buffer());
                    }
                    ...
                    if (!isSoldierAlreadyInVehicle)
                        Commands->Send_Custom_Event(obj, vehicle, CUSTOM_AI_VEHICLE_REQUEST_ENTER,
                                                    soldierObject->Get_ID(), 0.0f);
    ...
    void Timer_Expired(GameObject *obj, int number) override {
        if (number == TIMER_AI_OBJECT_SPAWN_COUNTDOWN) {
            ...
            if (soldierObject) {
                soldierObject->As_PhysicalGameObj()->Peek_Physical_Object()->Set_Collision_Group(
                        Collision_Group_Type::SOLDIER_GHOST_COLLISION_GROUP);
            }
            ...
            bool isWithinPathfindGrid = Get_Closest_Pathfind_Spot_Size(Commands->Get_Position(vehicle), 1.0f,
                                                                       &runLocation, 2.0f);
            if (isWithinPathfindGrid) {
            ...
            if (soldier_has_ai_behavior || (vehicle_has_ai_behavior && enteredVeh)) {
                ...
                if (coopState == 1) {
                    ...
                    if (vehicle && !enteredVeh) {
                        ...
                        Commands->Send_Custom_Event(obj, coopObject, CUSTOM_AI_COOP_NEXT_RALLYPOINT,
                                                    MS_AI_Global_Controller::GetInstance().Get_Rally_Point_ID(team),
                                                    0.0);
                        if (MS_AI_Global_Controller::GetInstance().debugGrouprushMessagesEnabled) {
                            Vector3 rgb = Get_Color_For_Team(team);
                            StringClass msg;
                            const char *unitName = Get_Translated_Definition_Name(
                                    Commands->Get_Preset_Name(coopObject));
                            msg.Format("%s: Joining group rush! (%s)", botName.Peek_Buffer(), unitName);
                            Send_Message_Team(team, (int) (rgb.X * 255), (int) (rgb.Y * 255), (int) (rgb.Z * 255),
                                              msg.Peek_Buffer());
                            delete[] unitName;
                        }
                    }
                    ...
                    Commands->Send_Custom_Event(obj, coopObject, CUSTOM_AI_COOP_PLAYER_DEFEND_BASE, objectiveObjID,
                                                0.0);
    ...
    void Controller_Gone(GameObject *obj) {
        ...
        StringClass buff;
        buff.Format("%s %s", botName.Peek_Buffer(),
                    Get_Translated_String(Get_String_ID_By_Desc("IDS_MP_PLAYER_LEFT_GAME")));
        Send_Message(MS_RGB_JOINLEFT_MSG, buff.Peek_Buffer());
        ...
        if (auto vehicle = vehicleObject.Get_Ptr()) {
            if (vehicle->As_VehicleGameObj()->Get_Driver() == soldier) {
                ...
                if (vehicle->As_VehicleGameObj()->Get_Occupant_Count() == 1 &&
                    !vehicle->As_VehicleGameObj()->Is_Attached_To_An_Object())
		...
		*/
        if (strcmp(spawnAtPresetName, "USEPLAYERSPAWNERS") == 0) {
            ...
            for (int i = 0; i < fullList.Count(); i++) {
                ...
                MoveablePhysClass *phys = soldier->As_PhysicalGameObj()
                                          ? soldier->As_PhysicalGameObj()->Peek_Physical_Object()->As_MoveablePhysClass()
                                          : nullptr;
        ...
        if (objectiveType != 3 &&
            MS_AI_Global_Controller::GetInstance().spawnPositions[team].ID(Commands->Get_Position(soldier)) == -1)
    ...
    void Purchase_Presets(GameObject *obj) {
        ...
        bool buyInf = mustChangeCharacter || objectiveType == dp88_AI_Objective::TYPE_ENGINEERING ||
                      personality->Get_Random(BotPersonality::BUYINF_CHANCE) || money > 10000;
        ...
        if (vehicleObject) {
            if (GameObject *veh = vehicleObject) {
                ...
                if (vObject && vObject->Get_Occupant_Count() < vObject->Get_Definition().Get_Seat_Count() &&
                    !Is_Script_Attached(vObject, "MS_AI_Player_Controller_Spawned_Object")) {
        ...
        bool buyVeh = objectiveType == dp88_AI_Objective::TYPE_OFFENSIVE &&
                      (personality->Get_Random(BotPersonality::BUYVEH_CHANCE) || money > 10000);
    ...
    const char *Choose_Unit(const char *checkPresetList, bool isInfantry) {
        ...
        if (strcmp(checkPresetList, "AUTOENGINEER") == 0) {
            ...
            if (strcmp(checkPresetList, "USEPLAYERPURCHASELISTS") == 0) {
                if (isInfantry) {
                    ...
                    fullList = Combine_Lists(fullList,
                                             Get_Purchase_Items(team, PurchaseSettingsDefClass::TYPE_CLASSES));
                    if (Is_Extras_Enabled() || Is_Sidebar_Enabled())
                        fullList = Combine_Lists(fullList, Get_Purchase_Items(team,
                                                                              PurchaseSettingsDefClass::TYPE_SECRET_CLASSES));
                } else {
                    fullList = Combine_Lists(fullList,
                                             Get_Purchase_Items(team, PurchaseSettingsDefClass::TYPE_VEHICLES));
                    if (Is_Extras_Enabled() || Is_Sidebar_Enabled())
                        fullList = Combine_Lists(fullList, Get_Purchase_Items(team,
                ...
                TwiddlerClass *twiddler = (TwiddlerClass *) DefinitionMgrClass::Find_Named_Definition(checkPresetList,
                                                                                                      false);
                if (twiddler)
        ...
        for (int i = 0; i < namedList.Count(); i++) {
            ...
            bool available = isInfantry ? Is_Infantry_Purchaseable(presetName, team) : Is_Vehicle_Purchaseable(
                    presetName, team);
    ...
    void Activate_AI_Behaviour(GameObject *obj, GameObject *activateOnObj) {
        ...
        if (!activateOnObj) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: Trying to activate AI on a nullpointer object (%hs). Destroying script...\n",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name(),
                    Commands->Get_Preset_Name(activateOnObj));
            M_Console_Output(msg.Peek_Buffer());
        ...
        if (!preferencesScript) {
            ...
            int maxAttackRangeSecondary = 0;
            int preferredAttackRangeSecondary = 0;
            Get_Equipped_Weapon_Range_Preferences(activateOnObj, maxAttackRange, preferredAttackRange,
                                                  maxAttackRangeSecondary, preferredAttackRangeSecondary);
            ...
            for (int i = 0; i < parameters.Count(); i++) {
                ...
                if (i == 0) {
                    ...
                    if (maxAttackRange)
                        ...
                        behaviourScriptParams.Format("%s,%s", behaviourScriptParams.Peek_Buffer(),
                                                     parameter.value.Peek_Buffer());
                } else if (strcmp(parameter.name, "Preferred_Attack_Range") == 0) {
                    if (preferredAttackRange)
                        behaviourScriptParams.Format("%s,%d", behaviourScriptParams.Peek_Buffer(),
                ...
                else if ((strcmp(parameter.name, "Priority_Light_Vehicle") == 0 ||
                          strcmp(parameter.name, "Priority_Heavy_Vehicle") == 0 ||
                          strcmp(parameter.name, "Priority_VTOL") == 0) &&
                         objectiveType == dp88_AI_Objective::TYPE_ENGINEERING) {
        ...
        if (objectiveType == 1) {
            ...
            for (auto x = list.Head(); x != nullptr; x = x->Next()) {
                ...
                if ((vehicle->Get_Occupant_Count() == 0 && vehicle->Is_Locked()) ||
                    ...
                    (vehicle->Get_Occupant_Count() >= 1 && vehicle->Get_Definition().Get_Seat_Count() <
                                                           MS_AI_Global_Controller::GetInstance().transportVehicleSeatCount) &&
                    !Is_Script_Attached(vehicle, "MS_AI_Marker_Transport_Vehicle_Override"))
                    continue;

                // If too far away, don't bother
                if (Vector3::Distance_Squared(Commands->Get_Position(soldierObject), Commands->Get_Position(vehicle)) >
                    distanceLimit * distanceLimit)
                    continue;

                // Skip vehicles marked to be ignored
                if (auto ignoreScript = (MS_AI_Marker_Vehicle_Search_Ignore *) (Find_Script_On_Object(vehicle,
                                                                                                      "MS_AI_Marker_Vehicle_Search_Ignore"))) {
                ...
                if (MS_AI_Global_Controller::GetInstance().debugAcquireVehicleMessagesEnabled) {
                    ...
                    msg.Format("%s: Running for %s's vehicle (%s)!", botName.Peek_Buffer(),
                               Get_Player_Name(vehicle->Get_Driver()), unitName);
                    Send_Message_Team(team, (int) (rgb.X * 255), (int) (rgb.Y * 255), (int) (rgb.Z * 255),
                                      msg.Peek_Buffer());
                    if (strcmp(unitName, "ERROR: String Not Found") == 0) {
                        msg.Format("%s: Selected vehicle (%s) is probably wrong, has no string!", botName.Peek_Buffer(),
    ...
    bool Go_To_Vehicle(GameObject *controllerObj, GameObject *vehicle) {
        ...
        if (soldierObject && vehicle) {
            ...
            Commands->Send_Custom_Event(controllerObj, soldierObject, CUSTOM_AI_GO_TO_VEHICLE, vehicleObject->Get_ID(),
                                        1.5f);
            return true;
        }
        return false;
    ...
    void Play_Radio_Message(int stringID) {
        ...
        if (TDBObjClass *tdbobj = TranslateDBClass::Find_Object(Get_Radio_Command_String(stringID))) {
            const char *unit = Get_Vehicle(obj) ? Get_Translated_Definition_Name(
                    Commands->Get_Preset_Name(Get_Vehicle(obj))) : Get_Translated_Definition_Name(
                    Commands->Get_Preset_Name(obj));
            Vector3 rgb = Get_Color_For_Team(team);
            StringClass msg;
    ...
    MS_AI_Team_Controller *Get_Team_Controller() {
        if (auto teamCtrlObj = MS_AI_Global_Controller::GetInstance().teamControllers[team]) {
            if (auto teamCtrlScript = (MS_AI_Team_Controller *) (Find_Script_On_Object(teamCtrlObj,
                                                                                       "MS_AI_Team_Controller"))) {
        ...
        if (messageNumber == Radio_Message_Type::GET_IN_THE_VEHICLE) {
            ...
            if (sender == Get_GameObj(coopFriendPlayerID) && orderType == CUSTOM_AI_ORDER_ATTACK) {
                ...
                if (auto harvester = Find_Harvester(team == 1 ? 0 : 1)) {
                    ...
                    float distFromTarget = Vector3::Distance(Commands->Get_Position(soldier),
                                                             Commands->Get_Position(harvester));
                    if (distFromTarget < 40 ||
                        (distFromTarget < 120 && personality->Get_Random(BotPersonality::TEAMWORK_CHANCE)) ||
                        FreeRandom.Get_Float() < sympathy) {
            ...
            if ((coopState == 0 && distanceFromSender < 80 &&
                 personality->Get_Random(BotPersonality::TEAMWORK_CHANCE)) || coveringSamePlayer) {
            ...
            if ((coopState == 0 && distanceFromSender < 80 &&
                 personality->Get_Random(BotPersonality::TEAMWORK_CHANCE)) || followingSamePlayer) {
                if (long id = Get_Player_ID(sender))
                    coopFriendPlayerID = id;
            ...
            int objectiveObjID = MS_AI_Global_Controller::GetInstance().Get_Temporary_Objective_ID(team);
            float distanceFromBase = Vector3::Distance(Commands->Get_Position(soldier),
                                                       Commands->Get_Position(Commands->Find_Object(objectiveObjID)));
...
private:
    bool Init_Parameters(GameObject *obj) {
        ...
        if (strcmp(soldierPresetList, "") == 0 || strcmp(soldierPresetList, "null") == 0)
            soldierPresetList = nullptr;
        else if (valid && strcmp(soldierPresetList, "USEPLAYERPURCHASELISTS") != 0 &&
                 !Is_Valid_Preset(soldierPresetList)) {
        ...
        if (valid && (objectiveType < 1 || objectiveType > 3)) {
            errorMessage.Format("The script has an invalid ObjectiveType parameter: %d, this must be 1, 2 or 3",
                                objectiveType);
            valid = false;
        }
...
class MS_AI_Player_Controller_Spawned_Object : public ScriptImpClass {
    ...
    void Created(GameObject *obj) override {
        ...
        if (!Commands->Find_Object(controllerID) && !isVehicle) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: The script's controller object can't be found. Destroying script...\n",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
            M_Console_Output(msg.Peek_Buffer());
            Destroy_Script();
            return;
        ...
        if (isVehicle && !obj->As_VehicleGameObj()) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: The script's not on a vehicle when it should be, IsVehicle:%d  Destroying script...\n",
        ...
        if (isVehicle) {
            ...
            for (int i = 0; i < vObj->Get_Definition().Get_Seat_Count(); i++) {
                ...
                if (auto passenger = vObj->Get_Occupant(i)) {
                    if (auto spawnedScript = (MS_AI_Player_Controller_Spawned_Object *) (Find_Script_On_Object(
                            passenger, "MS_AI_Player_Controller_Spawned_Object"))) {
    ...
    void Custom(GameObject *obj, int type, int param, GameObject *sender) override {
        if (type == CUSTOM_EVENT_OWNED_VEHICLE) {
            ...
            if (isVehicle) {
                ...
                if (obj->As_VehicleGameObj()->Get_Occupant_Count() <
                    obj->As_VehicleGameObj()->Get_Definition().Get_Seat_Count())
                    Commands->Enable_Vehicle_Transitions(obj, true);
            }
            ...
            Commands->Send_Custom_Event(obj, Commands->Find_Object(passengerControllerIDs[param]),
                                        CUSTOM_AI_VEHICLE_EXITED, 1, 0.1f);
        ...
        } else if (type == CUSTOM_AI_REFILL_COMPLETE || type == CUSTOM_AI_CHECK_REPAIR_NEEDED ||
                   type == CUSTOM_AI_ARRIVED_TO_REFILL ||
                   type == CUSTOM_AI_ARRIVED_TO_EXIT_VEHICLE || type == CUSTOM_AI_REQUEST_PATROL_POINT ||
                   type == CUSTOM_AI_CHECK_GO_HOME ||
                   type == CUSTOM_AI_COOP_STOP_SECONDARY_OBJECTIVE) {
            Commands->Send_Custom_Event(sender, Get_Controller(), type, param, 0.0f);
        } else if (type == CUSTOM_AI_ENEMY_KILLED) {
            Commands->Send_Custom_Event(obj, Get_Controller(), CUSTOM_AI_SEND_PUBLIC_MESSAGE, 1,
                                        (float) FreeRandom.Get_Int(1, 4));
        ...
        if (isVehicle) {
            ...
            for (int i = 0; i < passengerControllerIDs.Count(); i++) {
                if (passengerControllerIDs[i] != 0)
                    Commands->Send_Custom_Event(obj, Commands->Find_Object(passengerControllerIDs[i]),
                                                CUSTOM_AI_VEHICLE_DIED, 0, 0.1f);
            }
        } else {
            ...
            if (killer && killer->As_SmartGameObj() && killer->As_SmartGameObj()->Is_Human_Controlled()) {
                ...
                Commands->Send_Custom_Event(obj, Get_Controller(), CUSTOM_AI_SEND_PUBLIC_MESSAGE, 0,
                                            (float) FreeRandom.Get_Int(1, 4));
                ...
                int killStringID =
                        Get_Object_Type(obj) == Get_Object_Type(killer) ? Get_String_ID_By_Desc("IDS_MP_TREASON_PHRASE")
                                                                        : Get_String_ID_By_Desc(
                                "IDS_MP_DEFAULT_KILL_PHRASE");

                Vector3 rgb = Get_Color_For_Team(Get_Object_Type(killer));
                StringClass msg;
                msg.Format("%s %s %s", Get_Player_Name(killer), Get_Translated_String(killStringID),
                           botName.Peek_Buffer());
...
class MS_AI_Bomb_Specialist : public ScriptImpClass {
    ...
    void Created(GameObject *obj) override {
        ...
        if (!obj->As_SoldierGameObj()) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: Only objects of type SoldierGameObj can use the MS_AI_Bomb_Specialist script. Destroying script...\n",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
            M_Console_Output(msg.Peek_Buffer());
            Destroy_Script();
            return;
...
class MS_AI_Bomb_Specialist_Zone : public ScriptImpClass {
    ...
    void Created(GameObject *obj) {
        ...
        if (!Is_Valid_Preset(presetName)) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: SendToPreset: %hs is not a valid preset name. Destroying script...\n",
    ...
    void Entered(GameObject *obj, GameObject *enterer) override {
        ...
        if (enterer->As_SoldierGameObj() &&
            ...
            MS_AI_Bomb_Specialist *bsScript = (MS_AI_Bomb_Specialist *) (Find_Script_On_Object(enterer,
                                                                                               "MS_AI_Bomb_Specialist"));
...
class MS_AI_Refinery : public ScriptImpClass {
    ...
    void Created(GameObject *obj) override {
        if (!obj->As_BuildingGameObj() || !obj->As_BuildingGameObj()->As_RefineryGameObj()) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: Only objects of type RefineryGameObj can use the MS_AI_Refinery script. Destroying script...\n",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
            M_Console_Output(msg.Peek_Buffer());
            Destroy_Script();
            return;
...
class MS_AI_Spectator : public JFW_Key_Hook_Base {
    ...
    void Created(GameObject *obj) override {
        ...
        if (!obj->As_SoldierGameObj()) {
            StringClass msg;
            msg.Format(
                    "[%d:%hs:%hs] Critical Error: Only objects of type SoldierGameObj can use the MS_AI_Spectator script. Destroying script...\n",
                    Commands->Get_ID(obj), Commands->Get_Preset_Name(obj), this->Get_Name());
            M_Console_Output(msg.Peek_Buffer());
            Destroy_Script();
            return;
    ...
    void KeyHook() {
        ...
        if (GameObject *cur_bot = Commands->Find_Object(bots[currentBotIndex])) {
            ...
            if (auto playerControllerScript = (MS_AI_Player_Controller *) (Find_Script_On_Object(cur_bot,
                                                                                                 "MS_AI_Player_Controller"))) {
                StringClass message;
                message.Format("[AI Spectator]: Now Spectating %s",
                               playerControllerScript->Get_Bot_Name().Peek_Buffer());
    ...
    void Spectate_Current() {
        ...
        if (auto botCtrlObj = Commands->Find_Object(currentBotCtrlID)) {
            auto playerControllerScript = (MS_AI_Player_Controller *) (Find_Script_On_Object(botCtrlObj,
                                                                                             "MS_AI_Player_Controller"));
            ...
            if (!botObj) {
                StringClass message;
                message.Format("[AI Spectator]: %s: waiting to spawn...",
                               playerControllerScript->Get_Bot_Name().Peek_Buffer());
                Send_Message_Player(obj, DP88_RGB_GENERAL_MSG, message);
                return;
            }
    ...
    void Check_Camera(GameObject *obj) {
        if (auto botObj = Commands->Find_Object(currentBotID)) {
            ...
            auto logObj =
                    Get_Vehicle(botObj) && Get_Vehicle(botObj)->As_VehicleGameObj()->Get_Driver() == botObj ? cameraObj
                                                                                                            : botObj;
            if (auto behaviourScript = (MS_AI_Behaviour *) (Find_Script_On_Object(logObj, "MS_AI_Behaviour"))) {
                ...
                if (auto overrideTask = behaviourScript->Get_Override_Task()) {
                    StringClass buff;
                    buff.Format("Override task ID: %d, Type: %d", overrideTask->Get_ID(),
                                behaviourScript->Get_Override_Task_Type());
                    Send_Message_Player(obj, 0, 255, 0, buff.Peek_Buffer());
                ...
                if (auto objective = behaviourScript->Get_Current_Objective()) {
                    StringClass buff;
                    buff.Format("Objective dist: %d", (int) Vector3::Distance(Commands->Get_Position(objective),
                                                                              Commands->Get_Position(logObj)));
                    Send_Message_Player(obj, 0, 255, 0, buff.Peek_Buffer());
                }
            }
        }
    ...
    void Refresh_Bot_List() {
        ...
        auto botObjects = Combine_Lists(MS_AI_Global_Controller::GetInstance().botControllersTeam[1],
                                        MS_AI_Global_Controller::GetInstance().botControllersTeam[0]);
...
void MS_AccessHelper::Give_Bot_Credits(int team, float amount) {
    ...
    for (int i = 0; i < controllers.Count(); i++) {
        MS_AI_Player_Controller *playerControllerScript = (MS_AI_Player_Controller *) (Find_Script_On_Object(
                controllers[i], "MS_AI_Player_Controller"));
        if (playerControllerScript)
            playerControllerScript->Increment_Money(
                    amount * MS_AI_Global_Controller::GetInstance().grantCreditsMultiplier);
    }
}
...
void MS_AccessHelper::Send_Radio_Order_To_Bot(GameObject *sender, GameObject *receiver, int radioIndex, int orderType,
                                              int orderObjID) {
    if (receiver) {
        if (auto playerControllerScript = (MS_AI_Player_Controller *) (Find_Script_On_Object(receiver,
                                                                                             "MS_AI_Player_Controller"))) {
...
float MS_AccessHelper::Get_Money(GameObject *object) {
    if (auto botScript = (MS_AI_Player_Controller_Spawned_Object *) (Find_Script_On_Object(object,
                                                                                           "MS_AI_Player_Controller_Spawned_Object"))) {
        if (auto botControllerObj = botScript->Get_Controller()) {
            if (auto botControllerScript = (MS_AI_Player_Controller *) (Find_Script_On_Object(botControllerObj,
                                                                                              "MS_AI_Player_Controller"))) {
