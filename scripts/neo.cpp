                    ...
                    if (vehicle) //is player in a tank?
                    {
                        if (Commands->Is_A_Star(old)) {
                            ...
                            } else //old not in a vehicle
                            {
                                ...
                                float distance = Commands->Get_Distance(Commands->Get_Position(obj),
                                                                        Commands->Get_Position(old));
                                if (distance > Get_Float_Parameter("MaxRange")) {
                                    distance = Commands->Get_Distance(Commands->Get_Position(obj),
                                                                      Commands->Get_Position(target));
                                    if (distance < Get_Float_Parameter("MaxRange"))
                                        selected = target;
                                }
                            }
...
ScriptRegistrant<Neo_CreateObject_RandomTimer> Neo_CreateObject_RandomTimer_Registrant("Neo_CreateObject_RandomTimer",
                                                                                       "Preset_Name=blank:string,Min=0.0:float,Max=60.0:float,Repeat=1:int");
ScriptRegistrant<Neo_Vehicle_AI> Neo_Vehicle_AI_Registrant("Neo_Vehicle_AI",
                                                           "MaxRange=0.0:float,vsSoldier=1:int,vsVehicle=1:int,vsAircraft=1:int");
