...
void RA_Soviet_Defence::Enemy_Seen(GameObject *obj, GameObject *seen) {
    ...
    if ((IgnorePreset != Get_Int_Parameter("Spy_Preset_ID")) &&
        (IgnorePreset != Get_Int_Parameter("Aircraft_Preset_ID"))) {
        pos = Commands->Get_Position(obj);
...
void RA_Naval_PT::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    ...
    if (type == CUSTOM_EVENT_VEHICLE_ENTERED) {
        ...
        GameObject * newobj;
        char params[100];
        sprintf(params, "%s,%s,%s,%s,%s,%d", Get_Parameter("Preset_Name"), Get_Parameter("Cost"), Get_Parameter("Team"),
                Get_Parameter("Controller_ID"), Get_Parameter("Zone_ID"), storedID); //Get parameters
        //time to die
        ...
        if (send) //should I bother the boss?
        {
            ...
            Commands->Send_Custom_Event(newobj, Commands->Find_Object(Get_Int_Parameter("Controller_ID")), 951951, 0,
                                        0.0);
        }
    } else if (type == 159159) {
        if (param == 1) {
...
void RA_Naval_PT::Timer_Expired(GameObject *obj, int number) {
    ...
    Commands->Set_Facing(newobj, Commands->Get_Facing(
            Commands->Find_Object(Get_Int_Parameter("Zone_ID"))));//Set_Facing won't work on a script zone
...
/********************************************************************/
ScriptRegistrant<RA_Thief_Script> RA_Thief_Script_Registrant("RA_Thief_Script",
                                                             "Credits_To_Award=500.00:float,Prevent_Theft_Timer=30.00:float,Thief_Preset_ID:int");
ScriptRegistrant<RA_Soviet_Defence> RA_Soviet_Defence_Registrant("RA_Soviet_Defence",
                                                                 "MinAttackDistance=0.0:float,MaxAttackDistance=300.0:float,AttackTimer=10.00:float,Spy_Preset_ID:int,Aircraft_Preset_ID:int");
ScriptRegistrant<RA_Soviet_Refinery_Controller> RA_Soviet_Refinery_Controller_Registrant(
        "RA_Soviet_Refinery_Controller", "ScriptZoneID=0:int");
ScriptRegistrant<RA_Soviet_Refinery_Theft_Zone> RA_Soviet_Refinery_Theft_Zone_Registrant(
        "RA_Soviet_Refinery_Theft_Zone", "Thief_Preset_ID=0:int");
ScriptRegistrant<RA_Allied_Thief> RA_Allied_Thief_Registrant("RA_Allied_Thief",
                                                             "Credits_To_Award=500.00:float, Prevent_Theft_Timer=30.00:float");
ScriptRegistrant<RA_Naval_PT> RA_Naval_PT_Registrant("RA_Naval_PT",
                                                     "Preset_Name=blank:string,Cost=0.0:float,Team=0:int,Controller_ID=0:int,Zone_ID=0:int,TestID=0:int");
ScriptRegistrant<RA_Conyard_Controller> RA_Conyard_Controller_Registrant("RA_Conyard_Controller",
