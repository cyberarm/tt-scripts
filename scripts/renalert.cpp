...
void RA_Helipad_Zone::Timer_Expired(GameObject *obj, int number) {
    ...
    distance = Commands->Get_Distance(pos1, pos2);
    if ((distance <= 10.0) &&
        (Get_Vehicle_Driver(Commands->Find_Object(number)))) //reloads vehicle up to 10m from zone center
    {
...
void RA_MineLayer::Damaged(GameObject *obj, GameObject *damager, float amount) {
    if (driverID) //If there's no driver, do nothing
    {
        ...
        if (hitter == driverID)//checking if the driver 'shot' the vehicle
        {
            ...
            {
                Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("Mine_Manager_ID")), 300300,
                                            driverID, 0.0);
            }
        }
    }
...
void RA_Mine::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == CUSTOM_EVENT_VEHICLE_DRIVER) {
        if (ownerID) {
            Commands->Send_Custom_Event(obj, sender, CUSTOM_EVENT_VHEICLE_DRIVER_ID,
                                        Commands->Get_ID(Get_GameObj(ownerID)), 0);
        } else {
	...
	*/
    if (AP_mine) //Is this mine anti-personnel?
    {
        if (enemy->As_SoldierGameObj())//using newer, more specific code
        {
            ...
            {
                Commands->Apply_Damage(obj, Get_Float_Parameter("Trigger_Damage"), Get_Parameter("Trigger_Warhead"),
                                       0); //Die Infidel!
    ...
    if (AT_mine) //Is this mine anti-tank?
    {
        if (enemy->As_VehicleGameObj())//using newer, more specific code
        {
            ...
            {
                Commands->Apply_Damage(obj, Get_Float_Parameter("Trigger_Damage"), Get_Parameter("Trigger_Warhead"),
                                       0); //Good-bye cruel world!
            } else {
                Commands->Apply_Damage(obj, Get_Float_Parameter("Trigger_Damage"), Get_Parameter("Trigger_Warhead"),
                                       damager); //Good-bye cruel world!
            }
            SetExplosionObj(0);
            return; //the mine is dead, end now
        }
...
void RA_Naval_PT_Pokable::Timer_Expired(GameObject *obj, int number) {
    //Create the object
    GameObject * newobj;
    newobj = Commands->Create_Object(Get_Parameter("Preset_Name"),
                                     Commands->Get_Position(Commands->Find_Object(Get_Int_Parameter("Zone_ID"))));
...
void RA_Demolition_Truck_Improved::Killed(GameObject *obj, GameObject *killer) {
    ...
    } else if (triggered)//then was it the driver?
    {
        ...
        Ranged_Scale_Damage_To_Buildings(damage, warhead, Commands->Get_Position(obj), radius,
                                         Commands->Find_Object(driverID));
    }
...
void RA_CTF_Zone::Entered(GameObject *obj, GameObject *enterer) {
    if (Get_Int_Parameter("Team_ID") != Commands->Get_Player_Type(enterer)) {
        ...
        GameObject * tmpObj = Commands->Create_Object_At_Bone(enterer, Get_Parameter("Flag_Preset_Name"),
                                                              "LHAND"); //create the flag object
        int objID = Commands->Get_ID(tmpObj);                                //get the new flags ID
        FlagId = objID;
        Commands->Send_Custom_Event(obj, enterer, 10003, objID,
                                    0);            //tell the player script what the flag id is
        ...
        Commands->Start_Timer(obj, this, 10,
                              411);                            //needed to add this for the suicide and lag flags
...
void RA_CTF_Zone::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    ...
    if ((type == 10002) && (GotFlag)) {
        ...
        Commands->Send_Custom_Event(obj, sender, 10002, 1,
                                    0);                //tell player script "kill your flag, it is now captured"
        ...
        if (NumCaptured == Get_Int_Parameter(
                "Max_Capture"))                                //check to see if the max captures have been met
        {
...
void RA_MAD_Tank_Improved::Animation_Complete(GameObject *obj, const char *animation_name) {
    if (!_stricmp(animation_name, Get_Parameter("Deployed_Animation"))) //Is this the right animation?
    {
        if (Get_GameObj(ownerID)) //sonic pulse only if owner still valid
        {
            ...
            Ranged_Percentage_Damage_To_Buildings(percentage, warhead, Commands->Get_Position(obj), range,
                                                  Get_GameObj(ownerID));
            Ranged_Percentage_Damage_To_Vehicles(percentage, warhead, Commands->Get_Position(obj), range,
                                                 Get_GameObj(ownerID));
        }
        //Kill MAD Tank
    ...
    if (!attacking)    //skip facing action if attacking
    {
        ...
        if (!(target)) //If target invalid, end now
        {
            ...
            if (((Get_Vehicle_Mode(target) == VEHICLE_TYPE_FLYING) ||
                 (Get_Vehicle_Mode(Get_Vehicle(target)) == VEHICLE_TYPE_FLYING)) &&
                !Is_Script_Attached(target, "JFW_Submarine")) {
                if (!(Get_Int_Parameter("AntiAir"))) {
        ...
        if (call_attack && obj->As_DamageableGameObj()->Is_Enemy(
                target->As_DamageableGameObj())) //if attack and if call_attack was not turned false
        {
...
void RA_Ore_Field::Entered(GameObject *obj, GameObject *enterer) {
    if (Is_Script_Attached(enterer, "RA_Ore_Truck") ||
        Is_Script_Attached(enterer, "RA_Ore_Truck_2")) //Is this an Ore Truck?
    {
...
void RA_Ore_Field::Exited(GameObject *obj, GameObject *exiter) {
    if (Is_Script_Attached(exiter, "RA_Ore_Truck") ||
        Is_Script_Attached(exiter, "RA_Ore_Truck_2")) //Is this an Ore Truck?
    {
        ...
        if (Is_Stealth(obj)) {
            ...
            if (driverID) {
                if (!modelID) {
                    ...
                    GameObject * object = Commands->Create_Object("Invisible_Object", position);
                    Commands->Attach_To_Object_Bone(object, obj, Get_Parameter("BoneName"));
                    Commands->Send_Custom_Event(obj, Commands->Find_Object(driverID), Get_Int_Parameter("Message"),
                                                Commands->Get_ID(object), 0);
                    modelID = Commands->Get_ID(object);
                }
            ...
            if (!underwater) //Underwater takes priority
            {
                if (param == 1) //turn on effect
                {
                    if (!blackout) {
                        ...
                        Set_Screen_Fade_Opacity_Player(obj, 150, 0);
                        if (Get_Vehicle(obj)) {
                            Set_Fog_Override_Player(obj, Get_Float_Parameter("VehicleFogStart"),
                                                    Get_Float_Parameter("VehicleFogEnd"));
                        } else {
                            Set_Fog_Override_Player(obj, Get_Float_Parameter("InfantryFogStart"),
                                                    Get_Float_Parameter("InfantryFogEnd"));
                        }
                        Set_Fog_Color_Player(obj, 0, 0, 0);
                        blackout = true;
                    }
...
void RA_MAD_Tank_Devolved::Animation_Complete(GameObject *obj, const char *animation_name) {
    if (!_stricmp(animation_name, Get_Parameter("Deployed_Animation"))) //Is this the right animation?
    {
        if (Get_GameObj(ownerID)) //sonic pulse only if owner still valid
        {
            ...
            //Time to break a few windows
            SetExplosionObj(obj);
            Ranged_Percentage_Damage_To_Buildings(Epercentage, warhead, Commands->Get_Position(obj), range,
                                                  Get_GameObj(ownerID));
            Ranged_Variable_Percent_Vehicle_Damage(Epercentage, Fpercentage, FF, warhead, Commands->Get_Position(obj),
                                                   range, Get_GameObj(ownerID));
            SetExplosionObj(0);
        }
        //Kill MAD Tank
        //'0' Damager used so owner won't get points for tank itself
...
ScriptRegistrant<RA_Helipad_Zone> RA_Helipad_Zone_Registrant("RA_Helipad_Zone",
                                                             "TeamSelection=0:int,Reload_Interval=2.00:float,Apache_Preset_ID=0:int,Apache_Reload_Powerup=Blank:string,Hind_Preset_ID=0:int,Hind_Reload_Powerup=Blank:string");
ScriptRegistrant<RA_MineLayer> RA_MineLayer_Registrant("RA_MineLayer", "Mine_Manager_ID=0:int");
ScriptRegistrant<RA_Mine> RA_Mine_Registrant("RA_Mine",
                                             "Mine_Manager_ID=0:int,Is_Anti-Personnel=0:int,Is_Anti-Tank=0:int,Trigger_Damage=1.0:float,Trigger_Warhead=Death:string");
ScriptRegistrant<RA_Mine_Manager> RA_Mine_Manager_Registrant("RA_Mine_Manager",
                                                             "Allied_Mine=Blank:string,Soviet_Mine=Blank:string,Mine_Reload=Blank:string,BoneName=mine:string,Mine_Limit=30:int");
ScriptRegistrant<RA_ObjectDeath_OnCreate> RA_ObjectDeath_OnCreate_Registrant("RA_ObjectDeath_OnCreate",
                                                                             "Object=null:string");
ScriptRegistrant<RA_Naval_Yard> RA_Naval_Yard_Registrant("RA_Naval_Yard", "Limit=8:int,Disable_Time=5.0:float");
ScriptRegistrant<RA_Naval_PT_Pokable> RA_Naval_PT_Pokable_Registrant("RA_Naval_PT_Pokable",
                                                                     "Preset_Name=blank:string,Cost=0.0:float,Team=0:int,Controller_ID=0:int,Zone_ID=0:int");
...
ScriptRegistrant<RA_GameStart_Detector> RA_GameStart_Detector_Registrant("RA_GameStart_Detector",
                                                                         "Interval=5.0:float,ReceiverID=0:int,Message=0:int,Parameter=0:int");
...
ScriptRegistrant<RA_ObjectDeath_OnDeath> RA_ObjectDeath_OnDeath_Registrant("RA_ObjectDeath_OnDeath",
                                                                           "Preset=blank:string");
ScriptRegistrant<RA_Demolition_Truck_Improved> RA_Demolition_Truck_Improved_Registrant("RA_Demolition_Truck_Improved",
                                                                                       "Explosion=Explosion_Invis_Nuke:string,Warhead=Nuke:string,DamageStrength=480.0:float,DamageRadius=120.0:float");
ScriptRegistrant<RA_CTF_Zone> RA_CTF_Zone_Registrant("RA_CTF_Zone",
                                                     "Team_ID=0:int,Max_Capture=5:int,Flag_Preset_Name:string");
ScriptRegistrant<RA_MAD_Tank_Improved> RA_MAD_Tank_Improved_Registrant("RA_MAD_Tank_Improved",
                                                                       "Attach_Model=pistons:string,Deployed_Animation=V_SO_MADTANK.ANIM_MADTANK:string,Sonic_Preset=Sonic_Box:string,Percentage=0.34:float,Warhead=Sonic:string,DamageRadius=250.0:float,Announcement=MAD_Tank_Deployed:string,Bone=ROOTTRANSFORM:string");
ScriptRegistrant<RA_Conyard_Controller_Improved> RA_Conyard_Controller_Improved_Registrant(
        "RA_Conyard_Controller_Improved",
        "Team=0:int,Preset1=blank:string,Preset2=blank:string,Preset3=blank:string,Preset4=blank:string,Preset5=blank:string");
ScriptRegistrant<RA_Conyard_Controller_Improved_2> RA_Conyard_Controller_Improved_2_Registrant(
        "RA_Conyard_Controller_Improved_2",
        "Team=0:int,Preset1=blank:string,Preset2=blank:string,Preset3=blank:string,Preset4=blank:string,Preset5=blank:string,Preset6=blank:string,Preset7=blank:string,Preset8=blank:string");
ScriptRegistrant<RA_Building_DeathSound> RA_Building_DeathSound_Registrant("RA_Building_DeathSound",
                                                                           "HalfSound=ExploSound_Bldg_Half:string,DeadSound=ExploSound_Bldg_Dead:string");
ScriptRegistrant<RA_Base_Defense_Simple> RA_Base_Defense_Simple_Registrant("RA_Base_Defense_Simple",
                                                                           "MinRange=0.0:float,MaxRange=100.0:float,AntiAir=0:int,AntiGround=1:int,AdjustAim=0:int,TargetMode=0:int");
ScriptRegistrant<RA_Base_Defense_Powered> RA_Base_Defense_Powered_Registrant("RA_Base_Defense_Powered",
                                                                             "MinRange=0.0:float,MaxRange=100.0:float,AntiAir=0:int,AntiGround=1:int,AdjustAim=0:int,TargetMode=0:int");
ScriptRegistrant<RA_Base_Defense_Chargeup> RA_Base_Defense_Chargeup_Registrant("RA_Base_Defense_Chargeup",
                                                                               "MinRange=0.0:float,MaxRange=100.0:float,AntiAir=0:int,AntiGround=1:int,AdjustAim=0:int,TargetMode=0:int,ChargeAnim:string,ChargeSound:string");
ScriptRegistrant<RA_Vehicle_Regen> RA_Vehicle_Regen_Registrant("RA_Vehicle_Regen",
                                                               "Percentage=0.5:float,Heal=1.0:float,Time=1.0:float,TimerID=87654:int");
ScriptRegistrant<RA_Thief> RA_Thief_Registrant("RA_Thief", "Enter_Message=357753:int,Percentage=0.5:float");
ScriptRegistrant<RA_Thief_Improved> RA_Thief_Improved_Registrant("RA_Thief_Improved",
                                                                 "Enter_Message=357753:int,Percentage=0.5:float,Sound:string");
ScriptRegistrant<RA_Credit_Theft_Zone> RA_Credit_Theft_Zone_Registrant("RA_Credit_Theft_Zone",
                                                                       "Enter_Message=357753:int,Team=0:int");
ScriptRegistrant<RA_Credit_Theft_Zone_Timer> RA_Credit_Theft_Zone_Timer_Registrant("RA_Credit_Theft_Zone_Timer",
                                                                                   "Enter_Message=357753:int,Team=0:int,Time:float");
ScriptRegistrant<RA_DestroyNearest_OnDeath> RA_DestroyNearest_OnDeath_Registrant("RA_DestroyNearest_OnDeath",
                                                                                 "Preset_Name:string");
ScriptRegistrant<RA_Ore_Truck> RA_Ore_Truck_Registrant("RA_Ore_Truck",
                                                       "Field_Enter=456654:int,Field_Exit=75321:int,Deposit_Enter=321654:int,Max_Capacity=7:int,Harvest_Anim:string,Deliver_Anim:string");
ScriptRegistrant<RA_Ore_Truck_2> RA_Ore_Truck_2_Registrant("RA_Ore_Truck_2",
                                                           "Field_Enter=456654:int,Field_Exit=75321:int,Deposit_Enter=321654:int,Max_Capacity=7:int,Harvest_Anim:string,Deliver_Anim:string,Harvest_Sound:string,Dump_Sound:string");
ScriptRegistrant<RA_Ore_Field> RA_Ore_Field_Registrant("RA_Ore_Field",
                                                       "Field_Value=700:int,Enter=456654:int,Exit=75321:int");
ScriptRegistrant<RA_Ore_Delivery_Zone> RA_Ore_Delivery_Zone_Registrant("RA_Ore_Delivery_Zone",
                                                                       "Team=0:int,Enter=321654");
...
ScriptRegistrant<RA_Visible_Driver> RA_Visible_Driver_Registrant("RA_Visible_Driver",
                                                                 "BoneName=seat0:string,Message=471174:int");
ScriptRegistrant<RA_Vision_Control> RA_Vision_Control_Registrant("RA_Vision_Control",
                                                                 "FogController=RA_Game_Manager:string");
ScriptRegistrant<RA_Vision_Control_2> RA_Vision_Control_2_Registrant("RA_Vision_Control_2",
                                                                     "FogController=RA_Game_Manager:string,InfantryFogStart=0:float,InfantryFogEnd=9:float,VehicleFogStart=0:float,VehicleFogEnd=9:float");
ScriptRegistrant<RA_Fog_Level_Settings> RA_Fog_Level_Settings_Registrant("RA_Fog_Level_Settings",
                                                                         "FogEnable=1:int,FogMin=200.0:float,FogMax=300.0:float");
ScriptRegistrant<RA_Missile_Beacon> RA_Missile_Beacon_Registrant("RA_Missile_Beacon", "SiloPresetName:string");
ScriptRegistrant<RA_Superweapon_Beacon> RA_Superweapon_Beacon_Registrant("RA_Superweapon_Beacon",
                                                                         "SiloPresetName:string");
ScriptRegistrant<RA_Beacon_Terminal> RA_Beacon_Terminal_Registrant("RA_Beacon_Terminal",
                                                                   "BeaconPowerup:string,BeaconReadySound:string,BeaconTakenSound:string,SiloDisableSound:string,SpyNotifySound:string");
ScriptRegistrant<RA_Beacon_Terminal_2> RA_Beacon_Terminal_2_Registrant("RA_Beacon_Terminal_2",
                                                                       "BeaconPowerup:string,BeaconReadySound:string,BeaconTakenSound:string,SiloDisableSound:string,OfflineCustom:string,OnlineCustom:string,Warhead:string");
...
ScriptRegistrant<RA_Missile_Controller> RA_Missile_Controller_Registrant("RA_Missile_Controller",
                                                                         "SiloPresetName:string,TerminalPresetName:string");
ScriptRegistrant<RA_Demolition_Truck_Retarded> RA_Demolition_Truck_Retarded_Registrant("RA_Demolition_Truck_Retarded",
                                                                                       "Explosion=Explosion_Invis_Nuke:string,Warhead=Nuke:string,DamageStrength=480.0:float,DamageRadius=120.0:float");
ScriptRegistrant<RA_MAD_Tank_Devolved> RA_MAD_Tank_Devolved_Registrant("RA_MAD_Tank_Devolved",
                                                                       "Attach_Model=pistons:string,Deployed_Animation=V_SO_MADTANK.ANIM_MADTANK:string,Sonic_Preset=Sonic_Box:string,EnemyPercentage=0.34:float,FriendPercentage=0.17:float,ForceFF=1:int,Warhead=Sonic:string,DamageRadius=250.0:float,Announcement=MAD_Tank_Deployed:string,Bone=ROOTTRANSFORM:string");
