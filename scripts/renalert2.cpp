...
void RA_ABomb_Terminal::Poked(GameObject *obj, GameObject *poker) {
    if (active) {
        ...
        if (type == Get_Int_Parameter("Team")) {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(Get_Int_Parameter("ControllerID")), 985741,
                                        Commands->Get_ID(poker), 0.0);
        } else {
...
void RA_CanyonRiver_Weather::Timer_Expired(GameObject *obj, int number) {
    if (number == 789456) {
        ...
        Commands->Set_Lightning(Get_Float_Parameter("Lightning_Intensity"), Get_Float_Parameter("Lightning_Start"),
                                Get_Float_Parameter("Lightning_End"), heading,
                                Get_Float_Parameter("Lightning_Distribution"), time);
        Commands->Set_Wind(heading, Get_Float_Parameter("Wind_Strength"), Get_Float_Parameter("Wind_Variability"),
                           time);
    }
}
...
void RA_Underwater_OnCustom::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == Get_Int_Parameter("Disable_Message")) {
        ...
        if (allowed) {
            ...
            Set_Screen_Fade_Color_Player(obj, Get_Float_Parameter("Dive_Red"), Get_Float_Parameter("Dive_Green"),
                                         Get_Float_Parameter("Dive_Blue"), 0);
...
void RA_Vehicle::Killed(GameObject *obj, GameObject *killer) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        {
            Commands->Send_Custom_Event(obj, Commands->Find_Object(IDarray[i]), 310013, 1,
                                        0.5); //0.5 second delay so driver has returned to normal gameplay
            IDarray[i] = 0;
        }
    }
}
...
ScriptRegistrant<RA_Missile_Manager> RA_Missile_Manager_Registrant("RA_Missile_Manager",
                                                                   "AlliedSiloID=0:int,Allied_ZRotation=0.0:float,SovietSiloID=0:int,Soviet_ZRotation=0.0:float");
ScriptRegistrant<RA_Silo_Animation> RA_Silo_Animation_Registrant("RA_Silo_Animation",
                                                                 "ControllerID:int,Mode:int,Animation:string");
ScriptRegistrant<RA_ABomb_Terminal> RA_ABomb_Terminal_Registrant("RA_ABomb_Terminal",
                                                                 "Team:int,ControllerID:int,Powerup:string");
ScriptRegistrant<RA_CanyonRiver_Weather> RA_CanyonRiver_Weather_Registrant("RA_CanyonRiver_Weather",
                                                                           "Message=754754:int,Storm_Delay=900.0:float,Storm_Transition=300.0:float,Storm_Heading=60.0:float,Fog_Start=100.0:float,Fog_End=150.0:float,Rain_Density=1.0:float,Lightning_Intensity=1.0:float,Lightning_Start=0.2:float,Lightning_End=0.8:float,Lightning_Distribution=0.5:float,Wind_Strength=10.0:float,Wind_Variability=0.9:float");
ScriptRegistrant<RA_Metro_Weather> RA_Metro_Weather_Registrant("RA_Metro_Weather", "Time=300.0:float");
ScriptRegistrant<RA_Underwater_OnCustom> RA_Underwater_OnCustom_Registrant("RA_Underwater_OnCustom",
                                                                           "Dive_Message=789789:int,Surface_Message=987987:int,Disable_Message=852852:int,Enable_Message=258258:int,Dive_Red=0.0:float,Dive_Green=0.0:float,Dive_Blue=1.0:float,Dive_Opacity=150.0:float");
ScriptRegistrant<RA_Underwater_Zone> RA_Underwater_Zone_Registrant("RA_Underwater_Zone",
                                                                   "Enter_Message=0:int,Exit_Message=0:int");
