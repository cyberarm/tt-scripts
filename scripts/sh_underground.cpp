    ...
    bool CheckCooldown(GameObject *requester) {
        if (cooldown > 0 && cooldown > time(NULL) - lastTransition) {
            if (-1 != Get_Player_ID(requester)) {
                Send_Message_Player(requester, 255, 50, 50,
                                    StringClass::getFormattedString("You cannot %s for another %d seconds",
                                                                    (underground ? "surface" : "dig"),
                                                                    cooldown - (time(NULL) - lastTransition)));
            }
...
REGISTER_SCRIPT(SH_UndergroundVehicle,
                "DefaultLockState=0:int,DigAnimation:string,SurfaceAnimation:string,Velocity:float,DigExplosion:string,SurfaceExplosion:string,DigMessage:string,SurfaceMessage:string,Cooldown:int");
