...
class SH_HurtUnitTimerZone : public ScriptImpClass {
    void Entered(GameObject *obj, GameObject *referer) {
        ...
        sprintf_s(param_string, "%f,%s,%f", Get_Float_Parameter("Time"), Get_Parameter("Warhead"),
                  Get_Float_Parameter("Amount"));
...
class SH_DangerZone : public ScriptImpClass {
    void Entered(GameObject *obj, GameObject *referer) {
        char param_string[128];
        sprintf_s(param_string, "%f,%f,%f,%s", Get_Float_Parameter("Time"), Get_Float_Parameter("TransitionIn"),
                  Get_Float_Parameter("TransitionOut"), Get_Parameter("Color"));
    ...
    void Entered(GameObject *obj, GameObject *referer) {
        ...
        if (physical_obj) {
            ...
            if (c4_obj) {
                GameObject * stuck_object = c4_obj->Get_Stuck_Object();
                if (stuck_object && stuck_object->As_VehicleGameObj() &&
                    stuck_object->As_PhysicalGameObj()->Peek_Physical_Object()->As_VTOLVehicleClass()) {
                    c4_obj->Defuse();
                }
            }
        }
...
ScriptRegistrant<SH_Spawn_Difficulty> SH_Spawn_Difficulty_Created("SH_Spawn_Difficulty",
                                                                  "ObjectEasy:string,ObjectEasyEnabled:int,ObjectMedium:string,ObjectMediumEnabled:int,ObjectHard:string,ObjectHardEnabled:int");
ScriptRegistrant<SH_PTHidePresetOnDeath> SH_PTHidePresetOnDeath_Registant("SH_PTHidePresetOnDeath",
                                                                          "Presets:string,Player_Type:int");
