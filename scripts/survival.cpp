...
void SUR_Generator_Zone::Timer_Expired(GameObject *obj, int number) {
    ...
    switch (number) {
        case 1:
            ...
            if (energy <= 0) {
                for (int i = 1; i <= 3; i++) {
                    tmpObj = Commands->Find_Object(Get_Int_Parameter(5 +
                                                                     i));                //start getting all the Building ID's and Destorying them for game over.
...
void SUR_NHB_Soldier::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    ...
    if (param == 500010) {
        ...
        sprintf(sparam, "%d,%d,%d", Get_Int_Parameter("VPad_Path"), Get_Int_Parameter("VPad_Path_Start"),
                Get_Int_Parameter("VPad_Path_End"));
        Commands->Attach_Script(obj, "PDS_Test_Follow_Waypath", sparam);
    }
    if (param == 500011) {
        Commands->Set_Innate_Aggressiveness(obj, 50.500);
        sprintf(sparam, "%d,%d,%d", Get_Int_Parameter("HeliPath"), Get_Int_Parameter("HeliPath_Start"),
                Get_Int_Parameter("HeliPath_End"));
        Commands->Attach_Script(obj, "PDS_Test_Follow_Waypath", sparam);
    }
    if (param == 500020) {
        Commands->Set_Innate_Aggressiveness(obj, 50.500);
        sprintf(sparam, "%d,%d,%d", Get_Int_Parameter("NodGen_Path"), Get_Int_Parameter("NodGen_Path_Start"),
                Get_Int_Parameter("NodGen_Path_End"));
        Commands->Attach_Script(obj, "PDS_Test_Follow_Waypath", sparam);
    }
    if (param == 500021) {
        Commands->Set_Innate_Aggressiveness(obj, 50.000);
        sprintf(sparam, "%d,%d,%d", Get_Int_Parameter("GDIGen_Path"), Get_Int_Parameter("GDIGen_Path_Start"),
                Get_Int_Parameter("GDIGen_Path_End"));
        Commands->Attach_Script(obj, "PDS_Test_Follow_Waypath", sparam);
    }
...
void SUR_GrantPowerup_OnEntry::Entered(GameObject *obj, GameObject *enterer) {
    if ((!_stricmp(Commands->Get_Preset_Name(enterer), Get_Parameter("Preset"))) ||
        (!_stricmp(Get_Parameter("Preset"), "Any"))) {
        Commands->Give_PowerUp(enterer, Get_Parameter("Powerup"), true);
    }
}

ScriptRegistrant<SUR_Spawn_Delay> SUR_Spawn_Delay_Registrant("SUR_Spawn_Delay",
                                                             "Spawn_Delay:float,Spawn_Pos:vector3,Spawn_Obj:string,Spawn_Path:int,Do_Path:int");
ScriptRegistrant<SUR_Generator_Zone> SUR_Generator_Zone_Registrant("SUR_Generator_Zone",
                                                                   "Time_Delay:float,Supply_Truck_Preset:string,Emitter_Pos:vector3,Emitter_Preset_Good:string,Emitter_Preset_Ok:string,Emitter_Preset_Bad:string,Build_D_One:int,Build_D_Two:int,Build_D_Three:int");
ScriptRegistrant<SUR_MiniGen> SUR_MiniGen_Registrant("SUR_MiniGen",
                                                     "Time_Delay:float,Turret_ID_One:int,Turret_ID_Two:int,Turret_ID_Three:int,Turret_ID_Four:int");
ScriptRegistrant<SUR_NHB> SUR_NHB_Registrant("SUR_NHB",
                                             "GDI_Gen:int,Nod_Gen:int,Strike_Preset:string,Taunt1:int,Taunt2:int,OrcaWarning:int");
ScriptRegistrant<SUR_NHB_Soldier> SUR_NHB_Soldier_Registrant("SUR_NHB_Soldier",
                                                             "NHB_ID:int,VPad_Path:int,VPad_Path_Start:int,VPad_Path_End:int,HeliPath:int,HeliPath_Start:int,HeliPath_End:int,NodGen_Path:int,NodGen_Path_Start:int,NodGen_Path_End:int,GDIGen_Path:int,GDIGen_Path_Start:int,GDIGen_Path_End:int,NodGen_ID:int,GDIGen_ID:int");
...
ScriptRegistrant<SUR_New_PT_Reciever> SUR_New_PT_Reciever_Registrant("SUR_New_PT_Reciever",
                                                                     "Preset_Name:string,Message:int");
...
ScriptRegistrant<SUR_Chinook_AI> SUR_Chinook_AI_Registrant("SUR_Chinook_AI",
                                                           "Preset_Name:string,Position_Offset:vector3");
ScriptRegistrant<SUR_Dep_Turret> SUR_Dep_Turret_Registrant("SUR_Dep_Turret",
                                                           "MinAttackDistance=0.0:float,MaxAttackDistance=300.0:float,AttackTimer=10.0:int");
ScriptRegistrant<SUR_Timed_Death> SUR_Timed_Death_Registrant("SUR_Timed_Death",
                                                             "Delay:float,Damage_Amount:float,Damage_Type:string");
ScriptRegistrant<SUR_GrantPowerup_OnEntry> SUR_GrantPowerup_OnEntry_Registrant("SUR_GrantPowerup_OnEntry",
                                                                               "Preset:string,Powerup:string");
