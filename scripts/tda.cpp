...
void TDA_CTF_Zone::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    ...
    if ((type == 10002) && (GotFlag)) {
        ...
        {
            GameObject *tmpObj;
            tmpObj = Commands->Find_Object(Get_Int_Parameter(
                    "Building_To_Destroy1"));                //start getting all the Building ID's and Destorying them for game over.
            maxHealth = Commands->Get_Max_Health(tmpObj);                    //get buildings max health
            Commands->Apply_Damage(tmpObj, 10000, "Explosive",
                                   0);            //Apply_Damage (only one that don't crash game)
            tmpObj = Commands->Find_Object(Get_Int_Parameter(
                    "Building_To_Destroy2"));                //start getting all the Building ID's and Destorying them for game over.
            maxHealth = Commands->Get_Max_Health(tmpObj);                    //get buildings max health
            Commands->Apply_Damage(tmpObj, 10000, "Explosive",
                                   0);            //Apply_Damage (only one that don't crash game)
            tmpObj = Commands->Find_Object(Get_Int_Parameter(
                    "Building_To_Destroy3"));                //start getting all the Building ID's and Destorying them for game over.
            maxHealth = Commands->Get_Max_Health(tmpObj);                    //get buildings max health
            Commands->Apply_Damage(tmpObj, 10000, "Explosive",
                                   0);            //Apply_Damage (only one that don't crash game)
            tmpObj = Commands->Find_Object(Get_Int_Parameter(
                    "Building_To_Destroy4"));                //start getting all the Building ID's and Destorying them for game over.
            maxHealth = Commands->Get_Max_Health(tmpObj);                    //get buildings max health
            Commands->Apply_Damage(tmpObj, 10000, "Explosive",
                                   0);            //Apply_Damage (only one that don't crash game)
            tmpObj = Commands->Find_Object(Get_Int_Parameter(
                    "Building_To_Destroy5"));                //start getting all the Building ID's and Destorying them for game over.
            maxHealth = Commands->Get_Max_Health(tmpObj);                    //get buildings max health
            Commands->Apply_Damage(tmpObj, 10000, "Explosive",
                                   0);            //Apply_Damage (only one that don't crash game)
        }
...
ScriptRegistrant<TDA_Send_Custom_Zone> TDA_Send_Custom_Zone_Registrant("TDA_Send_Custom_Zone",
                                                                       "ID:int,EnterMessage:int,EnterParam:int,ExitMessage:int,ExitParam:int,Team_ID:int");
ScriptRegistrant<TDA_Conyard_Controller> TDA_Conyard_Controller_Registrant("TDA_ConYard_Controller",
                                                                           "Building1_ID=0:int,Building2_ID=0:int,Building3_ID=0:int,Building4_ID=0:int,Building5_ID=0:int,Building6_ID=0:int,Building7_ID=0:int,Building8_ID=0:int,Building9_ID=0:int,Building10_ID=0:int");
ScriptRegistrant<TDA_User_Purchased_VTOL_Object> TDA_User_Purchased_VTOL_Object_Registrant(
        "TDA_User_Purchased_VTOL_Object", "Preset_Name:string,HelipadLocation:vector3");
ScriptRegistrant<TDA_Helipad_Controller> TDA_Helipad_Controller_Registrant("TDA_Helipad_Controller",
                                                                           "VTOL_Controller1=0:int,VTOL_Controller2=0:int,VTOL_Controller3=0:int,VTOL_Controller4=0:int");
...
ScriptRegistrant<TDA_Toggle_Stealth_Zone> TDA_Toggle_Stealth_Zone_Registrant("TDA_Toggle_Stealth_Zone",
                                                                             "Trigger_Type:int,Player_Type:int");
ScriptRegistrant<TDA_Teleport_Zone> TDA_Teleport_Zone_Registrant("TDA_Teleport_Zone",
                                                                 "Location:vector3,Object_ID=0:int");
ScriptRegistrant<TDA_Disable_Building_Zone> TDA_Disable_Building_Zone_Registrant("TDA_Disable_Building_Zone",
                                                                                 "Building_ID:int,Team_ID:int");
ScriptRegistrant<TDA_Conyard_Repair> TDA_Conyard_Repair_Registrant("TDA_Conyard_Repair",
                                                                   "Repair_Frequency=1.0:float,Timer_ID=0:int");
ScriptRegistrant<TDA_Stealth_Generator> TDA_Stealth_Generator_Registrant("TDA_Stealth_Generator", "Zone_ID=0:int");
ScriptRegistrant<TDA_Stealth_Generator_Zone> TDA_Stealth_Generator_Zone_Registrant("TDA_Stealth_Generator_Zone",
                                                                                   "Player_Type=0:int");
ScriptRegistrant<TDA_CTF_Attached> TDA_CTF_Attached_Registrant("TDA_CTF_Attached", "");
ScriptRegistrant<TDA_CTF_Zone> TDA_CTF_Zone_Registrant("TDA_CTF_Zone",
                                                       "Team_ID=0:int,Max_Capture=5:int,Flag_Preset_Name:string,Building_To_Destroy1=0:int,Building_To_Destroy2=0:int,Building_To_Destroy3=0:int,Building_To_Destroy4=0:int,Building_To_Destroy5=0:int,Play_Capture_Sounds=1:int");
