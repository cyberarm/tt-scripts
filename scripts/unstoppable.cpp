...
void UP_HUD_Message_Zone_Enter::Entered(GameObject *obj, GameObject *enterer) {
    if (Commands->Is_A_Star(enterer)) {
        if (Get_Parameter("Message")) {
            Set_HUD_Help_Text_Player_Text(enterer, 7811, Get_Parameter("Message"),
                                          Vector3(Get_Int_Parameter("Red") / 255.f, Get_Int_Parameter("Green") / 255.f,
                                                  Get_Int_Parameter("Blue") / 255.f));
        }
...
ScriptRegistrant<UP_HUD_Message_Zone_Enter> UP_HUD_Message_Zone_Enter_Registrant("UP_HUD_Message_Zone_Enter",
                                                                                 "Message=Your Message:string,Sound=SoundName.wav:string,Red=255:int,Green=255:int,Blue=255:int");
...
void UP_HUD_Message_Zone_Exit::Exited(GameObject *obj, GameObject *exiter) {
    if (Commands->Is_A_Star(exiter)) {
        if (Get_Parameter("Message")) {
            Set_HUD_Help_Text_Player_Text(exiter, 7811, Get_Parameter("Message"),
                                          Vector3(Get_Int_Parameter("Red") / 255.f, Get_Int_Parameter("Green") / 255.f,
                                                  Get_Int_Parameter("Blue") / 255.f));
        }
...
ScriptRegistrant<UP_HUD_Message_Zone_Exit> UP_HUD_Message_Zone_Exit_Registrant("UP_HUD_Message_Zone_Exit",
                                                                               "Message=Your Message:string,Sound=SoundName.wav:string,Red=255:int,Green=255:int,Blue=255:int");
...
ScriptRegistrant<UP_Stop_Vehicle_Engine> UP_Stop_Vehicle_Engine_Registrant("UP_Stop_Vehicle_Engine",
                                                                           "Duration=0:float");
...
ScriptRegistrant<UP_Change_Vehicle_Weapon> UP_Change_Vehicle_Weapon_Registrant("UP_Change_Vehicle_Weapon",
                                                                               "Preset=Weapon:string,Rounds=1:int");
    ...
    if (MCTObj) {
        if (DamageableGameObj *damageable = MCTObj->As_DamageableGameObj()) {
            if (DamageableGameObj *damageable2 = obj->As_DamageableGameObj()) {
                ...
                StringClass healthSkin = ArmorWarheadManager::Get_Armor_Name(
                        damageable2->Get_Defense_Object()->Get_Skin());
                StringClass shieldSkin = ArmorWarheadManager::Get_Armor_Name(
                        damageable2->Get_Defense_Object()->Get_Shield_Type());
                ...
                Attach_Script_V(MCTObj, "UP_Master_Control_Terminal", "%d,%f", Commands->Get_ID(damageable2),
                                Get_Float_Parameter("Multiplier"));
            } else {
                Destroy_Script();
            }
        } else {
...
ScriptRegistrant<UP_Master_Control_Terminal> UP_Master_Control_Terminal_Registrant("UP_Master_Control_Terminal",
                                                                                   "Object:int,Multiplier=1.00:float");
...
ScriptRegistrant<UP_Grant_Weapon> UP_Grant_Weapon_Registrant("UP_Grant_Weapon",
                                                             "Preset=Weapon:string,ClipRounds=1:int,InventoryRounds=1:int,AutoSelect=1:int");
...
ScriptRegistrant<UP_Grant_Weapon_Zone_Enter> UP_Grant_Weapon_Zone_Enter_Registrant("UP_Grant_Weapon_Zone_Enter",
                                                                                   "Preset=Weapon:string,ClipRounds=1:int,InventoryRounds=1:int,AutoSelect=1:int");
...
ScriptRegistrant<UP_Grant_Weapon_Zone_Exit> UP_Grant_Weapon_Zone_Exit_Registrant("UP_Grant_Weapon_Zone_Exit",
                                                                                 "Preset=Weapon:string,ClipRounds=1:int,InventoryRounds=1:int,AutoSelect=1:int");
...
ScriptRegistrant<UP_Grant_Character_Zone_Enter> UP_Grant_Character_Zone_Enter_Registrant(
        "UP_Grant_Character_Zone_Enter", "Preset=Character:string");
...
ScriptRegistrant<UP_Grant_Character_Zone_Exit> UP_Grant_Character_Zone_Exit_Registrant("UP_Grant_Character_Zone_Exit",
...
ScriptRegistrant<UP_Spawn_And_Destroy_When_Fully_Repaired> UP_Spawn_And_Destroy_When_Fully_Repaired_Registrant(
        "UP_Spawn_And_Destroy_When_Fully_Repaired", "Preset=Object:string");
...
ScriptRegistrant<UP_Refinery_Object> UP_Refinery_Object_Registrant("UP_Refinery_Object",
                                                                   "Team=0:int,Interval=1.0:float,Amount=2.0:float");
...
void UP_Vehicle_Purchase_Terminal::Poked(GameObject *obj, GameObject *poker) {
    if (Pokable) {
        ...
        if (Commands->Get_Player_Type(playerObj) == Get_Int_Parameter("Team")) {
            ...
            Send_Message_Player(playerObj, 255, 255, 255,
                                StringClass::getFormattedString("Access Denied! You aren't in team %s.",
                                                                Get_Team_Name(Get_Int_Parameter("Team"))));
        }
    }
}
...
ScriptRegistrant<UP_Vehicle_Purchase_Terminal> UP_Vehicle_Purchase_Terminal_Registrant("UP_Vehicle_Purchase_Terminal",
                                                                                       "Team=0:int,SpawnerID=0:int,VehiclePreset:string,Credits=400:int");
...
void UP_Character_Purchase_Terminal::Poked(GameObject *obj, GameObject *poker) {
    if (Pokable) {
        ...
        if (Commands->Get_Player_Type(Player) == Get_Int_Parameter("Team")) {
            ...
            Send_Message_Player(Player, 255, 255, 255,
                                StringClass::getFormattedString("Access Denied! You aren't in team %s.",
                                                                Get_Team_Name(Get_Int_Parameter("Team"))));
        }
    }
...
ScriptRegistrant<UP_Character_Purchase_Terminal> UP_Character_Purchase_Terminal_Registrant(
        "UP_Character_Purchase_Terminal", "Team=0:int,CharacterPreset:string,Credits=400:int");
...
ScriptRegistrant<UP_Select_Weapon_On_Zone_Enter> UP_Select_Weapon_On_Zone_Enter_Registrant(
        "UP_Select_Weapon_On_Zone_Enter", "WeaponPreset:string");
...
ScriptRegistrant<UP_Select_Weapon_On_Zone_Exit> UP_Select_Weapon_On_Zone_Exit_Registrant(
        "UP_Select_Weapon_On_Zone_Exit", "WeaponPreset:string");
            ...
            if (Is_Valid_Preset(preset1) || presetObj1) {
                ...
                if (presetObj1) {
                    if (Check_Object(presetObj1)) {
                        ...
                        Commands->Set_Animation(presetObj1, "s_a_human.H_A_X5D_ParaT_1", false, NULL, 0.0f, -1.0f,
                                                false);
                    }
            ...
            if (Is_Valid_Preset(preset2) || presetObj2) {
                ...
                if (presetObj2) {
                    if (Check_Object(presetObj2)) {
                        ...
                        Commands->Set_Animation(presetObj2, "s_a_human.H_A_X5D_ParaT_2", false, NULL, 0.0f, -1.0f,
                                                false);
                    }
            ...
            if (Is_Valid_Preset(preset3) || presetObj3) {
                ...
                if (presetObj3) {
                    if (Check_Object(presetObj3)) {
                        ...
                        Commands->Set_Animation(presetObj3, "s_a_human.H_A_X5D_ParaT_3", false, NULL, 0.0f, -1.0f,
                                                false);
                    }
...
ScriptRegistrant<UP_Chinook_Reinforcements_Paradrop> UP_Chinook_Reinforcements_Paradrop_Registrant(
        "UP_Chinook_Reinforcements_Paradrop", "FirstPreset:string,SecondPreset:string,ThirdPreset:string");
...
ScriptRegistrant<UP_Unkillable_Until_Custom> UP_Unkillable_Until_Custom_Registrant("UP_Unkillable_Until_Custom",
                                                                                   "Message:int");
...
ScriptRegistrant<UP_AttachScript_Interval_Self> UP_AttachScript_Interval_Self_Registrant(
        "UP_AttachScript_Interval_Self", "Script:string,Parameters:string,LoopCount=-1:int,Interval=1:float");
...
ScriptRegistrant<UP_AttachScript_Interval> UP_AttachScript_Interval_Registrant("UP_AttachScript_Interval",
                                                                               "ObjectID:int,Script:string,Parameters:string,LoopCount=-1:int,Interval=1:float");
...
ScriptRegistrant<UP_Object_Follower_AI> UP_Object_Follower_AI_Registrant("UP_Object_Follower_AI",
                                                                         "TargetChangeCustomID=123:int,DestroyScriptCustomID=124:int");
...
void UP_Private_Color_Message_On_Poke::Poked(GameObject *obj, GameObject *poker) {
    if (Pokable) {
        Send_Message_Player(poker, Get_Int_Parameter("Red"), Get_Int_Parameter("Green"), Get_Int_Parameter("Blue"),
...
ScriptRegistrant<UP_Private_Color_Message_On_Poke> UP_Private_Color_Message_On_Poke_Registrant(
        "UP_Private_Color_Message_On_Poke", "Red=255:int,Green=255:int,Blue=255:int,Message=Message:string");
...
void UP_Seize_Weapon_Enter::Entered(GameObject *obj, GameObject *enterer) {
    ...
    if (Has_Weapon(enterer, weaponName)) {
        ...
        Attach_Script_V(enterer, "UP_Grant_Weapon_Custom_Then_Detach", "%s,%d,%d,0,%d", weaponName, clipAmmo, invAmmo,
                        Get_Int_Parameter("CustomID"));
...
ScriptRegistrant<UP_Seize_Weapon_Enter> UP_Seize_Weapon_Enter_Registrant("UP_Seize_Weapon_Enter",
                                                                         "Weapon:string,CustomID:int");
...
void UP_Seize_Weapon_Exit::Exited(GameObject *obj, GameObject *exiter) {
    ...
    if (Has_Weapon(exiter, weaponName)) {
        ...
        Attach_Script_V(exiter, "UP_Grant_Weapon_Custom_Then_Detach", "%s,%d,%d,0,%d", weaponName, clipAmmo, invAmmo,
                        Get_Int_Parameter("CustomID"));
        Remove_Weapon(exiter, weaponName);
    }
}

ScriptRegistrant<UP_Seize_Weapon_Exit> UP_Seize_Weapon_Exit_Registrant("UP_Seize_Weapon_Exit",
                                                                       "Weapon:string,CustomID:int");
    ...
    for (int i = 1; i < Get_Weapon_Count(enterer); i++) {
        if (const char *weaponName = Get_Weapon(enterer, i)) {
            ...
            if (Weapons.ID(weaponName) == -1) {
                int clipAmmo = Get_Clip_Bullets(enterer, weaponName);
                int invAmmo = Get_Bullets(enterer, weaponName);
                Attach_Script_V(enterer, "UP_Grant_Weapon_Custom_Then_Detach", "%s,%d,%d,0,%d", weaponName, clipAmmo,
                                invAmmo, Get_Int_Parameter("CustomID"));
                Remove_Weapon(enterer, weaponName);
                i--;
            }
        }
...
ScriptRegistrant<UP_Seize_All_Weapons_Enter> UP_Seize_All_Weapons_Enter_Registrant("UP_Seize_All_Weapons_Enter",
                                                                                   "WeaponsExcept:string,CustomID:int");
    ...
    for (int i = 1; i < Get_Weapon_Count(exiter); i++) {
        if (const char *weaponName = Get_Weapon(exiter, i)) {
            ...
            if (Weapons.ID(weaponName) == -1) {
                int clipAmmo = Get_Clip_Bullets(exiter, weaponName);
                int invAmmo = Get_Bullets(exiter, weaponName);
                Attach_Script_V(exiter, "UP_Grant_Weapon_Custom_Then_Detach", "%s,%d,%d,0,%d", weaponName, clipAmmo,
                                invAmmo, Get_Int_Parameter("CustomID"));
                Remove_Weapon(exiter, weaponName);
                i--;
            }
...
ScriptRegistrant<UP_Seize_All_Weapons_Exit> UP_Seize_All_Weapons_Exit_Registrant("UP_Seize_All_Weapons_Exit",
                                                                                 "WeaponsExcept:string,CustomID:int");
...
void UP_Grant_Weapon_Custom_Then_Detach::Custom(GameObject *obj, int type, int param, GameObject *sender) {
    if (type == Get_Int_Parameter("CustomID")) {
        Attach_Script_V(obj, "UP_Grant_Weapon", "%s,%d,%d,%d", Get_Parameter("Preset"), Get_Int_Parameter("ClipRounds"),
                        Get_Int_Parameter("InventoryRounds"), Get_Int_Parameter("AutoSelect"));
        Destroy_Script();
    }
}

ScriptRegistrant<UP_Grant_Weapon_Custom_Then_Detach> UP_Grant_Weapon_Custom_Then_Detach_Registrant(
        "UP_Grant_Weapon_Custom_Then_Detach",
        "Preset=Weapon:string,ClipRounds=1:int,InventoryRounds=1:int,AutoSelect=1:int,CustomID=10123:int");
